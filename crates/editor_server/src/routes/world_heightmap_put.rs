...
use crate::{resource::マザーハイトマップ, server_state::サーバー状態, storage::プロジェクト保管庫, storage::保存要求エラー};
...
fn 検証して保存する(状態: &サーバー状態, バイト列: Vec<u8>) -> Result<(), Box<Response>> {
    ...
    let 頂点数 = 区画割り.マザーハイトマップの格子解像度().map_err(|エラー| Box::new(保存要求エラー::検証に失敗(エラー).into_response()))?;
    let 格子 = マザーハイトマップ::生成する(頂点数, バイト列).map_err(|エラー| Box::new(保存要求エラー::検証に失敗(エラー).into_response()))?;
    状態.保管庫().大域世界の高さ格子を検証して保存する(格子).map_err(|エラー| Box::new(エラー.into_response()))
}
