...
pub async fn チャンク材質重みを保存する(State(状態): State<サーバー状態>, Path((x, z)): Path<(i32, i32)>, 本文: Bytes) -> Response {
...
fn 検証して保存する(状態: &サーバー状態, 座標: チャンク座標, バイト列: Vec<u8>) -> Result<(), Box<Response>> {
    ...
    let 頂点数 = 区画割り.チャンク格子の頂点数().map_err(|エラー| Box::new(保存要求エラー::検証に失敗(エラー).into_response()))?;
    let 重み = 地表材質の重み::生成する(頂点数, バイト列).map_err(|エラー| Box::new(保存要求エラー::検証に失敗(エラー).into_response()))?;
    状態.保管庫().チャンクの材質重みを検証して保存する(座標, 重み).map_err(|エラー| Box::new(エラー.into_response()))
}
