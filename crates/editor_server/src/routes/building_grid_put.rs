...
pub async fn 建物の格子を保存する(State(状態): State<サーバー状態>, Path(識別子の文字列): Path<String>, 本文: Bytes) -> Response {
    match tokio::task::spawn_blocking(move || 検査して保存する(&状態, &識別子の文字列, &本文)).await {
...
fn 検査して保存する(状態: &サーバー状態, 識別子の文字列: &str, 本文: &[u8]) -> Result<(), Box<Response>> {
    let 格子: 建物の格子 = serde_json::from_slice(本文).map_err(拒否応答へ写す)?;
    if 格子.建物定義ID.文字列() != 識別子の文字列 {
        return Err(拒否応答へ写す(建物の格子の保存エラー::経路と本文の識別子が食い違う {
            経路の識別子: 識別子の文字列.to_string(),
            本文の識別子: 格子.建物定義ID.文字列().to_string(),
        }));
    }
