...
//! 見下ろし図の下書きの粗マスの一辺が格子解像度を割り切るかは、区画割りを知るここで検査する。区画割りがまだ無い間は
...
fn データを検証して保存する(状態: &サーバー状態, カタログ: &建物外形カタログ, 座標: チャンク座標, 本文: &[u8]) -> Result<(), 保存要求エラー> {
    ...
    粗マスの一辺を区画割りと突き合わせる(状態, &データ)?;
    状態.保管庫().チャンクの構造を検証して保存する(座標, データ)
}

fn 粗マスの一辺を区画割りと突き合わせる(状態: &サーバー状態, データ: &チャンク構造) -> Result<(), 保存要求エラー> {
    let Some(大域構造) = 状態.保管庫().大域世界の構造を読む()? else {
        return Ok(());
    };
    見下ろし図の下書き::粗マスの一辺が格子解像度を割り切ることを確かめる(データ.見下ろし図の下書き.粗マスの一辺の升目数, 大域構造.区画割り.チャンクあたり格子解像度)?;
    Ok(())
}
