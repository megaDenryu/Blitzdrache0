...
//! (参照: `_doc/設計/楽曲エディター.md`「判断17」)。トラックの種類は既定の出どころとして残るが、
//! 楽器の検査には使わない。音の並びの検査を種類のメソッドにするのは、打楽器のトラックの行に音高番号が
//! 並んだ楽曲を読める形にしないためである。トラックの構成は実装の定数でなく楽曲データの側が持つ(参照: `_doc/設計/楽曲エディター.md`「判断7」)。
...
impl トラック定義 {
    pub(super) fn 検証する(&self, 進行の名簿: &進行の名簿<'_>) -> Result<(), 資源検証エラー> {
        文字列が空でないことを確かめる("トラック定義.表示名", &self.表示名)?;
        ...
        if !self.音の並び.受け入れる楽器か(self.楽器) {
            return Err(資源検証エラー::組み合わせが成り立たない {
                フィールド名: "トラック定義.楽器",
                説明: format!("{:?}の行を持つトラックへ{:?}の楽器は割り当てられない", self.音の並び.音の種類を得る(), self.楽器.音の種類を得る()),
            });
        }
        self.音の並び.検証する()?;
