...
use super::instrument::{打楽器の種類, 楽器, 楽器の音の種類};
...
impl 音の並び {
    ...
    pub(super) fn 音の種類を得る(&self) -> 楽器の音の種類 {
        match self {
            音の並び::音高の行一覧(_) => 楽器の音の種類::音高,
            音の並び::打楽器の行一覧(_) => 楽器の音の種類::打楽器,
        }
    }

    /// トラックの割り当ての検査は、この一致だけを見る(参照: `_doc/設計/楽曲エディター.md`「判断17」)。
    /// トラックの種類は既定の出どころであり、ここでは使わない。
    pub(super) fn 受け入れる楽器か(&self, 楽器: 楽器) -> bool {
        self.音の種類を得る() == 楽器.音の種類を得る()
    }
