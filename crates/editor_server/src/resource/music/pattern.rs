...
impl パターン {
    pub(super) fn 検証する(&self, トラック構成: &[トラック定義], 進行の名簿: &進行の名簿<'_>) -> Result<(), 資源検証エラー> {
        文字列が空でないことを確かめる("パターン.表示名", &self.表示名)?;
        整数が範囲内であることを確かめる("パターン.小節数", i64::from(self.小節数), 1, i64::from(パターンの小節数の上限))?;
