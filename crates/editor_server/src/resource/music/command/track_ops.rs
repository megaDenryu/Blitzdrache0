...
impl トラックの楽器を変える {
    /// 音の並びの音の種類(音高・打楽器)と食い違う楽器を拒むのは、打楽器の行へ音高の楽器が載った楽曲
    /// (逆も同様)を作らせないためである(参照: `_doc/設計/楽曲エディター.md`「判断5」「判断7」)。
    pub(super) fn 検証する(&self, 解決係: &コマンドの指し先の解決係<'_>) -> Result<(), 資源検証エラー> {
        let トラック = 解決係.トラックを引く(self.トラックの位置)?;
        if トラック.音の並び.受け入れる楽器か(self.新しい楽器) {
            return Ok(());
        }
        Err(資源検証エラー::組み合わせが成り立たない {
            フィールド名: "楽曲編集コマンド.新しい楽器",
            説明: format!("{:?}の行を持つトラックへ{:?}の楽器は割り当てられない", トラック.音の並び.音の種類を得る(), self.新しい楽器.音の種類を得る()),
        })
    }
}
