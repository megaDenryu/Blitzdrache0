...
impl 打ち込みの対象 {
    /// 指し先が実在することを確かめ、そのパターンの小節数を返す(呼び出し元がステップの範囲検査に要るため)。
    pub(super) fn 検証する(&self, 解決係: &コマンドの指し先の解決係<'_>) -> Result<u32, 資源検証エラー> {
        let パターン = 解決係.パターンを引く(&self.パターンの名乗り)?;
...
pub(super) fn ステップの位置が格子の内側であることを確かめる(フィールド名: &'static str, ステップ: u32, 小節数: u32) -> Result<(), 資源検証エラー> {
    整数が範囲内であることを確かめる(フィールド名, i64::from(ステップ), 0, ステップの位置の上限(小節数))
}

/// 始まりが終わりより後ろの範囲は、どのステップを指すのか決まらないため拒む。
pub(super) fn 範囲の向きが正しいことを確かめる(始まりのステップ: u32, 終わりのステップ: u32) -> Result<(), 資源検証エラー> {
