...
impl 打点を置く {
    pub(super) fn 検証する(&self, 解決係: &コマンドの指し先の解決係<'_>) -> Result<(), 資源検証エラー> {
        let 小節数 = self.対象.検証する(解決係)?;
        ステップの位置が格子の内側であることを確かめる("楽曲編集コマンド.ステップ", self.ステップ, 小節数)
    }
}
...
impl 音を伸ばす {
    pub(super) fn 検証する(&self, 解決係: &コマンドの指し先の解決係<'_>) -> Result<(), 資源検証エラー> {
        let 小節数 = self.対象.検証する(解決係)?;
        範囲を確かめる(self.始まりのステップ, self.終わりのステップ, 小節数)
    }
}
...
fn 範囲を確かめる(始まりのステップ: u32, 終わりのステップ: u32, 小節数: u32) -> Result<(), 資源検証エラー> {
    ステップの位置が格子の内側であることを確かめる("楽曲編集コマンド.始まりのステップ", 始まりのステップ, 小節数)?;
    ステップの位置が格子の内側であることを確かめる("楽曲編集コマンド.終わりのステップ", 終わりのステップ, 小節数)?;
