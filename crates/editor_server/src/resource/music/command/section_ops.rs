//! 曲構成の節を増やし・変え・減らし・並べ替え・差し込む5つの操作コマンドの型契約。節は並びの位置で指す。
//!
...
/// 曲の節を挿入するとは、名指した位置の手前へ節を1つ差し込む操作コマンドのことである。
/// 節の位置が曲構成の長さと等しいなら、末尾へ足すのと同じ意味になる(判断11の続き、issue #72)。
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[cfg_attr(feature = "typescript", derive(ts_rs::TS))]
pub struct 曲の節を挿入する {
    pub 節の位置: u32,
...
impl 曲の節を挿入する {
    pub(super) fn 検証する(&self, 解決係: &コマンドの指し先の解決係<'_>) -> Result<(), 資源検証エラー> {
        解決係.節の挿入位置が曲構成の範囲内であることを確かめる(self.節の位置)?;
        解決係.パターンを引く(&self.パターンの名乗り)?;
        繰り返し回数を確かめる("楽曲編集コマンド.繰り返し回数", self.繰り返し回数)
    }
}
...
fn 繰り返し回数を確かめる(フィールド名: &'static str, 繰り返し回数: u32) -> Result<(), 資源検証エラー> {
    整数が範囲内であることを確かめる(フィールド名, i64::from(繰り返し回数), i64::from(曲の節の繰り返し回数の下限), i64::from(曲の節の繰り返し回数の上限))
}
