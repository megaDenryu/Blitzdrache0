//! 編集コマンドの型契約。判断1(操作コマンドはブラウザの画面とブラウザなしの2つの入り口から、
...
pub use plan_view_ops::{
    等高線から高さ場を生成する, 等高線を削除する, 等高線を変更する, 等高線を追加する, 粗マスから地形を生成する, 粗マスを塗る, 高さ場から等高線を導く, 高さ場から粗マスを導く
};
...
pub use terrain_bake_ops::{建物基礎を平坦化する, 急勾配を岩肌へベイクする, 道路に合わせて切土盛土する, 道路下を泥へベイクする};

/// 編集コマンドとは、人間の画面操作とAIのブラウザなしの操作の両方が同じ形で積む、
...
pub enum 編集コマンド {
    ...
    粗マスから地形を生成する(粗マスから地形を生成する),
    高さ場から等高線を導く(高さ場から等高線を導く),
    高さ場から粗マスを導く(高さ場から粗マスを導く),
}
