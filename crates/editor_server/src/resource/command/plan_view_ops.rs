...
use crate::resource::coarse_cell_paint::大升の塗り;
...
/// 大升を塗るとは、対象チャンクの下書きの複数の大升へ高さ・層・またはその両方を一括で置く操作コマンドのことである。
/// 高さも層も無い項目はその大升の塗りを消す。一辺が下書きの一辺と違うときは塗りを空にしてから置く。
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[cfg_attr(feature = "typescript", derive(ts_rs::TS))]
pub struct 大升を塗る {
    pub チャンク座標: チャンク座標,
...
/// 大升から地形を生成するとは、対象チャンクの大升の塗りから高さ格子と材質重み格子を作り直す操作コマンドのことである。
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[cfg_attr(feature = "typescript", derive(ts_rs::TS))]
pub struct 大升から地形を生成する {
    pub チャンク座標: チャンク座標,
}
...
/// 高さ場から大升を導くとは、対象チャンクの高さ格子と材質重みから大升の塗りを導いて下書きを置き換える操作コマンドのことである。
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[cfg_attr(feature = "typescript", derive(ts_rs::TS))]
pub struct 高さ場から大升を導く {
    pub チャンク座標: チャンク座標,
    pub 大升の一辺の升目数: u32,
}
