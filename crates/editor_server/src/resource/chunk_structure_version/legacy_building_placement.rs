...
pub(super) fn 旧版の建物一覧を最新の形へ変換する(旧一覧: Vec<種別を持つ旧版の建物配置>) -> Result<Vec<建物の配置>, チャンク構造移行エラー> {
...
mod tests {
    ...
    /// 移行先の文字列は別クレートにある建物定義の正本と独立に書かれているため、機械で突き合わせる。
    /// 正本の識別子一覧は外部アセットの置き場に依らず答えられるので、この照合はどの環境でも走る。
    #[test]
    fn 移行先の建物定義の識別子は正本に実在する() {
        let 識別子一覧 = blitz_asset_compiler::建物定義の識別子一覧();
        assert!(識別子一覧.contains(&家屋の移行先ID), "旧版の家屋の移行先{家屋の移行先ID}が建物定義の正本に無い(正本: {識別子一覧:?})");
    }
}
