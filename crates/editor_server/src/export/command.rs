...
//! (グローバルCLAUDE.md「デザインパターンの既定」)。初版の範囲は地形(高さ場)とチャンク目録のみであり、
//! 建物・散布・道路のエンジン変換はしない(参照: `_doc/設計/ゲーム開発用エディター基盤.md`「判断5」)。
...
impl ソースアセット書き出しコマンド {
    ...
    pub fn 実行する(&self) -> Result<書き出し結果, 書き出しエラー> {
        ...
        let 書いたチャンク数 = 項目一覧.len();

        let 目録本文 = 本文を組み立てる(&項目一覧);
