...
fn main() -> std::io::Result<()> {
    ...
    本文をファイルへ書き出す(&生成先ディレクトリ.join("生存確認契約.ts"), editor_server::契約ファイルの本文を組み立てる())?;
    本文をファイルへ書き出す(&生成先ディレクトリ.join("編集資源契約.ts"), editor_server::編集資源契約の本文を組み立てる())?;
    Ok(())
}

fn 本文をファイルへ書き出す(出力先: &PathBuf, 本文: String) -> std::io::Result<()> {
    std::fs::write(出力先, &本文)?;
    println!("{}", 出力先.display());
    Ok(())
}
