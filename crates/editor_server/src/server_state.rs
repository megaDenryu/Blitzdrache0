...
impl サーバー状態 {
    pub fn 生成する(リポジトリルート: &リポジトリルート, プロジェクトルート: &プロジェクトルート, 建物の格子の保存係: 建物の格子の保存係) -> Self {
        Self {
            保管庫: ファイル保管庫::生成する(プロジェクトルート),
            プロジェクト情報: プロジェクト情報応答::生成する(プロジェクトルート.パス文字列(), プロジェクトルート.プロジェクト名()),
    ...
    pub fn 建物の格子の保存係を借りる(&self) -> Result<MutexGuard<'_, 建物の格子の保存係>, Box<Response>> {
        self.建物の格子の保存係.lock().map_err(|誤り| Box::new(失敗応答を組み立てる(StatusCode::INTERNAL_SERVER_ERROR, "内部エラー", 誤り.to_string())))
    }
