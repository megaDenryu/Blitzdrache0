...
//! 型付き変換を持たず、`as`による縮小変換はリポジトリの規約が禁じているため、文字列の往復
//! (`crates/blitz_asset_compiler/src/loader/joint/value_read.rs`と同じ技法)で狭める。

use super::error::高さの切り出しエラー;

pub(super) fn 求める(世界の一辺メートル: f64, 軸あたりチャンク数: u32) -> Result<f32, 高さの切り出しエラー> {
    if 軸あたりチャンク数 == 0 {
        return Err(高さの切り出しエラー::数値変換に失敗("軸あたりチャンク数が0である".to_string()));
    }
    let チャンク一辺メートル = 世界の一辺メートル / f64::from(軸あたりチャンク数);
    if !チャンク一辺メートル.is_finite() {
        return Err(高さの切り出しエラー::数値変換に失敗(format!("チャンク一辺メートルが有限でない: {チャンク一辺メートル}")));
    }
    チャンク一辺メートル
        .to_string()
        .parse::<f32>()
        ...
        .ok_or_else(|| 高さの切り出しエラー::数値変換に失敗(format!("チャンク一辺メートルをf32へ狭められない: {チャンク一辺メートル}")))
}
