...
pub use resource::{
    チャンクの道路, チャンクの高さ編集, チャンク座標, チャンク構造, マザーハイトマップ, マテリアル台帳, マテリアル定義, 世界の区画割り, 位置3次元,
    地表材質の重み, 地表材質層, 大域世界構造, 層割当, 広域道路, 建物の配置, 建物を削除する, 建物を移動する, 建物を配置する, 建物基礎を平坦化する,
    建物種別, 急勾配を岩肌へベイクする, 散布の設定, 散布設定を変更する, 材質の筆致, 編集コマンド, 読み込んだチャンク構造の版,
