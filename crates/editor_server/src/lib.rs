...
pub use resource::{
    はめ口の値, コード進行, コード進行参照, チャンクの道路, チャンクの高さ編集, チャンク座標, チャンク構造, トラックの格子, トラックの種類,
    トラック定義, パターン, パターンID, パターンのステップ数, ベイ構造, マザーハイトマップ, マテリアル台帳, マテリアル定義, ミキサー設定,
    世界の区画割り, 位置3次元, 升目の宣言, 升目の屋根, 升目の床, 升目の座標, 升目の複体, 和音, 和音の種類, 地表材質の重み, 地表材質層,
    壁の外面の飾り, 壁の種類, 大域世界構造, 層割当, 広域道路, 建物の入口方向, 建物の外接箱, 建物の格子, 建物の格子の一覧項目,
    建物の格子の現在の形式版, 建物の格子の装飾, 建物の配置, 建物を削除する, 建物を移動する, 建物を配置する, 建物基礎を平坦化する, 建物外形カタログ,
    建物外形カタログの現在の形式版, 建物外形カタログ読み込みエラー, 建物外形定義, 建物定義ID, 建物定義の用途, 急勾配を岩肌へベイクする, 打楽器の種類,
    散布の個体, 散布の設定, 散布設定を変更する, 既定のコード進行, 既定のコード進行一覧, 曲の節, 材質の筆致, 楽器, 楽曲, 楽曲ID, 楽曲の版の移行エラー,
    楽曲の現在の形式版, 編集コマンド, 読み込んだ大域世界構造の版, 造成筆致, 造成筆致種別, 道路に合わせて切土盛土する, 道路を削除する, 道路を追加する,
    道路下を泥へベイクする, 道路対象, 道路点を削除する, 道路点を挿入する, 道路点を移動する, 道路点を追加する, 音の並び,
};
