...
use editor_server::{
    ...
    散布の個体, 散布の設定, 既定の大升の一辺の升目数, 見下ろし図の下書き,
};
...
fn チャンク構造を保存して読み戻すと一致する() {
    let (_一時, 保管庫) = common::保管庫を作る("chunk_structure_roundtrip");
...
fn 建物のなじみ半径が基礎半径未満なら保存を拒む() {
    let (_一時, 保管庫) = common::保管庫を作る("chunk_structure_building_invalid");
...
fn チャンクの高さ格子と材質重みを保存して読み戻すとバイト一致する() {
    let (_一時, 保管庫) = common::保管庫を作る("chunk_grids_roundtrip");
