...
async fn 格子から作った建物は配置されて実行時の部品群まで焼かれる() {
    if !common::外部アセットの置き場があるか() {
        println!("外部アセットの置き場が無いため、格子由来の建物の焼きの検査をスキップする");
        return;
    }
    let 一時 = common::一時プロジェクト::生成する("grid_building_bake");
    let ルーター = common::ルーターを作る(&一時);
    ...
    let 保存 = ルーター
        .clone()
        .oneshot(
            Request::put(format!("/api/建物/{}/格子", common::試験の建物定義の識別子))
                .header("content-type", "application/json")
                .body(Body::from(common::初期の格子のjson().to_string()))
                .unwrap(),
        )
        .await
        .unwrap();
    ...
    保管庫
        .チャンクの構造を検証して保存する(チャンク座標::生成する(0, 0), common::格子の建物を1件置いた構造())
        .unwrap();
    let 書き出し = ルーター
        .clone()
...
fn 書き出したチャンクソースを確かめる(一時: &common::一時プロジェクト) {
...
fn 焼いた部品群を確かめる(一時: &common::一時プロジェクト) {
