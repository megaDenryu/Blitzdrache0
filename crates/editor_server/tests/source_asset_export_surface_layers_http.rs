...
fn 一時プロジェクトを整える(識別子: &str) -> (common::一時プロジェクト, editor_server::ファイル保管庫) {
...
async fn 書き出しの応答の状態を得る(一時: &common::一時プロジェクト) -> StatusCode {
    common::ルーターを作る(一時)
...
async fn 書き出して地表層テクスチャ集を読む(一時: &common::一時プロジェクト) -> 地表層テクスチャ集 {
