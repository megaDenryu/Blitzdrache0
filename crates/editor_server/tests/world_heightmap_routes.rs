...
async fn 区画割り未保存での高さ格子putは400を返す() {
    let 一時 = common::一時プロジェクト::生成する("world_heightmap_no_layout");
...
async fn 高さ格子は区画割りの寸法どおりなら保存して取得できる() {
    let 一時 = common::一時プロジェクト::生成する("world_heightmap_roundtrip");
...
async fn 高さ格子が未保存なら204を返す() {
    let 一時 = common::一時プロジェクト::生成する("world_heightmap_get_empty");
    let 応答 = common::ルーターを作る(&一時)
        .oneshot(Request::get("/api/大域世界/高さ格子").body(Body::empty()).unwrap())
        .await
        .unwrap();
