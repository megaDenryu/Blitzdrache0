...
fn 格子のパス(一時: &common::一時プロジェクト) -> std::path::PathBuf {
    一時
        .ルート()
        .join("editor_data")
        .join("建物")
...
async fn 升目を1つも持たない格子は拒まれ正本が作られない() {
    let 一時 = common::一時プロジェクト::生成する("building_grid_empty");
...
async fn 経路と本文の建物定義の識別子が食い違う要求は拒まれる() {
    let 一時 = common::一時プロジェクト::生成する("building_grid_id_mismatch");
    let ルーター = common::ルーターを作る(&一時);
...
async fn 未保存の建物の格子はnullで返り一覧は空になる() {
    let 一時 = common::一時プロジェクト::生成する("building_grid_absent");
    let ルーター = common::ルーターを作る(&一時);
...
async fn 初期の格子は保存されて一覧とカタログに載る() {
    ...
    let 一時 = common::一時プロジェクト::生成する("building_grid_accept");
    let ルーター = common::ルーターを作る(&一時);
