...
async fn 未保存のチャンクへはマザーの同じ格子点が配られる() {
    let 一時 = common::一時プロジェクト::生成する("chunk_height_cutout_values");
    common::大域を一意な値で用意する(&一時);

    let (状態, 本体) = common::高さ格子を取得する(&一時, 1, 0).await;
    assert_eq!(状態, StatusCode::OK);
    let 高さ一覧 = common::高さ一覧へ解く(&本体);
...
async fn 隣り合う未保存チャンクの縁は一致する() {
    let 一時 = common::一時プロジェクト::生成する("chunk_height_cutout_seam");
    common::大域を一意な値で用意する(&一時);

    let (_, 左の本体) = common::高さ格子を取得する(&一時, 0, 0).await;
    let (_, 右の本体) = common::高さ格子を取得する(&一時, 1, 0).await;
    let 左 = common::高さ一覧へ解く(&左の本体);
    let 右 = common::高さ一覧へ解く(&右の本体);
    for 格子z in 0..チャンク一辺頂点数 {
...
async fn 大域が未保存なら従来どおり204を返す() {
    let 一時 = common::一時プロジェクト::生成する("chunk_height_cutout_no_mother");
...
async fn 大域の構造も未保存なら204を返す() {
    let 一時 = common::一時プロジェクト::生成する("chunk_height_cutout_empty_project");

    let (状態, _) = common::高さ格子を取得する(&一時, 0, 0).await;
    assert_eq!(状態, StatusCode::NO_CONTENT);
}
