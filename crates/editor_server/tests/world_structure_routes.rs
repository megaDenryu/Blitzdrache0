...
async fn 大域世界構造の初期状態はnullを返す() {
    let 一時 = common::一時プロジェクト::生成する("world_structure_get_null");
...
async fn 大域世界構造を保存して取得できる() {
    let 一時 = common::一時プロジェクト::生成する("world_structure_put_get");
    ...
    assert_eq!(取得データ, common::区画割りJson());
}

#[tokio::test]
async fn 一辺のメートルが0の区画割りは422を返し正本を変えない() {
    let 一時 = common::一時プロジェクト::生成する("world_structure_reject");
