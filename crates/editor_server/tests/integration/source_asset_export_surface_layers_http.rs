...
async fn 保存した層割当のとおりに層とタイルが結ばれる() {
    ...
    assert_eq!(テクスチャ集.層のタイル(0).材質名().文字列(), "rock_a");
    assert_eq!(テクスチャ集.層のタイル(1).材質名().文字列(), "dirt_a");
    assert_eq!(テクスチャ集.層のタイル(2).材質名().文字列(), "grass_a");
    assert_eq!(テクスチャ集.層のタイル(3).材質名().文字列(), "sand_a");
}
...
async fn 台帳を保存していないプロジェクトは既定の層割当で焼ける() {
    ...
    let テクスチャ集 = 書き出して地表層テクスチャ集を読む(&一時).await;
    assert_eq!(テクスチャ集.層のタイル(0).材質名().文字列(), "grass_a");
    assert_eq!(テクスチャ集.層のタイル(3).材質名().文字列(), "sand_a");
}
