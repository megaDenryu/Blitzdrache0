...
fn 旧版のチャンク構造のjson() -> serde_json::Value {
    serde_json::json!({
...
fn 建物種別を持つ旧版のチャンク構造のjson(種別: &str) -> serde_json::Value {
    serde_json::json!({
        "道路一覧": [],
        "建物一覧": [{
            "識別子": "old-building-1",
...
fn 旧版の大域世界構造のjson() -> serde_json::Value {
    serde_json::json!({
...
fn 道路が1本だけの旧版のチャンク構造は道路一覧1本として読める() {
    let (一時, 保管庫) = crate::common::保管庫を作る("legacy_chunk_structure");
    let 座標 = チャンク座標::生成する(0, 0);
    let 構造パス = 一時.ルート().join("editor_data/チャンク/0_0/構造.json");
    std::fs::create_dir_all(構造パス.parent().unwrap()).unwrap();
    std::fs::write(&構造パス, serde_json::to_vec_pretty(&旧版のチャンク構造のjson()).unwrap()).unwrap();
...
fn 旧版の家屋は一間四方の家の定義IDへ移行する() {
    let (一時, 保管庫) = crate::common::保管庫を作る("legacy_house_definition");
    let 座標 = チャンク座標::生成する(0, 0);
    let 構造パス = 一時.ルート().join("editor_data/チャンク/0_0/構造.json");
    std::fs::create_dir_all(構造パス.parent().unwrap()).unwrap();
    std::fs::write(
...
fn 対応定義の無い旧版の塔は明示的に拒否する() {
    let (一時, 保管庫) = crate::common::保管庫を作る("legacy_tower_rejected");
    let 座標 = チャンク座標::生成する(0, 0);
    let 構造パス = 一時.ルート().join("editor_data/チャンク/0_0/構造.json");
    std::fs::create_dir_all(構造パス.parent().unwrap()).unwrap();
...
fn 広域道路が1本だけの旧版の大域世界構造は広域道路一覧1本として読める() {
    let (一時, 保管庫) = crate::common::保管庫を作る("legacy_world_structure");
    let 構造パス = 一時.ルート().join("editor_data/大域世界/構造.json");
    std::fs::create_dir_all(構造パス.parent().unwrap()).unwrap();
    std::fs::write(&構造パス, serde_json::to_vec_pretty(&旧版の大域世界構造のjson()).unwrap()).unwrap();
