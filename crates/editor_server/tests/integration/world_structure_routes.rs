...
async fn 大域世界構造の初期状態はnullを返す() {
    ...
    let 応答 = crate::common::ルーターを作る(&一時).oneshot(Request::get("/api/大域世界/構造").body(Body::empty()).unwrap()).await.unwrap();
...
async fn 一辺のメートルが0の区画割りは422を返し正本を変えない() {
    ...
    let 応答 = crate::common::ルーターを作る(&一時)
        .oneshot(Request::put("/api/大域世界/構造").header("content-type", "application/json").body(Body::from(serde_json::to_vec(&不正データ).unwrap())).unwrap())
        .await
        .unwrap();
