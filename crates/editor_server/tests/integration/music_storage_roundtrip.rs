...
fn 保存前は無しを返し一覧も空である() {
    ...
    assert!(保管庫.楽曲を読む(&crate::common::名乗りを作る("試験の楽曲")).unwrap().is_none());
...
fn 保存して読み戻すと同じ値になる() {
    ...
    assert_eq!(保管庫.楽曲を読む(&crate::common::名乗りを作る("試験の楽曲")).unwrap(), Some(crate::common::楽曲の例()));
}

#[test]
fn 一覧は保存済みの名乗りを昇順で返す() {
    let (_一時, 保管庫) = crate::common::保管庫を作る("music_list");
    for 文字列 in ["ぬ の曲", "あ の曲", "た の曲"] {
        let mut 楽曲 = crate::common::楽曲の例();
        楽曲.名乗り = crate::common::名乗りを作る(文字列);
        保管庫.楽曲を検証して保存する(楽曲).unwrap();
    }
    let 一覧 = 保管庫.楽曲の一覧を読む().unwrap();
    assert_eq!(一覧, vec![crate::common::名乗りを作る("あ の曲"), crate::common::名乗りを作る("た の曲"), crate::common::名乗りを作る("ぬ の曲")]);
}
...
fn ファイル名と中の名乗りが食い違う正本は読みが拒む() {
    ...
    assert!(保管庫.楽曲を読む(&crate::common::名乗りを作る("別の名乗り")).is_err());
}
...
fn 伴奏のトラックへ弦楽合奏を置いた楽曲は保存の往復を通る() {
    ...
    assert_eq!(保管庫.楽曲を読む(&crate::common::名乗りを作る("試験の楽曲")).unwrap(), Some(楽曲));
}
...
fn 一覧はjson以外のファイルを飛ばす() {
    ...
    assert_eq!(保管庫.楽曲の一覧を読む().unwrap(), vec![crate::common::名乗りを作る("試験の楽曲")]);
}
