...
async fn 保存した家屋は中心基準座標を変換して高さ格子上へ部品群として焼かれる() {
    let 一時 = crate::common::一時プロジェクト::生成する("export_building");
        ...
        .チャンクの構造を検証して保存する(
            ...
            チャンク構造 {
                ...
                建物一覧: vec![建物の配置 {
                    ...
                    建物定義ID: editor_server::建物定義ID::生成する(crate::common::一間四方の家の識別子).unwrap(),
...
fn 焼いた建物を確かめる(一時: &crate::common::一時プロジェクト) {
