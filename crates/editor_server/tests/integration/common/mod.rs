...
pub use building_grid_fixture::{初期の格子のjson, 升目を空にした格子のjson, 外部アセットの置き場があるか, 格子の台帳を読む, 格子の建物を1件置いた構造, 試験の建物定義の識別子};
pub use building_outline_catalog_fixture::{カタログに無い識別子, 一間四方の家の識別子, 建物外形カタログを作る};
pub use chunk_height_cutout_fixture::{チャンク一辺頂点数, マザーの高さ, 大域を一意な値で用意する, 高さ一覧へ解く, 高さ格子を取得する};
pub use music_fixture::{パターンの名乗りを作る, 名乗りを作る, 打点のない格子, 最初のパターン, 楽曲の例, 独自の進行};
pub use scatter_export_fixture::{散布を載せて書き出す, 書き出したチャンクソースを読む, 焼いたチャンクのバイト列を読む};
pub use scatter_fixture::{
    個体の水平位置一覧, 建物が散布を避ける距離メートル, 建物までの距離, 散布の個体一覧を作る, 散布を載せたチャンク構造を保存する, 検査のチャンク一辺メートル, 道路の中心線までの距離, 道路の散布除外バッファメートル
};
pub use source_asset_export_fixture::{
    エディターの区画割り, フォックスのソースを配置する, マザーを一意な値で保存する, 地表層のタイルを配置する, 大域世界を保存する, 小さな区画割り, 建物を据えられる区画割り, 零のマザーを保存する
};
...
impl 一時プロジェクト {
    ...
    /// 文字列が1つであることを呼び出し側の規律へ任せず、ここで落とす。
    pub fn 生成する(識別子: &str) -> Self {
        static 使った識別子: Mutex<BTreeSet<String>> = Mutex::new(BTreeSet::new());
        assert!(使った識別子.lock().unwrap().insert(識別子.to_string()), "一時プロジェクトの識別子{識別子}を2箇所が使っている。試験の実行ファイルの中で1つに定める");
...
pub fn ルーターを作る(一時プロジェクト: &一時プロジェクト) -> editor_server::経路正規化アプリ {
    let カタログのファイル = blitz_asset_compiler::建物外形カタログのファイル::リポジトリルートから生成する(一時プロジェクト.リポジトリルート().パス());
    let 保存係 = match editor_server::建物の格子の保存係::起動時の材料から生成する(&一時プロジェクト.プロジェクトルート(), カタログのファイル, 建物外形カタログを作る()) {
        Ok(保存係) => 保存係,
        Err(原因) => panic!("一時プロジェクトの建物の格子の台帳を読めない: {原因}"),
    };
    editor_server::ルーターを組み立てる(editor_server::サーバー状態::生成する(&一時プロジェクト.リポジトリルート(), &一時プロジェクト.プロジェクトルート(), 保存係))
}
