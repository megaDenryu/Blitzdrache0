...
async fn 建物外形カタログは形式版と定義一覧を返す() {
    ...
    let 応答 = crate::common::ルーターを作る(&一時).oneshot(Request::get("/api/建物外形カタログ").body(Body::empty()).unwrap()).await.unwrap();
    ...
    assert!(本文["建物定義一覧"][0]["高さメートル"].is_null(), "外接箱から導ける高さを二重に持っている");
}
