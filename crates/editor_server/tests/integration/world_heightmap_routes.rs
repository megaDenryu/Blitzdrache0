...
async fn 区画割り未保存での高さ格子のPUT要求は400を返す() {
    ...
    let 応答 = crate::common::ルーターを作る(&一時)
        .oneshot(Request::put("/api/大域世界/高さ格子").header("content-type", "application/octet-stream").body(Body::from(vec![0u8; 4])).unwrap())
        .await
        .unwrap();
...
async fn 高さ格子は区画割りの寸法どおりなら保存して取得できる() {
    ...
    let 応答 = crate::common::ルーターを作る(&一時)
        .oneshot(Request::put("/api/大域世界/高さ格子").header("content-type", "application/octet-stream").body(Body::from(バイト列.clone())).unwrap())
        .await
        .unwrap();
    assert_eq!(応答.status(), StatusCode::NO_CONTENT);

    let 応答 = crate::common::ルーターを作る(&一時).oneshot(Request::get("/api/大域世界/高さ格子").body(Body::empty()).unwrap()).await.unwrap();
