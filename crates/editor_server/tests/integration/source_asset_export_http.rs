...
async fn 正常な書き出しはファイル数と出力先を返しチャンク目録を読み戻せる() {
    let 一時 = crate::common::一時プロジェクト::生成する("export_ok");
    ...
    for 項目 in &項目一覧 {
        ...
        assert_eq!(ソース["建物の格子一覧"], serde_json::json!([]));
...
async fn 次回書き出す目録は既存のエディター世界の更新済み目録と一致する() {
    let 一時 = crate::common::一時プロジェクト::生成する("export_editor_directory");
    let 保管庫 = editor_server::ファイル保管庫::生成する(&一時.プロジェクトルート());
    let 区画割り = crate::common::エディターの区画割り();
    crate::common::大域世界を保存する(&保管庫, 区画割り);
