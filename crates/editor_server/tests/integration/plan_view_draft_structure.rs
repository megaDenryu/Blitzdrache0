...
//! 粗マスの一辺が格子解像度を割り切らない保存の拒否の3件を確かめる。
...
use editor_server::{チャンク座標, チャンク構造, プロジェクト保管庫, 地表材質層, 平面の位置, 散布の設定, 既定の粗マスの一辺の升目数, 等高線, 粗マスの塗り, 見下ろし図の下書き};
use tower::ServiceExt;

fn 下書きを載せた構造(粗マスの一辺の升目数: u32) -> チャンク構造 {
    チャンク構造 {
        道路一覧: Vec::new(),
        建物一覧: Vec::new(),
        散布: 散布の設定 {
...
fn 下書きを持つチャンク構造を保存して読み戻すと一致する() {
    let (_一時, 保管庫) = crate::common::保管庫を作る("plan_view_draft_roundtrip");
    let 座標 = チャンク座標::生成する(1, -2);
    let 構造 = 下書きを載せた構造(既定の粗マスの一辺の升目数);
...
fn 下書きを持たない旧版は空の下書きとして読める() {
    let (一時, 保管庫) = crate::common::保管庫を作る("plan_view_draft_legacy");
    ...
    assert_eq!(読み込み結果.見下ろし図の下書き, 見下ろし図の下書き::空の下書きを作る(既定の粗マスの一辺の升目数));
}

#[tokio::test]
async fn 粗マスの一辺が格子解像度を割り切らない構造の保存は拒まれる() {
    let 一時 = crate::common::一時プロジェクト::生成する("plan_view_draft_indivisible");
