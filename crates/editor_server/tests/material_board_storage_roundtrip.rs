...
fn マテリアル台帳は保存前は無しを返す() {
    let (_一時, 保管庫) = common::保管庫を作る("material_board_none");
    assert!(保管庫.マテリアル台帳を読む().unwrap().is_none());
}

#[test]
fn マテリアル台帳を保存して読み戻すと一致する() {
    let (_一時, 保管庫) = common::保管庫を作る("material_board_roundtrip");
...
fn 層割当が一覧に無い材質名を参照していると保存を拒む() {
    let (_一時, 保管庫) = common::保管庫を作る("material_board_invalid_reference");
    let mut 不正な台帳 = 台帳例();
...
#[test]
fn 材質名が重複していると保存を拒む() {
    let (_一時, 保管庫) = common::保管庫を作る("material_board_duplicate_name");
    let mut 不正な台帳 = 台帳例();
