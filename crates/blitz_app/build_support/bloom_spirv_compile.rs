//! 光のにじみピラミッド(判断41)のシェーダーをSPIR-Vへコンパイルする。
...
pub(super) fn 縮小側をコンパイルする(slangc: &スラングコンパイラの所在, ソース絶対パス: &Path, 出力先ディレクトリ: &Path) -> Result<(), String> {
    エントリ一覧をコンパイルする(slangc, ソース絶対パス, 出力先ディレクトリ, &縮小側エントリ一覧)
}

pub(super) fn 拡大側をコンパイルする(slangc: &スラングコンパイラの所在, ソース絶対パス: &Path, 出力先ディレクトリ: &Path) -> Result<(), String> {
