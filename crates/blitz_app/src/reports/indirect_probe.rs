...
pub(crate) struct 照合の材料<'a> {
    ...
    pub(crate) 太陽方向: [f32; 3], // 太陽から見た向き。ワールドの向きを太陽相対座標へ写すフレームがこの向きから決まる
}
...
fn 板1枚を照合する(材料: &照合の材料<'_>, 期待の材料: &expected_color::期待の材料<'_>, 板: &plates::板の宣言, 番号: usize) -> Result<(), report_error::代表板の照合エラー> {
    let 投影 = projection::板の中心を投影する(材料.視点情報, 板.中心, 材料.大域ずらし量, 材料.画像.幅(), 材料.画像.高さ())?;
    let 期待 = expected_color::期待を求める(期待の材料, 板, 投影.視線);
    let 実測 = 材料.画像.ピクセル(投影.横, 投影.縦).ok_or(report_error::代表板の照合エラー::画素が画像の外にある { 横: 投影.横, 縦: 投影.縦 })?;
