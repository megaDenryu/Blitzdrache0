...
pub(crate) fn 参照比較を表示する(比較: &布の参照比較, 読み戻し: Option<Result<Option<布の読み戻し>, レンダラーエラー>>) {
    lines::題材を出す(比較);
    let 読み戻し = match 読み戻し {
    ...
    // 硬さの記録はCPUだけで長く回すため、題材の形で分ける。垂直に吊るした布は距離の硬さ(反復回数と刻み幅)、水平に敷いた布は曲げの硬さを読む。
    match 比較.題材().題材の形 {
        参照比較の題材の形::垂直に吊るして上端の行を固定 => stiffness::硬さを出す(比較),
        参照比較の題材の形::水平に敷いて上端の行を固定 | 参照比較の題材の形::水平に敷いて上端の行と右端の列を固定 | 参照比較の題材の形::水平に敷いて上端の左の一点を固定 => {
            bending_stiffness::曲げの硬さを出す(比較)
        }
    }
}
