...
//!
//! 同じ入力で2回走らせて突き合わせるのは、3方式の数値決定性(グラフ彩色と二段階は書く先が自分の拘束と自分の点だけであり、固定小数の
//! 整数の原子加算は結合的でスレッドの順に依らない。数値決定性を持たないのは浮動小数の原子加算だけである)を事実として残すためである。CPUの参照計算との突き合わせは短い実行(比較の刻み数)で別に走らせる。長い実行では単精度の演算順の
...
//! 突き合わせは`comparison`、指紋は`sha256`、行の文言は`lines`が持つ。
