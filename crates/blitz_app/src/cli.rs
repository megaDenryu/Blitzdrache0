...
pub(crate) use types::{布モード, 起動モード, 起動設定};
...
/// `--frames N` `--shader-source <path>` `--scene <id>` `--asset-root <dir>` `--unlit` `--particles`
/// `--report-gpu-times` `--dev-ui` `--dump-frame <base>` `--no-post` `--exposure <倍率>` `--blend <0..1>` `--cloth` `--cloth-cape` を解析する。
