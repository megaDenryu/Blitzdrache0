...
pub(super) fn 名前からidを参照する(命令一覧: &[命令], 名前: &str) -> Result<u32, シェーダー中間表現の読み解きエラー> {
    for 命令 in 命令一覧 {
        if 命令.命令コード != 命令コード_名前指定 || 命令.語一覧.is_empty() {
            continue;
        }
...
pub(super) fn 位置の組み込み出力のidを参照する(命令一覧: &[命令]) -> Result<u32, シェーダー中間表現の読み解きエラー> {
    命令一覧
        .iter()
        .filter(|命令| 命令.命令コード == 命令コード_装飾指定 && 命令.語一覧.len() >= 3)
        .find(|命令| 命令.語一覧[1] == 装飾種別_組み込み変数 && 命令.語一覧[2] == 組み込み変数種別_位置)
        .map(|命令| 命令.語一覧[0])
...
pub(super) fn 装飾の付いたidを集める(命令一覧: &[命令], 装飾: u32) -> Vec<u32> {
    命令一覧
        .iter()
        .filter(|命令| 命令.命令コード == 命令コード_装飾指定 && 命令.語一覧.len() >= 2 && 命令.語一覧[1] == 装飾)
        .map(|命令| 命令.語一覧[0])
        .collect()
}
