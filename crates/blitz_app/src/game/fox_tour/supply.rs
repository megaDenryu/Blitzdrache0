...
impl キツネの場所巡りの配線 {
    ...
    pub(in crate::game) fn カメラと高さ場の読み口を借りる(&mut self) -> (&mut プレイヤーカメラの配線, &高さ場の読み口) {
    ...
    pub(in crate::game) fn 進行の要約を作る(&self) -> ゲーム進行の要約 {
        ゲーム進行の要約 {
            ...
            移動の問い合わせ件数: self.移動の記録.問い合わせ件数の要約(),
            刻みの所要時間: self.移動の記録.刻みの所要時間の要約(),
            問い合わせ件数別の刻みの所要時間: self.移動の記録.問い合わせ件数別の刻みの所要時間の要約(),
        }
    }
}
