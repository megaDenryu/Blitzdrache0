...
//! 移動の分は移動状態・直前の刻みの観測(水平の速さ・問い合わせ件数)・評価が終わらずに動かなかった刻みの数、カメラの分は系統・
//! 直前の描画のカメラ・評価が終わらずに保った描画の数である。
...
pub(crate) struct 移動の計器 {
    ...
    pub(crate) 評価が終わらずに動かなかった刻みの数: u32, // 問い合わせが評価できない領域を含み、位置も移動状態も動かさなかった刻みの累計
}

// 直前の刻みの移動の観測。まだ1度も刻んでいない状態をそのまま選択肢で持つ。
#[derive(Debug, Clone, Copy, PartialEq)]
...
pub(crate) struct カメラの計器 {
    ...
    pub(crate) 評価が終わらずに保った描画の数: u32, // 掃引の答えが評価できない領域を含み、前の表示距離を保った描画の累計
}

// 直前の描画で決めたカメラの値。まだ1度も決めていない描画をそのまま選択肢で持つ。
#[derive(Debug, Clone, Copy, PartialEq)]
pub(crate) enum 直前の描画のカメラ {
    まだ描いていない,
    描いた { 理想距離: メートル, 表示距離: メートル, 判定: 遮蔽の判定 },
}
