//! 読み戻し画像のピクセル判定。中央=三角形色、隅=クリアカラーをsRGB8bit・許容誤差±8で判定する。
...
const 初期三角形色線形値: [f32; 3] = [1.0, 0.45, 0.1];
const 書き換え後三角形色線形値: [f32; 3] = [0.1, 0.9, 0.2];

pub(crate) fn ピクセルを判定する(
    画像: &読み戻し画像,
    アクション: スモークアクション,
) -> Result<(), 起動エラー> {
    let 三角形期待色 = match アクション {
        スモークアクション::初期色判定 => 初期三角形色線形値,
        スモークアクション::最終判定 => 書き換え後三角形色線形値,
        _ => return Ok(()),
    };

    let 中央x = 画像.幅() / 2;
    let 中央y = 画像.高さ() / 2;
    比較する(画像, 中央x, 中央y, 三角形期待色, "中央(三角形)")?;
