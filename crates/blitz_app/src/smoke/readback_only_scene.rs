...
/// 判定する入口は順に`instance-*`・`prop-draw`と`village-draw`と`cluster-lights`の屋内・`multi-material-draw`・`indirect-probe`・`terrain-visual`・`texture-compression`・`game-fox-tour`・`cluster-lights`の夜である。
