...
pub(crate) enum 起動エラー {
    ...
    #[error("地表層テクスチャ集を持たない世界のシーンに、地表の層の重ね合わせの材質があった")]
    地表層テクスチャ集を持たない世界の地表材質,
    #[error("地表層テクスチャ集の読み込みに失敗した: {0}")]
    地表層テクスチャ集読込失敗(#[from] blitz_engine::surface_layer_textures::地表層テクスチャ集読込エラー),
    ...
    表面流の計算条件不正(#[from] blitz_sim::表面流の計算条件エラー),
    ...
    #[error("--local-light-countの指定を読まない世界である: {0}")]
    局所光の件数の上書きを読まない世界(String),
    ...
    #[error("チャンク目録の差し替えに失敗した: {0}")]
    チャンク目録差し替え失敗(#[from] blitz_engine::チャンク目録差し替えエラー),
    #[error("チャンクストリーミングにアセットカタログが必要だが、まだ構築されていない")]
    ストリーミングカタログ未構築,
    #[error("必要チャンク集合の中心をゲームのプレイヤーの大域位置で決める配線だが、そのフレームにゲームが位置を1つも供給しなかった")]
    必要チャンク集合の中心にするプレイヤーの大域位置が未供給,
    ...
    #[error("チャンク座標(x={}, z={})の描画束IDが起動時シーンまたは遠景の予約束IDと衝突する", .0.x(), .0.z())]
    ストリーミング束ID衝突(blitz_engine::チャンク座標),
    #[error("束の描画入力を組み立てられない: {0}")]
    描画入力不正(#[from] crate::app::scene_load::描画入力エラー),
    ...
    ピクセル判定失敗(String),
    #[error("そのフレームの露出倍率を組めない: {0}")]
    露出倍率不正(#[from] crate::cli::露出倍率エラー),
    #[error("世界の形を尋ねる口の実装が答えられない: {0}")]
    世界の形を尋ねる口の失敗(#[from] crate::world_execution::contract::世界の形を尋ねる口の実装エラー),
