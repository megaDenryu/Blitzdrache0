...
fn 読込設定の三値を別々に変えられる() {
    ...
    for (名前, 値) in [("--streaming-loader-workers", "3"), ("--streaming-request-capacity", "96"), ("--streaming-completion-capacity", "12")] {
        let 引数 = 語(値);
...
fn 固定経路の端点と速さを変えられる() {
    ...
    assert_eq!(
        (設定.始点東メートル, 設定.始点南メートル, 設定.終点東メートル, 設定.終点南メートル, 設定.一フレーム移動量メートル),
...
fn 起動引数の全配線が二次元経路と読込設定へ届く() {
    ...
    assert_eq!((経路.始点東メートル, 経路.始点南メートル, 経路.終点東メートル, 経路.終点南メートル), (-3000.0, -4000.0, 3000.0, 4000.0));
    assert_eq!(設定.ストリーミング.読込, チャンク読込設定::生成する(3, 96, 12).ok().unwrap_or(チャンク読込設定::既定値()));
}
