...
//!
//! 知らない語を既定の条件へ落とさず失敗にするのは、文字列の誤りが「別の条件で通った検収」になるためである。
...
pub(super) fn 引数を処理する(引数: &mut Iter<String>) -> Result<遠方環境の検収条件, 起動引数エラー> {
    ...
    遠方環境の検収条件::語から選ぶ(値.as_str()).ok_or_else(|| 起動引数エラー::遠方環境の検収条件不正(format!("diffuse・specular-level・specular-faceのどれでもない: {値}")))
}
