//! `--cloth-xpbd-reference-shape <文字列>`が選ぶ、XPBDの参照比較の布の敷き方と固定の形。垂直に吊るした平らな布は面の内側の力しか受けず
    ...
    pub(crate) fn 文字列から解析する(文字列: &str) -> Result<Self, 起動引数エラー> {
        match 文字列 {
            ...
            _ => Err(起動引数エラー::参照比較の題材の形不正(format!(
                "vertical-top-row / horizontal-top-row / horizontal-two-edges / horizontal-one-point のいずれかでない({文字列})"
            ))),
        }
    }

    /// 報告の行へ書く文字列。`文字列から解析する`の逆である。
