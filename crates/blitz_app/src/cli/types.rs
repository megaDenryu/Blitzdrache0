...
/// CLI引数から得た起動設定一式。
///
/// 各フィールドはCLI引数1つと既定値に対応する。フィールド名で対応させて詳細を次に記す。
///
/// - `シェーダーの入口ファイル`: ホットリロードの監視対象となる入口ファイル。既定は`shaders/scene.slang`(存在しなければ監視無効)。`import`で参照される他の.slangファイル(`pbr.slang`等)はこの入口ファイルと同じディレクトリから解決されるため、個別指定はしない。ディレクトリ全体をmtime走査で監視する。
/// - `シーン`: 表示するシーン。綴りは実行時アセットの安定IDであり、既定は平面板の世界(常に存在し決定的)。
/// - `アセットの置き場`: カタログ・チャンク目録・生成物がぶら下がる実行時アセットの置き場。既定は`target/runtime_assets`。
/// - `描画対象の並べ方`: 起動時シーンの描画対象の並べ方。`--object-count`が複製する件数を、`--reverse-draw-order`が束の中の走査順を決める。
/// - `ライティング`: 世界が宣言したライティングへの起動側の上書き。`--unlit`と`--local-light-count`が据える。
/// - `gpu時間報告`: `--report-gpu-times`指定でtrue。既定はfalse(パス別GPU時間の終了時コンソール出力、判断30)。
/// - `gpu時間のフレーム別生値報告`: `--report-gpu-frame-times`指定でtrue。既定はfalse(パス別GPU時間の窓へ入る前の生の値をフレーム別に全件出す)。分位を独立に計算し直して報告値と突き合わせる検収だけが使う指定であり、行数が測定フレーム数×区間数まで増える。
/// - `大気のベイク済み画像パス数報告`: `--report-atmosphere-passes`指定でtrue。既定はfalse(フレームごとの大気のベイク済み画像生成パス本数を終了時に出力する)。
/// - `フレーム時間報告`: `--report-frame-times`指定でtrue。最初の120フレームを除いたCPU側フレーム間隔分布を終了時に出力する。
/// - `gpuメモリ報告`: `--report-memory`指定でtrue。Vulkan専用メモリ確保の現在数・上限・用途別量を終了時に出力する。
/// - `描画発行報告`: `--report-draw-issue`指定でtrue。最終フレームのパス別描画発行数・候補数・可視数・個体数を終了時に出力する。
/// - `太陽角度報告`: `--report-sun-angle`指定でtrue。その実行が使った太陽の高度と方位を終了時に出力する。
/// - `キャスター距離分布報告`: `--report-caster-distance`指定でtrue。最終フレームのキャスター候補をカメラからの距離帯へ振り分けた分布を終了時に出力する。
/// - `インスタンス区間報告`: `--report-instance-sections`指定で、可視個体の選別とレンダラーCPU区間を終了時に出す。物量計測専用なのでフレーム時間報告へ相乗りしない。
/// - `インスタンス可視判定有効`: `--no-instance-cull`指定でfalse。既定はtrue。falseは全個体を描くため、可視判定の有無で読み戻し画像を比べる検収が成立する。
/// - `インスタンス段選択有効`: `--no-instance-lod`指定でfalse。既定はtrue。falseは全個体を最詳細段で描くため、段の選択の有無で読み戻し画像を比べる検収が成立する。
/// - `インスタンス影キャスター有効`: `--no-instance-shadow`指定でfalse。既定はtrue。falseはどの距離区分も個体を影の候補にしないため、シャドウパスに残る費用が地形と添付処理だけになる計測用の対照が作れる。
/// - `影キャスター全体有効`: `--no-shadow-casters`指定でfalse。既定はtrue。falseは地形も個体も布も距離区分のパスへ積まないため、シャドウパスに残る費用が深度配列の消去と保存だけになる。個体だけを外す指定との差が地形の描画費用になる。
/// - `遠景影キャスター有効`: `--no-distant-shadow`指定でfalse。遠景束だけを影の距離区分へ積まない帰属診断に使う。
/// - `個体詳細段探査刻み`: `--lod-probe-step <メートル>`指定でSome。1フレームおきにカメラをZ方向へこの距離だけ動かして戻し、段の境界をまたぐ往復を作る。
/// - `開発ui初期有効`: `--dev-ui`指定でtrue。既定はfalse(開発用UIの起動時有効化、判断34。実行中はF3でも切替可能)。
/// - `画素診断`: シーンの画素段が本番の色の代わりに出す診断。`--debug-cascade-bands`が距離区分の可視化を、`--debug-shadow-loss`が影可視度と受光距離帯の計器を選ぶ。既定は出さない。
/// - `フレームダンプ先`: `--dump-frame <ベース名>`と`--dump-hdr-frame <ベース名>`指定で、最終フレーム(--frames必須)の読み戻しを書き出す。書き出す画像と外部形式と、同時指定を拒むことは指定の型が持つ。既定は指定なし。
/// - `時間帯`: 空と時刻の起動指定。空を描くかどうかは既定では世界の方針が決め、この指定はその上書きである。
/// - `シャドウ計測`: 多段シャドウの費用を測るための起動指定。指定が無ければ本番の多段設定と同じ値になる。
...
/// - `布モード`: 布シミュレーションの方式(判断52・56)。`--cloth`=吊るし布(全シーン可)、`--cloth-cape`=マント(fox限定、キャラ追従)、`--cloth-xpbd-reference <コンプライアンス>`=CPUの参照計算と突き合わせる吊るし布(Issue #36の検証)。既定はなし。
...
/// - `ストリーミング`: チャンクストリーミングの有効化と容量上限。既定は無効(既存のスモークとベンチの挙動を変えない)。
/// - `深度プリパス方式`: 深度プリパスを積むかどうかと、色パスの深度の比べ方。`--depth-prepass`が据える。未指定を`使わない`と同じ値にしないのは、局所可視性補正を宣言した世界が未指定なら方式を引き上げ、明示の`none`とは衝突として落とすためである(2つを1つの値へ潰すと、引き上げが利用者の明示を黙って覆す)。
/// - `時間再構成`: 時間再構成を効かせるかどうかの上書き。既定は世界の宣言に従う。`--no-taa`が使わない側へ落とす。
/// - `平行移動`: 世界全体とカメラへ加える平行移動。大域ずらし量はカメラ大域原点・照明の大域位置と、起動時シーンとチャンク束の描画の基準原点へ効く。基準原点は所有チャンクから導出した後にこの平行移動を合成する。チャンク格子と必要集合の判定は平行移動を含まない世界座標で行うため、位置源には加えない。
...
pub(crate) struct 起動設定 {
    ...
    pub(crate) 布モード: 布モード,                                       // --cloth / --cloth-cape / --cloth-xpbd-reference。既定なし
    ...
    pub(crate) 平行移動: 平行移動起動設定,                                                      // 世界全体とカメラへ加える大域平行移動
    pub(crate) 遊ぶゲーム: 遊ぶゲームの指定,                                                    // --game。無指定はゲームを遊ばない
}
