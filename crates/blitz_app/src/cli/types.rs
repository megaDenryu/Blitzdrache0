...
pub(crate) struct 起動設定 {
    ...
    /// `--report-instance-sections`指定で、可視個体の選別とレンダラーCPU区間を終了時に出す。物量計測専用なのでフレーム時間報告へ相乗りしない。
    ...
    /// `--no-distant-shadow`指定でfalse。遠景束だけを影の距離区分へ積まない帰属診断に使う。
    pub(crate) 遠景影キャスター有効: bool,
