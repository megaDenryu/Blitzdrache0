...
pub(crate) struct 起動設定 {
    ...
    /// `--no-shadow-casters`指定でfalse。既定はtrue。falseは地形も個体も布も距離区分のパスへ積まないため、
    /// シャドウパスに残る費用が深度配列の消去と保存だけになる。個体だけを外す指定との差が地形の描画費用になる。
    ...
    /// 深度プリパスを積むかどうかと、色パスの深度の比べ方。既定は積まない(本番の絵をこの軸で変えない)。`--depth-prepass`が据える。
    pub(crate) 深度プリパス方式: blitz_render::深度プリパス方式,
