...
//!
//! 大半の選択肢は引数の値そのものが不正な場合を表すが、`局所可視性補正が深度プリパスを要る`と
...
pub(crate) enum 起動引数エラー {
    ...
    段差走査範囲不正(String),
    #[error("局所可視性補正を宣言した世界へ--depth-prepass noneを指定した。この組み合わせでは深度が消去値のままになり、局所可視性補正が1画素も効かない")]
    局所可視性補正が深度プリパスを要る, // 引数は読めたが世界の宣言と組み合わせられない場合(深度プリパスの深度を読むため)
    #[error("--depth-prepass引数が不正だった: {0}")]
    深度プリパス方式不正(String),
    #[error("局所可視性補正の検収の指定が不正だった: {0}")]
    局所可視性の検収指定不正(String),
    #[error("時間再構成を宣言した世界へ--no-postを指定した。この組み合わせでは再構成の結果を書くHDR中間画像が無い。--no-taaを重ねて時間再構成を切ること")]
