...
use super::{参照比較の床の下の固定点, 参照比較の題材の形, 布のコンプライアンス指定, 布の曲げのコンプライアンス指定, 布モード, 起動引数エラー};
...
pub(super) fn 布のxpbd参照比較の引数を処理する(引数: &mut Iter<String>, 引数名: &str, 床の下の固定点: 参照比較の床の下の固定点) -> Result<布モード, 起動引数エラー> {
    let 値 = 次の値を読む(引数, 引数名, 起動引数エラー::布のコンプライアンス不正)?;
    Ok(布モード::XPBD参照比較 {
        コンプライアンス: 布のコンプライアンス指定::文字列から解析する(値)?,
        床の下の固定点,
...
pub(super) fn 布の曲げのコンプライアンスの引数を処理する(引数: &mut Iter<String>, 布モード: 布モード) -> Result<布モード, 起動引数エラー> {
    let 値 = 次の値を読む(引数, "--cloth-xpbd-reference-bending", 起動引数エラー::布の曲げのコンプライアンス不正)?;
    let 曲げ = 布の曲げのコンプライアンス指定::文字列から解析する(値)?;
    match 布モード {
        布モード::XPBD参照比較 {
            コンプライアンス, 床の下の固定点, 題材の形,
        ..
        } => Ok(布モード::XPBD参照比較 {
            コンプライアンス,
            床の下の固定点,
        ...
        _ => Err(起動引数エラー::布の曲げのコンプライアンス不正("--cloth-xpbd-referenceの後に指定する".to_string())),
    }
}

/// `--cloth-xpbd-reference-shape <文字列>`。既に参照比較の方式になっている布モードへ題材の形(敷き方と固定)を与える。
pub(super) fn 参照比較の題材の形の引数を処理する(引数: &mut Iter<String>, 布モード: 布モード) -> Result<布モード, 起動引数エラー> {
    let 値 = 次の値を読む(引数, "--cloth-xpbd-reference-shape", 起動引数エラー::参照比較の題材の形不正)?;
    let 形 = 参照比較の題材の形::文字列から解析する(値)?;
    match 布モード {
        布モード::XPBD参照比較 {
            コンプライアンス,
            床の下の固定点,
        ...
        _ => Err(起動引数エラー::参照比較の題材の形不正("--cloth-xpbd-referenceの後に指定する".to_string())),
    }
}
