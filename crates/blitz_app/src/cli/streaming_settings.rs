...
use blitz_engine::{ストリーミングメモリ量, チャンク座標, チャンク読込設定, 固定経路};
...
const 既定経路始点Xメートル: f64 = -150.0;
const 既定経路終点Xメートル: f64 = 250.0;
const 既定一フレーム移動量メートル: f64 = 5.0;

#[derive(Debug, Clone, Copy, PartialEq)]
pub(crate) struct 固定経路起動設定 {
    pub(crate) 始点xメートル: f64,
    pub(crate) 終点xメートル: f64,
