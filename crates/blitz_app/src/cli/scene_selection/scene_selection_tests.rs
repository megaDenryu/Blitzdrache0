//! 文字列から世界の種別への見分けの検査。名前つきの世界が群の既定より先に選ばれること、群の接頭辞が効くこと、
//! どちらにも当たらない文字列が方針を1つも持たない選択肢へ落ちることを確かめる。
...
fn 床を同居させた植生の検収世界が3つとも名前で選ばれる() {
    for 文字列 in ["vegetation_cull", "vegetation_shadow_range", "vegetation_single"] {
        assert_eq!(種別を解く(文字列), 世界の種別::植生の検収世界(植生の検収世界の種別::床を同居させた), "{文字列}");
    }
}

#[test]
fn 接頭辞の群が名前つきでない文字列を受ける() {
...
fn 材質差し替えの検収世界だけが自分の差し替えの対を持つ() {
    let 材質差し替え = 起動時シーン::文字列から解析する("material_reload").unwrap().差し替える生成物の対();
