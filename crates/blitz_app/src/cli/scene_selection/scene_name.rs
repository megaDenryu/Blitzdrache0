...
pub(super) const キツネのシーン名: &str = "fox";
/// 材質差し替えの検収世界のシーン名。上書きされる側の生成物の安定IDでもある。
pub(super) const 材質差し替えの検収世界のシーン名: &str = "material_reload";
pub(super) const 間接照明の検収世界のシーン名: &str = "indirect_probe";
pub(super) const 両視錐台外の群の検収世界のシーン名: &str = "instance_all_culled";
pub(super) const 地形の起点世界のシーン名: &str = "terrain_origin";
/// 参照: `crates/blitz_asset_compiler/examples/generate_source_assets/terrain_visual_world.rs`
pub(super) const 地形の目視見本世界のシーン名: &str = "terrain_visual";
/// 参照: `crates/blitz_asset_compiler/src/runtime_compilation/world/fox_tour_declaration.rs`
pub(super) const 地形の場所巡り世界のシーン名: &str = "terrain_fox_tour";
pub(super) const 地形のエディター世界のシーン名: &str = "terrain_editor_world";
...
pub(super) const 部品で組んだ木の並びの小物世界のシーン名: &str = "prop_part_tree_row";
/// 参照: `crates/blitz_asset_compiler/examples/generate_source_assets/part_frame_row_world.rs`
pub(super) const 部品で組んだ一間四方の骨格の並びの小物世界のシーン名: &str = "prop_part_frame_row";
/// 局所光の件数の上書きを読む2つの世界の一方。失敗の文面が`--scene`へ渡すべき値をそのまま出すため公開する。
