...
use crate::cli::{
    世界の種別, 地形の夜灯り世界のシーン名, 地形世界の種別, 小物世界の種別, 局所光の件数の起動指定, 影を落とす灯の件数の起動指定, 石の小屋の屋内の小物世界のシーン名, 起動時シーン
};
...
fn 局所光の件数の上書きを読まない世界であることを確かめる(シーン: &起動時シーン, 局所光の件数: 局所光の件数の起動指定) -> Result<(), 起動エラー> {
    match 局所光の件数 {
        局所光の件数の起動指定::世界の宣言に従う => Ok(()),
        局所光の件数の起動指定::件数を上書きする(_) => Err(起動エラー::局所光の件数の上書きを読まない世界(format!(
            "{}(読むのは{地形の夜灯り世界のシーン名}と{石の小屋の屋内の小物世界のシーン名}だけである)",
            シーン.安定id()
        ))),
    }
}
