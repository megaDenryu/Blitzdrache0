...
pub(in crate::app) struct シーン読込回数 {
    pub(in crate::app) 総数: u64,
    pub(in crate::app) 起動後: u64,
}

/// 終了時の報告が読む、検収の観測の統計。
pub(in crate::app) struct 検収の観測の統計 {
    pub(in crate::app) フレーム時間統計: Option<フレーム時間統計>, // `--report-frame-times`が指定され、ウォームアップ後のフレームがあれば`Some`
    pub(in crate::app) 選別の区間統計: Option<フレーム時間統計>,   // `--report-instance-sections`が指定され、ウォームアップ後のフレームがあれば`Some`
    pub(in crate::app) シーン読込回数: シーン読込回数,
}
