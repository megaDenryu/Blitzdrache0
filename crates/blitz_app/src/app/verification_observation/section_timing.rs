...
use crate::app::frame_timing::{ウォームアップフレーム数, フレーム時間統計, フレーム間隔から統計を集計する, 標本容量};
use crate::cli::起動モード;

pub(super) struct 区間計測 {
    呼出回数: u32,
    時間一覧ms: Vec<f64>,
}
...
impl 区間計測 {
    ...
    pub(super) fn 集計する(&self) -> Option<フレーム時間統計> {
        フレーム間隔から統計を集計する(&self.時間一覧ms)
    }
}
