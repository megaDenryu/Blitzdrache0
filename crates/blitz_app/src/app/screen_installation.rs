...
impl 画面の据え付け {
    ...
    /// 据え付けの枠(`Option`)を引数で受けるのは、呼び出し側が`アプリ`の他のフィールドと同時に借りられるようにするためである。
    pub(in crate::app) fn 描画の最中に借りる(据え付けの枠: &mut Option<Self>) -> &mut Self {
        match 据え付けの枠 {
            Some(据え付け) => 据え付け,
    ...
    pub(in crate::app) fn 描画の最中に見る(据え付けの枠: &Option<Self>) -> &Self {
        match 据え付けの枠 {
            Some(据え付け) => 据え付け,
            None => panic!("描画の下流が据え付けの無い状態へ到達した(描画の入口が据え付けの無い描画機会を弾く不変条件が破れている)"),
        }
