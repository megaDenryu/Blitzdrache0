...
impl アプリ {
    ...
    pub(in crate::app) fn 時間再構成の合成入力を突き合わせるフレームか(&self, フレーム番号: フレーム番号, アクション: スモークアクション) -> bool {
...
fn 読み戻す(レンダラー: &mut blitz_render::レンダラー, 描画入力: フレーム描画入力<'_>) -> Result<Option<blitz_render::HDR読み戻し画像>, 起動エラー> {
