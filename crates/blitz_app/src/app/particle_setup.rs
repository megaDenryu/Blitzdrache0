...
fn 表面流素材を作る() -> Result<粒子素材, 起動エラー> {
    let 仕様 = blitz_sim::表面流の計算条件::生成する([表面流一辺セル数; 2], 1.0, 1.0 / 60.0, [0.0, -1.5], 0.99)?;
    let 状態 = blitz_sim::表面流状態::液膜分布で生成する(&仕様, |[列, 行]| if (42..86).contains(&列) && (84..116).contains(&行) { 1.0 } else { 0.0 })?;
