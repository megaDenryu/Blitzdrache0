...
impl アプリ {
    ...
    pub(super) fn ダンプ対象フレームか(&self, フレーム番号: フレーム番号, アクション: スモークアクション) -> bool {
        if self.天空.段差走査で読み戻すフレームか(フレーム番号) {
            return true;
        }
        let 起動モード::スモーク実行 { フレーム数 } = self.起動モード else {
            return false;
        };
        let 採るフレームか = フレーム番号.最後のフレームか(フレーム数) || self.先行ダンプのフレームか(フレーム番号, フレーム数) || アクション == スモークアクション::差し替え前ダンプ;
        self.フレームダンプ先.書き出すか() && 採るフレームか
    }

    /// 同一起動内の再現性を見るために、最終フレームの1つ前も書き出すか。指定が無ければ撮らない。
    pub(super) fn 先行ダンプのフレームか(&self, フレーム番号: フレーム番号, フレーム数: u32) -> bool {
        self.読み戻し検収.先行フレームも書き出すか && フレーム番号.最後の一つ前のフレームか(フレーム数)
    }
    ...
    pub(super) fn 読み戻してダンプする(&mut self, 描画入力: blitz_render::フレーム描画入力<'_>, 視点情報: &フレーム視点, アクション: スモークアクション) -> Result<描画の到達, 起動エラー> {
        match self.フレームダンプ先.clone() {
            ...
            フレームダンプ指定::提示画像を書き出す { 基準名 } => {
                let 先 = dump_destination::ダンプ先のパスを決める(self, 視点情報, &基準名, アクション);
