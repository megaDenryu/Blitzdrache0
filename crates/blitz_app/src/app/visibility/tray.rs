//! 毎フレーム使い回す選択の受け皿。エンジンが書く段別ID受け皿と、群ごとの選択の一覧と、そのフレームの計数と、
//! 判定に使った影の解像度密度を持つ。
//! 4つを同じ型が持つのは、選択が指す区間が必ず同じフレームのID列と区間を指し、計数がその中身を数えた値であり、
//! 解像度密度がその判定に使った正射影の値であるという対応を1箇所で保つためであり、毎フレームVecを確保して直後に捨てることも避ける。
...
impl 可視選択受け皿 {
    ...
    /// 直近のフレームの判定範囲が持っていた距離区分ごとの解像度密度。
    pub(in crate::app) fn 解像度密度(&self) -> Option<影の解像度密度> {
        self.解像度密度
    }
