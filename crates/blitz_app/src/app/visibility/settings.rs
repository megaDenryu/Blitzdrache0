...
use blitz_engine::{個体詳細段選択設定, 影の視距離, 段選択方式};
use blitz_math::メートル;
...
pub(super) fn 段の閾値設定を作る() -> 個体詳細段選択設定 {
    let 設定 = 個体詳細段選択設定::生成する(メートル::生成する(基準距離メートル), メートル::生成する(ヒステリシス幅メートル));
    match 設定 {
        Ok(値) => 値,
        Err(誤り) => panic!("個体詳細段選択設定の定数が検証を満たさない: {誤り}"),
    }
}
...
pub(super) fn 段選択方式を選ぶ(段選択有効: bool, 設定: 個体詳細段選択設定) -> 段選択方式 {
