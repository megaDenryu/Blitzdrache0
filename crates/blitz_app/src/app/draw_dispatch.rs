...
impl アプリ {
    pub(super) fn 描画まで進める(&mut self, 材料: フレーム材料<'_>) -> Result<(), 起動エラー> {
        ...
        let mut 描画入力 = self.描画入力を組み立てる(焼き上げ入力, 材料.視点情報, 材料.地形詳細段選択一覧, 可視個体選択一覧, 材料.プリミティブ発行, 材料.動く個体の大域の位置と向きの指定一覧)?;
        描画入力.布 = 材料.布入力;
        描画入力.ui描画 = 材料.ui描画;
        let 描画機会の番号 = 材料.視点情報.描画機会の番号;
        let 撮影 = self.天空.この描画の撮影の有無(描画機会の番号);
        let 到達 = match self.検収の起動設定.この描画の検収の経路を選ぶ(描画機会の番号, self.起動モード, 材料.アクション, 撮影) {
            ...
            検収の経路::時間再構成の観測 { 局面 } => self.時間再構成を観測する(描画入力, 局面, 描画機会の番号)?,
