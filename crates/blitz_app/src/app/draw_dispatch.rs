...
pub(super) struct フレーム材料<'a> {
    ...
    pub(super) 動く個体の大域の位置と向きの指定一覧: &'a [blitz_render::frame_input::動く個体の大域の位置と向きの指定],
...
impl アプリ {
    pub(super) fn 描画まで進める(&mut self, 材料: フレーム材料<'_>) -> Result<(), 起動エラー> {
        let 可視個体選択一覧 = blitz_render::可視個体選択一覧::生成する(材料.可視受け皿.選択一覧(), 材料.可視受け皿.id列(), 材料.可視受け皿.段範囲列())?;
        // 焼き直しの判定は保留中の鍵を書き換えるため、借用が重ならないよう描画入力の組み立てより前に済ませる。
        let 焼き上げ入力 = self.天空.焼き上げ入力(材料.視点情報);
        let mut 描画入力 = super::frame::描画入力を組み立てる(self, 焼き上げ入力, 材料.視点情報, 材料.地形詳細段選択一覧, 可視個体選択一覧, 材料.プリミティブ発行, 材料.動く個体の大域の位置と向きの指定一覧)?;
        描画入力.布 = 材料.布入力;
        描画入力.ui描画 = 材料.ui描画;
        let フレーム番号 = 材料.視点情報.フレーム番号;
        let 到達 = if let Some(形) = self.局所可視性を検収する形(フレーム番号, 材料.アクション) {
            self.局所可視度を検収する(描画入力, 材料.視点情報, 形)?
        } else if self.動きベクトルを報告するフレームか(フレーム番号, 材料.アクション) {
            self.動きベクトルを報告する(描画入力)?
        } else if self.時間再構成の合成入力を突き合わせるフレームか(フレーム番号, 材料.アクション) {
            self.時間再構成の合成入力を突き合わせる(描画入力, 材料.視点情報)?
        } else if self.時間再構成を観測するフレームか(フレーム番号, 材料.アクション) {
            self.時間再構成を観測する(描画入力, フレーム番号)?
        } else if self.ダンプ対象フレームか(フレーム番号, 材料.アクション) {
        ...
        if 到達 == 描画の到達::提示した {
            self.天空.焼き上げを確定する();
            self.視点の履歴.記録する(材料.視点情報.ビュー射影の組.ずらし無し(), 材料.視点情報.カメラ大域位置);
        }
        Ok(())
    }
}
