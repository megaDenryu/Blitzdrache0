//! そのフレームの材料から描画入力を組み立て、検収の起動設定が判定した検収の経路へ渡して実行する入口。
//! 入口を1つ置くのは、地形詳細段選択と可視個体選択の受け皿がアプリの外へ預けられている間だけ描画入力を作れるためであり、
//! 呼び出し元は預けと戻しだけを見ればよい。経路の判定は材料だけを読む純粋な判定であり、ここは経路ごとの実行の工程へ振り分けるだけである。
...
use super::verification_launch_settings::検収の経路;
...
impl アプリ {
    pub(super) fn 描画まで進める(&mut self, 材料: フレーム材料<'_>) -> Result<(), 起動エラー> {
        ...
        let 撮影 = self.天空.この描画の撮影の有無(フレーム番号);
        let 到達 = match self.検収の起動設定.この描画の検収の経路を選ぶ(フレーム番号, self.起動モード, 材料.アクション, 撮影) {
            検収の経路::局所可視性 { 形 } => self.局所可視度を検収する(描画入力, 材料.視点情報, 形)?,
            検収の経路::動きベクトル => self.動きベクトルを報告する(描画入力)?,
            検収の経路::時間再構成の突き合わせ => self.時間再構成の合成入力を突き合わせる(描画入力, 材料.視点情報)?,
            検収の経路::時間再構成の観測 { 局面 } => self.時間再構成を観測する(描画入力, 局面, フレーム番号)?,
            検収の経路::ダンプ { 画像, 書き出し先 } => self.読み戻してダンプする(描画入力, 材料.視点情報, 画像, &書き出し先)?,
            検収の経路::通常 => self.実行して判定する(材料.アクション, 描画入力)?,
        };
