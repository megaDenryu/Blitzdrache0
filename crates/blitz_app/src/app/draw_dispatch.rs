...
impl アプリ {
    pub(super) fn 描画まで進める(&mut self, 材料: フレーム材料<'_>) -> Result<(), 起動エラー> {
        ...
        // 大気LUTの焼き直し判定は前回焼いた鍵を書き換えるため、借用が重ならないよう描画入力の組み立てより前に済ませる。
        let 大気lut入力 = self.天空.大気lut入力(材料.視点情報);
        let mut 描画入力 = super::frame::描画入力を組み立てる(self, 大気lut入力, 材料.視点情報, 材料.地形詳細段選択一覧, 可視個体選択一覧);
        描画入力.布 = 材料.布入力;
        描画入力.ui描画 = 材料.ui描画;
        if self.ダンプ対象フレームか() {
            self.読み戻してダンプする(描画入力, 材料.視点情報)
        } else {
