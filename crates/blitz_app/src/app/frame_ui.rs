//! 開発用UI(egui)の1フレームぶんの描画データ組み立て。
//! 触れるフィールドは`画面へ重ねるui`・`露出`・`ブレンド`・`window`・`レンダラー`に限る。露出とブレンドはスライダーが書き換えるため可変で渡す。
...
impl アプリ {
    ...
    pub(super) fn ui描画データを組み立てる(&mut self) -> Result<Option<blitz_render::UI描画データ>, 起動エラー> {
        ...
        let Some(画面へ重ねるui) = &mut self.画面へ重ねるui else { return Ok(None) };
        ...
        画面へ重ねるui.描画データを作る(window, レンダラー, 統計, &mut self.露出, &mut self.ブレンド)
    }
}
