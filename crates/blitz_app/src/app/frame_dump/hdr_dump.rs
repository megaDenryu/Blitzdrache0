//! 明るさの圧縮前のHDR中間画像の読み戻しと書き出しの工程。受け取るのはレンダラーと1フレームの描画入力とダンプ先と読み戻し検収の起動設定、返すのはそのフレームが提示まで届いたかどうかである。
//!
...
use blitz_render::{HDR読み戻し画像, レンダラー};

use super::super::draw_dispatch::描画の到達;
use super::寸法を書く;
use crate::cli::読み戻し検収起動設定;
use crate::error::起動エラー;

pub(super) fn 明るさ圧縮前画像を読み戻して書き出す(
    レンダラー: &mut レンダラー, 描画入力: blitz_render::フレーム描画入力<'_>, ダンプ先: &Path, 読み戻し検収: &読み戻し検収起動設定
) -> Result<描画の到達, 起動エラー> {
    let 自動露出を報告するか = 読み戻し検収.自動露出を報告するか;
    let 探り色 = 読み戻し検収.自動露出の探り色;
    match レンダラー.一フレーム描画して圧縮前のhdrを読み戻す(描画入力)? {
        blitz_render::読み戻し結果::読み戻した(画像) => {
            書き出す(&画像, ダンプ先)?;
            if 自動露出を報告するか {
                自動露出を報告する(レンダラー, &画像, 探り色)?;
            }
            Ok(描画の到達::提示した)
        }
...
fn 自動露出を報告する(レンダラー: &blitz_render::レンダラー, 画像: &HDR読み戻し画像, 探り色: Option<[f32; 3]>) -> Result<(), 起動エラー> {
