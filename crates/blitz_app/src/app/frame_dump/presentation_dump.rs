//! 提示画像の読み戻しと書き出しの工程。受け取るのはアプリと1フレームの描画入力と視点とダンプ先、
//! 返すのはそのフレームが提示まで届いたかどうかである。
//!
//! 書き出す形式は`<ベース名>.raw`(RGBA8連結)と`<ベース名>.size`(幅 高さ)である。
//! 代表画素の照合をこの工程が呼ぶのは、どの照合も提示画像の8ビット値を材料にしており、
//! 読み戻した画像がここにしか無いためである。
...
use super::super::アプリ;
use super::{indirect_probe_check, sky_pixel_check, 寸法を書く};
