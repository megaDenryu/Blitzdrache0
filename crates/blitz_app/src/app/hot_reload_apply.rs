...
//! 触れるフィールドは`ホットリローダー`・`レンダラー`・`描画対象数`・`起動シーン大域アンカー`・`ストリーミング`に限る。
...
impl アプリ {
    pub(super) fn ホットリロードを確認する(&mut self) {
        ...
        let 起動シーン大域アンカー = self.起動シーン大域アンカー;
        ...
        match 結果 {
            ...
            ホットリロード結果::アセット再読込成功 { シーン } => {
                ...
                if アセット再読込を反映する(レンダラー, &シーン, 描画対象数, 起動シーン大域アンカー)
...
fn アセット再読込を反映する(
    ...
    大域アンカー: blitz_math::大域ワールド位置,
) -> bool {
    match super::scene_load::シーンをレンダラー入力に変換する(シーン, 描画対象数, 大域アンカー) {
