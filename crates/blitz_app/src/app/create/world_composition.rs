...
//! 返すのはフレーム段階の並びと局所可視性補正の描画設定である。
//!
//! 2つを1つの値で返すのは、拡散間接方式が深度プリパスの方式へ条件を課すためである。局所可視性補正は深度プリパスが
//! 書いた深度を読むため、プリパスを積まない画面では深度が消去値のままになり、遮蔽の標本化は全画素を空と見て
//! 可視度1を返す。絵は破綻せず、ただ局所可視性補正が1画素も効かない。この無言の空振りを起こさないため、
