...
//! 触れるフィールドはゲーム配線・据え付け・永続束・可視台帳・ストリーミング・ホットリローダーに限る。
...
impl アプリ {
    pub(super) fn 公開済みの実行時アセット一式を反映する(&mut self, 一式: 公開済みの実行時アセット一式) {
        ...
        if let Some(配線) = self.ストリーミング.配線を可変で借りる()
            && let Err(誤り) = 配線.チャンク目録と描画束を差し替える(一式.チャンク目録, レンダラー, &mut self.可視判定, &mut self.プリミティブ描画項目台帳)
        {
...
fn 起動時シーンを差し替える(
    ...
    if let Err(誤り) = ゲーム配線.束の描画シーン素材へ動く個体を宣言する(super::scene_load::起動時シーンの束ID, &mut 描画入力.描画シーン) {
