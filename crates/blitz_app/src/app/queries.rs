...
use blitz_render::{
    CPU区間時間, GPUメモリ統計, レンダラー, 大気LUT生成パス数の記録, 実表示観測, 実表示計測状況, 描画発行内訳, 検証カウンタ
};
...
impl アプリ {
    ...
    /// 大気LUT生成パスの実行数の記録。レンダラー未生成なら1フレームも数えていないため`None`。
    pub(crate) fn 大気lut生成パス数の記録を取得する(&self) -> Option<&大気LUT生成パス数の記録> {
        self.レンダラー.as_ref().map(レンダラー::大気lut生成パス数の記録を取得する)
    }
