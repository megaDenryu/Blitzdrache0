...
//! 毎フレームの進行と、そのうちゲームが見る世界の形を変える命令は`advance`にある。
//! `実破棄受け皿`は毎フレーム使い回し、破棄バッチごとのヒープ再確保を避ける。
//! `差し替え前の実破棄待ち束`は、世界差し替えで旧台帳を消した後に届く実破棄を新世界の台帳へ報告しないために持つ。
...
pub(super) struct ストリーミング配線 {
    ...
    実破棄受け皿: Vec<blitz_render::描画束ID>,             // 実破棄済み束IDの受け皿
    差し替え前の実破棄待ち束: Vec<blitz_render::描画束ID>, // 世界差し替えをまたぐ実破棄の識別一覧
...
impl ストリーミング配線 {
    ...
    /// 世界の形を尋ねる口が読込済みチャンクの形状の候補の選別を組むために借りる。
    pub(super) fn 調停(&self) -> &ストリーミング調停 {
        &self.調停
    }

    pub(super) fn チャンク目録の一辺を検査する(&self, 新しい目録: &blitz_engine::チャンク目録) -> Result<(), blitz_engine::チャンク目録差し替えエラー> {
        let 新一辺 = 新しい目録.一辺();
        if self.チャンク一辺 == 新一辺 {
            return Ok(());
        }
        Err(blitz_engine::チャンク目録差し替えエラー::チャンク一辺変更 { 旧: self.チャンク一辺, 新: 新一辺 })
    }

    /// 見送りフレーム数はレンダラーの計器であり配線は保持しないため、呼出し側が渡す。
    pub(super) fn 要約を作る(&self, 見送りフレーム数: u64) -> ストリーミング要約 {
        self.計測.要約を作る(self.調停.転送量(), self.上限, 見送りフレーム数, self.地形lod.段別の束数())
    }
