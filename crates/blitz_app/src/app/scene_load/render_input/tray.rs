...
impl 変換の受け皿 {
    ...
    pub(super) fn 積む(
        &mut self, 元: &描画対象データ, 位置: usize, 大域の基準原点: 大域ワールド位置, ローカルからワールド: 変換<ローカル, ワールド>, 地表の層のタイル: &地表の層のタイル一式
    ) -> Result<(), 起動エラー> {
        self.プリミティブ描画項目一覧.描画対象1つぶんを積む(元.形状(), 元.材質集合(), 位置).map_err(super::描画入力エラー::from)?;
