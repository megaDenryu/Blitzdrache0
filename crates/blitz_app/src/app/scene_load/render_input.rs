...
pub(super) fn シーンを束の描画入力へ変換する(
    シーン: &シーンデータ, 並べ方: crate::cli::描画対象の並べ方, 束座標: チャンク座標, 材料: シーンを描画入力へ写す材料<'_>
) -> Result<束の描画入力, 起動エラー> {
    let 大域の基準原点 = anchor::基準原点を導出する(シーン, 束座標, 材料.大域平行移動(), 材料.チャンク一辺())?;
    let 件数 = 並べ方.件数.map_or(シーン.描画対象一覧().len(), crate::cli::描画対象数::件数として取り出す);
    ...
    for 位置 in 0..件数 {
        ...
        let 変換 = 並べ方.件数.map_or(元.ローカルからワールド(), |_| 元.ローカルからワールド().合成する(layout::格子内の配置変換を求める(添字, 件数)));
