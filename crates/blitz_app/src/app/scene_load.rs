...
pub(super) fn シーンを読み込んで変換する(
    ...
    let 描画シーン = シーンをレンダラー入力に変換する(&シーン, 描画対象数, 起動時シーンの所有チャンク, 大域平行移動)?;
    Ok((シーン, 描画シーン))
}

/// 既に読み込み済みの`シーンデータ`をレンダラー入力へ変換する(ホットリロード再読込とチャンク束の追加で使う)。
/// `束座標`はそのシーンを載せる描画束のチャンク座標である。描画アンカーはこの座標から導出し、シーン内の全描画対象がこの座標を所有チャンクに持つことを検証する。
/// `大域平行移動`は`--global-offset`が世界全体へ加える平行移動であり、導出したアンカーへ最後に合成する。
pub(crate) fn シーンをレンダラー入力に変換する(
    ...
    render_input::変換する(シーン, 描画対象数, 束座標, 大域平行移動)
}
