...
pub(super) fn ウィンドウとレンダラーを作る(
    ...
    世界実行: &mut crate::world_execution::世界実行,
    ...
    let カタログ = scene_load::カタログを構築して高さ場を据える(世界.アセットの置き場, 世界実行)?;
    let 地表の層のタイル = scene_load::地表の層のタイル一式::カタログから読み込む(&カタログ)?;
    let 材料 = scene_load::シーンを描画入力へ写す材料::生成する(世界.大域平行移動, 世界.チャンク一辺, &地表の層のタイル);
    let (シーンデータ, mut 描画入力) = scene_load::シーンを読み込んで変換する(&カタログ, 世界.シーン.安定id(), 世界.描画対象の並べ方, 材料)?;
    let 遠景 = scene_load::遠景を読み込んで変換する(&カタログ, 材料)?;
    // 動く個体の宣言は束の読込より前でなければならない。宣言した個体だけがフレームスロットごとのバッファを持ち、そのバッファを読込時のディスクリプタが結ぶ。
    世界実行.動く個体を描画束へ宣言する(scene_load::起動時シーンの束ID, &mut 描画入力.描画シーン)?;
    ...
    let (布素材, 布プリセット, 布の参照比較) = match super::cloth_setup::布モードから構築する(世界.布モード, &描画入力.描画シーン, 世界.基本刻み)? {
        Some(構築物) => (Some(構築物.素材), Some(構築物.プリセット), 構築物.参照比較),
        None => (None, None, None),
    };
