...
pub(super) fn ウィンドウとレンダラーを作る(
    ...
    let (シーンデータ, mut 描画入力) =
        scene_load::シーンを読み込んで変換する(&カタログ, 世界.シーン.安定id(), 世界.描画対象の並べ方, 世界.大域平行移動)?;
    let 遠景 = scene_load::遠景を読み込んで変換する(&カタログ, 世界.大域平行移動)?;
    ...
    ホットリローダー.アセット監視を設定する(カタログ, 世界.シーン.安定id().clone(), &シーンデータ.参照ファイル一覧);
