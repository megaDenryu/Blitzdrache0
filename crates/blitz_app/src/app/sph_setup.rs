...
pub(super) fn 素材を作る(粒子数: u32) -> Result<粒子素材, 起動エラー> {
    ...
    for 添字 in 0..粒子数 {
        ...
        let y = (添字 / 一辺) % 一辺;
        let z = 添字 / (一辺 * 一辺);
        let 位置 = [格子座標へ変換する(x, 一辺), 格子座標へ変換する(y, 一辺) + 0.7, 格子座標へ変換する(z, 一辺)];
        for 成分 in [位置[0], 位置[1], 位置[2], 0.0, 0.0, 0.0, 0.0, if 添字 == 0 { 粒子数をf32へ(粒子数) } else { 0.0 }] {
...
fn 格子座標へ変換する(添字: u32, 一辺: u32) -> f32 {
    (粒子数をf32へ(添字) - 粒子数をf32へ(一辺 - 1) * 0.5) * 0.11
}

fn 粒子数をf32へ(値: u32) -> f32 {
