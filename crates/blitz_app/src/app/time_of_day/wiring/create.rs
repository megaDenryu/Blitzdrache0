...
//! 担当するのは「世界の種別と起動指定から世界の空の宣言と間接照明方針を決め、時計と下ろし済みの大気を組み、
...
pub(in crate::app) struct 生成材料<'材料> {
    ...
    pub(in crate::app) 明示境界を使わないか: bool, // --no-explicit-shadow-bandsで真になる。大規模世界でも距離区分を再配分せず実用分割のまま走る
}

pub(super) fn 天空配線を生成する(材料: 生成材料<'_>) -> Result<天空配線, レンダラーエラー> {
    let (種別, 設定, 基準) = (材料.種別, 材料.時間帯, 材料.基準ライティング);
    let 明示境界を使わない = 材料.明示境界を使わないか;
    let 方針 = scene_policy::世界の空の宣言を決める(種別, 設定.空);
    let 方針 = sun_disk_override::太陽円盤を反映する(方針, 設定.太陽円盤);
    let (時間帯, 大気) = match 方針 {
        世界の空の宣言::空なし => (None, None),
        世界の空の宣言::空あり { 空と太陽, 既定時刻 } => (
    ...
    let mut 配線 = 天空配線 {
        時間帯,
        空を描くか: scene_policy::空を描くか(方針, 設定.空),
        大気,
        大気更新判定: 大気更新判定::新規(),
        空中遠近の最遠距離の記録: 空中遠近の最遠距離の記録::新規(),
        照明問い合わせ契約: scene_policy::世界の間接照明の方式を決める(種別).照明問い合わせ契約へ写す(大気.is_some())?,
