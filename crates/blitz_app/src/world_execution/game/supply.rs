...
impl ゲーム配線 {
    ...
    pub(super) fn プレイヤーのエンティティid(&self) -> Option<エンティティID> {
        match self {
            Self::ゲームを遊ばない => None,
    ...
    pub(super) fn 刻み境界で確定したプレイヤーの大域位置(&self) -> Option<大域ワールド位置> {
        match self {
            Self::ゲームを遊ばない => None,
    ...
    pub(super) fn プレイヤーの移動状態(&self) -> Option<移動状態> {
        match self {
            Self::ゲームを遊ばない => None,
    ...
    pub(super) fn 画面へ重ねる表示内容を作る(&self) -> Option<ゲーム画面の表示内容> {
        match self {
    ...
    pub(super) fn 進行の要約を作る(&self, 移動の観測: &移動の観測の記録) -> Option<ゲーム進行の要約> {
        match self {
            Self::ゲームを遊ばない | Self::歩くだけ(_) => None,
