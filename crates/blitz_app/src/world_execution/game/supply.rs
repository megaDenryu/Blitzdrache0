...
//! 終了が決まったか・画面へ重ねる表示内容・終了時の要約であり、どれもゲームの状態を読むだけで書き換えない。
...
impl ゲーム配線 {
    ...
    /// 刻み境界で確定したプレイヤー。刻みの確定の処理位置と高さ場の据え付けが台帳へ写す材料であり、遊ばない起動はプレイヤーを持たず`None`である。
    ...
    /// この刻みまでにゲームの終了が決まったか。刻みの確定の処理位置が刻み結果を組むために読む。歩くだけと遊ばない起動は終了を決めない。
    pub(in crate::world_execution) fn 終了が決まったか(&self) -> bool {
        match self {
            Self::ゲームを遊ばない | Self::歩くだけ(_) => false,
            Self::キツネの場所巡り(配線) => 配線.終了が決まったか(),
        }
    }
