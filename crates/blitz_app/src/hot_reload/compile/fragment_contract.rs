...
impl 画素段の組 {
    ...
    pub(super) const 全ての組: [Self; 4] = [Self::定数近似の標準金属粗さpbr, Self::定数近似の地表の層の重ね合わせ, Self::遠方環境の標準金属粗さpbr, Self::遠方環境の地表の層の重ね合わせ];
    ...
    pub(super) fn ソースパスを解決する(self, 監視先: &Path) -> Result<PathBuf, シェーダー再コンパイルエラー> {
        ...
        let 親 = 監視先.parent().ok_or_else(|| シェーダー再コンパイルエラー::監視先に親ディレクトリが無い { 監視先: 監視先.to_path_buf() })?;
        Ok(親.join(ファイル名))
    }
}
