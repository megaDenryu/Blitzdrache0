...
pub(super) fn シーン描画シェーダーをコンパイルする(監視先: &Path) -> Result<契約別のシーン描画シェーダー, シェーダー再コンパイルエラー> {
    ...
    let [定数近似の標準金属粗さpbr, 定数近似の地表の層の重ね合わせ, 遠方環境の標準金属粗さpbr, 遠方環境の地表の層の重ね合わせ] = 組ごとのバイト列.try_into().unwrap_or_else(|_| panic!("画素段の組の数と焼いたバイト列の本数が食い違った"));
...
fn 画素段をコンパイルする(slangc: &スランガー位置, 組: 画素段の組, 監視先: &Path) -> Result<Vec<u8>, シェーダー再コンパイルエラー> {
...
fn エントリを1つコンパイルする(
    slangc: &スランガー位置, ソースパス: &Path, エントリ名: &str, ステージ: &str, 出力ファイル名: &str, 段の説明: &str
