...
use blitz_engine::surface_layer_textures::{
    エンジン材質名, 世界の地表層テクスチャ集の安定IDの文字列, 地表層ごとの材質索引, 地表層のタイル, 地表層の数, 地表層テクスチャ集, 地表層テクスチャ集を実行時形式へ格納する
};
...
pub(super) const 起動時シーンの安定IDの文字列: &str = "startup_scene";
...
/// カタログ・チャンク目録・起動時シーンを置き、生成台帳へ名指した世代の文字列を書く。
/// 地表層テクスチャ集を載せるかどうかで、タイルを持たない世界と持つ世界を作り分ける。
pub(super) fn 一式を書き出す(置き場: &Path, 世代の文字列: &str, 種別: 試験の世界の種別) {
    let シーンのパス = 置き場.join(format!("{起動時シーンの安定IDの文字列}.blitzasset"));
    std::fs::write(&シーンのパス, b"startup-scene-bytes").unwrap();
    let mut カタログ = カタログ::空を作る();
    カタログ.登録する(アセットID::生成する(起動時シーンの安定IDの文字列).unwrap(), シーンのパス);
    if 種別 == 試験の世界の種別::タイルを持つ世界 {
        let タイルのパス = 置き場.join(format!("{世界の地表層テクスチャ集の安定IDの文字列}.blitzasset"));
        std::fs::write(&タイルのパス, 地表層テクスチャ集を実行時形式へ格納する(&四層のテクスチャ集()).unwrap()).unwrap();
        カタログ.登録する(アセットID::生成する(世界の地表層テクスチャ集の安定IDの文字列).unwrap(), タイルのパス);
    }
    ...
    std::fs::write(置き場.join(実行時アセットの公開完了印::ファイル名()), 世代の文字列).unwrap();
}
