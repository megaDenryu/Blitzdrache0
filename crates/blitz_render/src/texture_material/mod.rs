...
impl テクスチャ素材 {
    ...
    pub fn 生成する(幅: u32, 高さ: u32, 格納形式: テクスチャ格納形式, 段ごとのバイト列: Vec<Vec<u8>>, 用途: テクスチャ用途) -> Result<Self, テクスチャ素材エラー> {
        縮小段の並びを検査する(幅, 高さ, 格納形式, &段ごとのバイト列)?;
        Ok(Self {
            幅, 高さ, 格納形式, 段ごとのバイト列, 用途
        })
    }

    /// 非圧縮のRGBA8の原寸1枚から生成する。縮小段はGPUが原寸から作るため、段の列は1本になる。
    pub fn rgba8の原寸から生成する(幅: u32, 高さ: u32, rgba8: Vec<u8>, 用途: テクスチャ用途) -> Result<Self, テクスチャ素材エラー> {
