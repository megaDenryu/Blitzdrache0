...
pub enum シェーダー一式エラー {
    ...
    空のバイト列, // 頂点・画素段いずれかのSPIR-Vバイト列が空だった
    #[error("シェーダーのSPIR-Vバイト長が4の倍数でなかった: {0}バイト")]
    非4バイト整列(usize), // SPIR-Vは32bit語単位のバイナリ形式のため、バイト長は4の倍数であるべき

    #[error("頂点段へ位置の不変装飾を付けられなかった: {0}")]
    位置の不変装飾を付けられない(#[from] 位置の不変装飾エラー), // 深度プリパスと色パスが同じ位置を出す根拠が立たないため生成を通さない
}
