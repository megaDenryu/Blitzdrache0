...
//! 無しの材質は特徴ビットが下り、材質テクスチャ表の正準の既定テクスチャのスロットを指す
...
impl 金属粗さPBR素材 {
    ...
    pub fn 生成する(
        ベースカラー: Option<テクスチャ素材>, 金属粗さ: Option<テクスチャ素材>, 法線マップ: Option<テクスチャ素材>, ベースカラー係数: [f32; 4], 金属度係数: f32, 粗さ係数: f32
    ...
    /// 役割で1枚を引く。この選択肢が持たない役割が`None`になるのは、その材質がその役割のテクスチャを持たないという意味そのものである。
    pub(crate) fn 役割のテクスチャ(&self, 役割: 材質テクスチャ役割) -> Option<&テクスチャ素材> {
        match 役割 {
            ...
            材質テクスチャ役割::地表の層の重み | 材質テクスチャ役割::地表の層0のタイル | 材質テクスチャ役割::地表の層1のタイル | 材質テクスチャ役割::地表の層2のタイル | 材質テクスチャ役割::地表の層3のタイル => {
                None
            }
        }
    }
