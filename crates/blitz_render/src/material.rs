...
impl マテリアル素材 {
    ...
    /// 自分の選択肢に無い役割が`None`になるのは、その材質がその役割のテクスチャを持たないという意味そのものである。
    ...
    /// 地表の層の重ね合わせがベースカラー係数を等倍で返すのは、その選択肢のアルベドを層の混ぜ合わせだけが決めるためである。
    /// 材質レコードの欄はシェーディングモデルに依らず1本であり、その選択肢の画素段はこの欄を読まない。
    ...
    /// 標準金属粗さPBRが等倍を返すのは、ベースカラー係数と同じ理由である。その選択肢の画素段はこの欄を読まない。
