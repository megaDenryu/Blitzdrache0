//! マテリアル素材: PBR metallic-roughnessの3テクスチャ+係数(判断23)。

use thiserror::Error;

use crate::texture_material::テクスチャ素材;

/// `マテリアル素材::生成する`が失敗したときのエラー。
#[derive(Debug, Error, Clone, Copy, PartialEq)]
pub enum マテリアル素材エラー {
    /// 係数(baseColor/metallic/roughness)が0.0〜1.0の範囲外だった。
    #[error("マテリアル係数が0.0から1.0の範囲外だった: {0}")]
    係数範囲外(f32),
}

/// ベースカラー・metallicRoughness・法線マップの3テクスチャと、それぞれに
/// 掛け合わせる係数。テクスチャは呼び出し側(blitz_app)が既定素材を明示適用済みの
/// ものを渡す(無言のデフォルト適用禁止)。
#[derive(Debug, Clone)]
pub struct マテリアル素材 {
    ベースカラー: テクスチャ素材,
