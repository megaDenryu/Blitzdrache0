...
pub(super) fn テクスチャを生成する(転送係: ステージング経由の転送係<'_>, 問い合わせ: 物理デバイス問い合わせ<'_>, 素材: &テクスチャ素材) -> Result<テクスチャ, レンダラーエラー> {
    let 確保係 = 転送係.確保係();
...
fn 機材の対応を確認する(問い合わせ: 物理デバイス問い合わせ<'_>, 形式: vk::Format, 積み方: 縮小段の積み方) -> Result<(), レンダラーエラー> {
    match 積み方 {
        縮小段の積み方::原寸の転送とGPUのblit => format_support::blitフィルタ対応を確認する(問い合わせ, 形式),
        縮小段の積み方::全段の転送 => format_support::ブロック圧縮の標本化と転送先の対応を確認する(問い合わせ, 形式),
    }
}
