...
pub(super) fn 素材の縮小段を画像へ転送する(
    転送係: ステージング経由の転送係<'_>, image: vk::Image, 素材: &テクスチャ素材, 縮小段数: u32, 積み方: 縮小段の積み方
) -> Result<(), レンダラーエラー> {
    let 転送するバイト列 = 段を1本のバイト列へ束ねる(素材.段ごとのバイト列());
    let ステージング = 転送係.確保係().ホスト可視バッファを確保して書き込む(&転送するバイト列, vk::BufferUsageFlags::TRANSFER_SRC)?;

    let 実行結果 = 積み込んで送信する(転送係.転送環境(), ステージング.バッファのハンドル(), image, 素材, 縮小段数, 積み方);
...
fn 積み込んで送信する(
    転送環境: &転送実行環境, ステージングバッファ: vk::Buffer, image: vk::Image, 素材: &テクスチャ素材, 縮小段数: u32, 積み方: 縮小段の積み方
) -> Result<(), レンダラーエラー> {
