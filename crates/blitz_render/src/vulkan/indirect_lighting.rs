...
//! ビューを借りるだけである。
//!
//! `注入`が`Some`になるのは検収が中身を注入した実行だけであり、本番のフレーム経路は注入の入口を1度も呼ばない。
//! どちらの選択肢を注入したかでそのフレームの焼き上げの計画が変わる(`inject`と`draw_input`)。
...
pub(crate) struct 遠方環境の照明資源 {
    ...
    派生表現: 派生表現一式,
    焼き始め: 焼き始めの記録,
    注入: Option<injection::検収の注入>, // 検収が中身を注入した実行だけが`Some`
}
