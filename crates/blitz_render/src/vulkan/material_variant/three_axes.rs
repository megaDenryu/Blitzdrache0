...
impl 材質の3軸 {
    pub(crate) const fn 生成する(モデル: シェーディングモデル種別, 特徴集合: 材質特徴集合, 表面描画状態: 表面描画状態) -> Self {
        Self { モデル, 特徴集合, 表面描画状態 }
    }

    /// GPUの材質レコードへ運ぶ軸。パイプラインを変えないため変種キーには現れない。
    /// 現行の画素段はこのビットを読まず、絵を決めるのは梱包工程が役割ごとに選んだ実テクスチャか正準の既定テクスチャのスロットである。
    /// それでもレコードへ載せるのは、シェーダーの宣言に欄があり、材質が持つテクスチャの有無を計器と検査が読めるようにするためである。
