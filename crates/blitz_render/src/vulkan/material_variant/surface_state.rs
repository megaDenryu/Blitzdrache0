...
/// 固定機能のカリングの設定。`両面`はカリングを無効にして裏向きの面も描く状態、`片面`は裏向きの面を捨てる状態である。
/// 名前が「向き」なのは材質の宣言の側の語彙だからであり、パイプラインの側ではラスタライズのカリングの有無に対応する。
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub(crate) enum 面の向き {
    片面,
    両面,
...
impl 表面描画状態 {
    ...
    /// 現行の入力境界が通す唯一の状態。実物のシーンのパイプラインがカリングを無効にしているため、面の向きは`両面`である
    /// (参照: `crates/blitz_render/src/vulkan/pipeline/graphics_pipeline.rs`のラスタライズ設定)。
    /// glTFのアルファ様式と片面表示の宣言は実行時形式へ運ばれず、glb契約検査器が作者へ警告して落とすため、読み込んだ材質はすべてこの状態になる
    /// (参照: `_doc/設計/マルチマテリアルと材質境界.md`「Blender段4までの受理範囲」)。
    pub(crate) const fn 不透明両面() -> Self {
        Self::生成する(アルファ様式::不透明, 面の向き::両面, 深度書込::書く)
    }
