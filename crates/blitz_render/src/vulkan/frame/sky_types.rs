...
/// 空パスが束縛する資源。夜空放射輝度や太陽円盤の値はフレームシェーダー定数(binding3)に載るため、ここには載らない。
///
/// `ディスクリプタセット`は走査順で最初の描画対象のものである。空のシェーダーがそこから読むのはフレームシェーダー定数だけであり、
/// どの描画対象のセットでも同じ資源を指すため、可視判定の結果でこの値は変わらない(布の描画と同じ理由)。
pub(crate) struct 空描画入力 {
    pub(crate) pipeline: vk::Pipeline,
    pub(crate) layout: vk::PipelineLayout,
    pub(crate) ディスクリプタセット: vk::DescriptorSet,
    /// set1。スカイビューのベイク済み画像と透過率のベイク済み画像と媒体のシェーダー定数を結ぶ。
    pub(crate) 標本セット: vk::DescriptorSet,
}

/// 空中遠近合成パス1フレームぶんの入力。大気のベイク済み画像方式で合成を切っていないフレームだけ`Some`で渡す。
///
/// 最遠距離をここが運ぶのは、ボリュームを焼いた条件と参照する条件を同じ値から作るためである。フレームシェーダー定数へ
/// 写すと、焼いた刻みと参照する刻みが別の経路で更新されて食い違いうる。
pub(crate) struct 空中遠近合成描画入力 {
    pub(crate) pipeline: vk::Pipeline,
    pub(crate) layout: vk::PipelineLayout,
    /// set0。フレームシェーダー定数を読むために走査順で最初の描画対象のセットを借りる(空パスと同じ理由)。
