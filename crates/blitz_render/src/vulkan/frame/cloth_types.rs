...
pub(crate) struct 布描画の外部資源 {
    /// シャドウパスで布を描くパイプラインとそのレイアウト。通常の描画対象と同じ、レンダラーが1つだけ持つシャドウパイプラインである。
    pub(crate) シャドウpipeline: vk::Pipeline,
    pub(crate) シャドウlayout: vk::PipelineLayout,
    /// 両パスが束縛するシーン用ディスクリプタセット。布のシェーダーが読むのはフレームユニフォーム(binding3)と
