...
use crate::vulkan::graph::{カラー添付列, クリア指定, パス宣言, パス種別, 深度アタッチメント, 画像ハンドル, 画像用途};

pub(super) fn 空パスを宣言する<'a>(
    カラー: 画像ハンドル, 動きベクトル: 画像ハンドル, 深度: 画像ハンドル, 読み画像: Vec<(画像ハンドル, 画像用途)>, 入力: &'a 空描画入力, 寸法: vk::Extent2D
) -> パス宣言<'a> {
    パス宣言::生成する(
        "空",
        読み画像,
        vec![(カラー, 画像用途::カラー出力), (動きベクトル, 画像用途::カラー出力), (深度, 画像用途::深度出力)],
        Vec::new(),
        Vec::new(),
        パス種別::グラフィックス {
            カラー: カラー添付列::色と動きベクトル { 色: カラー, 動きベクトル },
