...
use crate::vulkan::graph::{GPU命令の積み先と宣言済み資源の取り出し口, バッファハンドル, バッファ用途, パス宣言, パス種別, 画像ハンドル, 画像用途};
...
pub(super) fn ヒストグラム消去パスを宣言する<'a>(ヒストグラム: バッファハンドル) -> パス宣言<'a> {
    パス宣言::生成する(
        ...
        パス種別::転送,
        move |文脈| {
            let バッファ = 文脈.宣言済みのバッファを参照する(ヒストグラム);
            // 安全性: コマンドバッファは記録中で、埋める長さは確保長ぴったりである。
            unsafe { 文脈.積み先().論理デバイス().cmd_fill_buffer(文脈.積み先().コマンドバッファ(), バッファ, 0, ヒストグラムのバイト数(), 0) };
        },
    )
}

pub(super) fn hdr集計パスを宣言する<'a>(hdr: 画像ハンドル, ヒストグラム: バッファハンドル, 入力: &'a 自動露出描画入力, 寸法: vk::Extent2D) -> パス宣言<'a> {
    パス宣言::生成する(
        "自動露出のヒストグラム集計",
        ...
        パス種別::コンピュート,
        move |文脈| {
            let _ = 文脈.宣言済みのバッファを参照する(ヒストグラム);
            ...
            コンピュートを積む(文脈, 入力.集計パイプライン, 入力.集計レイアウト, 入力.セット, &定数, 班数);
        },
    )
}

pub(super) fn 露出導出と適応パスを宣言する<'a>(ヒストグラム: バッファハンドル, 露出状態: バッファハンドル, 入力: &'a 自動露出描画入力) -> パス宣言<'a> {
    パス宣言::生成する(
        "自動露出の導出と適応",
        Vec::new(),
        Vec::new(),
        ...
        パス種別::コンピュート,
        move |文脈| {
            let _ = 文脈.宣言済みのバッファを参照する(露出状態);
            let 定数: Vec<u8> = 入力.導出の即時定数.iter().flat_map(|値| 値.to_le_bytes()).collect();
            コンピュートを積む(文脈, 入力.導出パイプライン, 入力.導出レイアウト, 入力.セット, &定数, [1, 1, 1]);
        },
    )
}

fn コンピュートを積む(文脈: &GPU命令の積み先と宣言済み資源の取り出し口, pipeline: vk::Pipeline, layout: vk::PipelineLayout, セット: vk::DescriptorSet, 定数: &[u8], 班数: [u32; 3]) {
