...
pub(super) fn ヒストグラム消去パスを宣言する<'a>(ヒストグラム: バッファハンドル) -> パス宣言<'a> {
    パス宣言::生成する(
        ...
        パス種別::転送,
        move |積み先と取り出し口| {
            let バッファ = 積み先と取り出し口.宣言済みのバッファを参照する(ヒストグラム);
            // 安全性: コマンドバッファは記録中で、埋める長さは確保長ぴったりである。
            unsafe { 積み先と取り出し口.積み先().論理デバイス().cmd_fill_buffer(積み先と取り出し口.積み先().コマンドバッファ(), バッファ, 0, ヒストグラムのバイト数(), 0) };
        },
    )
}
...
fn コンピュートを積む(積み先と取り出し口: &GPU命令の積み先と宣言済み資源の取り出し口, pipeline: vk::Pipeline, layout: vk::PipelineLayout, セット: vk::DescriptorSet, 定数: &[u8], 班数: [u32; 3]) {
