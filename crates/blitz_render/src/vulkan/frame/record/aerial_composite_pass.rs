//! 空中遠近合成パスの宣言: シーンが書いたカラーをLOADし、深度と空中遠近ボリュームをフラグメント段で読んで、
//! 全画面三角形でジオメトリの画素へ霞を混ぜる。混ぜ方はパイプラインのブレンドが担い、このパスは宣言と発行だけを持つ。
//!
//! 深度をアタッチメントとして宣言しないのは、同じパスで深度を読みながら深度アタッチメントにも指定すると
//! フィードバックループになるためである。深度がクリア値のままの画素の除外はフラグメントの`discard`が担う。
//! ボリュームの用途に`大気LUTフラグメント読み`を使うのは、大気LUTが休むレイアウトをGENERALに保つ不変条件が
//! あるためであり、通常の`シェーダー読みフラグメント`を使ってはならない(参照: `graph/initial_state/atmosphere_lut.rs`)。
...
pub(super) fn 作る<'a>(
    ...
    パス宣言::生成する(
        "空中遠近合成",
        vec![(深度, 画像用途::深度シェーダー読み), (空中遠近, 画像用途::大気LUTフラグメント読み)],
        vec![(カラー, 画像用途::カラー出力)],
        Vec::new(),
        Vec::new(),
        パス種別::グラフィックス {
...
fn コマンドを積む(device: &ash::Device, command_buffer: vk::CommandBuffer, 入力: &空中遠近合成描画入力, 寸法: vk::Extent2D) {
    ...
    // 安全性: command_bufferは記録中で、pipelineとlayoutとセットは互換の組として生成済み。押し込み定数の
