//! シーンの処理位置を展開する工程。深度プリパスを積み、その深度を読む局所可視性補正の2本を積み、続いて本体のシーン描画を1本積む。
//! 検収が合成深度を据えた実行では、プリパスと局所可視性補正の間へ深度を書き戻す転送を1本挟む。
//!
//! 処理位置の中の4種類のパスを1つの工程が持つのは、この順そのものが担当する関心事だからである。プリパスを外へ出すと、
...
/// 局所可視性補正を積んだフレームだけ、両側ぼかしが書いた画像のハンドルを返す。読み戻しの処理位置がこの画像を転送元に選べるのは、
...
pub(in crate::vulkan::frame::record::graph_build) fn シーンを積む<'a>(
    ...
    // 選別を処理位置の先頭へ置く。シーン描画と布描画がその結果を画素段で読むため、この順が保たれることで
    // 書き込みと読みの間にバリアが立つ。
