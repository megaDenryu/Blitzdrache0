...
pub(super) fn 積む<'a>(
    ...
    // 間接照明の表現は大気のベイク済み画像の後ろへ積む。遠方環境がスカイビューのベイク済み画像を読むためであり、レンダーグラフが宣言順に実行することでこの前後関係が保たれる。焼き直さないフレームは1本も積まないが、消費側が読む3枚は登録する。
    let 間接照明 = stages::間接照明を積む(グラフ, 任意.間接照明, 大気のベイク済み画像.スカイビュー);
    let mut スキン済み = None;
    let mut 布 = None;
    for 段階 in 構成.段階一覧() {
        match 段階 {
