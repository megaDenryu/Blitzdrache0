...
pub(super) fn 積む<'a>(
    ...
    // 間接照明の表現は大気のベイク済み画像の後ろへ積む。遠方環境がスカイビューのベイク済み画像を読むためであり、
    // レンダーグラフが宣言順に実行することでこの前後関係が保たれる。焼き直さないフレームでは1本も積まれない。
    let 間接照明生成パス数 = stages::間接照明を積む(グラフ, 任意.間接照明, 大気のベイク済み画像.スカイビュー);
    let mut スキン済み = None;
    let mut 布 = None;
    for 段階 in 構成.段階一覧() {
        match 段階 {
            ...
            フレーム段階::シーン => {
                stages::シーンを積む(グラフ, 基本, カラー, スキン済み, 布, クリア色, 描画対象.ジオメトリ, 描画対象.共有, 寸法);
            }
            フレーム段階::空 => {
