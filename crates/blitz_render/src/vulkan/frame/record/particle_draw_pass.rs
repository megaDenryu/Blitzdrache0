...
use crate::vulkan::graph::{カラー添付列, クリア指定, バッファハンドル, バッファ用途, パス宣言, パス種別, 深度アタッチメント, 画像ハンドル, 画像用途};

pub(super) fn 粒子描画パスを宣言する<'a>(カラー: 画像ハンドル, 深度: 画像ハンドル, 粒子ハンドル: バッファハンドル, 粒子入力: &'a 粒子描画入力, 寸法: vk::Extent2D) -> パス宣言<'a> {
    パス宣言::生成する(
        "粒子描画",
        Vec::new(),
