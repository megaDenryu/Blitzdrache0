...
use crate::vulkan::graph::{カラー添付列, バッファハンドル, バッファ用途, パス宣言, パス種別, 画像ハンドル, 画像用途};

/// 露出状態のバッファは、そのフレームで導出と適応を積んだときだけ読みとして宣言する。宣言することが、
/// 同じフレームの更新を画素段の読みが跨がないというメモリ依存そのものである。
pub(super) fn 明るさの圧縮パスを宣言する<'a>(
    hdr: 画像ハンドル, 光のにじみa: 画像ハンドル, スワップチェーン: 画像ハンドル, 露出状態: Option<バッファハンドル>, 入力: &'a 明るさの圧縮描画入力, 寸法: vk::Extent2D
) -> パス宣言<'a> {
    パス宣言::生成する(
        "明るさの圧縮",
