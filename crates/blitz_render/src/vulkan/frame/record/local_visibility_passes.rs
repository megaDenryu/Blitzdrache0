...
pub(super) fn 遮蔽の標本化パスを宣言する<'a>(深度: 画像ハンドル, 生: 画像ハンドル, 入力: &'a 局所可視性描画入力, 寸法: vk::Extent2D) -> パス宣言<'a> {
    パス宣言::生成する(
        "局所可視性の遮蔽の標本化",
        ...
        Vec::new(),
        パス種別::コンピュート,
        move |積み先と取り出し口| コンピュートを積む(積み先と取り出し口, 入力, 入力.遮蔽の標本化パイプライン, 班数(寸法)),
    )
}

pub(super) fn 両側ぼかしパスを宣言する<'a>(深度: 画像ハンドル, 生: 画像ハンドル, ぼかし後: 画像ハンドル, 入力: &'a 局所可視性描画入力, 寸法: vk::Extent2D) -> パス宣言<'a> {
    パス宣言::生成する(
        "局所可視性の両側ぼかし",
        ...
        Vec::new(),
        パス種別::コンピュート,
        move |積み先と取り出し口| コンピュートを積む(積み先と取り出し口, 入力, 入力.両側ぼかしパイプライン, 班数(寸法)),
    )
}
...
fn コンピュートを積む(積み先と取り出し口: &GPU命令の積み先と宣言済み資源の取り出し口, 入力: &局所可視性描画入力, pipeline: vk::Pipeline, 班数: [u32; 3]) {
