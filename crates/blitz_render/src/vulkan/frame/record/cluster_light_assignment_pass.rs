...
use crate::vulkan::graph::{GPU命令の積み先と宣言済み資源の取り出し口, バッファハンドル, バッファ用途, パス宣言, パス種別};

pub(in crate::vulkan::frame) fn クラスタ選別パスを宣言する<'a>(格子: バッファハンドル, 光添字列: バッファハンドル, 入力: クラスタ選別の描画入力) -> パス宣言<'a> {
    パス宣言::生成する(
        "クラスタの選別",
        Vec::new(),
        Vec::new(),
        ...
        パス種別::コンピュート,
        move |積み先と取り出し口| {
            let _ = 積み先と取り出し口.宣言済みのバッファを参照する(格子);
            let _ = 積み先と取り出し口.宣言済みのバッファを参照する(光添字列);
            クラスタ選別のgpu命令を積む(積み先と取り出し口, &入力);
        },
    )
}

fn クラスタ選別のgpu命令を積む(積み先と取り出し口: &GPU命令の積み先と宣言済み資源の取り出し口, 入力: &クラスタ選別の描画入力) {
