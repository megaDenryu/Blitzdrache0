//! ブルーム縮小チェーンのパス宣言(判断41)。前処理(初段)はHDRを読みKaris平均+ソフト閾値で
...
const 縮小パス名一覧: [&str; 5] = ["ブルーム前処理", "ブルーム縮小2", "ブルーム縮小3", "ブルーム縮小4", "ブルーム縮小5"];
...
pub(super) fn 前処理を作る<'a>(
    ...
    作る(縮小パス名一覧[0], hdr, 縮小0, 入力.前処理pipeline, 入力.前処理layout, 入力.前処理set, 寸法)
}
...
pub(super) fn 縮小を作る<'a>(
    ...
    作る(縮小パス名一覧[段], 読み, 書き, 入力.縮小pipeline, 入力.縮小layout, 入力.縮小set一覧[段 - 1], 寸法)
}
