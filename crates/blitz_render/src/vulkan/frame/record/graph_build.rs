...
pub(super) fn グラフを構築する<'a>(
    ...
    // 時間再構成のパスを積むフレームだけは今のフレームの色の画像へ描く。パスがその色を読みながらHDR中間画像へ
    // 結果を書くため、同じ1枚を読みながら書く形になるのを避ける(`参照: _doc/設計/時間再構成.md`「判断d」)。
    let ポスト = post_setup::登録する(&mut グラフ, 画像一式, 任意入力.明るさの圧縮, 任意入力.光のにじみ, 任意入力.自動露出, 寸法);
    let ポストの描画先 = ポスト.as_ref().map_or(基本.スワップチェーン, |構成| 構成.hdrハンドル);
    let シーンカラーハンドル = 基本.時間再構成.as_ref().map_or(ポストの描画先, |登録| 登録.今のフレームの色);

    let 実績 = composition::フレーム段階列を展開して積む(
        &mut グラフ,
        フレーム構成,
        画像一式,
        &基本,
