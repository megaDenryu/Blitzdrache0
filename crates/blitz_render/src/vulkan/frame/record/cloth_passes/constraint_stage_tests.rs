//! 拘束の工程が零化→(距離の色ごと→曲げの色ごと→目標拘束)×反復回数の並びで積まれることと、衝突の後に目標拘束の最終の成立が仕上げの前に積まれることの検証。材料は`test_fixtures`が持つ。
...
/// 拘束の計算の発行は色の数×反復回数であり、同じ色の並びを反復のたびに繰り返す。
#[test]
...
fn 目標拘束は各反復の距離拘束の後ろに一本ずつ積まれ零本なら積まれない() {
    ...
    assert_eq!(名前一覧[位置一覧[0] - 1], "布曲げ拘束");
    let 零本 = 積んだパス名一覧(&目標拘束の数を据えた入力(1, 0));
    assert!(!零本.contains(&"布目標拘束"));
}

/// 反証: 曲げ拘束を目標拘束の後ろに積むと、曲げが固定した粒子を各反復の最後に動かして固定が緩む。曲げの色は各反復で距離の色の直後・目標拘束の直前に、
/// 曲げの色の数だけ積む。曲げ拘束0本の布には積まない。
#[test]
fn 曲げ拘束は各反復の距離拘束の後ろ目標拘束の前に曲げの色の数だけ積まれ零本なら積まれない() {
    ...
    let mut 零本 = 刻み数を据えた入力(1);
    零本.曲げ拘束の数 = 0;
    零本.曲げの色の区間一覧 = Vec::new();
    assert!(!積んだパス名一覧(&零本).contains(&"布曲げ拘束"));
}
