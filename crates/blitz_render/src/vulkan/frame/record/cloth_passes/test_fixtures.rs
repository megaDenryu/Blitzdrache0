...
pub(super) fn 目標拘束の数を据えた入力(刻み数: u32, 目標拘束の数: u32) -> 布描画入力 {
    布描画入力 {
        レイアウト: vk::PipelineLayout::null(),
        介入パイプライン: vk::Pipeline::null(),
        積分パイプライン: vk::Pipeline::null(),
        目標の確定パイプライン: vk::Pipeline::null(),
        乗数零化パイプライン: vk::Pipeline::null(),
        拘束パイプライン: vk::Pipeline::null(),
        曲げ拘束パイプライン: vk::Pipeline::null(),
        目標拘束パイプライン: vk::Pipeline::null(),
        ハッシュ消去パイプライン: vk::Pipeline::null(),
        ハッシュ格納パイプライン: vk::Pipeline::null(),
        分離パイプライン: vk::Pipeline::null(),
        床とカプセルの押し出しパイプライン: vk::Pipeline::null(),
        仕上げパイプライン: vk::Pipeline::null(),
        頂点生成パイプライン: vk::Pipeline::null(),
        ディスクリプタセット: vk::DescriptorSet::null(),
        粒子数: 1024,
        拘束の数: 30,
        目標拘束の数,
        曲げ拘束の数: 8,
        色の区間一覧: vec![布の彩色の区間 { 開始: 0, 本数: 10 }, 布の彩色の区間 { 開始: 10, 本数: 10 }, 布の彩色の区間 { 開始: 20, 本数: 10 }],
        ...
        インデックス数: 6,
        描画パイプライン: vk::Pipeline::null(),
        描画レイアウト: vk::PipelineLayout::null(),
        相対の基準原点: カメラ相対の基準原点::世界原点から生成する(blitz_math::大域ワールド位置::原点()).unwrap(),
        外部資源: 布描画の外部資源 {
            シャドウ: 布シャドウ描画入力 {
                パイプライン: vk::Pipeline::null(),
                レイアウト: vk::PipelineLayout::null(),
            },
        },
    }
