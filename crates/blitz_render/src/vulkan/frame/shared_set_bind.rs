...
pub(super) fn シーンの共有セットを束縛する(積み先: GPU命令の積み先<'_>, layout: vk::PipelineLayout, 共有: 共有セット束縛) {
    let 対応 = [(ビューとパスのセット番号, 共有.ビューとパス), (材質のセット番号, 共有.材質), (照明問い合わせのセット番号, 共有.照明問い合わせ)];
...
pub(super) fn 布の共有セットを束縛する(積み先: GPU命令の積み先<'_>, layout: vk::PipelineLayout, 共有: 共有セット束縛) {
    let 対応 = [(ビューとパスのセット番号, 共有.ビューとパス), (照明問い合わせのセット番号, 共有.照明問い合わせ)];
    束縛する(積み先, layout, &対応, 共有.計器.セット別束縛());
}

fn 束縛する(積み先: GPU命令の積み先<'_>, layout: vk::PipelineLayout, 対応: &[(u32, vk::DescriptorSet)], 計数: &セット別束縛計数) {
...
pub(super) fn ビューとパスのセットを束縛する(積み先: GPU命令の積み先<'_>, layout: vk::PipelineLayout, 共有: 共有セット束縛<'_>) {
    let device = 積み先.論理デバイス();
    let command_buffer = 積み先.コマンドバッファ();
    共有.計器.セット別束縛().数える(ビューとパスのセット番号);
    // 安全性: command_bufferは記録中で、layoutとセットは互換の組として生成済みである。
    unsafe {
        device.cmd_bind_descriptor_sets(command_buffer, vk::PipelineBindPoint::GRAPHICS, layout, ビューとパスのセット番号, &[共有.ビューとパス], &[]);
    }
}
