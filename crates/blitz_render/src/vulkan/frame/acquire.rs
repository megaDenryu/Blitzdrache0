...
/// 次画像取得の結果。
pub(crate) enum 取得結果 {
    取得した { 添字: スワップチェーン画像添字, 劣化: bool }, // 画像を取得できた。`劣化`はサブオプティマル(描画は可能だが再構築が望ましい)。
    再構築が必要,                                            // スワップチェーンが陳腐化しており、取得できなかった。
}

impl GPU環境 {
    pub(crate) fn スワップチェーンの次の画像を取得する(&self, スワップチェーン: vk::SwapchainKHR, 取得セマフォ: vk::Semaphore) -> Result<取得結果, レンダラーエラー> {
        // 安全性: スワップチェーン・取得セマフォはいずれも生成済みで、呼び出し元が
        // フェンス待機によりセマフォが未シグナル状態であることを保証する。
        let 結果 = unsafe { self.swapchain_loader().acquire_next_image(スワップチェーン, u64::MAX, 取得セマフォ, vk::Fence::null()) };
