...
use super::{
    UI描画入力, ジオメトリ入力, スキニング描画入力, 光のにじみ描画入力, 共有セット束縛, 布描画入力, 明るさの圧縮描画入力, 点光源の影の描画発行, 点光源の影の束縛, 空中遠近合成描画入力, 空描画入力, 粒子描画入力, 距離区分別のシャドウ入力
};
...
/// - `間接照明`: 照明問い合わせ契約が遠方環境の選択肢のフレームだけ`Some`。定数近似の契約では資源そのものが無い
