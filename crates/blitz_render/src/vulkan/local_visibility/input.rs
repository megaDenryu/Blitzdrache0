...
pub(crate) struct 局所可視性描画入力 {
    ...
    pub(crate) 遮蔽の標本化パイプライン: vk::Pipeline,
    pub(crate) 両側ぼかしパイプライン: vk::Pipeline,
...
impl 局所可視性一式 {
    pub(crate) fn 描画入力を作る(&self, 射影: 射影の復元, 寸法: vk::Extent2D) -> 局所可視性描画入力 {
        局所可視性描画入力 {
            ...
            遮蔽の標本化パイプライン: self.パイプライン.遮蔽の標本化,
            両側ぼかしパイプライン: self.パイプライン.両側ぼかし,
            レイアウト: self.パイプライン.レイアウト,
            セット: self.ディスクリプタ.セットのハンドル(),
            即時定数: 即時定数を組み立てる(射影, 寸法, self.描画設定.補正の設定),
        }
    }
