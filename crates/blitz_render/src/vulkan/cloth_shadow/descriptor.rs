//! 布専用シャドウ経路のディスクリプタの所有者。多段影定数1本だけを結ぶレイアウトと、
...
/// 多段影定数のバインディング番号。shaders/cascade_shadow_uniform.slangの`[[vk::binding(8, 0)]]`と一致させる。
/// 8にそろえるのは、同じ定数を読むscene.slang・shadow.slangが取り込む宣言と番号を食い違わせないためである。
const 多段影定数のバインディング番号: u32 = 8;
