//! ディスクリプタ索引の機能による選び分けの検査。索引の機能を欠く候補を飛ばすこと、discreteを優先すること、
//! 1台も残らないときに候補別の不足を報告することを見る。機材の機能(ブロック圧縮・立方体の配列画像)による
//! 選び分けは`feature_choose_tests`が持つ。

#![allow(clippy::unwrap_used)]

use super::candidate::選定候補;
use super::candidate_fixture::候補;
use super::choose::選ぶ;
...
fn 全候補が索引の機能を欠くと候補別の不足を報告する() {
    let 欠ける候補 = 選定候補::生成する(1, "統合GPU".to_string(), false, ディスクリプタ索引機能::生成する(false, true), true, true);
