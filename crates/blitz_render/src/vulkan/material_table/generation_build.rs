...
type 構築結果<供給元> = Result<資源表世代<<供給元 as 常駐テクスチャ供給元>::常駐画像, <供給元 as 常駐テクスチャ供給元>::世代付属資源>, レンダラーエラー>;
...
pub(in crate::vulkan::material_table) fn 資源表世代を構築する<供給元: 常駐テクスチャ供給元>(
    ...
    let 正準の既定テクスチャの枚数 = u32::try_from(正準の既定テクスチャの用途::全用途.len()).unwrap_or_else(|_| panic!("正準フォールバックの用途数がu32に収まらない"));
...
fn 部分生成を退役させる<供給元: 常駐テクスチャ供給元>(供給元: &mut 供給元, 画像集合: Vec<供給元::常駐画像>, 付属資源: Option<供給元::世代付属資源>) {
