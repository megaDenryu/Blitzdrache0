...
impl 保持材質 {
    ...
    pub(super) fn 梱包対象へ写す(&self) -> 梱包対象材質<'_> {
        let 役割別指定 = std::array::from_fn(|添字| self.役割別テクスチャ[添字].as_ref().map(|保持| テクスチャ指定::生成する(保持.テクスチャid, 保持.画像id, &保持.素材)));
        梱包対象材質::生成する(self.材質id, self.シェーディングモデル, self.ベースカラー係数, self.金属度係数, self.粗さ係数, 役割別指定, self.地表の層ごとのタイル倍率)
    }
}
