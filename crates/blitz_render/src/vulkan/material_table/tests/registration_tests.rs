...
use super::material_fixture::{検査用素材, 正準の既定テクスチャと1枚だけの容量, 画像を選んだ材質};

fn 資源を作らずに試す(材質一覧: &[梱包対象材質<'_>], 容量: テクスチャ表レイアウト容量) -> Result<(), レンダラーエラー> {
...
fn 検査専用供給元は容量超過を資源を作る前に返す() {
    ...
    assert!(資源を作らずに試す(&[画像を選んだ材質(1, 1, 1, &素材)], 容量).is_ok(), "容量ちょうどは通る");

    let 超える一覧 = [画像を選んだ材質(1, 1, 1, &素材), 画像を選んだ材質(2, 2, 2, &別素材)];
    let 誤り = 資源を作らずに試す(&超える一覧, 容量).expect_err("容量を1枚超える組は型付きの失敗になる");
    assert!(matches!(誤り, レンダラーエラー::材質資源表不正(材質資源表エラー::レイアウト容量超過 { 常駐枚数: 6, レイアウト容量: 5 })));
}
...
fn 検査専用供給元と資源を作る供給元の判定が一致する() {
    ...
    let 素材 = 検査用素材(テクスチャ用途::色);
    let 別素材 = 検査用素材(テクスチャ用途::線形データ);
    for 材質一覧 in [vec![画像を選んだ材質(1, 1, 1, &素材)], vec![画像を選んだ材質(1, 1, 1, &素材), 画像を選んだ材質(2, 2, 2, &別素材)]] {
...
fn 取り外した束は元の位置へ戻る() {
    ...
    assert_eq!(登録簿.束idの並び(), vec![描画束ID::生成する(1), 描画束ID::生成する(2), 描画束ID::生成する(3)], "戻した後の並びが取り外す前と同じである");
}
