...
/// 正準の既定テクスチャが常に占める枚数。材質のテクスチャ枚数はこの枚数を引いて数える。
const 正準の既定テクスチャの枚数: usize = 4;

#[test]
fn 同じ画像を指す3つのテクスチャidを1枚へ重複除去する() {
    let 素材 = 検査用素材(テクスチャ用途::色);
    let mut 供給元 = 検査用供給元::常に成功する();
    let 材質一覧 = [画像を選んだ材質(1, 101, 7, &素材), 画像を選んだ材質(2, 102, 7, &素材), 画像を選んだ材質(3, 103, 7, &素材)];
    let 世代 = 資源表世代を構築する(&mut 供給元, 資源表世代ID::最初(), 余裕のあるレイアウト容量(), &材質一覧).unwrap();
    assert_eq!(世代.画像枚数() - 正準の既定テクスチャの枚数, 1);
...
fn 同じ画像でもビュー契約が違えば別のスロットにする() {
    ...
    let 材質一覧 = [梱包対象材質::生成する(
        ...
        0.0,
        1.0,
        [Some(ベースカラー), Some(金属粗さ), None, None, None, None, None, None],
        [1.0; 4],
    )];
    let 世代 = 資源表世代を構築する(&mut 供給元, 資源表世代ID::最初(), 余裕のあるレイアウト容量(), &材質一覧).unwrap();
    assert_eq!(世代.画像枚数() - 正準の既定テクスチャの枚数, 2);
}
...
fn 同じテクスチャidが別の画像を指したら拒む() {
    ...
    assert!(matches!(結果, Err(レンダラーエラー::材質資源表不正(材質資源表エラー::テクスチャIDの衝突 { テクスチャid: 301 }))));
}
