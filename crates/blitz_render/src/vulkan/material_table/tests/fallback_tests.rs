//! 正準の既定テクスチャの検査。段4aの(vi)用途型からスロットへの解決が梱包工程に閉じ、予約添字を約束せずに
...
fn テクスチャ無しの材質は用途ごとの実在スロットを指し特徴ビットが立たない() {
    ...
    assert_eq!(役割別スロット(レコード), 役割別スロット(レコードを引く(&世代, 2)), "同じ世代の中では用途ごとの正準の既定テクスチャは1つに定まる");
}
...
fn テクスチャ有りの役割だけビットが立ち正準の既定テクスチャと違うスロットを指す() {
    ...
    assert_ne!(テクスチャ有り.スロット(材質テクスチャ役割::ベースカラー), テクスチャ無し.スロット(材質テクスチャ役割::ベースカラー));
    assert_eq!(テクスチャ有り.スロット(材質テクスチャ役割::法線マップ), テクスチャ無し.スロット(材質テクスチャ役割::法線マップ));
}
