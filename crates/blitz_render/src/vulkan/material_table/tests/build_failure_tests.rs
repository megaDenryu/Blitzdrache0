...
fn 構築の失敗は部分生成資源だけを破棄し公開中の世代を変えない() {
    ...
    let 初期世代 = 資源表世代を構築する(&mut 供給元, 資源表世代ID::最初(), 余裕のあるレイアウト容量(), &[材質を作る(1, Some(&素材))]).unwrap();
    let mut 台帳 = 資源表世代台帳::最初の世代を公開する(初期世代);
    let 公開前の世代番号 = 台帳.公開中().世代id().番号();
    let 公開前の画像枚数 = 台帳.公開中().画像枚数();

    // 正準の既定テクスチャ3枚を常駐させた後、材質のテクスチャを常駐させる4回目で失敗させる。
    let mut 失敗する供給元 = 検査用供給元::指定回で失敗する(4);
    let 新世代 = 資源表世代を構築する(&mut 失敗する供給元, 台帳.次の世代idを発行する().unwrap(), 余裕のあるレイアウト容量(), &[材質を作る(2, Some(&素材))]);

    assert!(matches!(新世代, Err(レンダラーエラー::テクスチャ形式不正(テクスチャ形式エラー::リニアフィルタblit非対応))));
