...
pub(in crate::vulkan::material_table) struct 梱包結果 {
    ...
    pub(in crate::vulkan::material_table) 材質別レコード添字: HashMap<大域材質ID, 材質レコード添字>,
}
...
pub(in crate::vulkan::material_table) fn 梱包する(
    ...
    let mut 材質別レコード添字 = HashMap::with_capacity(材質一覧.len());
    for 材質 in 材質一覧 {
        let 添字番号 = u32::try_from(レコード列.len()).unwrap_or_else(|_| panic!("材質レコードの件数がu32に収まらない"));
        let レコード = 一件を梱包する(材質, 台帳, フォールバック)?;
        validate::検証する(材質.材質id(), &レコード, フォールバック)?;
        if 材質別レコード添字.insert(材質.材質id(), 材質レコード添字::生成する(添字番号)).is_some() {
...
/// テクスチャの有無から特徴ビットとスロットを同時に決める。無しなら実在するフォールバックのスロットを置き、
/// 有りなら台帳が解決したスロットを置く。台帳に無いIDは世代の構築が常駐させ損ねた場合だけであり、材質の入力の誤りではない。
fn 一件を梱包する(
    材質: &梱包対象材質<'_>,
    台帳: &テクスチャ台帳,
    フォールバック: &正準フォールバック解決,
) -> Result<世代内材質レコード, 材質資源表エラー> {
    let mut 有無 = [false; 3];
