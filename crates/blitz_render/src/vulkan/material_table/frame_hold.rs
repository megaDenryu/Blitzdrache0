...
//!
//! 注意: 1つのスロットは、フェンス通過を記録するまで次の束縛を受け付けない。未通過のまま上書きすると、
//! 先に束縛した世代を誰も保持していないと台帳が誤判定し、GPUが読んでいる最中の画像を退役させる。
...
impl フレーム保持状況 {
    ...
    /// そのスロットで発行するフレームが束縛する世代を記録する。
    ///
    /// 注意: 受け付けるのは保持なし(未束縛かフェンス通過済み)のときだけである。未通過のスロットへ重ねて束縛するのは
    /// フェンス待機と束縛の呼び出し順が壊れた呼び出し元のバグであり、通すと使用中の画像を退役させるため通さない。
    pub(in crate::vulkan::material_table) fn 束縛を記録する(&mut self, スロット: フレームスロット添字, 世代: 資源表世代ID) {
        assert!(
            self.スロット別[スロット.配列添字()] == フレーム保持::保持なし,
            "フェンス通過を記録していないスロット{}へ資源表世代{}を重ねて束縛した",
            スロット.配列添字(),
            世代.番号()
        );
    ...
    pub(in crate::vulkan::material_table) fn 保持しているか(&self, 世代: 資源表世代ID) -> bool {
        self.スロット別.iter().any(|保持| matches!(保持, フレーム保持::進行中 { 世代: 保持中 } if *保持中 == 世代))
    }
}
