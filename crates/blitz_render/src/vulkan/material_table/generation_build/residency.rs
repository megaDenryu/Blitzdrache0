//! 世代の画像を常駐させる工程。受け取るのは供給元・容量・材質一覧で、返すのはテクスチャ台帳と正準の既定テクスチャの解決である。
...
/// 正準の既定テクスチャを先に常駐させるのは、材質が1件も無い世代でも表が実在する画像だけを持つ状態にするためである。
fn 正準の既定テクスチャを常駐させる<供給元: 常駐テクスチャ供給元>(
    ...
    let 用途別スロット: [テクスチャスロット; 正準の既定テクスチャの用途::全用途.len()] = 用途別スロット.try_into().unwrap_or_else(|_| panic!("正準の既定テクスチャの用途数とスロット数が食い違った"));
...
fn 積んで並びを確かめる<供給元: 常駐テクスチャ供給元>(
    ...
    assert_eq!(画像集合.len(), スロット.配列添字(), "テクスチャスロットの発番順と画像集合の並びが食い違った");
    let 画像 = 供給元.常駐させる(素材)?;
    画像集合.push(画像);
    Ok(())
}
