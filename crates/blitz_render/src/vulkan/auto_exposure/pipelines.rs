...
impl 自動露出のパイプライン一式 {
    pub(crate) fn 生成する(
        確保係: &GPU資源の確保係<'_>, ディスクリプタ: &自動露出のディスクリプタ, 集計シェーダー: &コンピュートシェーダー, 導出シェーダー: &コンピュートシェーダー
    ) -> Result<Self, レンダラーエラー> {
        let device = 確保係.論理デバイス();
        let 集計レイアウト = layout::集計のレイアウトを作る(device, ディスクリプタ.レイアウトのハンドル())?;
        match 集計パイプラインと導出レイアウトと導出パイプラインを作る(確保係, ディスクリプタ, 集計シェーダー, 導出シェーダー, 集計レイアウト) {
            Ok((集計, 導出, 導出レイアウト)) => Ok(Self {
                集計, 集計レイアウト, 導出, 導出レイアウト
            }),
            Err(誤り) => {
...
fn 集計パイプラインと導出レイアウトと導出パイプラインを作る(
    確保係: &GPU資源の確保係<'_>,
    ...
    let 導出レイアウト = match layout::導出のレイアウトを作る(device, ディスクリプタ.レイアウトのハンドル()) {
        Ok(レイアウト) => レイアウト,
        Err(誤り) => {
