//! 全画面三角形パイプラインの固定機能ステート・パイプラインレイアウト・VkPipelineの構築。
//! 前提: シェーダーモジュールの生存期間は呼び出し元(`fullscreen_pipeline`)が持ち、ここでは受け取ったモジュールを参照するだけで破棄しない。
...
pub(super) fn 組み立てる(
    ...
    let プッシュ定数範囲一覧 = if プッシュ定数バイト数 > 0 {
        vec![vk::PushConstantRange::default().stage_flags(vk::ShaderStageFlags::FRAGMENT).offset(0).size(プッシュ定数バイト数)]
    } else {
        Vec::new()
    };
    let layout一覧 = [ディスクリプタlayout];
    let layout_info = vk::PipelineLayoutCreateInfo::default().set_layouts(&layout一覧).push_constant_ranges(&プッシュ定数範囲一覧);
