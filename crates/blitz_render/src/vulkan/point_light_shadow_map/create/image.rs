...
pub(super) fn 立方体配列ビューを作る(確保係: &GPU資源の確保係<'_>, 画像: vk::Image) -> Result<vk::ImageView, レンダラーエラー> {
    点光源の影の画像のビューを作る(確保係, 画像, vk::ImageViewType::CUBE_ARRAY, 0, 点光源の影の層数())
}

/// 指定した層だけを見る2Dビュー。面ごとの影の記録が深度アタッチメントとして使う。
pub(super) fn 層ビューを作る(確保係: &GPU資源の確保係<'_>, 画像: vk::Image, 層: u32) -> Result<vk::ImageView, レンダラーエラー> {
    点光源の影の画像のビューを作る(確保係, 画像, vk::ImageViewType::TYPE_2D, 層, 1)
}

fn 点光源の影の画像のビューを作る(確保係: &GPU資源の確保係<'_>, 画像: vk::Image, 種別: vk::ImageViewType, 開始層: u32, 層数: u32) -> Result<vk::ImageView, レンダラーエラー> {
    let 部分範囲 = vk::ImageSubresourceRange::default()
        ...
        .layer_count(層数);
    let create_info = vk::ImageViewCreateInfo::default().image(画像).view_type(種別).format(点光源の影の形式).subresource_range(部分範囲);
