...
impl 専用メモリ付きバッファ {
    ...
    pub(crate) fn ホスト可視のバイト列を写し取る(&self, device: &ash::Device, バイト数: usize) -> Result<Vec<u8>, レンダラーエラー> {
    ...
    pub(crate) fn ホスト可視の32ビット語列を写し取る(&self, device: &ash::Device, 語数: usize) -> Result<Vec<u32>, レンダラーエラー> {
