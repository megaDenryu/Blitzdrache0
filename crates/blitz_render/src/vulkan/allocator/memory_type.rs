...
impl メモリの置き場 {
    pub(super) fn 適合する型の添字を選ぶ(self, プロパティ一覧: &vk::PhysicalDeviceMemoryProperties, 要求メモリ型ビット: u32) -> Result<u32, レンダラーエラー> {
        match self {
            Self::ホスト可視 => 選ぶ(プロパティ一覧, 要求メモリ型ビット, vk::MemoryPropertyFlags::HOST_VISIBLE | vk::MemoryPropertyFlags::HOST_COHERENT).ok_or(レンダラーエラー::ホスト可視メモリ型なし),
            Self::デバイスローカル => 選ぶ(プロパティ一覧, 要求メモリ型ビット, vk::MemoryPropertyFlags::DEVICE_LOCAL).ok_or(レンダラーエラー::デバイスローカルメモリ型なし),
        }
    }
}

fn 選ぶ(プロパティ一覧: &vk::PhysicalDeviceMemoryProperties, 要求メモリ型ビット: u32, 要求プロパティ: vk::MemoryPropertyFlags) -> Option<u32> {
