...
impl GPU資源の確保係<'_> {
    ...
    pub(super) fn 要件に合う専用メモリを確保する(&self, 要件: vk::MemoryRequirements, 置き場: メモリの置き場, メモリ用途: GPUメモリ用途) -> Result<vk::DeviceMemory, レンダラーエラー> {
        let メモリ型添字 = 置き場.適合する型の添字を選ぶ(&self.メモリプロパティ, 要件.memory_type_bits)?;
        let alloc_info = vk::MemoryAllocateInfo::default().allocation_size(要件.size).memory_type_index(メモリ型添字);
