...
use crate::vulkan::descriptor::{宣言から作ったセットレイアウト, 宣言から割り当てたセット, 照明問い合わせのバッファ組, 結ぶ現物};
...
impl クラスタ選別のディスクリプタ {
    ...
    pub(super) fn 生成する(device: &ash::Device, バッファ組一覧: &[照明問い合わせのバッファ組]) -> Result<Self, レンダラーエラー> {
...
fn バッファ組を結ぶ(device: &ash::Device, セット: &宣言から割り当てたセット<4>, バッファ組: 照明問い合わせのバッファ組) {
