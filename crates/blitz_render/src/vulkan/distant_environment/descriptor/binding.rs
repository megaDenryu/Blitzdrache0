//! 遠方環境ディスクリプタのバインド番号と型の対応を持つ。レイアウト・プールの容量・セットへの書き込みが
//! すべてこの1箇所の対応を読むため、番号と型の食い違いが起こらない。
//!
//! 注意: 番号は`shaders/distant_environment.slang`の`vk::binding`と一致させる。
//! 注意: 読むスカイビューも書き込み先も、レイアウトはすべてGENERALである。
