...
//!
//! ビューを2つ持つのは、書く側と読む側で必要な種別が違うためである。生成のコンピュートは`配列ビュー`へ層番号を奥行きに取る
//! 2次元配列として書き、消費側は`立方体ビュー`を向きで参照できる立方体として読む。Vulkanは同じ画像から両方のビューを作れるため、
//! 画像そのものは1つで足りる。
//!
...
pub(in crate::vulkan) struct 遠方環境の立方体画像 {
    ...
    pub(in crate::vulkan) 配列ビュー: vk::ImageView,   // 生成のコンピュートが書き込む先
    pub(in crate::vulkan) 立方体ビュー: vk::ImageView, // 消費側が向きで参照する先。立方体として標本する
