...
//! 検査が読み戻すためである。画素形式は大気のベイク済み画像と同じR16G16B16A16_SFLOATであり、
//! ストレージ書き込みはVulkan仕様の必須対応形式のため実行時の対応確認は行わない。
