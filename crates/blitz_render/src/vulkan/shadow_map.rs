//! 方向光の多段シャドウマップ(判断35・OW5第5段): 2048x2048の層を距離区分数ぶん重ねた
...
pub(crate) const シャドウマップ一辺: u32 = crate::cascade::距離区分の解像度;
...
/// 1テクセルが占める正規化座標の幅。PCFの標本位置の刻みとして多段影定数へ載せ、シェーダーが読む。
/// 解像度は2の冪であるため、この逆数はf32で誤差なく表せる。
pub(crate) fn テクセル尺度() -> f32 {
    let 一辺 = u16::try_from(シャドウマップ一辺).unwrap_or_else(|_| panic!("シャドウマップ一辺がu16に収まらない: {シャドウマップ一辺}"));
    1.0 / f32::from(一辺)
}
...
pub(crate) struct シャドウマップ {
    ...
    /// 距離区分ごとに1層だけを見るビュー。距離区分別のシャドウ記録が深度アタッチメントとして束縛する。
    pub(crate) 距離区分別のビュー一覧: [vk::ImageView; 距離区分数],
