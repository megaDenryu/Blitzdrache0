...
pub(crate) fn バッファバリアを導出する(初期状態: &HashMap<バッファハンドル, バッファ状態>, パス列: &[パスリソース使用]) -> Vec<Vec<バッファバリア記述>> {
    let mut 現在状態 = 初期状態.clone();
    パス列
        .iter()
        .map(|パス| パス.読みバッファ.iter().chain(パス.書きバッファ.iter()).filter_map(|&(ハンドル, 用途)| 差分を計算する(&mut 現在状態, ハンドル, 用途)).collect())
        .collect()
}
