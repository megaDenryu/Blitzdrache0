...
pub(crate) use initial_state::{
    前フレームhdr中間画像読み直後状態, 前フレームコンピュート読み直後状態, 前フレームシャドウマップ読み直後状態,
    前フレーム今のフレームの色読み直後状態, 前フレーム動きベクトル書き込み直後状態, 前フレーム深度書き込み直後状態, 前フレーム画素段読み直後状態,
    前フレーム粒子読み直後状態, 前フレーム頂点入力読み直後状態, 取得直後の色画像状態, 局所可視度の画像の前フレーム直後状態,
    履歴画像の前フレーム直後状態, 焼いた画像を参照するだけのときの初期状態, 焼いた画像を焼き直すときの初期状態, 直前の送信でコンピュート書き直後状態,
};
