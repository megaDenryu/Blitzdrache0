...
use super::usage::image_usage_mapping::{状態へ写像する, 書き込みを含むか};
...
pub(crate) fn 導出する(
    ...
    for パス in パス列 {
        let バリア一覧 = パス
            ...
            .collect();
        結果.push(地点別バリア { 地点: 遷移地点::パスの前 { 名前: パス.名前 }, バリア一覧 });
    }
    ...
    let 終端バリア一覧 = 最終用途
        ...
        .filter_map(|&(ハンドル, 用途)| 差分を計算する(&mut 現在状態, ハンドル, 用途))
        .collect();
    結果.push(地点別バリア { 地点: 遷移地点::グラフ終端, バリア一覧: 終端バリア一覧 });
...
fn 差分を計算する(
    ...
    let 省略可能 = 前状態.layout == 今状態.layout
        && !書き込みを含むか(前状態.access)
        && !書き込みを含むか(今状態.access);
    if 省略可能 {
        return None;
