...
pub(crate) fn バリアを導出する(初期状態: &HashMap<画像ハンドル, 画像状態>, パス列: &[パスリソース使用], 最終用途: &[(画像ハンドル, 画像用途)]) -> Vec<地点別バリア> {
...
fn 差分を計算する(現在状態: &mut HashMap<画像ハンドル, 画像状態>, ハンドル: 画像ハンドル, 今用途: 画像用途) -> Option<画像バリア記述> {
    let 今状態 = 状態へ写像する(今用途);
    let 前状態 = *現在状態.get(&ハンドル).unwrap_or_else(|| panic!("グラフに未登録の画像ハンドルがパスで使用された"));
