...
//! `クエリプール`が`Some`ならパス前後にタイムスタンプを書く。
...
pub(crate) struct グラフ実行器<'デバイス> {
    積み先: GPU命令の積み先<'デバイス>,
    クエリプール: Option<vk::QueryPool>, // 計測が有効なフレームだけ`Some`
}
...
impl<'デバイス> グラフ実行器<'デバイス> {
    ...
    pub(crate) fn グラフを積む(&self, グラフ: グラフ<'_>) -> Vec<(&'static str, u32)> {
        ...
        let 地点別画像バリア列 = barrier_derivation::バリアを導出する(&画像初期状態表, &リソース使用列, &最終用途表);
        let パス別バッファバリア列 = buffer_barrier_derivation::バッファバリアを導出する(&バッファ初期状態表, &リソース使用列);

        let mut 地点別画像バリア列 = 地点別画像バリア列.into_iter();
        let mut 計測マッピング = Vec::new();
        for (パス添字, パス) in パス列.into_iter().enumerate() {
            let 地点別: 地点別バリア = 地点別画像バリア列.next().unwrap_or_else(|| panic!("パス数とバリア導出結果の数が一致しない(実装のバグ)"));
            let バッファバリア一覧 = パス別バッファバリア列.get(パス添字).unwrap_or_else(|| panic!("パス数とバッファバリア導出結果の数が一致しない(実装のバグ)"));
            barrier_apply::バリアを発行する(self.積み先, &画像レジストリ, &バッファレジストリ, &地点別.バリア一覧, バッファバリア一覧);
        ...
        let 終端: 地点別バリア = 地点別画像バリア列.next().unwrap_or_else(|| panic!("グラフ終端のバリア導出結果が欠けている(実装のバグ)"));
        barrier_apply::バリアを発行する(self.積み先, &画像レジストリ, &バッファレジストリ, &終端.バリア一覧, &[]);
