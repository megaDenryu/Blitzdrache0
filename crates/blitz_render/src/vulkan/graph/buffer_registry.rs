...
impl バッファレジストリ {
    ...
    fn ハンドルの指すエントリを参照する(&self, ハンドル: バッファハンドル) -> &バッファエントリ {
        let 添字usize = usize::try_from(ハンドル.添字()).unwrap_or_else(|_| panic!("バッファハンドルの添字がusizeに収まらない"));
        let エントリ = self.エントリ一覧.get(添字usize).unwrap_or_else(|| panic!("グラフに登録されていないバッファハンドルが参照された"));
        if エントリ.世代 != ハンドル.世代() {
            panic!("世代の異なるバッファハンドルが参照された(古いグラフのハンドルの使い回し)");
        }
        エントリ
    }

    pub(crate) fn バッファを取得する(&self, ハンドル: バッファハンドル) -> vk::Buffer {
        self.ハンドルの指すエントリを参照する(ハンドル).バッファ
    }
}
