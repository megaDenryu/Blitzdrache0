...
//! 大気のベイク済み画像の初期状態だけは、焼く場合と参照する場合で1つの不変条件の表裏になるため`atmosphere_lut`が持つ。

mod atmosphere_lut;

pub(crate) use atmosphere_lut::{
    大気のベイク済み画像を参照する画像の初期状態, 大気のベイク済み画像を焼く画像の初期状態
};
...
/// シャドウマップの、前フレーム「フラグメントシェーダー読み」直後を想定した状態。
///
...
/// HDR中間画像の、前フレーム「トーンマップパスのフラグメント読み」直後を想定した状態(判断38)。
///
/// 注意: HDR画像はグラフの外(レンダラー)で毎フレーム使い回す。グラフ内で最後に
/// この画像を使うパスは常にトーンマップ(フラグメント読み)のため、次フレーム冒頭の
