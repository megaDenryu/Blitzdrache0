...
/// 直前の送信のコンピュートパスが書いたまま、まだ何も同期していないバッファの状態。
/// 提示のフレームの外で1回だけ積む読み戻し(布の粒子と乗数)が、コンピュート書きから転送読みへのバリアを導かせるために使う。
/// 読み直後の状態で登録すると読み→読みとして省略され、直前の送信の書き込みが転送へ見えない。
pub(crate) fn 直前の送信でコンピュート書き直後状態() -> バッファ状態 {
