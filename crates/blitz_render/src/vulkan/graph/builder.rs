...
impl<'a> グラフ<'a> {
    ...
    pub(crate) fn 画像を登録する(&mut self, 画像: vk::Image, 画像ビュー: vk::ImageView, アスペクト: 画像アスペクト, 初期状態: 画像状態, 寸法: vk::Extent2D) -> 画像ハンドル {
    ...
    pub(crate) fn 分解する(
        ...
        (self.画像レジストリ, self.バッファレジストリ, self.画像初期状態表, self.バッファ初期状態表, self.最終用途表, self.パス列)
    }
}
