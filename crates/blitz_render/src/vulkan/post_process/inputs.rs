...
pub(crate) struct ポスト描画入力 {
    ...
    pub(crate) 自動露出: Option<自動露出描画入力>, // 露出方式がヒストグラム自動の世界だけ`Some`。時刻別固定の世界ではパスを1本も積まない。
}

impl ポスト処理一式 {
    pub(crate) fn 描画入力を作る(&self, 露出: f32, 自動露出の経過秒: f32) -> ポスト描画入力 {
        let (前処理set, 縮小set一覧, 拡大set一覧) = self.光のにじみ.確保済みのセット群().束縛するセットのハンドル();
        ポスト描画入力 {
            光のにじみ: 光のにじみ描画入力 {
                前処理パイプライン: self.光のにじみ.前処理.パイプラインのハンドル(),
                前処理レイアウト: self.光のにじみ.前処理.パイプラインレイアウトのハンドル(),
                縮小パイプライン: self.光のにじみ.縮小.パイプラインのハンドル(),
                縮小レイアウト: self.光のにじみ.縮小.パイプラインレイアウトのハンドル(),
                拡大パイプライン: self.光のにじみ.拡大.パイプラインのハンドル(),
                拡大レイアウト: self.光のにじみ.拡大.パイプラインレイアウトのハンドル(),
                前処理セット: 前処理set,
                縮小セット一覧: 縮小set一覧,
                拡大セット一覧: 拡大set一覧,
            },
            明るさの圧縮: 明るさの圧縮描画入力 {
                パイプライン: self.明るさの圧縮.パイプライン.パイプラインのハンドル(),
                レイアウト: self.明るさの圧縮.パイプライン.パイプラインレイアウトのハンドル(),
                ディスクリプタセット: self.明るさの圧縮.束縛するセットのハンドル(),
                露出,
                芸術的バイアスの補正段: self.自動露出.方式().芸術的バイアスの補正段(),
                自動か: u32::from(self.自動露出.方式().パスを積むか()),
            },
            自動露出: self.自動露出.方式().パスを積むか().then(|| self.自動露出.描画入力を作る(自動露出の経過秒)),
        }
    }
