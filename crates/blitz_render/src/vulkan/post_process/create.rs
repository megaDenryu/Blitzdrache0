...
fn 束を生成する(材料: &生成材料<'_>, hdr: &HDRターゲット, ピラミッド: &光のにじみピラミッド) -> Result<(光のにじみ一式, 明るさの圧縮一式, 自動露出一式), レンダラーエラー> {
    let device = 材料.転送係.論理デバイス();
    let 光のにじみ = 光のにじみ一式::生成する(材料.転送係.確保係(), &材料.シェーダー.光のにじみ前処理, &材料.シェーダー.光のにじみ縮小, &材料.シェーダー.光のにじみ拡大, hdr.画像ビュー, ピラミッド)?;
    let 圧縮結果 = 明るさの圧縮一式::生成する(材料.転送係.確保係(), 材料.スワップチェーン画像形式, &材料.シェーダー.明るさの圧縮, hdr.画像ビュー, ピラミッド.最終ビュー());
