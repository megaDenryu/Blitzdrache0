//! ポストプロセス段の資源一式(判断38・39・41): HDR中間画像・ブルームピラミッド・ブルーム一式・トーンマップ一式。
...
impl ポスト処理一式 {
    ...
    /// 読み手であるトーンマップ・ブルームのパイプラインとディスクリプタを先に、読まれる側の画像を後に破棄する。
    /// 前提: レンダラー全体の破棄順は renderer/destroy.rs が持ち、この束はその1段として呼ばれる(GPU待機済み)。
    pub(crate) fn 破棄する(&self, device: &GPUデバイス) {
        self.トーンマップ.破棄する(device);
        self.ブルーム.破棄する(device);
        self.ブルームピラミッド.破棄する(device);
