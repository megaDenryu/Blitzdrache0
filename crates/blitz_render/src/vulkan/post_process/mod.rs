//! ポスト処理の処理位置(`光のにじみと明るさの圧縮`)の資源一式(判断38・39・41): HDR中間画像・光のにじみピラミッド・光のにじみ一式・明るさの圧縮一式・自動露出一式。
//! この5つはフレーム構成にポスト処理の処理位置があるときだけ同時に存在し、一部だけが存在する状態を持たないため、
...
impl ポスト処理一式 {
    pub(crate) fn 生成する(
        転送係: ステージング経由の転送係<'_>, スワップチェーン画像形式: vk::Format, 寸法: vk::Extent2D, シェーダー: &シェーダー束, 自動露出の設定: 自動露出の設定
    ...
    pub(crate) fn 自動露出の観測を読み戻す(&self, 転送係: ステージング経由の転送係<'_>) -> Result<crate::auto_exposure::自動露出の観測, レンダラーエラー> {
