...
impl スキニングバッファ {
    pub(in crate::vulkan::skinning) fn 生成する(転送係: ステージング経由の転送係<'_>, 頂点一覧: &[頂点], 素材: &スキンメッシュ素材) -> Result<Self, レンダラーエラー> {
        let 確保係 = 転送係.確保係();
        ...
        let 行列一覧 = 台帳.フレームスロットごとに積む(確保係.フレームスロットごとのホスト可視バッファを確保して書き込む(&行列初期値, vk::BufferUsageFlags::STORAGE_BUFFER))?;

        // スキン済み頂点: コンピュートが書き、シーン/シャドウが頂点入力として読む(判断44の合流点)。
        let 出力サイズ = u64::try_from(頂点バイト列.len()).unwrap_or_else(|_| panic!("出力バッファ長がu64に収まらない"));
        let 出力 = 台帳.積む(確保係.デバイスローカルバッファを確保する(出力サイズ, vk::BufferUsageFlags::STORAGE_BUFFER | vk::BufferUsageFlags::VERTEX_BUFFER))?;

        Ok(Self { レスト頂点, 属性, 行列一覧, 出力 })
    }
}
