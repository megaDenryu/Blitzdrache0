...
impl スキニング一式 {
    pub(crate) fn 生成する(転送係: ステージング経由の転送係<'_>, 頂点一覧: &[頂点], 素材: &スキンメッシュ素材, シェーダー: &コンピュートシェーダー) -> Result<Self, レンダラーエラー> {
        ...
        let ディスクリプタ = match descriptor::スキニングディスクリプタを確保する(device, &バッファ) {
            Ok(ディスクリプタ) => ディスクリプタ,
            Err(誤り) => {
                バッファ.破棄する(device);
                return Err(誤り);
        ...
        let パイプライン = match pipeline::スキニングパイプラインを生成する(確保係, ディスクリプタ.レイアウトのハンドル(), シェーダー.コード()) {
            Ok(パイプライン) => パイプライン,
            Err(誤り) => {
                ディスクリプタ.破棄する(device);
                バッファ.破棄する(device);
