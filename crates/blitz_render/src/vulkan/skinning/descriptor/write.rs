...
pub(super) fn 書く(device: &ash::Device, セット: &宣言から割り当てたセット<4>, バッファ: &スキニングバッファ, フレーム添字: フレームスロット添字) {
    セット.書き込み先(device).並びの位置ごとに結ぶ([
        結ぶ現物::バッファ全体(バッファ.レスト頂点のバッファ()),
        結ぶ現物::バッファ全体(バッファ.属性のバッファ()),
        結ぶ現物::バッファ全体(バッファ.行列のバッファ(フレーム添字)),
