...
impl 空中遠近合成資源 {
    ...
    pub(super) fn 生成する(
        確保係: &GPU資源の確保係<'_>, カラー形式: vk::Format, ビューとパスのセットのレイアウト: vk::DescriptorSetLayout, 束縛先: &空中遠近合成の束縛先, シェーダー: &シェーダー一式
    ) -> Result<Self, レンダラーエラー> {
        let device = 確保係.論理デバイス();
        let ディスクリプタ = 空中遠近合成ディスクリプタ::生成する(確保係, 束縛先)?;
        let レイアウト一覧 = [ビューとパスのセットのレイアウト, ディスクリプタ.レイアウトのハンドル()];
        match 空中遠近合成パイプライン::生成する(確保係, カラー形式, &レイアウト一覧, シェーダー) {
            Ok(パイプライン) => Ok(Self { パイプライン, ディスクリプタ }),
            Err(誤り) => {
                ディスクリプタ.破棄する(device);
                Err(誤り)
            }
    ...
    pub(crate) fn 描画入力を作る(&self, ディスクリプタセット: vk::DescriptorSet, フレーム添字: フレームスロット添字, 最遠距離: f32) -> 空中遠近合成描画入力 {
        空中遠近合成描画入力 {
