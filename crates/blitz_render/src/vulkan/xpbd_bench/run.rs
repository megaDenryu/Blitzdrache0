...
impl XPBD計測一式 {
    pub(super) fn 走らせる(&self, 環境: &ウィンドウなし実行GPU環境, 条件: &XPBD計測の条件, 受け皿: &読み戻しの受け皿) -> Result<読み戻しの中身, レンダラーエラー> {
...
pub(crate) fn 走らせて読み戻す(素材: &XPBD計測素材, 条件: &XPBD計測の条件, シェーダー: &XPBDシェーダー一式) -> Result<(読み戻しの中身, 検証観測), レンダラーエラー> {
...
fn 環境で走らせる(環境: &ウィンドウなし実行GPU環境, 素材: &XPBD計測素材, 条件: &XPBD計測の条件, シェーダー: &XPBDシェーダー一式) -> Result<読み戻しの中身, レンダラーエラー> {
    let device = 環境.device();
