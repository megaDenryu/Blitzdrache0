...
impl XPBD計測一式 {
    fn 生成する(転送係: ステージング経由の転送係<'_>, 素材: &XPBD計測素材, 条件: &XPBD計測の条件, シェーダー: &XPBDシェーダー一式) -> Result<Self, レンダラーエラー> {
        ...
        let パイプライン群 = match pipelines::XPBD計測パイプライン群::生成する(転送係.確保係(), ディスクリプタ.レイアウトのハンドル(), シェーダー, 条件.方式) {
            Ok(群) => 群,
            Err(誤り) => {
                ディスクリプタ.破棄する(device);
                バッファ.破棄する(device);
