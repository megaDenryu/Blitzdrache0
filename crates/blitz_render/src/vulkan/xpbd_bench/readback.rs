...
impl 読み戻しの受け皿 {
    ...
    pub(super) fn コピーを積む<'a>(&'a self, グラフ: &mut グラフ<'a>, 点: バッファハンドル, 乗数: バッファハンドル) {
        let 点のバイト数 = usizeの長さをバイト数u64へ変換する(self.点の数 * 16);
        let 乗数のバイト数 = usizeの長さをバイト数u64へ変換する(self.拘束の数 * 4);
...
fn usizeの長さをバイト数u64へ変換する(値: usize) -> u64 {
