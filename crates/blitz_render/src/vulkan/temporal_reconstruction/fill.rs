...
//!
//! 動きベクトル画像と今のフレームの色の画像を埋めないのは、シーン描画のパスが毎フレーム消去してから書くためである。
//! 前のフレームの内容を1画素も引き継がないため、確保直後の中身が結果へ現れる経路が無い。
...
fn 画像を汎用レイアウトへ遷移する(積み先: GPU命令の積み先<'_>, 画像: vk::Image) {
    let device = 積み先.論理デバイス();
    let command_buffer = 積み先.コマンドバッファ();
