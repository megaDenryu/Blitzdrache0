...
impl super::時間再構成一式 {
    ...
    pub(crate) fn 合成入力を据える(&mut self, 確保係: &GPU資源の確保係<'_>, 入力: &時間再構成の合成入力) -> Result<(), レンダラーエラー> {
...
impl 合成入力の注入一式 {
    pub(crate) fn 生成する(確保係: &GPU資源の確保係<'_>, 入力: &時間再構成の合成入力) -> Result<Self, レンダラーエラー> {
        let device = 確保係.論理デバイス();
        let バイト列一覧 = [
    ...
    pub(crate) fn 描画入力を作る(&self) -> 合成入力の注入入力 {
        合成入力の注入入力 {
            今のフレームの色: self.バッファ一覧[0].バッファのハンドル(),
            履歴: self.バッファ一覧[1].バッファのハンドル(),
            動きベクトル: self.バッファ一覧[2].バッファのハンドル(),
            深度: self.バッファ一覧[3].バッファのハンドル(),
            寸法: self.寸法,
        }
    }
