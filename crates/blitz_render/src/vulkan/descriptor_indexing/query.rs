...
pub(crate) fn 機能を採取する(instance: &ash::Instance, 物理デバイス: vk::PhysicalDevice) -> ディスクリプタ索引機能 {
    ...
    ディスクリプタ索引機能::生成する(vulkan12機能.shader_sampled_image_array_non_uniform_indexing == vk::TRUE, vulkan12機能.descriptor_binding_partially_bound == vk::TRUE)
}
...
pub(crate) fn 上限を採取する(instance: &ash::Instance, 物理デバイス: vk::PhysicalDevice) -> ディスクリプタ索引上限 {
    ...
    ディスクリプタ索引上限::生成する(性質.limits.max_per_stage_descriptor_sampled_images, 性質.limits.max_descriptor_set_sampled_images, 性質.limits.max_per_stage_resources)
}
