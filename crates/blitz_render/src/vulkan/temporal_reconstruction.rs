...
//!
//! 方式が使わないの世界でも4枚を確保するのは、確保するかどうかを方式で分けると、シーン描画のパイプラインが宣言するカラー添付の枚数が世界ごとに変わり、パイプラインの一致条件が1つ増えるためである。動きベクトルは方式に依らず毎フレーム書く。方式で分かれるのは、時間再構成のパスを積むかどうかと、履歴画像へ初期値を書き込むかどうかだけである(`参照: _doc/設計/時間再構成.md`「判断e: 世界宣言の第4軸と検収の保護」)。
...
impl 時間再構成一式 {
    ...
    /// 判断cの3条件が起きたときに呼ぶ。次に積むパスは履歴を1画素も混ぜず、今のフレームの結果をそのまま出す。
    pub(crate) fn 履歴を無効にする(&mut self) {
        self.履歴の状態.無効にする();
    }

    /// 提示まで到達したフレームの後で呼ぶ。書き込んだ側を次のフレームの読み側へ回し、履歴を有効にする。
    pub(crate) fn 提示したフレームを反映する(&mut self) {
        self.履歴の状態.提示したフレームを反映する();
    }
