...
pub(crate) fn 生成する(
    ...
    let 割当数 = u32::try_from(フレームインフライト数).unwrap_or_else(|_| panic!("フレームインフライト数がu32に収まらない"));
    ...
    let Ok(command_buffer一覧): Result<[vk::CommandBuffer; フレームインフライト数], _> = command_buffer一覧.try_into() else {
