...
pub(super) fn ui頂点のバインド記述と属性記述を組み立てる() -> (vk::VertexInputBindingDescription, [vk::VertexInputAttributeDescription; 3]) {
    ...
    let 属性記述一覧 = [
        ...
        vk::VertexInputAttributeDescription::default().location(1).binding(0).format(vk::Format::R32G32_SFLOAT).offset(uv開始位置),
        vk::VertexInputAttributeDescription::default().location(2).binding(0).format(vk::Format::R8G8B8A8_UNORM).offset(色開始位置),
    ];
    (バインド記述, 属性記述一覧)
}
