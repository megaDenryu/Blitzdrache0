...
pub(super) fn データをスロットへ確保して書き込む(
    スロット: &mut Option<バッファスロット>, 確保係: &GPU資源の確保係<'_>, データ: &[u8], 用途: vk::BufferUsageFlags
    ...
    Ok(スロット.as_ref().unwrap_or_else(|| panic!("UIジオメトリバッファスロットの確保に失敗した(実装のバグ)")).バッファのハンドル())
}
