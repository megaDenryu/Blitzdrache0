...
//! 進行中フレーム数ぶん多重化し、容量不足時のみ作り直す(判断33)。
...
pub(crate) struct UIジオメトリバッファ {
    頂点一覧: [Option<バッファスロット>; 進行中フレーム数],
    インデックス一覧: [Option<バッファスロット>; 進行中フレーム数],
}
...
impl UIジオメトリバッファ {
    ...
    pub(crate) fn 書き込む(
        ...
        let 頂点バッファ = slot::データをスロットへ確保して書き込む(&mut self.頂点一覧[フレーム添字.配列添字()], 確保係, &頂点バイト列, vk::BufferUsageFlags::VERTEX_BUFFER)?;
        let インデックスバッファ = slot::データをスロットへ確保して書き込む(&mut self.インデックス一覧[フレーム添字.配列添字()], 確保係, &インデックスバイト列, vk::BufferUsageFlags::INDEX_BUFFER)?;
