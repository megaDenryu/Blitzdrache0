...
pub(super) fn ホストの画素列を画像へ転送する(転送係: ステージング経由の転送係<'_>, image: vk::Image, 幅: u32, 高さ: u32, rgba8: &[u8]) -> Result<(), レンダラーエラー> {
    let ステージング = 転送係.確保係().ホスト可視バッファを確保して書き込む(rgba8, vk::BufferUsageFlags::TRANSFER_SRC)?;

    let 実行結果 = ステージングバッファから唯一の縮小段レベルへ転送する(転送係.転送環境(), ステージング.バッファのハンドル(), image, 幅, 高さ);

    ステージング.破棄する(転送係.論理デバイス());
    実行結果
}

fn ステージングバッファから唯一の縮小段レベルへ転送する(転送環境: &転送実行環境, ステージングバッファ: vk::Buffer, image: vk::Image, 幅: u32, 高さ: u32) -> Result<(), レンダラーエラー> {
    let 一時 = 転送環境.転送コマンドを積み始める()?;
    let 積み先 = 一時.積み先();
    barrier::画像を転送先レイアウトへ遷移する(積み先, image);
    ステージングバッファから画像へコピーする(積み先, ステージングバッファ, image, 幅, 高さ);
    barrier::画像をシェーダー読み取り専用レイアウトへ遷移する(積み先, image);
    一時.送信して完了を待つ()
}

fn ステージングバッファから画像へコピーする(積み先: GPU命令の積み先<'_>, ステージングバッファ: vk::Buffer, image: vk::Image, 幅: u32, 高さ: u32) {
