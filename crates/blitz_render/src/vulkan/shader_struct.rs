...
pub(crate) fn 読み取る(原文: &str, 構造体名: &str) -> Result<シェーダー構造体の並び, シェーダー構造体の読み取りの破れ> {
    let 宣言 = format!("struct {構造体名}");
    let mut 行一覧 = 原文.lines().skip_while(|行| 行.trim() != 宣言);
    if 行一覧.next().is_none() {
        return Err(シェーダー構造体の読み取りの破れ::宣言が見つからない { 構造体名: 構造体名.to_string() });
    }
    ...
    for 行 in 行一覧 {
        ...
        if 内容 == "};" {
            return Ok(シェーダー構造体の並び { フィールド一覧, バイト長: 位置 });
        }
        let (名前, 大きさ) = フィールドを読む(内容)?;
        フィールド一覧.push((名前, 位置));
        位置 += 大きさ;
    }
    Err(シェーダー構造体の読み取りの破れ::宣言が閉じていない { 構造体名: 構造体名.to_string() })
}

/// 「[column_major] 型 名前[個数];」の1行から、フィールド名と占めるバイト数を求める。
fn フィールドを読む(行: &str) -> Result<(String, usize), シェーダー構造体の読み取りの破れ> {
    let 語一覧: Vec<&str> = 行.trim_end_matches(';').split_whitespace().skip_while(|語| *語 == "column_major").collect();
    ...
    let 単位バイト数 = match *型 {
        ...
        "uint" => 4,
        その他 => {
            return Err(シェーダー構造体の読み取りの破れ::バイト詰めの規則が決まっていない型である { 型名: その他.to_string() });
        }
    };
