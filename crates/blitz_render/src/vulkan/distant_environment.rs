...
//! 大気のベイク済み画像方式の空を持つフレーム構成で、かつ遠方環境を焼く構成のときだけ`描画の処理位置の資源`が保持する。
//!
...
pub(crate) use inputs::{計算の班数を求める, 遠方環境が借りる束縛先, 遠方環境の即時定数, 遠方環境の生成入力, 遠方環境の画像入力};
...
impl 遠方環境一式 {
    pub(crate) fn 生成する(
        確保係: &GPU資源の確保係<'_>, 解像度: 遠方環境の解像度, 借りる束縛先: 遠方環境が借りる束縛先<'_>, シェーダー: &コンピュートシェーダー
    ...
    pub(crate) fn 生成入力を作る(&self, フレーム添字: フレームスロット添字, 即時定数: 遠方環境の即時定数) -> 遠方環境の生成入力 {
        遠方環境の生成入力 {
