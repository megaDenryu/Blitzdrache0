//! 布ディスクリプタセットへの14バッファ書き込み。並びの順と番号と種別は`descriptor`が持つ束縛の宣言が決める。
...
pub(super) fn 布の束縛先をディスクリプタセットへ書き込む(
    device: &ash::Device, セット: &宣言から割り当てたセット<束縛の本数>, バッファ: &布バッファ, スキン済み頂点buffer: Option<vk::Buffer>, フレーム添字: フレームスロット添字
) {
    セット.書き込み先(device).並びの位置ごとに結ぶ([
        ...
        結ぶ現物::バッファ全体(バッファ.曲げ拘束の引数.バッファのハンドル()),
    ]);
}
