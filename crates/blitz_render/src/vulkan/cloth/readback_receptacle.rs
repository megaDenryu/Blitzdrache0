...
impl 読み戻しの受け皿 {
    pub(super) fn 確保する(確保係: &GPU資源の確保係<'_>, 粒子数: u32, 拘束の数: u32) -> Result<Self, レンダラーエラー> {
        let 粒子のバイト数 = 件数をusizeへ変換する(粒子数) * 粒子1件のバイト数;
        let 乗数のバイト数 = 件数をusizeへ変換する(拘束の数) * 乗数1件のバイト数;
    ...
    pub(super) fn 読む(&self, device: &ash::Device) -> Result<布の読み戻し, レンダラーエラー> {
        ...
        Ok(布の読み戻し {
            位置一覧: 粒子のバイト列
                ...
                .collect(),
            ラグランジュ乗数一覧: 乗数のバイト列.chunks_exact(乗数1件のバイト数).map(|塊| 単精度(塊, 0)).collect(),
        })
    }
