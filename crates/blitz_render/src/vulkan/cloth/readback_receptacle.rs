...
//! 乗数のバッファは距離拘束の後ろに目標拘束、その後ろに曲げ拘束が続く1本であり、読む口が距離拘束の数と目標拘束の数で3つの並びへ分ける。
...
impl 読み戻しの受け皿 {
    pub(super) fn 確保する(確保係: &GPU資源の確保係<'_>, 粒子数: u32, 拘束の数: u32, 目標拘束の数: u32, 曲げ拘束の数: u32) -> Result<Self, レンダラーエラー> {
        let 粒子のバイト数 = 件数をusizeへ変換する(粒子数) * 粒子1件のバイト数;
        let 距離拘束の数 = 件数をusizeへ変換する(拘束の数);
        let 目標拘束の数 = 件数をusizeへ変換する(目標拘束の数);
        let 乗数のバイト数 = (距離拘束の数 + 目標拘束の数 + 件数をusizeへ変換する(曲げ拘束の数)) * 乗数1件のバイト数;
    ...
    pub(super) fn 読む(&self, device: &ash::Device) -> Result<布の読み戻し, レンダラーエラー> {
        ...
        let mut 目標拘束のラグランジュ乗数一覧 = 乗数一覧.split_off(self.距離拘束の数);
        let 曲げ拘束のラグランジュ乗数一覧 = 目標拘束のラグランジュ乗数一覧.split_off(self.目標拘束の数);
        Ok(布の読み戻し {
            位置一覧: 粒子のバイト列.chunks_exact(粒子1件のバイト数).map(|塊| [単精度(塊, 0), 単精度(塊, 4), 単精度(塊, 8)]).collect(),
            ラグランジュ乗数一覧: 乗数一覧,
            目標拘束のラグランジュ乗数一覧,
            曲げ拘束のラグランジュ乗数一覧,
        })
