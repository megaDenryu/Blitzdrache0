...
pub(super) fn バイト列にする(固定: &固定部, 入力: &布フレーム入力) -> Vec<u8> {
    ...
    // gravityDt
    f32列を積む(&mut バイト列, &[固定.定数.重力[0], 固定.定数.重力[1], 固定.定数.重力[2], 固定.定数.刻み幅.秒().値()]);
    // gridOrigin + セル寸法(=粒子間隔)
    f32列を積む(&mut バイト列, &[固定.定数.グリッド原点[0], 固定.定数.グリッド原点[1], 固定.定数.グリッド原点[2], 固定.定数.粒子間隔]);
    // capsuleA + 半径
    f32列を積む(&mut バイト列, &[入力.カプセル端点a[0], 入力.カプセル端点a[1], 入力.カプセル端点a[2], 入力.カプセル半径]);
    // capsuleB + 予備
    ...
    f32列を積む(&mut バイト列, &[固定.定数.粒子間隔 * 0.4, 固定.定数.刻み幅.刻み幅の2乗の逆数(), 固定.定数.速度減衰, 0.0]);
    バイト列
}
