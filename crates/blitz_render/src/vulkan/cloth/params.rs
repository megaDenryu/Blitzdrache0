...
pub(super) fn バイト列にする(固定: &固定部, 入力: &布フレーム入力) -> Vec<u8> {
    ...
    // gravityDt(dtは固定歩進1/60。判断47と同じ規律)
    f32列を積む(&mut バイト列, &[固定.定数.重力[0], 固定.定数.重力[1], 固定.定数.重力[2], 1.0 / 60.0]);
    // gridOrigin + セル寸法(=粒子間隔)
    f32列を積む(
        &mut バイト列,
        &[固定.定数.グリッド原点[0], 固定.定数.グリッド原点[1], 固定.定数.グリッド原点[2], 固定.定数.粒子間隔],
    );
    // capsuleA + 半径
    f32列を積む(
        &mut バイト列,
    ...
    // misc { 衝突半径(粒子間隔の0.4倍), 過緩和, 速度減衰, 予備 }
    f32列を積む(&mut バイト列, &[固定.定数.粒子間隔 * 0.4, 固定.定数.過緩和, 固定.定数.速度減衰, 0.0]);
    バイト列
}
