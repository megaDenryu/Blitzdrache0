...
impl 自動露出一式 {
    pub(crate) fn 生成する(転送係: ステージング経由の転送係<'_>, シェーダー: &自動露出のシェーダー一式, 設定: 自動露出の設定, hdrビュー: vk::ImageView) -> Result<Self, レンダラーエラー> {
        ...
        let パイプライン = match pipelines::自動露出のパイプライン一式::生成する(確保係, &ディスクリプタ, &シェーダー.集計シェーダー, &シェーダー.導出と適応) {
            Ok(パイプライン) => パイプライン,
            Err(誤り) => {
                ディスクリプタ.破棄する(device);
                バッファ.破棄する(device);
