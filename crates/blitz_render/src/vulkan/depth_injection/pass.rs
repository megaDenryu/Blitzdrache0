...
pub(crate) fn 合成深度の注入を作る<'a>(深度: 画像ハンドル, 入力: 合成深度の注入入力) -> パス宣言<'a> {
    パス宣言::生成する("合成深度の注入", Vec::new(), vec![(深度, 画像用途::転送先)], Vec::new(), Vec::new(), パス種別::転送, move |積み先と取り出し口| {
        コピーを積む(積み先と取り出し口, 深度, 入力)
    })
}

fn コピーを積む(積み先と取り出し口: &GPU命令の積み先と宣言済み資源の取り出し口, 深度: 画像ハンドル, 入力: 合成深度の注入入力) {
    let 画像 = 積み先と取り出し口.宣言済みの画像を参照する(深度);
    ...
    unsafe {
        積み先と取り出し口
            .積み先()
            .論理デバイス()
            .cmd_copy_buffer_to_image(積み先と取り出し口.積み先().コマンドバッファ(), 入力.バッファ, 画像, vk::ImageLayout::TRANSFER_DST_OPTIMAL, &領域一覧);
    }
}
