...
impl 個体変換内容 {
    ...
    pub(crate) fn 天頂軸まわりに回して平行移動成分を置き換えた内容を返す(&self, 回す角: 読込時の向きから天頂軸まわりに回す角, 基準原点からの相対位置: 位置<ワールド>) -> Self {
        ...
        差し替え後.ローカルからワールド[3] = [基準原点からの相対位置.x().値(), 基準原点からの相対位置.y().値(), 基準原点からの相対位置.z().値(), 1.0];
