...
/// 動く個体を持つ描画対象の個体レコードを、フレームスロットごとのホスト可視バッファで持つ置き場。
///
/// `動く個体一覧`は動く個体の添字と、その個体の読込時の内容を持つ。差し替えるのは平行移動と天頂軸まわりの
/// 向きだけであり、寸法と読込時の姿勢はここが持ち続けて毎フレームの書き込みの土台になる。
...
impl 動く個体の変換バッファ {
    ...
    pub(crate) fn 動く個体の位置と向きを書き込む(
        ...
        let 書き込む内容 = 読込時の内容.天頂軸まわりに回して平行移動成分を置き換えた内容を返す(読込時の向きから回す角, 基準原点からの相対位置);
        let レコード = bytes::バイト列にする(&書き込む内容);
        let 書き込む位置 = 個体レコードの先頭からの位置(個体添字);
        self.スロットごとのバッファ.スロットの一部を書き換える(device, フレーム添字, 書き込む位置, &レコード)
    }
}
