...
pub(crate) struct 動く個体の変換バッファ {
    ...
    /// 動く個体の添字と、その個体の読込時の内容。平行移動だけを差し替えるため、向きと寸法の出どころをここが持ち続ける。
...
impl 動く個体の変換バッファ {
    ...
    /// そのフレームの動く個体1体の位置を書き込む。宣言されていない個体を指す指定は無言で飛ばさず型付きエラーで拒む。
    pub(crate) fn 動く個体の位置を書き込む(
        &self,
        device: &ash::Device,
        フレーム添字: フレームスロット添字,
        個体添字: u32,
        ...
        let レコード = bytes::バイト列にする(&読込時の内容.平行移動成分を置き換えた内容を返す(基準原点からの相対位置));
