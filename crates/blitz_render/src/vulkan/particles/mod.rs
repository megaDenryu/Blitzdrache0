...
impl 粒子リソース一式 {
    ...
    pub(crate) fn 生成する(
        ...
        let コンピュートパイプライン = match 粒子コンピュートパイプライン::生成する(確保係, ディスクリプタ.レイアウトのハンドル(), シェーダー.コンピュートコード()) {
            Ok(パイプライン) => パイプライン,
            Err(誤り) => {
                ディスクリプタ.破棄する(device);
                バッファ.破棄する(device);
        ...
        let 描画パイプライン = match 粒子描画パイプライン::生成する(確保係, カラー形式, 深度形式, ディスクリプタ.レイアウトのハンドル(), シェーダー.頂点コード(), シェーダー.画素段コード())
        {
            Ok(パイプライン) => パイプライン,
            Err(誤り) => {
