...
impl<'書き込み> ビューとパスのセットの書き込み先<'書き込み> {
    ...
    /// そのフレームスロットの定数3本を結ぶ。空の処理位置を持たない構成でも空パス定数の束縛先は結ぶ(中身は書かれず誰も読まない)。
    pub(super) fn フレームの定数3本を結ぶ(&self, 定数: &フレームシェーダー定数一式, フレーム添字: フレームスロット添字) {
        let 対応 = [
            (ビュー定数の束縛番号, 定数.ビュー定数のバッファ(フレーム添字)),
            (多段影の定数の束縛番号, 定数.多段影のバッファ(フレーム添字)),
            (空パスの定数の束縛番号, 定数.空パスのバッファ(フレーム添字)),
        ];
        for (番号, buffer) in 対応 {
