...
/// 1つのセットが持つディスクリプタの件数。影の画像1つ・ヘッダの定数バッファ1つ・レコード列のストレージバッファ2つに、
/// 遠方環境の枝では拡散照度・鏡面畳込み・反射率積分表の3つの画像が加わる。
...
pub(super) fn 生成する(
    ...
    let プールサイズ一覧 = [
        件数(vk::DescriptorType::COMBINED_IMAGE_SAMPLER, 件数一式.画像 * スロット数),
