...
pub(crate) const 反射率積分表のバインディング番号: u32 = 6;
...
pub(super) fn バインド一覧() -> [vk::DescriptorSetLayoutBinding<'static>; 3] {
    [
        画素段の画像バインド(拡散照度のバインディング番号),
        画素段の画像バインド(鏡面畳込みのバインディング番号),
        画素段の画像バインド(反射率積分表のバインディング番号),
    ]
}
...
pub(crate) fn 結ぶ(device: &ash::Device, set: vk::DescriptorSet, 束縛先: 遠方環境の束縛先, サンプラー: vk::Sampler) {
    let 対応 = [
        (拡散照度のバインディング番号, 束縛先.拡散照度),
        (鏡面畳込みのバインディング番号, 束縛先.鏡面畳込み),
        (反射率積分表のバインディング番号, 束縛先.反射率積分表),
    ];
    for (番号, ビュー) in 対応 {
