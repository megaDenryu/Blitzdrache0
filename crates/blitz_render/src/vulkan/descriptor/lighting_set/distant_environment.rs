//! 照明問い合わせのセットのうち、遠方環境の選択肢だけが持つ3つの束縛先。触れるのは拡散照度の立方体画像(binding4)・
//! 鏡面畳込みの立方体画像(binding5)・反射率積分表(binding6)であり、直接光と影の束縛先には触れない。
//!
//! 不変条件: レイアウトがこの3つを宣言したセットは、使う前に必ず3つとも結ばれている。定数近似の選択肢はこの3つを
//! 1つも宣言しないため、未使用のダミー束縛を作らない(参照: `_doc/設計/放射輝度問い合わせ階層.md`「世界の間接照明の方式と契約の2つの選択肢(3-Ic)」)。
//!
...
impl 照明問い合わせのセットの書き込み先<'_> {
    ...
    pub(crate) fn 遠方環境の3つの画像を結ぶ(&self, 束縛先: 遠方環境の束縛先, サンプラー: &遠方環境を読むサンプラー) {
        let 対応 = [(拡散照度の束縛番号, 束縛先.拡散照度), (鏡面畳込みの束縛番号, 束縛先.鏡面畳込み), (反射率積分表の束縛番号, 束縛先.反射率積分表)];
        for (番号, ビュー) in 対応 {
            self.ディスクリプタの書き込み先().サンプラー付きの画像を結ぶ(番号, ビュー, サンプラー.サンプラーのハンドル(), vk::ImageLayout::GENERAL);
        }
    }
}
