...
pub(crate) const クラスタ格子のバインディング番号: u32 = 8;
pub(crate) const クラスタ光添字列のバインディング番号: u32 = 9;

pub(super) fn バインド一覧() -> [vk::DescriptorSetLayoutBinding<'static>; 2] {
    [
        画素段の記憶バインド(クラスタ格子のバインディング番号),
        画素段の記憶バインド(クラスタ光添字列のバインディング番号),
    ]
}
...
pub(super) fn 番号とバッファの対(バッファ組: 照明問い合わせのバッファ組) -> [(u32, vk::Buffer); 2] {
    [
        (クラスタ格子のバインディング番号, バッファ組.クラスタ格子),
        (クラスタ光添字列のバインディング番号, バッファ組.クラスタ光添字列),
    ]
}
