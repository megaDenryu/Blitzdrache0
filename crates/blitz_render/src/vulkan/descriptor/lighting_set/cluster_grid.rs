...
//!
//! 遠方環境の3つと違い、この2つは両方の照明束縛レイアウトの選択肢が必ず宣言する。全世界がクラスタの経路を通り、
//! 局所光が0件の世界は件数0のセルだけを持つ格子を読むためである。選択肢を分けないことで、照明束縛レイアウトの選択肢も
//! パイプラインキーの成分も増えない
//! (参照: `_doc/設計/クラスタ多光源と点光源の影.md`「判断f: 照明束縛レイアウトの選択肢を増やさず、パイプラインキーへ照明の能力の成分も足さない」)。
//!
...
pub(super) fn 番号とバッファの対(バッファ組: 照明問い合わせのバッファ組) -> [(束縛番号, vk::Buffer); 2] {
