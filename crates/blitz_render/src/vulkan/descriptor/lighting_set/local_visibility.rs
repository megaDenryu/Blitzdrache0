...
//!
//! 遠方環境の3つと違い、この束縛は両方の契約が必ず宣言する。拡散間接方式で選択肢を分けずに常に掛ける設計であり、
...
impl 照明問い合わせのセットの書き込み先<'_> {
    ...
    pub(crate) fn 局所可視度の画像を結ぶ(&self, ビュー: vk::ImageView) {
        self.ディスクリプタの書き込み先().画像1枚を結ぶ(局所可視度の束縛番号, vk::DescriptorType::SAMPLED_IMAGE, ビュー, vk::ImageLayout::GENERAL);
    }
}
