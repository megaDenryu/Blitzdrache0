...
//!
//! クラスタの2本と同じく、両方の照明束縛レイアウトの選択肢が必ず宣言する。影を1件も持たない世界も同じ資源を結び、
//! 局所光レコードの影の有無で標本するかどうかが決まる。選択肢を分けるとパイプラインキーの成分が増える(判断f)。
//!
...
pub(super) fn 点光源の影の束縛() -> vk::DescriptorSetLayoutBinding<'static> {
