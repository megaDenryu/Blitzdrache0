...
/// `個体レコード`は、個体が1体だけの対象も1要素ぶんの範囲を持つ専用のバッファを指し、
/// 動く個体を宣言した対象だけがフレームスロットごとに別のバッファを指す。
/// `可視id列`は、個体が1体だけの対象では束が共有する値0だけの列を指す。
pub(crate) struct ジオメトリセット参照 {
    pub(crate) 個体レコード: 個体レコード参照, // 個体レコードを読むバッファ
    pub(crate) 可視id列: 可視ID列参照,         // 可視ID列を読むバッファ
}

pub(crate) struct 描画対象ディスクリプタプール {
    pool: vk::DescriptorPool,
    ジオメトリset一覧: Vec<vk::DescriptorSet>, // 描画対象の順、その中でフレームスロットの順に並ぶ
    描画対象数: usize,
}
