...
//! `照明束縛`が持つのは照明問い合わせのセットが宣言した選択肢であり、資源を結ぶ側がこの値から結ぶ先を決める。
...
pub(crate) struct シーンセットレイアウト一式 {
    ...
    照明束縛: 照明束縛レイアウト, // 照明問い合わせのセットが宣言した選択肢
...
impl シーンセットレイアウト一式 {
    ...
    pub(crate) fn 生成する(確保係: &GPU資源の確保係<'_>, 表容量: テクスチャ表レイアウト容量, 照明束縛: 照明束縛レイアウト) -> Result<Self, レンダラーエラー> {
        create::シーンセットレイアウト一式を組み立てる(確保係, 表容量, 照明束縛)
    }
