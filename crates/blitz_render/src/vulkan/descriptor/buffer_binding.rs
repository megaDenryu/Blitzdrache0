//! バッファを指すバインディングの書き込み。フレームユニフォーム(binding3)・描画対象ユニフォーム(binding5)・
//! 個体変換(binding6)・可視ID列(binding7)の4つを担当し、画像バインディングには触れない。

use ash::vk;

/// binding3(フレームユニフォームバッファ)を書き込む。生成時にセットごとの固有バッファを結ぶ。
pub(super) fn フレームユニフォームを書き込む(device: &ash::Device, set: vk::DescriptorSet, buffer: vk::Buffer) {
    ユニフォームバッファを書き込む(device, set, 3, buffer);
}

/// binding5へ描画対象固有の変換とマテリアル係数を結ぶ。
pub(super) fn 描画対象ユニフォームを書き込む(device: &ash::Device, set: vk::DescriptorSet, buffer: vk::Buffer) {
    ユニフォームバッファを書き込む(device, set, 5, buffer);
}
...
fn ユニフォームバッファを書き込む(device: &ash::Device, set: vk::DescriptorSet, binding: u32, buffer: vk::Buffer) {
