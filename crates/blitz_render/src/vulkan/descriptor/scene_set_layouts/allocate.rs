...
impl シーンセットレイアウト一式 {
    ...
    pub(crate) fn 照明問い合わせのセットを割り当てる(&self, device: &ash::Device, pool: vk::DescriptorPool, セット数: usize) -> Result<Vec<照明問い合わせの割り当て済みセット>, レンダラーエラー> {
    ...
    pub(crate) fn 材質のセットを1つ割り当てる(&self, device: &ash::Device, pool: vk::DescriptorPool) -> Result<材質の割り当て済みセット, レンダラーエラー> {
