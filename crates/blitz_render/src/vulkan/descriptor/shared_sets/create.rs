...
pub(super) fn 共有ディスクリプタセットを生成する(
    device: &ash::Device, レイアウト: &シーンセットレイアウト一式, シェーダー定数: &フレームシェーダー定数一式
) -> Result<共有ディスクリプタセット, レンダラーエラー> {
    let pool = プールを生成する(device)?;
    match 割り当てて結ぶ(device, pool, レイアウト, シェーダー定数) {
        Ok(ビューとパス一覧) => Ok(共有ディスクリプタセット { pool, ビューとパス一覧 }),
        Err(誤り) => {
...
fn 割り当てて結ぶ(
    device: &ash::Device, pool: vk::DescriptorPool, レイアウト: &シーンセットレイアウト一式, シェーダー定数: &フレームシェーダー定数一式
    ...
    for フレーム添字 in フレームスロット添字::全スロット() {
        ビューとパスのセットの書き込み先::生成する(device, ビューとパス一覧[フレーム添字.配列添字()]).フレームの定数3本を結ぶ(シェーダー定数, フレーム添字);
    }
    Ok(ビューとパス一覧)
}
...
fn プールを生成する(device: &ash::Device) -> Result<vk::DescriptorPool, レンダラーエラー> {
    ...
    let プールサイズ一覧 = [vk::DescriptorPoolSize::default().ty(vk::DescriptorType::UNIFORM_BUFFER).descriptor_count(3 * フレーム数)];
