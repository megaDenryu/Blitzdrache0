...
impl<'書き込み> 材質のセットの書き込み先<'書き込み> {
    ...
    pub(crate) fn 世代の資源を結ぶ(&self, レコードバッファ: &材質レコードバッファ, 画像集合: &[テクスチャ]) {
        self.0
            .バッファの先頭からの範囲を結ぶ(材質レコードの束縛番号, vk::DescriptorType::STORAGE_BUFFER, レコードバッファ.バッファのハンドル(), レコードバッファ.範囲);
        let ビュー一覧: Vec<vk::ImageView> = 画像集合.iter().map(|画像| 画像.image_view).collect();
        self.0.サンプラー無しの画像の並びを結ぶ(材質テクスチャ表の束縛番号, &ビュー一覧, vk::ImageLayout::SHADER_READ_ONLY_OPTIMAL);
    }
}
