...
//!
//! 2つを組で持つのは、書き込みの工程へ論理デバイスとセットを別々に運ばせると、別の役割のセットと
//! 組み合わせた呼び出しが署名の上で成立してしまうためである。役割ごとのセットは、この組を包んだ自分の型を持ち、
//! 自分が持つ束縛番号だけを結ぶメソッドで公開する(参照: `lighting_set/set_write.rs`)。
//!
