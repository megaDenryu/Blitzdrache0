...
impl 共有ディスクリプタセット {
    pub(crate) fn 生成する(device: &ash::Device, レイアウト: &シーンセットレイアウト一式, シェーダー定数: &フレームシェーダー定数一式) -> Result<Self, レンダラーエラー> {
        create::共有ディスクリプタセットを生成する(device, レイアウト, シェーダー定数)
    }
    ...
    pub(crate) fn 束縛を作る<'計器>(&self, フレーム添字: フレームスロット添字, 材質: vk::DescriptorSet, 照明問い合わせ: vk::DescriptorSet, 計器: &'計器 記録の計器) -> 共有セット束縛<'計器> {
        共有セット束縛 {
