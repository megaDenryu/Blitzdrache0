//! そのフレームに描く個体の添字をLOD段ごとの範囲へ並べたストレージバッファ。シーンとシャドウの両方の頂点シェーダーが
//! これを参照する。フレームインフライトごとに1本を持ち、束の読込時に個体数ぶんを一度だけ確保する。
//! 可視判定の結果や段の選択が変わってもGPU確保は起こらず、毎フレーム行うのは書き込みだけである
//! (参照: `_doc/設計/植生インスタンスと物量計測.md`「描画発行」)。
//! 確保の直後は0から個体数-1までの恒等の列を書く。可視個体選択を持たない描画対象はこの初期値のまま全個体を1つの段で描く。
