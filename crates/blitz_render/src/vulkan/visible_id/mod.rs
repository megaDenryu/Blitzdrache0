//! そのフレームにシーンパスが描く個体の添字を並べたストレージバッファ。フレームインフライトごとに1本を持ち、
//! 束の読込時に最大個体数ぶんを一度だけ確保する。可視判定の結果が変わってもGPU確保は起こらず、毎フレーム行うのは書き込みだけである
//! (参照: `_doc/設計/植生インスタンスと物量計測.md`「描画発行」)。
//! 確保の直後は0から個体数-1までの恒等の列を書く。可視個体選択を持たない描画対象はこの初期値のまま全個体を描く。
//! 注意: この恒等の列は一度も書き換えられていないことが前提である。ある描画対象が可視個体選択を持つフレームと
//! 持たないフレームを行き来すると、選択を持たないフレームで前のフレームの絞り込んだ列を読んでしまう。
//! 可視判定の入切はプロセスの起動時に決まり、群は可視判定が有効なあいだ毎フレーム選択を受け取るため、この前提が保たれている。
//! 前提: 個体数は1以上である(描画対象素材の個体変換列が非空であることによる)。
//! 確保と破棄はこのファイル、ディスクリプタが結ぶ参照は`reference`、毎フレームの書き込みは`write`にある。
...
impl 可視ID列バッファ {
    pub(crate) fn 生成する(
        ...
        let 範囲 = u64::try_from(恒等列.len()).unwrap_or_else(|_| panic!("可視ID列のバイト長がu64に収まらない"));
        Ok(Self {
            buffer一覧,
            memory一覧,
            範囲,
...
fn 恒等バイト列を作る(個体数: u32) -> Vec<u8> {
    let 件数 = usize::try_from(個体数).unwrap_or_else(|_| panic!("個体数がusizeに収まらない"));
    let mut バイト列 = Vec::with_capacity(件数 * 可視IDバイト長);
    for 添字 in 0..個体数 {
