//! 空段階の資源: 空パイプラインと、LUTを参照する標本ディスクリプタと空中遠近合成の一式。
//! フレーム構成に空段階があるときだけ`描画段階資源`が保持し、空パスと合成パスの記録だけがこの一式を束縛する。
//!
//! 標本ディスクリプタをこの型が必ず持つのは、「空を描くのにLUTを参照するディスクリプタが無い」という状態を
//! 型で作れなくするためである。放射輝度の評価はパイプライン1つで完結し、フラグメントの中に方式の分岐を持たない
...
pub(crate) struct 空段階資源 {
    パイプライン: 空パイプライン,
    /// set1でスカイビューLUTと透過率LUTと媒体のユニフォームを結ぶ。
    標本: 大気LUT標本ディスクリプタ,
...
pub(crate) struct 空段階の生成要求<'a> {
    ...
    /// set1から参照するLUTの持ち主。フレーム構成に空段階があるときは必ず作られている。
