//! 大気のベイク済み画像のうち、生成パスが束縛する側の資源(5つのディスクリプタと5本のコンピュートパイプライン)の所有者。
//! 触れるのはこの10個だけであり、画像もメモリも持たない。生成の途中で失敗したらそれまでに作ったハンドルを逆順に片付ける。
//!
...
use super::base_resources::大気のベイク済み画像の基盤資源;
...
pub(super) struct 大気のベイク済み画像の束縛一式 {
    ...
    遠方環境用スカイビューディスクリプタ: 経路生成ディスクリプタ,
    ...
    pub(super) 遠方環境用スカイビューパイプライン: pipeline::生成パイプライン,
    pub(super) 空中遠近パイプライン: pipeline::生成パイプライン,
}

impl 大気のベイク済み画像の束縛一式 {
    pub(super) fn 生成する(
        確保係: &GPU資源の確保係<'_>, 基盤: &大気のベイク済み画像の基盤資源, シェーダー: &大気のベイク済み画像のシェーダー一式
    ) -> Result<Self, crate::error::レンダラーエラー> {
        create::大気のベイク済み画像の束縛一式を生成する(確保係, 基盤, シェーダー)
    }
    ...
    pub(super) fn 遠方環境用スカイビューセット(&self, フレーム添字: フレームスロット添字) -> vk::DescriptorSet {
        self.遠方環境用スカイビューディスクリプタ.set(フレーム添字)
    }
