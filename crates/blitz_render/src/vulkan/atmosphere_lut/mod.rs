...
//! 大気のベイク済み画像方式の空を持つフレーム構成のときだけ`描画の処理位置の資源`が保持し、大気のベイク済み画像の生成パスだけがこの一式を束縛する。
//!
...
pub(crate) use inputs::{大気のベイク済み画像の描画入力, 大気のベイク済み画像の生成入力, 大気のベイク済み画像の画像入力, 生成する組, 生成の即時定数};
...
impl 大気のベイク済み画像一式 {
    pub(crate) fn 生成する(確保係: &GPU資源の確保係<'_>, 解像度: 大気のベイク済み画像の解像度, シェーダー: &大気のベイク済み画像のシェーダー一式) -> Result<Self, レンダラーエラー> {
    ...
    pub(crate) fn 媒体を書き込む(&self, device: &ash::Device, フレーム添字: フレームスロット添字, 媒体: &大気散乱媒体) -> Result<(), レンダラーエラー> {
