//! プリコンピュート大気のベイク済み画像の資源一式: ベイク済み画像・媒体ユニフォーム・ディスクリプタ・生成パイプライン。
...
pub(crate) use inputs::{
    大気のベイク済み画像の描画入力, 大気のベイク済み画像の生成入力, 大気のベイク済み画像の画像入力, 生成する組, 生成の押し込み定数,
};
...
/// 解像度をこの型が覚えないのは、ベイク済み画像1枚ごとの形が`基盤`の画像そのものに付いており、ディスパッチの大きさも
...
impl 大気のベイク済み画像一式 {
    ...
    /// 空パスの大気のベイク済み画像方式が参照する先。媒体のユニフォームと2枚のベイク済み画像のビューを、標本ディスクリプタの生成へ渡す。
