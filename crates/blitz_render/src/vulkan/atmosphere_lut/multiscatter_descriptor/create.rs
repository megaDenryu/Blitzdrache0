//! 多重散乱ディスクリプタの生成局面。サンプラー・レイアウト・プール・セットを順に作り、束縛先を書き込む。
//! 呼ばれるのは大気のベイク済み画像一式の組み立て時の1回だけであり、以降のフレームはセットを参照するだけである。
//! 途中で失敗したら、それまでに作ったハンドルをその場で逆順に片付ける。

use ash::vk;

use super::{binding, 多重散乱の束縛先, 多重散乱ディスクリプタ};
use crate::error::レンダラーエラー;
use crate::vulkan::allocator::GPU資源の確保係;
use crate::vulkan::sync::フレームスロット添字;

pub(super) fn 多重散乱ディスクリプタを生成する(確保係: &GPU資源の確保係<'_>, 束縛先: 多重散乱の束縛先<'_>) -> Result<多重散乱ディスクリプタ, レンダラーエラー> {
    ...
    Ok(多重散乱ディスクリプタ { layout, pool, sampler, set一覧 })
}
