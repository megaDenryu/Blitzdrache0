//! 4枚のLUT画像を形の並び順に確保する工程。担当するのは「形の並びを受け取り、その順に画像を作り、
...
pub(super) fn 順に作る(
    ...
    形一覧: [大気LUTの形; 4],
) -> Result<[大気LUT画像; 4], レンダラーエラー> {
    let mut 作った: Vec<大気LUT画像> = Vec::with_capacity(形一覧.len());
    for 形 in 形一覧 {
        match 大気LUT画像::生成する(device, メモリプロパティ, 形) {
            Ok(画像) => 作った.push(画像),
            Err(誤り) => {
    ...
    Ok(作った
        .try_into()
        .unwrap_or_else(|_| panic!("大気LUT画像を{}枚要求したのに{件数}枚できた", 形一覧.len())))
}
