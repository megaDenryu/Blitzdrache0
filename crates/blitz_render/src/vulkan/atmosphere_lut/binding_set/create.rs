...
pub(super) fn 大気のベイク済み画像の束縛一式を生成する(
    ...
    let ディスクリプタ = descriptors::ディスクリプタ五点を作る(確保係, 基盤)?;
    match pipelines::五本の生成パイプラインを作る(確保係, &ディスクリプタ, シェーダー) {
        Ok([透過率パイプライン, 多重散乱パイプライン, スカイビューパイプライン, 遠方環境用スカイビューパイプライン, 空中遠近パイプライン]) => {
            Ok(大気のベイク済み画像の束縛一式 {
