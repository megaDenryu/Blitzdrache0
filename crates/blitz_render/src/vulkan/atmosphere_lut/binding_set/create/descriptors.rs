...
pub(super) fn ディスクリプタ五点を作る(確保係: &GPU資源の確保係<'_>, 基盤: &大気のベイク済み画像の基盤資源) -> Result<ディスクリプタ五点, レンダラーエラー> {
    ...
    let 書き込み先一覧 = [基盤.スカイビュー.画像ビュー, 基盤.遠方環境用スカイビュー.画像ビュー, 基盤.空中遠近.画像ビュー];
...
fn 多重散乱を作る(
    確保係: &GPU資源の確保係<'_>, 基盤: &大気のベイク済み画像の基盤資源, シェーダー定数一覧: &[vk::Buffer; 進行中フレーム数]
