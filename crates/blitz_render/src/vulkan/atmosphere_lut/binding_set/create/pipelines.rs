...
pub(super) fn 五本の生成パイプラインを作る(
    確保係: &GPU資源の確保係<'_>, ディスクリプタ: &ディスクリプタ五点, シェーダー: &大気のベイク済み画像のシェーダー一式
) -> Result<[生成パイプライン; 5], レンダラーエラー> {
    let device = 確保係.論理デバイス();
    let 仕様一覧 = [
        (ディスクリプタ.透過率.レイアウトのハンドル(), 即時定数の枠::無し, シェーダー.透過率.コード()),
        (ディスクリプタ.多重散乱.レイアウトのハンドル(), 即時定数の枠::無し, シェーダー.多重散乱.コード()),
        (ディスクリプタ.スカイビュー.レイアウトのハンドル(), 即時定数の枠::バイト数(スカイビュー押し込みバイト数), シェーダー.スカイビュー.コード()),
        (ディスクリプタ.遠方環境用スカイビュー.レイアウトのハンドル(), 即時定数の枠::バイト数(スカイビュー押し込みバイト数), シェーダー.スカイビュー.コード()),
        (ディスクリプタ.空中遠近.レイアウトのハンドル(), 即時定数の枠::バイト数(空中遠近押し込みバイト数), シェーダー.空中遠近.コード()),
    ];
    ...
    let 件数 = 作った.len();
    Ok(作った.try_into().unwrap_or_else(|_| panic!("生成パイプラインを5本要求したのに{件数}本できた")))
}
