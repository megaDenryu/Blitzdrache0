//! 読み戻しの受け皿3枚の確保と解放。担当する工程は「テクセル数の並びを受け取り、その順に受け皿を確保し、
//! 途中で失敗したらそれまでの確保を逆順に解放する」ことである。
//!
//! 3枚を1つの型で持つのは、コピーパスが3本そろって初めて意味を持ち、1枚だけ確保できた状態を呼び出し元へ
//! 見せる理由が無いためである。
...
impl 受け皿一式 {
    pub(super) fn 確保する(
        ...
        Ok(Self {
            バッファ一覧: 作った
                .try_into()
                .unwrap_or_else(|_| panic!("読み戻しバッファを3本要求したのに{件数}本できた")),
        })
    }

    /// 並びは透過率・多重散乱・スカイビューであり、確保に渡したテクセル数の並びと同じである。
