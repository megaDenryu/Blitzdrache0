//! 読み戻し検査のコマンド記録。担当する工程は「本番と同じ3本の生成パスと、その後ろへ画像からバッファへのコピーを積み、
...
pub(super) fn 焼いて読み戻す(
    ...
    let 生成する組::全部 {
        ...
        panic!("読み戻し検査は3枚すべてを焼く指示で呼ばれなければならない");
    };
    環境.一度きりで実行する(|device, command_buffer| {
        ...
        グラフ.パスを積む(pass::作る("スカイビュー生成", スカイビュー, 二枚を読む, スカイビュー入力));
        グラフ.パスを積む(copy_pass::作る("透過率読み戻し", 透過率, 入力.透過率画像.寸法, 受け皿.透過率().handle));
        グラフ.パスを積む(copy_pass::作る(
            "多重散乱読み戻し",
            多重散乱,
            入力.多重散乱画像.寸法,
            受け皿.多重散乱().handle,
...
/// 検査は3枚すべてを1回の記録で焼くため、どの画像も「焼く画像」として登録する。
