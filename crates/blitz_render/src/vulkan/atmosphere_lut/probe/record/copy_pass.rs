//! ベイク済み画像1枚をホスト可視バッファへ写す転送パスの宣言。担当する工程は「画像ハンドルと寸法と受けバッファを受け取り、
//! 転送種別のパス宣言を返す」ことである。
//!
...
pub(super) fn コピーパスを作る(名前: &'static str, 画像: graph::画像ハンドル, 範囲: vk::Extent3D, 受け: vk::Buffer) -> graph::パス宣言<'static> {
    graph::パス宣言::生成する(名前, vec![(画像, graph::画像用途::転送元)], Vec::new(), Vec::new(), Vec::new(), graph::パス種別::転送, move |積み先と取り出し口| {
