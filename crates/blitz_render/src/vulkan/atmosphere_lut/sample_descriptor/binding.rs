...
//! 注意: 2枚のベイク済み画像のレイアウトはGENERALである。レンダーグラフの画像用途「大気のベイク済み画像の画素段参照」が
...
pub(super) fn 書き込む(
    ...
    let スカイビュー情報 = [読み画像情報(sampler, 束縛先.スカイビュービュー)];
    let 書き込み一覧 = [
...
fn 読み画像情報(sampler: vk::Sampler, ビュー: vk::ImageView) -> vk::DescriptorImageInfo {
