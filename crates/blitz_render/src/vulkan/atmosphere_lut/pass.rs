...
//! begin/end renderingは無く、GPU命令をコマンドバッファへ積むクロージャはバインドと計算の発行だけを書く(粒子更新パスと同じ様式)。
//!
...
pub(crate) fn 大気のベイク済み画像の生成パスを作る<'a>(
    パス名: &'static str, 書き込み先: 画像ハンドル, 読み画像: Vec<(画像ハンドル, 画像用途)>, 入力: &'a 大気のベイク済み画像の生成入力
) -> パス宣言<'a> {
    パス宣言::生成する(
        パス名,
        読み画像,
        ...
        move |積み先と取り出し口| {
            ...
            let _ = 積み先と取り出し口.宣言済みの画像を参照する(書き込み先);
...
/// 検証層が範囲外として指摘するため、空のバイト列では発行しない。
