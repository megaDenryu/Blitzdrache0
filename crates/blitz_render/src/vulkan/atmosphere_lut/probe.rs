...
pub(crate) fn 大気のベイク済み画像をgpuで焼いて読み戻す(条件: 焼く条件<'_>) -> Result<(大気のベイク済み画像の読み戻し, 検証観測), レンダラーエラー> {
...
fn 環境で焼く(環境: &ウィンドウなし実行GPU環境, 条件: &焼く条件<'_>) -> Result<大気のベイク済み画像の読み戻し, レンダラーエラー> {
...
fn 一式で焼く(
    環境: &ウィンドウなし実行GPU環境, 一式: &大気のベイク済み画像一式, 条件: &焼く条件<'_>, 確保係: &crate::vulkan::allocator::GPU資源の確保係<'_>
    ...
    let 受け皿 = 受け皿一式::確保する(
        確保係,
        [条件.解像度.透過率のテクセル数(), 条件.解像度.多重散乱のテクセル数(), 条件.解像度.スカイビューのテクセル数(), 条件.解像度.空中遠近のボクセル数()],
    )?;
