...
//! 空パス定数は空の処理位置を持つフレーム構成でだけ書き、持たない構成では1度も書かない。
...
impl フレームシェーダー定数一式 {
    ...
    pub(crate) fn 多段影のバッファ(&self, フレーム添字: フレームスロット添字) -> vk::Buffer {
        self.多段影.フレームスロットのバッファ(フレーム添字)
    }

    pub(crate) fn 空パスのバッファ(&self, フレーム添字: フレームスロット添字) -> vk::Buffer {
