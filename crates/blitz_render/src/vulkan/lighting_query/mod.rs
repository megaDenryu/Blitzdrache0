...
//! `束縛レイアウト`はこのセットが宣言した選択肢であり、遠方環境の3つの画像を結ぶかどうかをこの値だけが決める。
...
pub(crate) struct 照明問い合わせ資源束 {
    ...
    スロット一覧: Vec<スロット資源>,
    束縛レイアウト: 照明束縛レイアウト,                   // このセットが宣言した選択肢
    遠方環境サンプラー: Option<遠方環境を読むサンプラー>, // 遠方環境の選択肢のときだけ持つ、3つの画像を読む固定サンプラー
}
...
impl 照明問い合わせ資源束 {
    ...
    pub(crate) fn セット(&self, フレーム添字: フレームスロット添字) -> vk::DescriptorSet {
        self.スロット一覧[フレーム添字.配列添字()].セット.セットのハンドル()
    }
