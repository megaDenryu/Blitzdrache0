...
pub(super) fn 照明問い合わせ資源束を生成する(
    確保係: &GPU資源の確保係<'_>, レイアウト: &シーンセットレイアウト一式, 影の資源: &影の資源の組
...
fn 遠方環境サンプラーを作る(確保係: &GPU資源の確保係<'_>, 束縛レイアウト: 照明束縛レイアウト) -> Result<Option<遠方環境を読むサンプラー>, レンダラーエラー> {
...
fn スロット一覧を作る(
    確保係: &GPU資源の確保係<'_>, pool: &照明問い合わせのディスクリプタプール, レイアウト: &シーンセットレイアウト一式, 影の資源: &影の資源の組
