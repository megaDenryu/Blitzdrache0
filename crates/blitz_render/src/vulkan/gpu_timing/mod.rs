...
/// M7のブルームピラミッド(判断41)で8→16、M9の布シミュレーション(判断54: 約8パス追加)で16→32へ拡張。
pub(crate) const パス数上限: u32 = 32;
...
impl パス別GPU計測 {
    ...
    pub(crate) fn クエリプール(&self, フレーム添字: usize) -> vk::QueryPool {
        self.プール一覧[フレーム添字]
    }
    ...
    /// 呼ぶこと。
    pub(crate) fn 読み取る(&mut self, device: &ash::Device, フレーム添字: usize) {
        readback::読み取る(
            device,
            self.プール一覧[フレーム添字],
            &self.直近マッピング一覧[フレーム添字],
            self.タイムスタンプ周期ns,
            &mut self.移動平均表,
        );
    }
    ...
    pub(crate) fn 直近マッピングを記録する(&mut self, フレーム添字: usize, マッピング: Vec<(&'static str, u32)>) {
        self.直近マッピング一覧[フレーム添字] = マッピング;
    }
