...
use crate::vulkan::frame::record::布シミュレーションの一刻みのパス数;
use crate::vulkan::sync::{フレームスロット添字, 進行中フレーム数};

/// 布を除いてグラフが1フレームに持てるパス数の上限。
...
const 布を除くパス数上限: u32 = 60;

/// 布が1フレームに積むパス数の上限。シミュレーションの工程を進める刻み数ぶん繰り返し、頂点生成を描画機会ごとに1本積む。
const 布のパス数上限: u32 = 布シミュレーションの一刻みのパス数 * 布の進める刻み数::上限の本数() + 1;

/// グラフが1フレームに持てるパス数の上限(クエリプール容量の元になる)。
/// 超えたときは`graph::timestamp_write`がクエリの開始添字を求める時点でpanicする。
pub(crate) const パス数上限: u32 = 布を除くパス数上限 + 布のパス数上限;
...
impl パス別GPU計測 {
    ...
    pub(crate) fn 直近マッピングを記録する(&mut self, フレーム添字: フレームスロット添字, マッピング: Vec<(&'static str, u32)>) {
