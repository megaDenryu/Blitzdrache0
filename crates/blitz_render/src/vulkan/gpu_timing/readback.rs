...
//!
//! 同じパス名がフレーム内に何度も現れたら、その回のぶんを足して1つの標本にする。鏡面畳込み生成は粗さの段ごとに
//! パスを積むため、回ごとに1標本を入れると窓の中身が段の混じった列になり、中央値も95パーセンタイルも
//! 「そのフレームでその区間に費やした時間」を答えなくなる(段ごとに大きさが違うため混じった列の中央値は
//! どの段の値でもない)。窓の1標本を1フレームぶんに保つことで、区間の分位がそのまま更新1回の費用になる。
...
/// 同じ名前の回を足して、名前ごとに1フレームぶんの合計を返す。回が1つでも読めなかった名前は、そのフレームの
/// ぶんを丸ごと捨てる。読めた回だけを足した合計はそのフレームの費用を過小に見せ、読み飛ばしたことが値から
/// 分からなくなるためである。並びは最初に現れた順を保つ(グラフの宣言順がそのまま報告の並びになる)。
