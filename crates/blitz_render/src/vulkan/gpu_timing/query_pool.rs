...
pub(super) fn 進行中フレーム別のタイムスタンプクエリプールを生成する(device: &ash::Device) -> Result<[vk::QueryPool; 進行中フレーム数], レンダラーエラー> {
    let mut プール一覧 = [vk::QueryPool::null(); 進行中フレーム数];
    for 添字 in 0..進行中フレーム数 {
        let create_info = vk::QueryPoolCreateInfo::default().query_type(vk::QueryType::TIMESTAMP).query_count(クエリ数上限);
        // 安全性: deviceは生成済みで有効。
