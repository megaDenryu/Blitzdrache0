...
pub(super) fn 形式を選ぶ(候補一覧: &[vk::SurfaceFormatKHR]) -> Result<vk::SurfaceFormatKHR, レンダラーエラー> {
    let 優先形式 = 候補一覧.iter().find(|形式| 形式.format == vk::Format::B8G8R8A8_SRGB && 形式.color_space == vk::ColorSpaceKHR::SRGB_NONLINEAR);
...
pub(super) fn 画像数を決める(能力: &vk::SurfaceCapabilitiesKHR) -> u32 {
    ...
    if 能力.max_image_count > 0 { 望ましい数.min(能力.max_image_count) } else { 望ましい数 }
}
