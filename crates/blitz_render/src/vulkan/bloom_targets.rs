//! 光のにじみピラミッドの中間画像(判断41)。縮小チェーンは1/2解像度から1/2ずつ最大5段
...
pub(crate) struct 光のにじみピラミッド {
    pub(crate) 縮小一覧: Vec<HDRターゲット>, // [0]が1/2解像度で、以降1/2ずつ小さくなる。長さは1以上・段数上限以下
    pub(crate) 拡大一覧: Vec<HDRターゲット>, // [i]は縮小一覧[i]と同解像度。長さは縮小一覧の長さ-1(1段のみなら空)
    pub(crate) 寸法一覧: Vec<vk::Extent2D>,  // 各段の寸法(縮小一覧と同じ添字)
}
...
impl 光のにじみピラミッド {
    ...
    /// 明るさの圧縮が読む最終結果のビュー。2段以上なら拡大チェーンの最上段、1段のみなら縮小結果。
