...
impl 焼かせる注入資源 {
    pub(crate) fn 生成する(確保係: &GPU資源の確保係<'_>, 派生表現: &派生表現一式, 解像度: 遠方環境の解像度, 入力: &遠方環境の焼かせる解析入力) -> Result<Self, レンダラーエラー> {
