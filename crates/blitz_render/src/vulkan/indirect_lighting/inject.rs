...
impl 遠方環境の照明資源 {
    ...
    pub(crate) fn 解析入力を注入する(&mut self, 確保係: &GPU資源の確保係<'_>, 入力: &遠方環境の解析入力) -> Result<(), レンダラーエラー> {
        if self.注入.is_some() {
            return Err(間接照明エラー::from(遠方環境の解析入力エラー::二重注入).into());
        }
        let 資源 = injection::解析入力の注入資源::生成する(確保係, &self.派生表現, 入力)?;
        self.注入 = Some(injection::検収の注入::派生表現へ直に与える(資源));
        Ok(())
    }

    /// 検収専用の入口。遠方環境の立方体画像と反射率積分表を与えた値で置き換え、拡散照度と鏡面畳込みは生成側に焼かせる。
    pub(crate) fn 焼かせる解析入力を注入する(&mut self, 確保係: &GPU資源の確保係<'_>, 入力: &遠方環境の焼かせる解析入力) -> Result<(), レンダラーエラー> {
        if self.注入.is_some() {
            return Err(間接照明エラー::from(遠方環境の解析入力エラー::二重注入).into());
        }
        let 解像度 = 遠方環境の解像度::既定値();
        let 資源 = injection::焼かせる注入資源::生成する(確保係, &self.派生表現, 解像度, 入力)?;
