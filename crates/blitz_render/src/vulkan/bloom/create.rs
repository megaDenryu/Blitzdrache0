...
pub(super) fn パイプライン部を生成する(
    確保係: &GPU資源の確保係<'_>, 前処理シェーダー: &シェーダー一式, 縮小シェーダー: &シェーダー一式, 拡大シェーダー: &シェーダー一式
    ...
    for (シェーダー, エントリ名, レイアウト) in 仕様一覧 {
        match 全画面パスのパイプライン::組み立てる(確保係, HDR形式, レイアウト, シェーダー, エントリ名, 0) {
    ...
    let [前処理, 縮小, 拡大] = 一覧.try_into().unwrap_or_else(|_| panic!("全画面パスのパイプラインが仕様の本数ぶん揃わなかった"));
