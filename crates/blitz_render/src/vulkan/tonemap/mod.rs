...
pub(crate) struct 明るさの圧縮一式 {
    ...
    sampler: vk::Sampler,
    ディスクリプタ: descriptor::明るさの圧縮ディスクリプタ,
}

impl 明るさの圧縮一式 {
    pub(crate) fn 生成する(
        確保係: &GPU資源の確保係<'_>, スワップチェーン形式: vk::Format, シェーダー: &シェーダー一式, hdrビュー: vk::ImageView, 光のにじみビュー: vk::ImageView
        ...
        let パイプライン = match 全画面パスのパイプライン::組み立てる(確保係, スワップチェーン形式, ディスクリプタ.layout.レイアウトのハンドル(), シェーダー, c"fragmentMain", 露出プッシュ定数バイト数)
        {
            Ok(パイプライン) => パイプライン,
            Err(誤り) => {
                ディスクリプタ.破棄する(device);
    ...
    /// パイプラインへの束縛へ渡す境界。
    pub(crate) fn 束縛するセットのハンドル(&self) -> vk::DescriptorSet {
        self.ディスクリプタ.set.セットのハンドル()
    }

    pub(crate) fn 破棄する(&self, device: &ash::Device) {
        // 安全性: samplerはSelfが唯一の所有者であり、破棄時点でGPU側の使用がdevice_wait_idle済みであることを呼び出し元が保証する。
