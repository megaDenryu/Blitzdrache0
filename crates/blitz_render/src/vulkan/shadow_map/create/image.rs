...
pub(super) fn シャドウマップの画像を作る(確保係: &GPU資源の確保係<'_>, 一辺: 影の一辺解像度) -> Result<vk::Image, レンダラーエラー> {
...
pub(super) fn 距離区分ビューを作る(確保係: &GPU資源の確保係<'_>, 画像: vk::Image, 層: u32) -> Result<vk::ImageView, レンダラーエラー> {
    シャドウマップの画像のビューを作る(確保係, 画像, vk::ImageViewType::TYPE_2D, 層, 1)
}

fn シャドウマップの画像のビューを作る(確保係: &GPU資源の確保係<'_>, 画像: vk::Image, 種別: vk::ImageViewType, 開始層: u32, 層数: u32) -> Result<vk::ImageView, レンダラーエラー> {
    let 部分範囲 = vk::ImageSubresourceRange::default()
        ...
        .layer_count(層数);
    let create_info = vk::ImageViewCreateInfo::default().image(画像).view_type(種別).format(シャドウマップ形式).subresource_range(部分範囲);
