...
impl 派生表現の画像三点 {
    pub(super) fn 生成する(
        ...
        let 拡散照度 = 派生の立方体画像::生成する(device, メモリプロパティ, 解像度.拡散照度.面の一辺().値(), 1)?;
        let 鏡面 = 解像度.鏡面畳込み;
        let 最詳細段の一辺 = 鏡面.段の一辺(最詳細段(鏡面)).値();
        let 鏡面畳込み = match 派生の立方体画像::生成する(device, メモリプロパティ, 最詳細段の一辺, 鏡面.粗さ段数())
        {
            Ok(画像) => 画像,
            Err(誤り) => {
                拡散照度.破棄する(device);
        ...
        let 表 = 反射率積分表の画像::生成する(device, メモリプロパティ, 解像度.反射率積分表.横(), 解像度.反射率積分表.縦());
        match 表 {
            Ok(反射率積分表) => Ok(Self {
                拡散照度,
                鏡面畳込み,
