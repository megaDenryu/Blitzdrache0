...
impl 受けバッファ四点 {
    pub(super) fn 生成する(確保係: &GPU資源の確保係<'_>, テクセル数: &受けバッファのテクセル数) -> Result<Self, レンダラーエラー> {
        let device = 確保係.論理デバイス();
        let mut 作った読み戻しバッファ一覧 = Vec::with_capacity(3);
        for 数 in [テクセル数.遠方環境, テクセル数.拡散照度, テクセル数.鏡面畳込み] {
            match ベイク済み画像の読み戻しバッファ::生成する(確保係, 数) {
                Ok(バッファ) => 作った読み戻しバッファ一覧.push(バッファ),
                Err(誤り) => return Err(作った途中を片付けて返す(device, &作った読み戻しバッファ一覧, 誤り)),
            }
        }
        ...
        let 鏡面畳込み = 作った読み戻しバッファ一覧.pop().unwrap_or_else(|| panic!("受けバッファの組み立てが3本を作れていない"));
        let 拡散照度 = 作った読み戻しバッファ一覧.pop().unwrap_or_else(|| panic!("受けバッファの組み立てが3本を作れていない"));
        let 遠方環境 = 作った読み戻しバッファ一覧.pop().unwrap_or_else(|| panic!("受けバッファの組み立てが3本を作れていない"));
        Ok(Self {
            遠方環境, 拡散照度, 鏡面畳込み, 反射率積分表
        })
    }

    pub(super) fn 口(&self) -> 受けバッファの口 {
        受けバッファの口 {
            遠方環境: self.遠方環境.バッファのハンドル(),
            拡散照度: self.拡散照度.バッファのハンドル(),
            鏡面畳込み: self.鏡面畳込み.バッファのハンドル(),
            反射率積分表: self.反射率積分表.バッファのハンドル(),
        }
    }
...
fn 作った途中を片付けて返す(device: &GPUデバイス, 一覧: &[ベイク済み画像の読み戻しバッファ], 誤り: レンダラーエラー) -> レンダラーエラー {
    for バッファ in 一覧 {
        バッファ.破棄する(device);
    }
    誤り
