...
pub(in crate::vulkan) fn 書き込みパスを作る(画像: graph::画像ハンドル, 範囲: vk::Extent3D, 層数: u32, 元: vk::Buffer) -> graph::パス宣言<'static> {
    転送パスを作る("検査用遠方環境の書き込み", 画像, vec![範囲], 層数, 元, 四成分テクセルのバイト数)
}

/// 立方体画像の全段を書き込む。段ごとの範囲は段番号の昇順で渡す。
pub(in crate::vulkan) fn 全段の書き込みパスを作る(パス名: &'static str, 画像: graph::画像ハンドル, 段ごとの範囲: Vec<vk::Extent3D>, 層数: u32, 元: vk::Buffer) -> graph::パス宣言<'static> {
    転送パスを作る(パス名, 画像, 段ごとの範囲, 層数, 元, 四成分テクセルのバイト数)
}

/// 層も縮小段も持たない2次元画像を書き込む。反射率積分表がこの形を取る。
pub(in crate::vulkan) fn 表の書き込みパスを作る(パス名: &'static str, 画像: graph::画像ハンドル, 範囲: vk::Extent3D, 元: vk::Buffer, テクセルのバイト数: u64) -> graph::パス宣言<'static> {
...
fn 領域を並べる(段ごとの範囲: &[vk::Extent3D], 層数: u32, テクセルのバイト数: u64) -> Vec<vk::BufferImageCopy> {
    ...
    for (段番号, 範囲) in 段ごとの範囲.iter().enumerate() {
        ...
        領域一覧.push(vk::BufferImageCopy::default().buffer_offset(位置).image_subresource(層の部分範囲(段, 層数)).image_extent(*範囲));
