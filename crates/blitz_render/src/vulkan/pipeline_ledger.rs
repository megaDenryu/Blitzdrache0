...
//!
//! 布・粒子・全画面・コンピュートはこの台帳の外にあり、それぞれの処理位置の資源が自分のパイプラインを持ち続ける。
...
//! `シーンの描画先`と`シーンの照明束縛`は起動時に決めた契約であり、キーを組み立てるのは台帳自身である
//! (呼び出し元が成分を持ち回らない)。`深度プリパス方式`からは色パスの深度の扱いを導くため、台帳が唯一の持ち主である。
...
pub(crate) struct 材質描画族パイプライン台帳 {
    ...
    シーンの描画先: 描画先の一意化,       // 起動時に決めた描画先の契約
    シーンの照明束縛: 照明束縛レイアウト, // 起動時に決めた照明の契約
    深度プリパス方式: 深度プリパス方式,   // 起動時の計測条件が選んだ深度プリパスの方式
    シャドウの描画先: 描画先の一意化,
    深度プリパスの描画先: 描画先の一意化,
...
impl 材質描画族パイプライン台帳 {
    /// 未知のキーは型付きの失敗である。記録の途中で作る選択肢を持たない。
    pub(crate) fn 参照する(&self, キー: パイプラインキー) -> Result<vk::Pipeline, レンダラーエラー> {
        let 表 = match キー {
    ...
    pub(crate) fn 破棄する(&mut self, device: &ash::Device) {
        ...
        self.シャドウ.破棄する(|pipeline| device_supplier::パイプラインを破棄する(device, pipeline));
        self.深度プリパス.破棄する(|pipeline| device_supplier::パイプラインを破棄する(device, pipeline));
