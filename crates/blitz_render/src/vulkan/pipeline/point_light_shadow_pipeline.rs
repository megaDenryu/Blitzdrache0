...
impl 点光源の影のパイプライン {
    ...
    pub(crate) fn 生成する(確保係: &GPU資源の確保係<'_>, ディスクリプタlayout一覧: &[vk::DescriptorSetLayout], シェーダー: &シェーダー一式) -> Result<Self, レンダラーエラー> {
        let layout = パイプラインレイアウト::確保する(確保係, ディスクリプタlayout一覧, 点光源の影の描画定数::プッシュ定数範囲())?;
        match 点光源の影のパイプラインを生成する(確保係, layout.レイアウトのハンドル(), シェーダー) {
...
fn 点光源の影のパイプラインを生成する(確保係: &GPU資源の確保係<'_>, layout: vk::PipelineLayout, シェーダー: &シェーダー一式) -> Result<vk::Pipeline, レンダラーエラー> {
