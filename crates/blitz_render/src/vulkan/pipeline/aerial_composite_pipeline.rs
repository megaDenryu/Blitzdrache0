...
//! set0はシーンのディスクリプタセットレイアウトを再利用して空パス定数とビュー・シーンパス定数を読み、set1で深度とボリュームを参照する。
//! 画素段の即時定数でそのフレームの最遠距離を受ける。
//!
//! 深度をアタッチメントとして束縛しないのは、同じパスで深度を読みながら深度アタッチメントにも指定すると
//! フィードバックループになるためである。画素の限定は深度比較でなく画素段の`discard`が担う。
...
/// 即時定数のバイト数。`shaders/aerial_composite.slang`の`AerialCompositeCondition`(実数1つ)と一致させる。
const 即時定数バイト数: u32 = 4;
