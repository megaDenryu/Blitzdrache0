...
impl 色パスの深度状態 {
    pub(crate) const fn 方式から決める(方式: 深度プリパス方式) -> Self {
        match 方式 {
            ...
            深度プリパス方式::使い色は近いか同値で比べる => Self::近いか同値を描いて書く,
    ...
    pub(crate) const fn 比較(self) -> vk::CompareOp {
        match self {
            Self::より近いものを描いて書く => vk::CompareOp::GREATER,
            Self::近いか同値を描いて書く => vk::CompareOp::GREATER_OR_EQUAL,
    ...
    pub(crate) const fn 書き込むか(self) -> bool {
        match self {
            Self::より近いものを描いて書く | Self::近いか同値を描いて書く => true,
