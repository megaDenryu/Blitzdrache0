//! 空パイプライン生成のモジュールライフサイクル管理: シェーダーモジュールの生成→固定機能組み立て→破棄。
//! 固定機能ステートの詳細は`assemble`に委ねる(`pipeline/create.rs`と同じ分け方)。
...
pub(super) fn 空パイプラインを生成する(
    ...
    let 結果 = assemble::空パイプラインの固定機能を組み立てる(device, カラー形式, 深度形式, ディスクリプタlayout一覧, 頂点モジュール, 画素段モジュール);
