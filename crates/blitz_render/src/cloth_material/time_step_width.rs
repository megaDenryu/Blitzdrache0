//! 布の固定刻み1本の長さ。値は時間の規律の正本(`crates/blitz_app/src/world_execution/step_seconds.rs`)からコンポジションルートが注入し、
//! レンダラーは刻み幅の値を1つも自分で持たない(`_doc/設計/XPBD共通拘束基盤.md`「判断9」)。`blitz_sim::刻み幅`の写しであり、
