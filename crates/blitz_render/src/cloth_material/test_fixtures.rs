...
pub(super) fn 更新対応(目標拘束: u32, スキン頂点: u32) -> 目標の更新対応 {
    目標の更新対応 {
        目標拘束添字: 布の目標拘束添字::生成する(目標拘束),
        スキン頂点添字: スキン頂点添字::生成する(スキン頂点),
    }
}
...
pub(super) fn 目標拘束つきの材料(拘束一覧: &[Vec<u8>], 色の区間一覧: Vec<布の彩色の区間>, 目標拘束一覧: &[Vec<u8>], 目標位置一覧: &[Vec<u8>]) -> 布素材の材料 {
