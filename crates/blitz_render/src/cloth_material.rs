...
pub struct 布素材の材料 {
    ...
    pub 目標の更新対応一覧: Vec<[u32; 2]>, // (目標拘束添字, スキン頂点添字)。反復の前にスキン済み頂点の位置をその目標へ写す
...
/// 粒子の逆質量が正であることまで検証する。スキン頂点添字(目標の更新対応の第2要素)の検証は、スキン済み頂点数を知るレンダラー生成時に行う。
#[derive(Debug, Clone)]
pub struct 布素材 {
...
impl 布素材 {
    pub fn 生成する(材料: 布素材の材料) -> Result<Self, 布素材エラー> {
        ...
        for 対応 in &材料.目標の更新対応一覧 {
            target_check::更新対応の拘束添字を検証する(対応[0], 目標拘束の数)?;
        }
        Ok(Self {
            粒子バイト列: 材料.粒子バイト列,
