...
pub(super) fn 実行する(
    ...
    let フレーム添字 = レンダラー.現在フレーム添字;
    let フェンス = レンダラー.フレーム同期.フェンス(フレーム添字);
    // 安全性: フェンスは前フレームまたは初期シグナル状態のGPU完了を表す。
    unsafe { レンダラー.device.wait_for_fences(&[フェンス], true, u64::MAX)? };
    if let Some(時計) = &mut cpu区間時計 {
        時計.フェンス待機を終了する();
    }
    if let Some(計測) = &mut レンダラー.gpu計測 {
        計測.読み取る(&レンダラー.device, フレーム添字);
    }
