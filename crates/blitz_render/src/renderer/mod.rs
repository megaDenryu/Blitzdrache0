...
pub struct レンダラー {
    ...
    /// ウィンドウ寸法に連動して揃って作り直す資源(スワップチェーン・深度バッファ・提示同期)と、その資源が要求寸法に追従しているかどうかの状態の束。
    /// 深度画像の寸法と提示セマフォの本数がスワップチェーンの決めた値と一致すること、およびゼロ寸法や陳腐化のまま描画へ進めないことはこの型が保つ。
    提示: presentation::提示,
    ...
    /// 索引化した材質テクスチャ表の資源表世代の所有者。段4aでは束縛するセットもシェーダーも無い(参照: `_doc/設計/GPU資源束縛の分離と索引化.md`「段階導入」)。
    ...
    /// フレームスロットで参照する資源(コマンドバッファ・描画完了フェンス・取得セマフォ)と、そのスロットの巡回状態の束。3つを同じスロットで参照する一致はこの型が保つ。
    ...
    /// フレーム構成にポスト処理段階があるときのみ`Some`(判断38・39)。HDR中間画像・光のにじみ・明るさの圧縮の
    ...
    /// 大気のベイク済み画像生成パスの実行数の記録。更新判定が働いているかを実測で見る計器であり、常に数える
