//! 起動時シーンの描画対象を材質資源表へ登録して束を作る工程。受け取るのは共有資源と材質資源表と描画シーン素材、
...
use crate::vulkan::material_table::材質資源表;
...
pub(super) fn 起動シーンの束を作る(
    ...
    let (材質id一覧, 下書き) =
        材質資源表.束の材質を下書きする(crate::renderer::scene_draw_resources::起動シーンの束ID, 描画シーン.描画対象一覧())?;
    let 結果 = シーン描画資源::生成する(
        device,
        シーン描画資源生成要求 {
            メモリプロパティ,
