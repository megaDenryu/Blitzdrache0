//! 提示資源の生成後に組み立てる残り資源の組み立て手順。段ごとの生成はサブモジュールへ分ける(基礎/パイプライン/粒子/ポスト処理/シミュレーション)。束の型は`frame_resources`。
...
pub(super) fn 組み立てる(要求: 生成要求<'_>) -> Result<フレーム資源, レンダラーエラー> {
    ...
    let シーンカラー形式 = 描画先.シーンカラー形式(要求.提示資源.画像形式());
    ...
    let ui一式 = vulkan::ui::UIリソース一式::生成する(device, 要求.提示資源.画像形式(), &要求.シェーダー.ui)?;
    let ポスト処理 = 描画先.組み立てる(device, &メモリプロパティ, 要求.提示資源, 要求.シェーダー)?;
