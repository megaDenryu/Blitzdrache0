...
pub(super) fn 組み立てる(要求: 生成要求<'_>) -> Result<フレーム資源, レンダラーエラー> {
    ...
    let 基礎 = base_resources::組み立てる(要求.環境, &確保係, 要求.描画シーン, 要求.影の一辺, 照明束縛)?;
    ...
    let 粒子 = particle_resources::組み立てる(&要求, &確保係, &基礎, シーンカラー形式)?;
    ...
    let 画面連動 = screen_linked_resources::組み立てる(&要求, &確保係, &基礎)?;
