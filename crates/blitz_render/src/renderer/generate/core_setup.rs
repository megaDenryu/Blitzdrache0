//! GPU環境の生成と、その環境を材料にする最初の資源(提示・実表示計測)の取得を1つの束にまとめる工程。
...
pub(super) fn 組み立てる(表示ハンドル: RawDisplayHandle, ウィンドウハンドル: RawWindowHandle, 寸法: ウィンドウ寸法, 実表示計測要求: 実表示計測要求) -> Result<コア資源, レンダラーエラー> {
    let 検証カウンタ = 検証カウンタ::生成する();
    let 環境 = GPU環境::生成する(表示ハンドル, ウィンドウハンドル, &検証カウンタ, 実表示計測要求)?;
    let 実表示計測 = 環境.実表示計測を作る();
    let 提示 = 提示::生成する(&環境, 寸法)?;
    let (タイムスタンプ対応か, タイムスタンプ周期ns) = 環境.物理デバイス問い合わせ().タイムスタンプ計測条件を調べる(環境.キューファミリ添字());
