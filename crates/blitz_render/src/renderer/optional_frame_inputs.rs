...
//! 担当する工程は「フレームスロットとそのフレームの値を受け取り、任意の処理位置の入力一式を作る」ことである。
//! 所有と借用を分けているのは、`任意描画入力`が借用の束であり、借用元をフレーム記録の呼び出しより長く生かす必要があるためである。開発用UIの入力だけはここへ含めない。UIは他と違い外部(blitz_app)から毎フレーム渡される値であり、集める対象を持たない。
...
impl レンダラー {
    pub(super) fn 任意入力の材料を集める(
        ...
        Ok(任意入力の材料 {
            ...
            時間再構成の合成入力: self.時間再構成.合成入力の注入入力(),
...
impl レンダラー {
    ...
    /// 時間再構成方式が履歴混合の世界だけ入力を作る。ポスト処理を組まない構成でも作らないのは、再構成の結果を書く先がHDR中間画像だからである。この組み合わせは起動引数の解析が型付きの失敗で拒むため、ここへ到達するのは資源が揃っている場合だけである。
