...
//! 空の処理位置・影の処理位置の布専用経路が使う現物を具体フィールドで持ち、資源の種類を鍵にした表では持たない。
//! シーンと影の本体のパイプラインはここに無い。材質を読む描画族はパイプラインキーで選ぶため、`vulkan::pipeline_ledger`の台帳が持つ。
//! 空の処理位置の大気のベイク済み画像のように処理位置が専有する画像もここが持つ。こうした専用の資源をレンダラー直下へ足さないことがこの器の目的であり、
...
//! 生成はレンダラー生成時の1回だけの局面であり、`create`が担う。一部の資源だけに触れる操作は、大気のベイク済み画像だけに触れるものを`atmosphere`、
//! 空の処理位置だけに触れるものを`sky`が持つ。
//! `空`の有無はフレーム構成の空の処理位置の有無に一致する。
//! `大気のベイク済み画像`の有無は、空の処理位置があるか照明問い合わせ契約が遠方環境の選択肢であることに一致する。
//! `遠方環境の照明`の有無は照明問い合わせ契約が遠方環境の選択肢であることに一致する。
//! `布シャドウ`の有無はフレーム構成の布シミュレーションの処理位置の有無に一致し、布一式の有無とは一致しない。
//! エンジンが布シミュレーションの処理位置を常に宣言するため、`レンダラー::生成する`の`構成と素材を検査する`が検査するのは布一式があるときに処理位置があることだけであり、布一式の無い起動でも布シャドウは作られる。
...
pub(super) struct 描画段階資源 {
    ...
    大気のベイク済み画像: Option<vulkan::atmosphere_lut::大気のベイク済み画像一式>, // 空の処理位置があるか照明問い合わせ契約が遠方環境の選択肢のときに`Some`
