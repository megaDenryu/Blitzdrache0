//! 描画の処理位置がGPUへ発行するときに束縛するパイプラインと、そのパイプラインレイアウト・専用ディスクリプタ・専用画像の所有者。
//! 空の処理位置・影の処理位置の布専用経路が使う現物を具体フィールドで持ち、処理位置の種類を鍵にした表では持たない。
//! シーンと影の本体のパイプラインはここに無い。材質を読む描画族はパイプラインキーで選ぶため、`vulkan::pipeline_ledger`の台帳が持つ。
//! 空の処理位置の大気のベイク済み画像のように処理位置が専有する画像もここが持つ。処理位置に属する資源をレンダラー直下へ足さないことがこの器の目的であり、
//! 資源の種類がパイプラインかどうかで置き場所を分けると目的が達せられないためである。
//! 処理位置が増えるたびにレンダラー直下のフィールドが増えることを止めるためにこの型がある
//! (参照: `_doc/設計/空と時間帯と遠距離シャドウ.md`「描画の処理位置の資源の器と布専用シャドウ経路」)。
//! 破棄はレンダラーの単一破棄元(`renderer/destroy.rs`)がこの型の`破棄する`を1回呼ぶ形を保つ。
//! 生成はレンダラー生成時の1回だけの局面であり、`create`が担う。処理位置ごとの操作は、大気のベイク済み画像だけに触れるものを`atmosphere`、
//! 空の処理位置だけに触れるものを`sky`が持つ。
//! `空`・`大気のベイク済み画像`・`遠方環境の照明`・`布シャドウ`の有無は、それぞれフレーム描画入力の空・大気媒体・遠方環境と、
//! レンダラーの布一式の有無に常に一致させる(布の対応は`レンダラー::生成する`の`構成と素材を検査する`が検査済み)。
//! `空`が持つ空の処理位置の資源は、空の方式もあわせて持つ。`合成深度の注入`はシーンの処理位置が専有する資源であり、据えるまでは転送パスを1本も積まない。
...
pub(super) struct 描画の処理位置の資源 {
    空: Option<vulkan::sky_stage::空の処理位置の資源>,                              // フレーム構成に空の処理位置があるときだけ`Some`
    大気のベイク済み画像: Option<vulkan::atmosphere_lut::大気のベイク済み画像一式>, // フレーム構成に空の処理位置があるときだけ`Some`
    遠方環境の照明: Option<vulkan::indirect_lighting::遠方環境の照明資源>,          // 照明問い合わせ契約が遠方環境の選択肢のときだけ`Some`
    布シャドウ: Option<vulkan::cloth_shadow::布シャドウ資源>,                       // フレーム構成に布シミュレーションの処理位置があるときだけ`Some`
