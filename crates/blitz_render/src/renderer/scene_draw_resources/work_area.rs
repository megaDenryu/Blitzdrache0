...
//! ディスクリプタセットはチャンク自身が自分の添字で参照して返すため、この走査はチャンクをまたいだ通し添字をセットの選択には使わない。1つの描画対象ぶんの組み立ては`object_entry`、1つの段ぶんの発行は`stage_issue`、数え方は`tally`、充填し終えた作業領域の読み出しは`readout`にある。
