...
impl シーン描画資源 {
    ...
    /// 積み終えた作業領域から、そのフレームの描画発行の内訳を数える。発行回数は作業領域の件数そのものであり、
    /// 個体数はその発行が描くインスタンス数の合計である。
    pub(in crate::renderer) fn 描画発行内訳を作る(&self) -> 描画発行内訳 {
        描画発行内訳::生成する(
            self.ジオメトリ入力作業領域.len(),
            self.シャドウ入力作業領域.len(),
            self.ジオメトリ入力作業領域.iter().map(|入力| u64::from(入力.インスタンス数)).sum(),
            self.シャドウ入力作業領域.iter().map(|入力| u64::from(入力.インスタンス数)).sum(),
        )
    }
