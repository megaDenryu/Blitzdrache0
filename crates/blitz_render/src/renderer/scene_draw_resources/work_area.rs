//! 毎フレームの描画入力作業領域の充填と受け渡し。チャンク一覧を先頭から走査し、描画対象1つにつき非空のLOD段ごとに、
//! そのパスへ描く個体が1体でもあればそのパスの描画入力を1件積む。
//! 作業領域はシーン1本と帯ごとに1本ずつであり、帯別カリングでは帯ごとに描く個体が違うため1本を4帯で共有できない。
//! 注意: 積む前に必ず空にする。各パスの入力の件数は、全チャンクの描画対象の非空段数の合計からそのパスの描画数が0の段を除いた数であり、パスごとに違う値になる。
//! ディスクリプタセットはチャンク自身が自分の添字で参照して返すため、この走査はチャンクをまたいだ通し添字をセットの選択には使わない。1つの描画対象ぶんの組み立ては`object_entry`、1つの段ぶんの発行は`stage_issue`、数え方は`tally`にある。
...
use crate::vulkan::frame::{シャドウ描画入力, ジオメトリ入力, 帯別シャドウ入力, 描画対象入力};
...
pub(super) struct 描画発行受け皿<'a> {
    ...
    pub(super) 帯別シャドウ: &'a mut [Vec<シャドウ描画入力>; 帯数],
...
impl シーン描画資源 {
    pub(in crate::renderer) fn 作業領域を更新する(&mut self, 入力: &作業領域更新入力<'_>) -> Result<(), レンダラーエラー> {
        ...
        for 列 in &mut self.帯別シャドウ入力作業領域 {
            列.clear();
        }
        self.計数集計.集計を始める();
        let mut 通し添字 = 0usize;
    ...
    /// 発行数だけは積んだ入力の件数そのものであるため、パスごとの列の長さをここで集計へ渡す。
    pub(in crate::renderer) fn 描画発行内訳を作る(&self) -> 描画発行内訳 {
        self.計数集計.内訳を作る(std::array::from_fn(|添字| match 添字.checked_sub(1) {
            None => self.ジオメトリ入力作業領域.len(),
            Some(帯添字) => self.帯別シャドウ入力作業領域[帯添字].len(),
        }))
    }
    ...
    pub(in crate::renderer) fn 描画対象入力を作る(&self) -> 描画対象入力<'_> {
        描画対象入力 {
            ...
            帯別シャドウ: 帯別シャドウ入力::生成する(&self.帯別シャドウ入力作業領域),
        }
    }
}
