...
pub(in crate::renderer::scene_draw_resources) fn 描画対象資源一覧を生成する(
    ...
    assert_eq!(描画対象一覧.len(), 材質id一覧.len(), "描画対象の件数と材質資源表が発番した材質IDの並びの件数が食い違った");
    ...
    for (描画対象添字, (描画対象, 材質id)) in 描画対象一覧.iter().zip(材質id一覧.iter()).enumerate() {
        ...
        match 描画対象資源::生成する(転送係, 描画対象, 材質id, &動く個体添字一覧) {
            Ok(資源) => 一覧.push(資源),
            Err(誤り) => {
                for 資源 in &一覧 {
                    資源.破棄する(device);
...
fn 描画対象の動く個体添字を集める(動く個体一覧: &[動く個体の宣言], 描画対象添字: usize) -> Vec<u32> {
    動く個体一覧.iter().filter(|宣言| 宣言.描画対象添字 == 描画対象添字).map(|宣言| 宣言.個体添字).collect()
}
