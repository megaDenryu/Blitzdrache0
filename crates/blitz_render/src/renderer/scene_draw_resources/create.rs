...
pub(in crate::renderer) struct チャンク描画資源生成材料<'a> {
    ...
    pub(in crate::renderer) 影方針: crate::描画束の影の出し方,
}
...
impl シーン描画資源 {
    ...
    pub(in crate::renderer) fn 生成する(確保係: &GPU資源の確保係<'_>, 要求: シーン描画資源生成要求<'_>) -> Result<Self, レンダラーエラー> {
        let 材料 = チャンク描画資源生成材料 {
            転送係: 要求.転送係,
            セットレイアウト: 要求.セットレイアウト,
            材質id一覧: 要求.材質id一覧,
            動く個体一覧: 要求.描画シーン.動く個体一覧(),
            影方針: crate::描画束の影の出し方::全距離区分,
        };
        let チャンク = チャンク描画資源::生成する(確保係, 材料, 起動シーンの束ID, 要求.描画シーン.描画対象一覧())?;
        let チャンク一覧 = vec![チャンク];
