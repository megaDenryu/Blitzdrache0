//! 束の追加と解除。触れるのは`チャンク一覧`と`破棄待ち`の2フィールドだけで、両者の間の移動規律をここが持つ。
//!
//! 解除された束をその場で破棄しないのは、過去のフレームで発行済みの描画コマンドがまだGPUで実行中でありうるためである。
//! 解除予約は描画発行より前に行うため、予約した時点でその束を使う新しい描画は発行されない。残るのは発行済みのものだけで、
//! それらはフレームインフライト数ぶんのフレームが進めば必ず完了している(同じスロットを再び使うときにフェンスを待つため)。
//! したがって`フレームインフライト数`回の`破棄待ちを一フレーム進める`を経てから破棄する。
...
impl シーン描画資源 {
    /// 注意: 同じIDの束を解除せずに追加するのは呼び出し元のバグである。上書きを許すと旧い束のGPU資源を辿れなくなり漏れるため、
    /// 回復可能な失敗ではなく破られた不変条件として扱う。
