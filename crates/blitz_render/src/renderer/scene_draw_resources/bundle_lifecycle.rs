...
impl シーン描画資源 {
    ...
    pub(in crate::renderer) fn 束を追加する(&mut self, 確保係: &GPU資源の確保係<'_>, 材料: 束追加材料<'_>, id: 描画束ID, 描画対象一覧: &[描画対象素材]) -> Result<(), レンダラーエラー> {
        assert!(!self.チャンク一覧.iter().any(|束| 束.id() == id), "同じ束ID{}を解除せずに二重追加した", id.番号を返す());
