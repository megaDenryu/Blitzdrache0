...
use crate::vulkan::descriptor::{ジオメトリセット参照, 描画対象ディスクリプタプール, 材質セット参照};
...
fn ディスクリプタを生成する(
    ...
    let 対象別材質参照一覧 = 描画対象資源一覧
        .iter()
        .map(対象のスロット別参照を作る)
        .collect::<Vec<Vec<材質セット参照<'_>>>>();
    描画対象ディスクリプタプール::生成する(device, 材料.セットレイアウト, &ジオメトリ参照一覧, &対象別材質参照一覧)
}

/// 材質スロットの添字順に参照を並べる。並びがそのままディスクリプタセットの並びになるため、
/// スロット資源が返す添字とセットの添字が同じ意味を持つ。
fn 対象のスロット別参照を作る(資源: &描画対象資源) -> Vec<材質セット参照<'_>> {
    (0..資源.材質スロット数())
        .map(|スロット添字| match 資源.スロットの材質セット参照(スロット添字) {
            Some(参照) => 参照,
