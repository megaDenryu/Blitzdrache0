...
impl チャンク描画資源 {
    ...
    pub(in crate::renderer::scene_draw_resources) fn 生成する(
        確保係: &GPU資源の確保係<'_>, 材料: チャンク描画資源生成材料<'_>, id: 描画束ID, 描画対象一覧: &[描画対象素材]
    ) -> Result<Self, レンダラーエラー> {
        let device = 確保係.論理デバイス();
        let 影方針 = 材料.影方針;
        let 描画対象資源一覧 = render_object_resources::描画対象資源一覧を生成する(材料.転送係, 描画対象一覧, 材料.材質id一覧, 材料.動く個体一覧)?;
        let 単一個体用可視id列 = match 可視ID列バッファ::生成する(材料.転送係.確保係(), 1) {
            Ok(値) => 値,
            Err(誤り) => {
...
fn ディスクリプタを生成する(
    ...
    let ジオメトリ参照一覧 = 描画対象資源一覧.iter().map(|資源| 資源.ジオメトリセット参照を作る(単一個体用可視id列)).collect::<Vec<ジオメトリセット参照>>();
    描画対象ディスクリプタプール::生成する(device, 材料.セットレイアウト, &ジオメトリ参照一覧)
}
