...
/// この対象へ来ているそのフレームの位置指定をすべてGPUへ書く。指定が1件も無い対象では走査だけで終わる。
/// 走査が線形なのは、動く個体の件数がゲーム状態の台帳に載る数であり、束ごとの描画対象数と同じ桁に収まるためである。
fn 動く個体の位置を書き込む(材料: &描画対象の材料<'_>) -> Result<(), レンダラーエラー> {
    for 指定 in 材料.入力.動く個体の大域位置の指定一覧 {
        if 指定.束id != 材料.束id || 指定.描画対象添字 != 材料.対象添字 {
            continue;
        }
        材料.資源.動く個体の位置を書き込む(材料.入力.device, 材料.入力.フレーム添字, *指定)?;
    }
    Ok(())
}
