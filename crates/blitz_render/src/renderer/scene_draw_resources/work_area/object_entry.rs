...
//! 非空のLOD段ごとのシーンパス入力と帯ごとのシャドウ描画入力である。
//! 可視ID列のGPUへの書き込みもここで行う。書き込む内容と各パスの描くインスタンス数は同じ選択から出るため、離すと食い違いうる。
//! 注意: 帯ごとのシャドウ描画入力のインスタンス数はその帯の判定が決めた区間であり、シーン側や他の帯の可視数を
//! 流用してはならない(参照: `_doc/設計/空と時間帯と遠距離シャドウ.md`「可視ID列の帯別区間(4区分並べの後継)」)。
...
fn 全個体を束の段で積む(
    材料: &描画対象の材料<'_>, 受け皿: &mut 描画発行受け皿<'_>, 相対アンカー: カメラ相対アンカー
) {
