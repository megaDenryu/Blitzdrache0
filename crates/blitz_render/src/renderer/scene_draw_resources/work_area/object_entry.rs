...
use super::{point_light_shadow_issue, stage_issue};
...
/// 描画対象1つを積む工程が受け取る材料一式。
///
/// `プリミティブ発行`はこの描画対象が全詳細段にわたって描くプリミティブの並びであり、段ごとの絞り込みは
/// 発行を積む工程が行う。`先頭対象か`はスキニングをシーンの先頭対象だけへ適用する(判断44の既存契約)ため
/// の判定で、可視判定で先頭が描かれなくなっても差し替え先の判断は走査順の先頭で決まる。
pub(super) struct 描画対象の材料<'a> {
    pub(super) 入力: &'a 作業領域更新入力<'a>,
    pub(super) 束id: 描画束ID,
    pub(super) 影方針: crate::描画束の影の出し方,
...
impl 描画対象の材料<'_> {
    /// この対象の基準原点をそのフレームのカメラ大域原点で狭めた位置。プッシュ定数の基準原点も、
    /// 点光源の影の絞りが読む球の中心も、この1つの値から出す(2度求めると同じ量を2箇所で持つことになる)。
    pub(super) fn 基準原点のカメラ相対位置を作る(&self) -> Result<カメラ相対位置, レンダラーエラー> {
        Ok(self.資源.大域の基準原点.カメラ相対へ変換する(self.入力.カメラ大域原点)?)
    }

    pub(super) fn 頂点バッファ差し替え先(&self) -> Option<vk::Buffer> {
        if self.先頭対象か { self.入力.スキン済み頂点バッファ } else { None }
    }
}

pub(super) fn 積む(材料: &描画対象の材料<'_>, 受け皿: &mut 描画発行受け皿<'_>) -> Result<(), レンダラーエラー> {
    let 基準原点のカメラ相対位置 = 材料.基準原点のカメラ相対位置を作る()?;
    let 相対の基準原点 = カメラ相対の基準原点::相対位置から生成する(基準原点のカメラ相対位置);
    動く個体の位置と向きを書き込む(材料)?;
    point_light_shadow_issue::点光源の影の発行を積む(材料, 受け皿.点光源の影, 基準原点のカメラ相対位置, 相対の基準原点)?;
    受け皿.集計.対象の個体数を加える(u64::from(材料.資源.個体数()));
    let Some(計画) = 材料.入力.可視個体選択一覧.参照する(材料.束id, 材料.対象添字) else {
...
/// この対象へ来ているそのフレームの位置と向きの指定をすべてGPUへ書く。指定が1件も無い対象では走査だけで終わる。
/// 走査が線形なのは、動く個体の件数がゲーム状態の台帳に載る数であり、束ごとの描画対象数と同じ桁に収まるためである。
fn 動く個体の位置と向きを書き込む(材料: &描画対象の材料<'_>) -> Result<(), レンダラーエラー> {
    for 指定 in 材料.入力.動く個体の大域の位置と向きの指定一覧 {
        if 指定.束id != 材料.束id || 指定.描画対象添字 != 材料.対象添字 {
            continue;
        }
        材料.資源.動く個体の位置と向きを書き込む(材料.入力.device, 材料.入力.フレーム添字, *指定)?;
    }
    Ok(())
}

/// 可視判定と段選択の対象でない描画対象。束の詳細段で全個体を1回だけ描き、可視ID列は生成時の恒等の列をそのまま読む。
fn 全個体を束の段で積む(材料: &描画対象の材料<'_>, 受け皿: &mut 描画発行受け皿<'_>, 相対の基準原点: カメラ相対の基準原点) -> Result<(), レンダラーエラー> {
