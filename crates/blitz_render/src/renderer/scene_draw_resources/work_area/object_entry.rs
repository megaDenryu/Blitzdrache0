...
//! 注意: シャドウ描画入力のインスタンス数は常にその段の全個体である。カメラ視錐台で絞った可視集合をシャドウへ流用してはならない
...
pub(super) fn 積む(材料: &描画対象の材料<'_>, 受け皿: &mut 描画発行受け皿<'_>) -> Result<(), レンダラーエラー> {
    ...
    for (段番号, 範囲) in 計画.段範囲一覧.iter().enumerate() {
        受け皿.集計.シーン可視数を加える(u64::from(範囲.可視数()));
...
fn 全個体を束の段で積む(
    ...
    let 個体数 = 材料.資源.個体数();
    受け皿.集計.シーン可視数を加える(u64::from(個体数));
    stage_issue::積む(材料, 受け皿, 材料.段.添字(), 段別描画範囲::生成する(0, 個体数, 個体数), 相対アンカー);
}
