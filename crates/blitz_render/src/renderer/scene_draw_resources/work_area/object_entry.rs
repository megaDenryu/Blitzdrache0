...
//! 非空のLOD段ごとのシーンパス入力とシャドウ描画入力である。
//! 可視ID列のGPUへの書き込みもここで行う。書き込む内容と各段の描くインスタンス数は同じ選択から出るため、離すと食い違いうる。
//! 注意: シャドウ描画入力のインスタンス数はシャドウ側の判定が決めた範囲であり、シーン側の可視数を流用してはならない
...
pub(super) fn 積む(材料: &描画対象の材料<'_>, 受け皿: &mut 描画発行受け皿<'_>) -> Result<(), レンダラーエラー> {
    ...
    材料.資源.可視id列を書き込む(材料.入力.device, 材料.入力.フレーム添字, 計画.id列)?;
    for (段番号, 範囲) in 計画.段範囲一覧.iter().enumerate() {
        受け皿
            .集計
            .パス別可視数を加える(u64::from(範囲.シーン().描画数()), u64::from(範囲.シャドウ().描画数()));
...
fn 全個体を束の段で積む(
    ...
    let 個体数 = 材料.資源.個体数();
    受け皿.集計.パス別可視数を加える(u64::from(個体数), u64::from(個体数));
    stage_issue::積む(材料, 受け皿, 材料.段.添字(), 段別描画範囲::全個体を描く(0, 個体数), 相対アンカー);
}
