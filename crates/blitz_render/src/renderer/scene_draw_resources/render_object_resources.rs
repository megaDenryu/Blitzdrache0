...
//! `個体変換`は描画が可視ID列を通した添字で1件を選び、動く個体を宣言した対象だけがスロット別の選択肢になる。
//! `境界球`を読むのは点光源の影の面ごとの絞りだけである。
//! `内容検査`は、この対象の個体数(個体変換列の件数と常に一致する)と、パスごとの重複を見るための既出記録を持つ。
...
pub(super) struct 描画対象資源 {
    ...
    段別ジオメトリ: geometry_list::段別ジオメトリ, // 詳細段の昇順に並んだ非空のジオメトリ
    スロット別材質id: スロット別材質ID,            // 材質スロットごとの大域材質ID
    個体変換: 個体レコードの置き場,                // 個体変換の列の置き場
    可視id列: 可視ID列の出どころ,
    境界球: 描画対象の境界球,     // 全個体を覆う球。読み込み時に決まり、以後変わらない
    内容検査: 可視ID列の内容検査, // 書き込む列の中身がこの対象の個体と整合していることの検査
}
