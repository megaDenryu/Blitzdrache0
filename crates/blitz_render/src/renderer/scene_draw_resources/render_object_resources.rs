...
use visible_id_range::全個体の並べ替え検査;
...
pub(super) struct 描画対象資源 {
    ...
    個体変換: 個体変換の出どころ,
    可視id列: 可視ID列の出どころ,
    /// 書き込む列が全個体の並べ替えであることの検査。この対象の個体数(個体変換列の件数と常に一致する)と、
    /// 重複を見るための既出記録をこれが持つ。
    並べ替え検査: 全個体の並べ替え検査,
}
...
impl 描画対象資源 {
    ...
    /// そのフレームに描く個体の添字を段別の並びで書く。可視個体選択を持たない対象へは呼ばない。
    /// 書き込む前に列が全個体の並べ替えであることを確かめるのは、この型が対象の個体数を持つ唯一の書き込み境界だからである。
    /// 並べ替えであることがそのまま「シャドウパスが全個体を描く」ことの担保になる。
