...
impl 描画対象資源 {
    ...
    /// 書き込む前に列の長さが個体数と一致することと各IDが個体数の内側を指すことを確かめるのは、
    /// この型が対象の個体数を持つ唯一の書き込み境界だからである。長さの一致がそのまま
    /// 「シャドウパスが全個体を描く」ことの担保になる。
    pub(super) fn 可視id列を書き込む(
        ...
        visible_id_range::全個体の並べ替えかを検査する(可視id列, self.個体数)?;
