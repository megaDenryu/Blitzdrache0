//! 描画の処理位置の資源の生成。呼ばれるのはレンダラー生成時の1回だけであり、以降のフレームは参照しかしない。
//! 途中で失敗したら、それまでに生成した資源をその場で逆順に破棄する。生成の途中経過を外へ出さないため、
//! 部分的に生成された器は呼び出し元から見えない。
//! フレーム構成しだいで作る処理位置の資源は`optional_stages`が担う。シーンと影の本体のパイプラインはパイプライン台帳が持つため、ここでは作らない。
...
/// 器を組み立てるのに要る材料一式。
///
/// `シーンカラー形式`はシーンの処理位置の色アタッチメントの形式(ポスト処理があればHDR中間画像、無ければ
/// スワップチェーン)であり、影の処理位置は深度だけへ書くため色形式を要らない。`照明問い合わせ契約`は世界の
/// 間接照明方針から起動時に決まる契約で、遠方環境の選択肢だけが遠方環境と派生表現の資源を作る。
/// `クラスタ選別が読むバッファ組一覧`は照明問い合わせ資源束が持つスロットごとのバッファの組であり、
/// 選別の生成側のセットがこれを結ぶ。
...
/// 選別を先に作るのは、この後に続く任意の処理位置の資源に破棄をまとめて呼ぶ口が無く、後から作ると
/// 任意の処理位置の失敗で選別だけが漏れる経路と、選別の失敗で任意の処理位置が漏れる経路のどちらかが必ず残るためである。
pub(super) fn 生成する(要求: 生成要求<'_>) -> Result<描画の処理位置の資源, レンダラーエラー> {
    let クラスタ選別 = クラスタ選別一式::生成する(要求.確保係, &要求.シェーダー.クラスタ選別, 要求.クラスタ選別が読むバッファ組一覧)?;
    let 点光源の影 = match vulkan::pipeline::点光源の影のパイプライン::生成する(要求.確保係, &要求.セットレイアウト.シャドウの並び(), &要求.シェーダー.点光源の影) {
        Ok(値) => 値,
        Err(誤り) => {
