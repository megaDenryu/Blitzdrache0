//! フレーム構成と照明問い合わせ契約しだいで作る処理位置の資源の生成局面。受け取るのは器の生成要求、返すのは
//! 4つの`Option`を束ねた組である。処理位置を持たない構成では`None`が「その処理位置を積まない」ことを型で表す。
//!
...
impl 任意の処理位置の資源 {
    ...
    fn 片付ける(&self, 要求: &生成要求<'_>) {
        if let Some(布シャドウ) = &self.布シャドウ {
            布シャドウ.破棄する(要求.確保係.論理デバイス());
        }
        if let Some(空) = &self.空 {
            空.破棄する(要求.確保係.論理デバイス());
        }
        if let Some(遠方環境の照明) = &self.遠方環境の照明 {
            遠方環境の照明.破棄する(要求.確保係.論理デバイス());
        }
        if let Some(大気のベイク済み画像) = &self.大気のベイク済み画像 {
            大気のベイク済み画像.破棄する(要求.確保係.論理デバイス());
        }
    }
}

pub(super) fn 組み立てる(要求: &生成要求<'_>) -> Result<任意の処理位置の資源, レンダラーエラー> {
    let mut 資源 = 任意の処理位置の資源 {
        空: None,
...
fn 大気の上へ積む(要求: &生成要求<'_>, 資源: &mut 任意の処理位置の資源) -> Result<(), レンダラーエラー> {
