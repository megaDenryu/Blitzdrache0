...
//! `記録の実行環境`はバッファの持ち主と積み始めの口を同じ型へ置き、記録を頼む側に論理デバイスとキューを運ばせない。
//! `見送り累計`は、提示停止に起因する異常(破棄待ちの滞留・フレームループの空転)を実行中に観測する計器である。
...
pub(super) struct フレーム進行 {
    ...
    記録の実行環境: フレームの記録の実行環境, // コマンド一式のバッファへ1フレームぶんの積み込みを始める操作サービス
    フレーム同期: vulkan::sync::フレーム同期,
    巡回: フレームスロット巡回,
    見送り累計: u64, // 提示へ到達しなかったフレームの累計
}
...
impl フレーム進行 {
    ...
    pub(super) fn フレームのgpu命令を積み始める(
        &self, スロット資源: &フレームスロット資源, クエリプール: Option<vk::QueryPool>
    ) -> Result<フレームのGPU命令を積むコマンドバッファ<'_>, レンダラーエラー> {
        self.記録の実行環境.フレームのgpu命令を積み始める(スロット資源.command_buffer, クエリプール)
    }
