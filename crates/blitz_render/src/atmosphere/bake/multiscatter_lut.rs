...
//! `NewMultiScattCS`(`MULTI_SCATTERING_POWER_SERIE`の選択肢)。
...
pub fn 多重散乱のベイク済み画像のテクセル値(媒体: &大気散乱媒体, 透過率表: &透過率表, 解像度: 大気のベイク済み画像の解像度, 横: u32, 縦: u32) -> Result<多重散乱RGB, 大気数学エラー> {
...
pub fn 多重散乱のベイク済み画像を焼く(媒体: &大気散乱媒体, 透過率表: &透過率表, 解像度: 大気のベイク済み画像の解像度) -> Result<Vec<多重散乱RGB>, 大気数学エラー> {
    let 一辺 = 解像度.多重散乱の一辺();
