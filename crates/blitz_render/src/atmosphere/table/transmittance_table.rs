//! CPU側に焼いた透過率のベイク済み画像と、そこからの双一次補間の標本取り。
//! 受け取るのはテクセル座標のUV、返すのはその位置の透過率である。
//!
//! 多重散乱とスカイビューの計算がこの表を参照するのは、GPU側の生成シェーダーが透過率のベイク済み画像をサンプラーで参照するからである。
...
impl 透過率表 {
    pub fn 焼く(媒体: &消散媒体, 解像度: 大気のベイク済み画像の解像度) -> Result<Self, 大気数学エラー> {
        Ok(Self {
            解像度,
            テクセル一覧: 透過率のベイク済み画像を焼く(媒体, 解像度)?,
        })
    }
    ...
    fn テクセル(&self, 横: u32, 縦: u32) -> [f64; 3] {
        ...
        let Some(値) = self.テクセル一覧.get(縦 * 幅 + 横) else {
            panic!("透過率表の範囲外を参照した(横{横} 縦{縦})。添字の切り詰めが効いていない");
        };
        値.成分().map(f64::from)
    }
}
