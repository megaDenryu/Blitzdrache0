...
fn メートルに包む(値: f32) -> メートル {
    メートル::生成する(値)
}

pub(super) fn 地球標準の媒体() -> 消散媒体 {
    let [赤, 緑, 青] = レイリー散乱係数;
    let レイリー = 減衰成分::生成する(消散係数RGB::毎メートルから(赤, 緑, 青).unwrap(), 密度分布::指数分布(メートルに包む(レイリー尺度高度)).unwrap());
    let ミー = 減衰成分::生成する(消散係数RGB::毎メートルから(ミー消散係数, ミー消散係数, ミー消散係数).unwrap(), 密度分布::指数分布(メートルに包む(ミー尺度高度)).unwrap());
    let [吸収赤, 吸収緑, 吸収青] = 吸収消散係数;
    let 吸収 = 減衰成分::生成する(消散係数RGB::毎メートルから(吸収赤, 吸収緑, 吸収青).unwrap(), 密度分布::テント(メートルに包む(吸収下端高度), メートルに包む(吸収頂点高度), メートルに包む(吸収上端高度)).unwrap());
    消散媒体::生成する(メートルに包む(下端半径), メートルに包む(上端半径), レイリー, ミー, 吸収).unwrap()
}
