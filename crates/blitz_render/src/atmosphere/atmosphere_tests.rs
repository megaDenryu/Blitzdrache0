...
pub(super) fn 地球標準の媒体() -> 消散媒体 {
    let [赤, 緑, 青] = レイリー散乱係数;
    let レイリー = 減衰成分::生成する(消散係数RGB::毎メートルから(赤, 緑, 青).unwrap(), 密度分布::指数分布(m(レイリー尺度高度)).unwrap());
    let ミー = 減衰成分::生成する(消散係数RGB::毎メートルから(ミー消散係数, ミー消散係数, ミー消散係数).unwrap(), 密度分布::指数分布(m(ミー尺度高度)).unwrap());
    let [吸収赤, 吸収緑, 吸収青] = 吸収消散係数;
    let 吸収 = 減衰成分::生成する(消散係数RGB::毎メートルから(吸収赤, 吸収緑, 吸収青).unwrap(), 密度分布::テント(m(吸収下端高度), m(吸収頂点高度), m(吸収上端高度)).unwrap());
