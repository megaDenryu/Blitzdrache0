...
/// 束の識別子から段を参照する。選択に無い束は最詳細段を描く。段を1つしか持たない束は選択の対象にならず、その束にとって最詳細段が唯一の段だからである。
/// 一覧の件数は常駐チャンク数と同じ数十件であり、束ごとの走査は描画発行の件数と同じ桁に収まる。
pub(crate) fn 段を参照する(一覧: &[地形詳細段選択], 束id: 描画束ID) -> 地形詳細段 {
    一覧.iter().find(|選択| 選択.束id() == 束id).map_or(地形詳細段::最詳細(), |選択| 選択.段())
}
