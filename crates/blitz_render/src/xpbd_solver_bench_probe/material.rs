...
impl XPBD計測素材 {
    pub fn 生成する(
        点の状態バイト列: Vec<u8>, 拘束の引数バイト列: Vec<u8>, 隣接の区間バイト列: Vec<u8>, 隣接の項目バイト列: Vec<u8>, 色の区間一覧: Vec<XPBD彩色の区間>, 点の数: u32, 拘束の数: u32
    ) -> Result<Self, XPBD計測素材エラー> {
        if 点の数 == 0 || 拘束の数 == 0 {
            return Err(XPBD計測素材エラー::空のグラフ { 点の数, 拘束の数 });
        }
        let 点 = u32の件数をusizeへ変換する(点の数);
        let 拘束 = u32の件数をusizeへ変換する(拘束の数);
