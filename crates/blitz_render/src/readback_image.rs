//! 読み戻した1フレームの結末を表す値オブジェクト。提示画像のRGBA8並び・明るさの圧縮前のHDR中間画像の単精度並び・最終深度の単精度並び・局所可視度の符号値の並び・動きベクトルの2成分の並びの5つの画像と、読み戻しが成立したかどうかの判別を持つ。
//! 深度の値オブジェクトは`depth`に、局所可視度の値オブジェクトは`local_visibility`に、動きベクトルの値オブジェクトは`motion_vector`に、読み戻しの成否の判別は`result`にある。
//!
//! 5つを1つのモジュール木へ置くのは、どれも同じ読み戻しの結末であり、読み戻し対象の別だけで表現が分かれるためである。
//! 提示画像だけを見せると、8ビットの値から圧縮前の放射輝度を復元しようとする誤りを招く。
//!
...
pub use depth::深度読み戻し画像;
pub use local_visibility::局所可視度読み戻し画像;
pub use motion_vector::動きベクトル読み戻し画像;
pub use result::読み戻し結果;
