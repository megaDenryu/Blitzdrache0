...
impl 表面流の計算条件 {
    pub fn 生成する(格子寸法: [u32; 2], セル幅: f32, 時間刻み: f32, 重力接線成分: [f32; 2], 速度減衰: f32) -> Result<Self, 表面流の計算条件エラー> {
        let [列数, 行数] = 格子寸法;
        if 列数 < 2 || 行数 < 2 {
            return Err(表面流の計算条件エラー::格子が小さすぎる { 列数, 行数 });
        }
        if !セル幅.is_finite() || セル幅 <= 0.0 {
            return Err(表面流の計算条件エラー::セル幅が不正 { 指定値: セル幅 });
        }
        if !時間刻み.is_finite() || 時間刻み <= 0.0 {
            return Err(表面流の計算条件エラー::時間刻みが不正 { 指定値: 時間刻み });
        }
        if !速度減衰.is_finite() || !(0.0..=1.0).contains(&速度減衰) {
            return Err(表面流の計算条件エラー::速度減衰が不正 { 指定値: 速度減衰 });
        }
        Ok(Self {
            列数,
            行数,
