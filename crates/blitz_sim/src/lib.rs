...
pub use gpu_layout::{
    xpbd as xpbd_gpu_layout, 拘束バイト列にする, 粒子バイト列にする, 表面流バイト列にする, 隣接拘束バイト列にする
};
...
pub use xpbd::{
    コンプライアンス, コンプライアンスエラー, ラグランジュ乗数, 刻み幅, 刻み幅エラー, 距離の誤差を逆質量の比で分配する, 距離拘束の一刻みの係数,
    距離拘束の一回の射影の結果, 距離拘束の参加点, 距離拘束の引数, 静止長, 静止長エラー,
};
