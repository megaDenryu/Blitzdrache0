...
pub(super) fn 近い点の対を選ぶ(生成器: &決定的な値の生成器, 位置一覧: &[位置<ワールド>], 上限: u32) -> Vec<点の対> {
    ...
    for (添字, 位置) in 位置一覧.iter().enumerate() {
        ...
        for 相手 in 近い順の添字(位置一覧, 添字, 位置).into_iter().take(本数) {
            ...
            対の集合.insert(点の対 { a: 添字へ(小), b: 添字へ(大) });
        }
    }
...
fn 本数を決める(生成器: &決定的な値の生成器, 添字: usize, 上限: u32) -> usize {
    let 値 = 生成器.零以上一未満の値を求める(u64::try_from(添字).unwrap_or(0), 用途番号::番号から生成する(本数の用途番号));
...
fn 近い順の添字(位置一覧: &[位置<ワールド>], 自分: usize, 位置: &位置<ワールド>) -> Vec<usize> {
    let mut 候補: Vec<(f32, usize)> = 位置一覧.iter().enumerate().filter(|(添字, _)| *添字 != 自分).map(|(添字, 相手)| ((*相手 - *位置).長さ().値(), 添字)).collect();
