...
//! GPUは距離拘束の色の後ろに曲げの色を積む(`_doc/設計/XPBD共通拘束基盤.md`「判断8」)。色ごとに連続した並びへ組み替えて持つのは、GPUが色ごとに1回の計算の発行で区間を処理するためである。
...
/// 色1つが占める、並べ替えた曲げ拘束一覧の中の連続した区間。GPUの計算の発行1回分である。
