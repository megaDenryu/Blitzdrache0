...
fn 同じ点を持つ拘束と範囲外の添字を拒む() {
    ...
    assert!(matches!(曲げ拘束のバッチ::生成する(3, vec![拘束(0, 1, 2, 3).unwrap()]), Err(拘束グラフエラー::曲げ拘束の点添字が範囲外 { .. })));
}

/// 4点を触る拘束は、辺を1本しか共有しない隣どうしでも同じ色になれない。
#[test]
fn 色の区間は全拘束を覆い同じ色の中で点を共有しない() {
    let 一覧 = vec![拘束(0, 1, 2, 3).unwrap(), 拘束(1, 2, 0, 4).unwrap(), 拘束(5, 6, 7, 8).unwrap(), 拘束(3, 4, 1, 9).unwrap()];
