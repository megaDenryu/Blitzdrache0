...
//! 回すことである。接触点集合1つの非貫通の中身は`substep_non_penetration`が、静止摩擦の中身は
//! `substep_static_friction`が、速度段階の巡回は`substep_velocity_stage`が持つ。
...
impl 一つの箱と静的な直方体の場面 {
    ...
    // 判断19の反復1回。まず接触点集合ごとに非貫通を同時に解き、次に接触点集合ごとに静止摩擦を射影する。
    pub(super) fn 一回反復する(&self, バッチ: &mut 接触拘束の二つのバッチ, 予測: 予測の状態) -> 予測の状態 {
        ...
        for 集合の番号 in 0..バッチ.剛体と静的世界の接触点集合の数() {
            let 接触点集合 = バッチ.剛体と静的世界の接触点集合を反復のために借りる(集合の番号);
            予測 = self.接触点集合の静止摩擦を射影する(接触点集合, 予測);
        }
        予測
    }
}
