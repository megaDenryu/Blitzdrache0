...
//! `relative_origin_direction_detail`・`relative_origin_slope_tolerance`)と段階A5の計器(`noise_floor_by_box_arm`・`contact_identity_by_posture`・`increment_uncertainty_record`・`discarded_component_on_rows`・`coordinate_free_discard`・`relative_eigenvalue_margin`)も
