...
//! 接線は接触点集合1つを1つの連立として同時に解く。点ごとに順に射影すると、各点が物体全体の滑りを1点で戻そうと
//! して接線の乗数が積み増され、傾きの正接が静止摩擦係数より小さい坂でも円錐を超えて滑走へ落ちる。
...
#[cfg(test)]
mod manifold_fixture;
#[cfg(test)]
mod manifold_order_tests;
mod manifold_row;
...
pub use manifold_row::静止摩擦の接触点集合の一行;
pub use manifold_row_result::静止摩擦の連立へ点を入れた結果;
pub use manifold_solution::接触点集合の静止摩擦の同時解;
pub use manifold_system::接触点集合の静止摩擦の連立;
