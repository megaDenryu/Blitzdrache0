...
//! 有効集合は取りうる部分集合を全部走査して相補条件を満たすものから選ぶ(`subset_search`)。枢軸を順に動かす形は、
//! 外す条件と入れ直す条件が同じ点について同時に成り立つ材料が坂の場面に実在するため終了しない。
...
// 解と結末を外へ出す口。いまこれを読むのは試験の場面と、連立を組み直して固有分解の内訳を見る計器だけである。
// 剛体どうしの本番の工程が入る作業で試験の印を外す。
#[cfg(test)]
