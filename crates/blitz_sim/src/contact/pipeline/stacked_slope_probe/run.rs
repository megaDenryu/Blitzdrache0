...
pub(in crate::contact::pipeline) fn 一つの適用規則を綴る(見出し: &str, 適用規則: 試験の許容差の適用規則) {
    ...
    for 刻み in 0..綴る刻みの数 {
        ...
        if 刻み < 内訳を綴る刻みの数 || 刻み % 100 == 99 {
            let いま = 下り向きの変位(&場面);
            let 内訳の文字列: Vec<String> = 内訳.iter().map(|(鍵, 数)| format!("{鍵}={数}")).collect();
            println!("  刻み{刻み} 判定{}件[{}] 下の箱の変位={:.3e} 上の箱の変位={:.3e}", 履歴.len(), 内訳の文字列.join(" "), いま.0 - 初め.0, いま.1 - 初め.1);
        }
    }
    let 全体の文字列: Vec<String> = 全体.iter().map(|(鍵, 数)| format!("{鍵}={数}")).collect();
    println!("  下の箱の坂の面に対する傾きの正接の最大={下の箱の傾きの正接の最大:.3e}");
    println!("  600刻みの結末の延べ数[{}] B1の破れ 比<10 {}件 比>=10 {}件", 全体の文字列.join(" "), 小さい群.len(), 大きい群.len());
