//! 箱の塔の試験用場面のうち、接触の一刻みの工程そのものを組む部分(判断18・19)。
//! 細分数16と細分の刻み幅1/960秒は判断19が確定した値である(2026-09-04の箱の塔の実測)。
//! 剛体の登録(`tower_fixture`)と分けているのは、こちらが刻み幅と品質と混合則という起動時の設定を決める工程であり、あちらが世界に物を置く工程だからである。
...
/// 判断19が確定した細分数16で組む。細分数を変えた比較の検査だけが `細分数を指定して箱の塔の工程を組む` を読む。
pub(super) fn 箱の塔の工程を組む() -> 剛体の接触の一刻みの工程 {
    細分数を指定して箱の塔の工程を組む(確定した細分数)
}

pub(super) fn 細分数を指定して箱の塔の工程を組む(細分の数: u32) -> 剛体の接触の一刻みの工程 {
