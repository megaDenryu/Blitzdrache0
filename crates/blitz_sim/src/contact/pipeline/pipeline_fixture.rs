...
pub(super) fn テスト工程を作る(細分の数: u32) -> (剛体の接触の一刻みの工程, 剛体の台帳, 剛体の識別子) {
    let mut 台帳 = 剛体の台帳::空();
    let id = 一辺1メートルの箱を台帳へ登録する(&mut 台帳);
...
// 材質1つに摩擦0.5と反発0の表面物性を与えた混合則。工程の組み立てから分けているのは、こちらが材質と物性を決める工程だからである。
fn 試験の材質の混合則を組む(材質: 材質の識別子) -> 混合則 {
    ...
    let Ok(()) = 組み立て.表面物性を登録する(材質, 物性) else {
...
// 質量2キログラム・一辺1メートルの動的な箱を原点へ1体置く。工程の組み立てから分けているのは、こちらが世界に物を置く工程だからである。
fn 一辺1メートルの箱を台帳へ登録する(台帳: &mut 剛体の台帳) -> 剛体の識別子 {
