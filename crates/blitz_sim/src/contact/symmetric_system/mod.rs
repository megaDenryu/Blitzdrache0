//! 接触点集合1つを1つの連立方程式として解くときの、対称な行列の数値の道具(判断12・判断13)。単位の無い実数だけを扱い、
//! 単位型は呼び出し側の連立方程式が持つ。面と面の接触では連立方程式が特異になる。剛体1つの法線方向の自由度は並進1つと
...
pub(in crate::contact) const 法線と接線の連立方程式の行の上限: usize = 2 * 接触点の上限;

// 特異な連立方程式を避けるために対角へ足す正則化の、行列の規模に対する比。
const 特異を避ける正則化の比: f32 = 1.0e-4;

/// 対角へ足す正則化。大きさは行列の規模(対角の最大)に比例し、質量と腕の取り方に依らない。
/// 増分の左辺だけへ足し、右辺へは足さない(収束の到達点を変えないため)。
pub(in crate::contact) fn 特異を避ける正則化を求める(対称行列: &[[f32; 接触点の上限]; 接触点の上限], 個数: usize) -> f32 {
