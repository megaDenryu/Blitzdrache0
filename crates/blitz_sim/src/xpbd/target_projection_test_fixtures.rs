...
pub(super) fn ワールドの位置を生成する(x: f32, y: f32) -> 位置<ワールド> {
    位置::生成する(メートル::生成する(x), メートル::生成する(y), メートル::生成する(0.0))
}

pub(super) fn 逆質量を生成する(値: f32) -> 逆キログラム {
    逆キログラム::生成する(値).unwrap_or_else(|誤り| panic!("{誤り}"))
}

pub(super) fn 目標拘束の一刻みの係数を生成する(コンプライアンス: f32, 刻み幅の秒: f32) -> 目標拘束の一刻みの係数 {
...
/// 1回の射影を当てて、補正後の位置と更新後の乗数を返す。退化の選択肢は検査の失敗である。
pub(super) fn 一回射影して進める(
    係数: &目標拘束の一刻みの係数, 粒子: 位置<ワールド>, 逆質量の値: f32, 目標: 位置<ワールド>, 乗数: ラグランジュ乗数
) -> (位置<ワールド>, ラグランジュ乗数) {
    let 参加点 = 距離拘束の参加点::生成する(粒子, 逆質量を生成する(逆質量の値));
    match 係数.一回射影する(参加点, 目標, 乗数) {
        目標拘束の一回の射影の結果::補正した { 補正, 更新後のラグランジュ乗数 } => (粒子.変位を足す(補正), 更新後のラグランジュ乗数),
