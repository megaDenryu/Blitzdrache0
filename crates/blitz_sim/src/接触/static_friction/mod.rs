...
//! 接線は接触点集合1つを1つの連立方程式として解き、その連立方程式へは非貫通が本来解く右辺を持つ法線の行も同じ行列で入る
//! (連立方程式そのものは `normal_tangential_system` が持ち、その解を粘着の候補と呼ぶ)。点ごとに順に射影すると、各点が物体全体の滑りを1点で
//! 戻そうとして接線の乗数が積み増され、傾きの正接が静止摩擦係数より小さい坂でも円錐を超えて滑走へ落ちる。
//! 法線の行を同じ連立方程式へ入れるのは、接線だけを解くと接触点に働く接線の力が回転を含む補正を作り、次の反復の
//! 非貫通がその回転を戻して錨の接線変位を作り直すためである。法線を先に確定してから足す2段の形も採らない。
//! 前段の非貫通が作った接線変位を後段がもう一度払うことになり、前段の有限回転を後段の線形化が取り消せない。
...
pub use manifold_row_result::静止摩擦の連立方程式へ点を入れた結果;
