...
impl 反発係数 {
    ...
    /// ニュートンの反発則が求める、衝突の後の法線方向の分離の速さ −e v_n⁻(判断14)。
    /// 法線相対速度は接近が負・分離が正であるため、既に離れていた接触では負になる。その場合は分離を要求しない0を返す。
    pub fn 跳ね返り後の分離の速さを求める(&self, 予測の前の法線相対速度: メートル毎秒) -> メートル毎秒 {
        let 速さ = 予測の前の法線相対速度.比で伸ばす(-self.0);
        if 速さ > メートル毎秒::生成する(0.0) { 速さ } else { メートル毎秒::生成する(0.0) }
    }
