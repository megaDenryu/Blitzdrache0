//! 剛体力学層のうち接触(判断1の`接触/`)。接触物性と混合則(判断15)・接触拘束の2つのバッチとその生成(判断11)・
...
//! 公開面は`blitz_app`のコンポジションルートが組み立てと登録に使う型だけであり、拘束・履歴・錨・行の型は`接触`の中に留める(判断23)。
//! 接触島(判断17の`island/`)・休止(判断18の`rest/`)・粗い選別(判断11の`broad_phase/`)・細分の工程(判断19の`pipeline/`)もここが持つ。
//! 場面を進めて判断13と判断14の定量の基準を測るのは`シーン/`の試験専用の材料である。
