...
//! 単精度の本番の経路(`normal_tangential_system`・`symmetric_system`・`static_friction`)が解く連立方程式と同じ式を
...
//! 診断が別に測る。この線引きにより、Issue #59が並べた連鎖のうち「擬似逆行列の階数落ちと許容差 → 残差」の環を
//! 単独で見られる。
//!
...
pub(in crate::接触) use cone_judgement::倍精度の円錐の判定;
pub(in crate::接触) use reference_solution::倍精度の参照の結末;
pub(in crate::接触) use solution_breakdown::倍精度の解の内訳;
pub(in crate::接触) use tolerance_origin::参照計算の許容差の由来;
