//! 接触点集合1つの法線と接線を同じ対称行列へ入れて1回で解く連立方程式(判断12・判断13)。粘着の候補はこの連立方程式の解である。
...
//! 法線を先に確定してから右辺が零の混合連立方程式を足す2段の形にすると、前段の非貫通が作った接線変位を後段がもう一度
...
//! この連立方程式と同じ式を倍精度で持つ試験専用の参照計算を`double_reference`が持つ(Issue #59の数値契約の診断)。
...
#[cfg(test)]
pub(in crate::接触) use acceptance::受理の契約を破った理由;
pub use acceptance::混合連立方程式の受理の結果;
#[cfg(test)]
pub(in crate::接触) use decade_histogram::桁ごとの度数;
#[cfg(test)]
pub(in crate::接触) use double_reference::{倍精度の円錐の判定, 倍精度の参照の結末, 倍精度の解の内訳, 参照計算の許容差の由来};
#[cfg(test)]
pub(in crate::接触) use eigen_distribution::偽の固有値の下限に対する固有の向きの分布;
#[cfg(test)]
pub(in crate::接触) use eigen_relative_spread::固有値を最大で割った相対値の広がり;
#[cfg(test)]
pub(in crate::接触) use increment_uncertainty_spread::増分と不確かさの率の広がり;
#[cfg(test)]
pub(in crate::接触) use solution_breakdown::{単精度の固有の向きの内訳, 単精度の解の内訳};
pub use system::接触点集合の法線と接線の連立方程式;
pub use tangential_row::錨の接線変位を零へ戻す一行;
#[cfg(test)]
pub(in crate::接触) use tolerance_application::{接線の行への許容差の適用規則, 試験の許容差の適用規則};
#[cfg(test)]
pub(in crate::接触) use tolerance_candidate::{偽の固有値の下限を導く取り決め, 擬似逆行列を構成するときに右辺の向きを捨てる判定の適用規則, 解けたと見なす許容差を導く座標の由来};
#[cfg(test)]
pub(in crate::接触) use tolerance_reach::{受理の倍率の届く先, 行を積む閾値の倍率};
#[cfg(test)]
pub(in crate::接触) use tolerance_scale::解けたと見なす許容差の倍率;
