...
//! 後ろまでの窓で、同じ配置へ1倍と2倍を当てた擬似逆行列の内訳(固有の向きの採否・行ごとの乗数の増分と元の行空間の残差)・
...
use crate::接触::normal_tangential_system::{受理の倍率の届く先, 接線の行への許容差の適用規則, 解けたと見なす許容差の倍率, 試験の許容差の適用規則};
...
fn 固有の向きの採否_遠方の正接零点五八を受理一倍と二倍の窓で読む() {
    for 比 in [1.0, 2.0] {
        let 適用規則 = 試験の許容差の適用規則::本番と同じ適用規則().受理の倍率を差し替える(解けたと見なす許容差の倍率::比から生成する(比));
        let 窓 = 細分を綴る窓::入口も読む {
            始まりの細分: 窓の始まりの細分(),
            終わりの細分: 窓の終わりの細分(),
        };
