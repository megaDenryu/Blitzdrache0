...
use crate::接触::restitution_coefficient::反発係数;
use crate::接触::solver_quality::接触を解く品質の設定;
use crate::接触::surface_property::表面物性;
...
pub(super) fn 細分数を指定して試験の接触の一刻みの工程を組む(細分の数: u32) -> 剛体の接触の一刻みの工程 {
    ...
    細分数と摩擦係数を指定して試験の接触の一刻みの工程を組む(細分の数, 摩擦)
}

/// 静止摩擦係数と動摩擦係数を同じ値に指定して組む。接線の荷重を持つ場面(坂の上の箱)の検査だけが読む。
pub(super) fn 細分数と摩擦係数を指定して試験の接触の一刻みの工程を組む(細分の数: u32, 摩擦: 摩擦係数) -> 剛体の接触の一刻みの工程 {
    ...
    let 混合則 = 試験の材質の混合則を組む(摩擦);
    let 重力 = 一様な加速度::成分から生成する(メートル毎秒毎秒::生成する(0.0), メートル毎秒毎秒::生成する(-9.8), メートル毎秒毎秒::生成する(0.0));
    ...
    剛体の接触の一刻みの工程::生成する(方針, 接触の空間と世界::生成する(空間索引), 接触履歴の保持::見込みの接触点の数で生成する(64), 接触の解法ソルバー::生成する(予測器, 速度段階))
}

// 試験の材質1つに指定の摩擦(既定0.5)と反発0の表面物性を与えた混合則。工程の組み立てから分けているのは、こちらが材質と物性を決める工程だからである。
fn 試験の材質の混合則を組む(摩擦: 摩擦係数) -> 混合則 {
