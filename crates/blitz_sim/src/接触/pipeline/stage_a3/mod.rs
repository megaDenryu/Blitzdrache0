...
//! 段階A5の5つ(`increment_uncertainty`・`tower_discard_count`・`alternative_discard_tower`・`counterfactual_discard`・`tower_rest_quality`)も同じ場面を読むため、この木に置く。
//! 9つの計器が共有する「場面を組んで刻みを進める」段取りは`body_scene_run`が持つ。各計器は集計の中身だけを持つ。
//! 坂(剛体と静的世界の接触)の側は `crates/blitz_sim/src/接触/シーン/numeric_contract_record/tolerance_dependence/eigen_direction_switch/` が持つ。
