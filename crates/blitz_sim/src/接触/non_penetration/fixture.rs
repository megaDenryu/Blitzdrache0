...
//! 参加者を高さから組むことである。基本の引数は、下向きの接触法線・符号付き貫通量0・局所点は重心そのもの・硬い接触である。
...
use blitz_math::{メートル, ワールド, 二段の位置, 変位, 方向};
...
pub(in crate::接触) const 試験の形の最小の厚み: f32 = 1.0;

pub(in crate::接触) fn 下向きの接触法線() -> 方向<ワールド> {
    方向::生成する(0.0, -1.0, 0.0).unwrap()
}

pub(in crate::接触) fn 試験の特徴の識別() -> 接触の特徴の識別 {
    let 接触点集合 = 上下の箱の接触点集合(姿勢::恒等());
    let 接触点 = 接触点集合.接触解法の候補点を順に並べる().next().unwrap();
    (*接触点.特徴の識別()).into()
}

pub(in crate::接触) fn 解き方を作る(コンプライアンスの値: f32) -> 非貫通の解き方 {
    非貫通の解き方::生成する(コンプライアンス::生成する(コンプライアンスの値).unwrap(), 形の最小の厚み::生成する(メートル::生成する(試験の形の最小の厚み)).unwrap())
}

// 剛体第1と剛体第2の識別子を台帳から発行し、下向きの法線と硬い接触を持つ引数を作る。局所点と符号付き貫通量は呼び出し側が差し替える。
pub(in crate::接触) fn 剛体どうしの基本の引数() -> 剛体と剛体の接触拘束の引数 {
    let mut 台帳 = 剛体の台帳::空();
    ...
    剛体と剛体の接触拘束の引数 {
        ...
        局所接触点第2: 変位::零(),
        接触法線: 下向きの接触法線(),
        生成時の符号付き貫通量: 符号付き貫通量::検査済みの長さから組む(メートル::生成する(0.0)),
        特徴の識別: 試験の特徴の識別(),
        接触物性: 単一の材質の混合則()
            .接触物性を求める(
...
// 静的世界の側は原点に固定した接触点を持つ。剛体の側の局所点と符号付き貫通量は呼び出し側が差し替える。
pub(in crate::接触) fn 剛体と静的世界の基本の引数() -> 剛体と静的世界の接触拘束の引数 {
    let 基本 = 剛体どうしの基本の引数();
    剛体と静的世界の接触拘束の引数 {
        剛体: 基本.剛体第1,
        静的世界の接触相手: 静的世界の接触相手の識別子::生成する(0),
        剛体側の局所接触点: 変位::零(),
        静的世界側の接触点: 二段の位置::位置から生成する(世界の位置を作る(0.0, 0.0, 0.0)),
        接触法線: 基本.接触法線,
        生成時の符号付き貫通量: 基本.生成時の符号付き貫通量,
        特徴の識別: 基本.特徴の識別,
        接触物性: 基本.接触物性,
        解き方: 基本.解き方,
    }
