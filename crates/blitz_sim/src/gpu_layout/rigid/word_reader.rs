...
impl<'a> 剛体のバイト列の読み手<'a> {
    ...
    fn 語のバイト(&self, 件: usize, 語番号: usize) -> [u8; 4] {
        ...
        [self.バイト列[開始], self.バイト列[開始 + 1], self.バイト列[開始 + 2], self.バイト列[開始 + 3]]
    }
    ...
    pub(super) fn 単精度3つ(&self, 件: usize, 開始の語番号: usize) -> [f32; 3] {
        [self.単精度(件, 開始の語番号), self.単精度(件, 開始の語番号 + 1), self.単精度(件, 開始の語番号 + 2)]
    }

    pub(super) fn 単精度4つ(&self, 件: usize, 開始の語番号: usize) -> [f32; 4] {
        [self.単精度(件, 開始の語番号), self.単精度(件, 開始の語番号 + 1), self.単精度(件, 開始の語番号 + 2), self.単精度(件, 開始の語番号 + 3)]
    }
}
