...
impl 剛体の目標拘束のバッチ {
    pub(super) fn 生成する(拘束一覧: Vec<添字付き剛体の目標拘束>, 刻み幅: 刻み幅) -> Result<Self, 剛体の参照計算エラー> {
    ...
    pub(super) fn 一回反復する(&mut self, 剛体一覧: &mut [細分の中の剛体]) {
        for (添字, 拘束) in self.拘束一覧.iter().enumerate() {
            ...
            if let 剛体の目標拘束の一回の射影の結果::補正した { 補正, 更新後のラグランジュ乗数 } = self.係数一覧[添字].一回射影する(&剛体.参加者(), 拘束.目標, self.乗数一覧[添字]) {
                剛体.補正を適用する(&補正);
