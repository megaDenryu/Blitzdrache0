...
impl 剛体の参照計算 {
    ...
    pub fn 生成する(台帳: 剛体の台帳, 点一覧: &[点自由度の初期状態], 拘束: 剛体の拘束の一覧, 予測器: 細分の予測器, 反復回数: u32) -> Result<Self, 剛体の参照計算エラー> {
        let 刻み幅 = 予測器.刻み幅();
        Ok(Self {
            台帳,
            ...
            点と剛体の距離拘束: point_distance_batch::点と剛体の距離拘束のバッチ::生成する(拘束.点と剛体の距離拘束, 点一覧.len(), 刻み幅)?,
            予測器,
            反復回数,
        })
    }
    ...
    // ゲーム側の作用の投入に当たる口。検査が力と衝撃を剛体へ与えるために台帳を借りる。呼び手は検査だけであり、公開面には出さない。
    #[cfg(test)]
    pub(crate) fn 台帳を書き換える(&mut self) -> &mut 剛体の台帳 {
        &mut self.台帳
    }
