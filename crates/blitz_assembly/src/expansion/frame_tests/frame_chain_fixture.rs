...
pub(super) fn 文字列ごとの配置(配置表: &部品ごとの配置表, 文字列: &str) -> Vec<個体配置> {
    配置表.据えた順().iter().filter(|据えた| 据えた.識別子() == &識別子(文字列)).map(|据えた| 据えた.配置()).collect()
}
...
pub(super) fn 平壁を必ず入れるはめ口一覧(面の文字列一覧: &[&str]) -> Vec<はめ口の指定> {
    面の文字列一覧.iter().map(|文字列| はめ口の指定::生成する(名前(文字列), vec![壁をはめる候補("平壁")]).unwrap()).collect()
}

pub(super) fn 四面すべてのはめ口の文字列() -> Vec<&'static str> {
    はめ口の宣言一覧.into_iter().map(|(文字列, _)| 文字列).collect()
}
