//! カタログの組み上げの失敗。破れうる前提を選択肢で数え上げ、どの前提が破れたかを型で言う。
//!
...
/// 噛み合う相手がカタログに無い接合点1件。どの部品のどの接合点が、どの種別の相手を探して見つからなかったかを持つ。
...
impl fmt::Display for 対の不足 {
    fn fmt(&self, 出力: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            出力,
            "{}の接合点「{}」が{}を宣言しているが、噛み合う{}がカタログのどの部品にも1件も無い",
            self.部品の識別子,
            self.接合点の名前,
            self.種別,
            self.種別.噛み合う相手の文字列()
...
pub enum カタログエラー {
    ...
    #[error("噛み合う相手がカタログに無い接合点が{}件ある\n{}", .不足一覧.len(), 不足一覧を綴る(.不足一覧))]
    噛み合う相手がカタログに無い { 不足一覧: Vec<対の不足> },

    #[error("境界箱の最小が最大を超えている(軸{軸})")]
    境界箱の最小が最大を超える { 軸: usize },

    #[error("配置で写した箱が静的物理形状として成り立たない: {0}")]
    静的物理形状として成り立たない(#[from] blitz_engine::static_shape::静的物理形状エラー),
}
