...
fn 地形の世界のソース一覧() -> Vec<チャンクの高さ格子ソース> {
    ...
    目録ソース
        .項目一覧()
        .iter()
        .map(|項目| チャンクの高さ格子ソース::生成する(項目.チャンク(), 高さ格子のファイル::パスから作る(目録ソースパス.with_file_name(項目.ソース相対パス()))))
        .collect()
}

/// チャンク座標の1成分を、端のチャンクを原点とする大域の添字へ写す。負になったら世界の範囲と端の値が食い違っている。
fn 端からの大域添字へ変換する(成分: i32) -> u32 {
    u32::try_from(成分 + 端のチャンク).unwrap_or_else(|_| panic!("チャンク座標{成分}が端のチャンク{端のチャンク}を足しても負である。世界の範囲と端の値の食い違い"))
}
