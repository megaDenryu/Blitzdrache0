...
fn foxの計算充填法線と接線は単位長かつ直交する() {
    ...
    for 頂点 in &シーン.先頭の描画対象().形状().最詳細メッシュ().頂点一覧 {
        ...
        assert!((法線長さ二乗 - 1.0).abs() < 1e-3, "計算充填した法線は単位長のはず: {:?}(長さ二乗={法線長さ二乗})", 頂点.法線);

        let 接線ベクトル = [頂点.接線[0], 頂点.接線[1], 頂点.接線[2]];
        let 接線長さ二乗 = 内積(接線ベクトル, 接線ベクトル);
        assert!((接線長さ二乗 - 1.0).abs() < 1e-3, "計算充填した接線は単位長のはず: {接線ベクトル:?}(長さ二乗={接線長さ二乗})");
        assert!(内積(頂点.法線, 接線ベクトル).abs() < 1e-3, "計算充填した接線は法線と直交するはず: 法線={:?} 接線={接線ベクトル:?}", 頂点.法線);
    }
}
