...
fn 同じ材質名を2つの層が指しても焼けて索引が一致する() {
    let 層割当 = 地表材質の層割当::文字列から生成する(["grass_a", "grass_a", "rock_a", "sand_a"]).unwrap();
...
fn 置き場にタイルが無い材質名を指す層割当は型付きエラーになる() {
    let 層割当 = 地表材質の層割当::文字列から生成する(["存在しない材質", "dirt_a", "rock_a", "sand_a"]).unwrap();
...
fn 空の材質名を持つ層割当は生成の時点で拒む() {
    assert!(地表材質の層割当::文字列から生成する(["  ", "dirt_a", "rock_a", "sand_a"]).is_err());
}
