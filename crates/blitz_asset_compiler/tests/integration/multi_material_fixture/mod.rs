...
/// この検収のglbは標準金属粗さPBRだけを宣言する。別の選択肢が出たら焼く側の破れであり、検収として落とす。
pub fn 金属粗さpbrを取り出す(マテリアル: &blitz_engine::マテリアルデータ) -> &blitz_engine::金属粗さPBRデータ {
    match マテリアル {
        blitz_engine::マテリアルデータ::金属粗さPBR(値) => 値,
        blitz_engine::マテリアルデータ::地表の層の重ね合わせ(_) => panic!("焼いた材質が金属粗さPBRの選択肢でない"),
    }
}
