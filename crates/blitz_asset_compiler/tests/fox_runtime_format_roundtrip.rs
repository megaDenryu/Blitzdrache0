...
use blitz_engine::{
    アセットID, アセット実行時形式エラー, カタログ, シーンを実行時形式へ格納する, チャンク座標, 実行時形式からシーンを読む
};
...
fn foxを決定的な実行時形式で往復する() {
    ...
    let mut 期待 = match ソースシーンを読み込む(&カタログ, &id, チャンク座標::生成する(0, 0), テクスチャ格納方針::全てRGBA8)
    {
        Ok(シーン) => シーン,
        Err(誤り) => panic!("Fox.glbの読込に失敗した: {誤り}"),
    };
    期待.参照ファイル一覧.clear();
    let 一回目 = match ソースシーンをコンパイルする(&カタログ, &id, チャンク座標::生成する(0, 0), テクスチャ格納方針::全てRGBA8)
    {
        Ok(バイト列) => バイト列,
        Err(誤り) => panic!("Foxを実行時形式へコンパイルできなかった: {誤り}"),
    };
    let 二回目 = match ソースシーンをコンパイルする(&カタログ, &id, チャンク座標::生成する(0, 0), テクスチャ格納方針::全てRGBA8)
    {
        Ok(バイト列) => バイト列,
