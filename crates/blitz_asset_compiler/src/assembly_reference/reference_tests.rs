//! 正解表の読みの試験。担当するのは、実物の文字列をそのまま読めることと、座標系が違う表を拒むことを固定することである。
//!
...
#[test]
fn 実物の文字列をそのまま読む() {
    let パス = 一時ファイルへ書く("正解表.json", 実物の文字列);
...
#[test]
fn 受け取らない座標系の正解表を拒む() {
    let パス = 一時ファイルへ書く("別の座標系.json", &実物の文字列.replace("\"glTF\"", "\"Blender\""));
...
#[test]
fn 成分の数が足りない姿勢を拒む() {
    let パス = 一時ファイルへ書く("成分不足.json", &実物の文字列.replace("[0.0, 3.0, 0.0]", "[0.0, 3.0]"));
    let 誤り = 組み立ての正解表のファイル::生成する(&パス).正解表を読み取る().unwrap_err();
