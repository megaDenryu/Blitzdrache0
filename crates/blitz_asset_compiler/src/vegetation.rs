...
impl ソースアセットのコンパイル係<'_> {
    pub fn 植生チャンクをコンパイルする(&self, id: &アセットID, 所有チャンク: チャンク座標, 個体数: usize) -> Result<コンパイル済みシーン, アセットコンパイルエラー> {
    ...
    fn 植生チャンクのシーンを組み立てる(&self, id: &アセットID, 所有チャンク: チャンク座標, 個体数: usize) -> Result<組み立てたシーン, アセットコンパイルエラー> {
        ...
        Ok(組み立てたシーン::置いた個体の数を添えて生成する(シーン, 群.置いた個体の数))
    }
}
