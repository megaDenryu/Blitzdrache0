//! コンパイル対象1件: 何を焼いてどこへ書き出すかを持つ型。出力パスを外から組ませないのは、拡張子の文字列が
...
impl コンパイル対象 {
    /// 出力パスは出力ルートが安定IDから導く。呼び出し側が拡張子の文字列を組まないため、文字列の写しが散らない。
    pub(crate) fn 生成する(id: アセットID, 所有チャンク: チャンク座標, 種別: ソース種別, 出力ルート: &実行時形式の出力ルート, 台帳での扱い: 台帳での扱い) -> Self {
        Self {
    ...
    pub(crate) fn 生成物を書き出す(&self, バイト列: &[u8]) -> Result<PathBuf, crate::実行時アセットのコンパイルエラー> {
        crate::atomic_file_write::一時ファイル経由で書き込む(&self.出力パス, バイト列).map_err(|誤り| crate::実行時アセットのコンパイルエラー::入出力に失敗(format!("{}を書き出せない: {誤り}", self.出力パス.display())))?;
        let ファイル名 = self.出力パス.file_name().ok_or_else(|| crate::実行時アセットのコンパイルエラー::ソースが不正("生成物のファイル名が無い"))?;
