...
use crate::{
    今回の宣言と依存一式の内容ハッシュを求める, 内容ハッシュ, 種類ごとの置いた個体の数, 置いた個体の数
};
...
impl 対象1件の仕上げ係<'_> {
    pub(super) fn 対象を焼いて書き出す(
        &self,
        対象: &コンパイル対象,
    ) -> Result<焼き上がり, crate::実行時アセットのコンパイルエラー> {
        let 結果 = self
            .対象をコンパイルする(対象)
            .map_err(crate::実行時アセットのコンパイルエラー::ソースが不正)?;
        let 実行時パス = 対象.生成物を書き出す(&結果.実行時バイト列)?;
        let 宣言 = self.コンパイル係.安定idが指すソースのパスを参照する(&対象.id).map_err(|誤り| {
            crate::実行時アセットのコンパイルエラー::ソースが不正(
                format!("{}の宣言が指すソースを参照できない: {誤り}", 対象.id),
            )
        })?;
        let 内容ハッシュ =
            今回の宣言と依存一式の内容ハッシュを求める(宣言, &結果.ソース依存一覧).map_err(crate::実行時アセットのコンパイルエラー::ソースが不正)?;
        Ok(焼き上がり {
            実行時パス,
            ソース依存一覧: 結果.ソース依存一覧,
            メタデータ: 結果.メタデータ,
