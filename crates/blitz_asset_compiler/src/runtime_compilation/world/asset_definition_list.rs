...
//!
//! 台帳を親から分けるのは、世界が増えるたびにこの一覧だけが伸び、対象世界の選択肢の定義と押し合うためである。
...
pub(super) fn アセット定義一覧を選ぶ(世界: 対象世界) -> Vec<アセット定義> {
    match 世界 {
        ...
        対象世界::植生の世界 => vegetation_declaration::植生の世界のアセット定義一覧を組む(),
        対象世界::見本の集落の世界 => village_declaration::見本の集落の世界のアセット定義一覧を組む(),
        対象世界::部品で組んだ家の並びの世界(_) => part_house_row_declaration::部品で組んだ家の並びの世界のアセット定義一覧を組む(),
        対象世界::部品で組んだ木の並びの世界 => part_tree_row_declaration::部品で組んだ木の並びの世界のアセット定義一覧を組む(),
        対象世界::部品で組んだ一間四方の骨格の並びの世界(種類) => part_frame_row_declaration::部品で組んだ一間四方の骨格の並びの世界のアセット定義一覧を組む(種類),
        対象世界::目視見本の世界 => visual_sample_declaration::目視見本の世界のアセット定義一覧を組む(),
        対象世界::頂点診断の世界(原型) => vertex_diagnostic_declaration::頂点診断の世界のアセット定義一覧を組む(原型),
        対象世界::ブロック圧縮の対照世界 => Vec::new(),
        対象世界::夜の多光源の世界 => night_lights_declaration::夜の多光源の世界のアセット定義一覧を組む(),
        対象世界::屋内の多光源の世界 => stone_hut_declaration::屋内の多光源の世界のアセット定義一覧を組む(),
        対象世界::場所巡りの世界 => fox_tour_declaration::場所巡りの世界のアセット定義一覧を組む(),
    }
}
