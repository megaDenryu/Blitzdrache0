...
//! `二段に積んだ骨格`と`横に二ベイ継いだ骨格`は骨格を連ねた見本2つであり、部品の一覧は`一間四方の骨格`と同じで違うのは規則だけである。継いだ面には壁を入れない。
//! `屋根を載せた一間四方の家`と`屋根を載せた二段の家`は骨格の上へ切妻屋根を載せた家2つであり、骨格ごとに床を1枚張り、部品の一覧に屋根が1件加わって6件になる。
//! `煙突を付けた一間四方の家`は部品の一覧に煙突が1件加わって8件になる。
...
/// ベイ格子の変換と建物外形カタログの組み立てが使う口。文字列と部品の一覧の正本はこの束の中にあり、
/// 呼び出し側が自分で文字列を書かないようにここから配る。
...
pub(crate) enum 部品の組み立て規則の種類 {
    ...
    樫の木,
    一間四方の骨格,
    二段に積んだ骨格,         // 骨格を連ねた見本。継いだ面には壁を入れない
    横に二ベイ継いだ骨格,     // 骨格を連ねた見本。継いだ面には壁を入れない
    屋根を載せた一間四方の家, // 骨格の上へ切妻屋根を載せ、骨格ごとに床を1枚張る
    屋根を載せた二段の家,     // 骨格の上へ切妻屋根を載せ、骨格ごとに床を1枚張る
    煙突を付けた一間四方の家, // 屋根を載せた1ベイの家の右面の壁へ煙突を立てた家
}
...
impl 部品の組み立て規則の種類 {
    ...
    pub(crate) fn 規則と種の起点を組む(self) -> Result<(組み立て規則, 生成の種), String> {
        match self {
            ...
            Self::屋根を載せた一間四方の家 => 骨格を連ねた規則へ種を添える(frame_chain_rule::屋根を載せた一間四方の家の規則を組む()?),
            Self::屋根を載せた二段の家 => 骨格を連ねた規則へ種を添える(frame_chain_rule::屋根を載せた二段の家の規則を組む()?),
            Self::煙突を付けた一間四方の家 => 骨格を連ねた規則へ種を添える(frame_chain_rule::煙突を付けた一間四方の家の規則を組む()?),
        }
    }
}
