...
pub(super) const 地形世界の起動時シーン: (&str, ソースアセットの相対パス) = ("terrain_origin", ソースアセットの相対パス::板);

/// 段を2つ持つ原型ソース。個体別LODの検収と、地形世界の同居植生がこちらを読む。
pub(super) const 植生詳細段原型ソース: ソースアセットの相対パス = ソースアセットの相対パス::植生診断原型;
...
const 小物の木箱シーン: (&str, ソースアセットの相対パス) = ("prop_wooden_crate", ソースアセットの相対パス::小物の木箱);
...
const 小物の金具付き木箱シーン: (&str, ソースアセットの相対パス) = ("prop_banded_chest", ソースアセットの相対パス::生成する("props/banded_chest.glb"));
...
const 材質境界の二材質シーン: (&str, ソースアセットの相対パス) = ("multi_material_two", ソースアセットの相対パス::二材質);
const 材質境界の単一材質シーン: (&str, ソースアセットの相対パス) = ("multi_material_one", ソースアセットの相対パス::単一材質);

/// 遠方環境の消費の検収シーン。金属と誘電体の板を粗さの水準ごとに並べる。この安定IDだけが世界の間接照明の方式として
/// 天空の遠方環境を選ぶ(`crates/blitz_app/src/app/time_of_day/scene_policy.rs`)。
/// 参照: `_doc/設計/放射輝度問い合わせ階層.md`「3-Icの消費式と実装段割り」
const 遠方環境の検収シーン: (&str, ソースアセットの相対パス) = ("indirect_probe", ソースアセットの相対パス::遠方環境の検収);

/// 資源表世代の差し替えの検収シーンと、検収の実行中にその生成物を上書きする側。安定IDを分ける理由は`crates/blitz_app/src/smoke/material_reload.rs`にある。
const 材質差し替えのシーン: (&str, ソースアセットの相対パス) = ("material_reload", ソースアセットの相対パス::二材質);
const 材質差し替えの代替シーン: (&str, ソースアセットの相対パス) = ("material_reload_alt", ソースアセットの相対パス::代替二材質);

pub(super) fn 板の世界の一覧() -> Vec<アセット定義> {
    vec![
        必須定義("quad", ソースアセットの相対パス::板, ソース種別::Gltfシーン),
        ...
        必須定義("quad_alt", ソースアセットの相対パス::代替板, ソース種別::Gltfシーン),
        必須定義("shadow_scene", ソースアセットの相対パス::影のシーン, ソース種別::Gltfシーン),
        任意定義("helmet", ソースアセットの相対パス::ヘルメット),
        任意定義("fox", ソースアセットの相対パス::フォックス),
...
pub(super) fn 地形の世界の一覧() -> Vec<アセット定義> {
    vec![必須定義(地形世界の起動時シーン.0, 地形世界の起動時シーン.1, ソース種別::Gltfシーン), ソース専用定義(地形世界の植生原型, 植生詳細段原型ソース)]
}
