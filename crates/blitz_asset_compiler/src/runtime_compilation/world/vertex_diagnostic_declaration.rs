...
pub(super) fn 頂点診断の世界のアセット定義一覧を組む(原型: 診断の原型) -> Vec<アセット定義> {
    vec![必須定義(地形世界の起動時シーン.0, 地形世界の起動時シーン.1, ソース種別::Gltfシーン), ソース専用定義(原型.安定id, 原型.ソース相対パス)]
}

pub(super) fn 同居植生(原型: 診断の原型, 個体数: usize) -> 同居植生宣言 {
    同居植生宣言 { 原型の安定id: 原型.安定id, 個体数 }
}
