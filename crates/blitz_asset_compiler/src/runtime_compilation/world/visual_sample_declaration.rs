...
const 材質見本の立体のソース: ソースアセットの相対パス = ソースアセットの相対パス::生成する("terrain_visual_world/material_sample_bodies.gltf");
...
pub(super) const 庭の小物一覧: &[原型と置き方の宣言] = &[
    ...
    小物("terrain_visual_wooden_crate", ソースアセットの相対パス::小物の木箱, 集まり(5, 56.5, 55.0, 2.0)),
    小物("terrain_visual_boulder", ソースアセットの相対パス::小物の岩, 集まり(4, 42.5, 45.0, 2.5)),
    小物("terrain_visual_tree_stump", ソースアセットの相対パス::小物の切り株, 集まり(4, 57.5, 44.5, 2.2)),
];

/// この世界がチャンク以外に焼くもの。材質見本の立体も遠景地面も小物の原型も、庭の地面のチャンクが素材として読むだけである。
pub(super) fn 目視見本の世界のアセット定義一覧を組む() -> Vec<アセット定義> {
    let mut 定義一覧 = vec![ソース専用定義(材質見本の立体, 材質見本の立体のソース), ソース専用定義(遠景地面, 遠景地面のソース)];
    定義一覧.extend(庭の小物一覧.iter().map(|宣言| 外部ソース専用定義(宣言.原型の識別.安定id(), 宣言.原型の識別.ソース相対パス())));
    定義一覧
}

/// 宣言をコンパイラが受け取る指定へ写す。材質見本の立体の安定IDと小物群の宣言を1つの指定へまとめる。
pub(crate) fn 目視見本の指定を作る(材質見本の立体の安定id: &str, 遠景地面の安定id: &str, 群一覧: &[原型と置き方の宣言]) -> Result<目視見本の指定, String> {
    Ok(目視見本の指定 {
