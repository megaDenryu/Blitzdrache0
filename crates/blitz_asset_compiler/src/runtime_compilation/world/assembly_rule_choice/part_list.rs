...
/// 台帳の1行が書いた2つの文字列を`原型の識別`へ組み上げる。表の1行を1行に収めたまま、
/// ソースの文字列が相対パスの型へ入る口をこの1箇所に保つ。
const fn 部品(安定id: &'static str, ソースの文字列: &'static str) -> 原型の識別 {
    原型の識別::生成する(安定id, ソースアセットの相対パス::生成する(ソースの文字列))
}
...
pub(super) fn 部品一覧を選ぶ(種類: 部品の組み立て規則の種類) -> &'static [原型の識別] {
    match 種類 {
        ...
        部品の組み立て規則の種類::一間四方の骨格 | 部品の組み立て規則の種類::二段に積んだ骨格 | 部品の組み立て規則の種類::横に二ベイ継いだ骨格 => 一間四方の骨格の部品一覧,
        部品の組み立て規則の種類::屋根を載せた一間四方の家 | 部品の組み立て規則の種類::屋根を載せた二段の家 => 屋根つきの家の部品一覧,
