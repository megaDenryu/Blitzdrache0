...
use blitz_assembly::{
    はめ口の指定, はめ口へ入れるもの, 壁の外面の飾りの指定, 接合点名, 法線方向のずらし, 部品ID, 骨格へ張る床, 骨格へ継ぎ足すもの
};
