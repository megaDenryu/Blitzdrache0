//! 場所巡りの世界が焼くものの宣言。担当するのは、起動時に読むキツネのシーンと目印の原型と、目的地の大域の水平位置を
//! 1箇所へ集めることだけである。宣言をコンパイラが受け取る据え付けへ写す手順は`fixed_placement_declaration`が持つ。
//!
//! 注意: 散らす岩の原型は外部のアセットリポジトリにしかない。このリポジトリを持たない環境ではその宣言が飛ばされ、
//! 散布するチャンクのコンパイルがカタログ未登録で落ちる。この世界が引数なしの一括生成に入っていないのは元からであり、
//! 焼くのは`gen-game-map`と`game-fox-tour`と`large-world-assets`だけである。
//!
//! 注意: 目的地の大域の水平位置は、ゲームロジック層の道順(`crates/blitz_game/src/fox_tour_route.rs`)と同じ数でなければならない。
//! 食い違うと、目印の柱が立っていない場所で到達が成立するか、柱の足元で到達が成立しないかのどちらかになる。
//! 2つが別のクレートに分かれているのは、あちらがblitz_mathだけに依存するゲームロジック層であり、
//! こちらがアセットコンパイラの宣言だからである。整数のメートルで書くのは、2つの表を目で並べて突き合わせるためである。

use crate::散布の焼き方;

use super::super::archetype_identity::原型の識別;
use super::super::catalog::アセット定義;
use super::super::source_kind::{ソース種別, 原型と置き方の宣言, 固定物の据え付け宣言};
use super::definition_kind::{ソース専用定義, 外部ソース専用定義};
use super::fox_declaration::キツネのシーン定義を作る;
use super::fox_tour_assembled_scatter_declaration::散らす部品で組んだ木の一覧;
use super::fox_tour_scatter_declaration::散らす原型の一覧;
use super::part_row_declaration::部品のアセット定義一覧を作る;

/// 起動時に読むキツネのシーンの安定ID。名前が`terrain`で始まることが、この世界の空の宣言・間接照明方針・露出方式を
/// 地形の本番世界と同じものにする。参照: `crates/blitz_app/src/app/time_of_day/scene_policy.rs`
pub(super) const キツネのシーン: 原型の識別 = 原型の識別::生成する("terrain_fox_tour", crate::ソースアセットの相対パス::フォックス);

/// 目的地に立てる目印の柱の原型を指す安定ID。実行時形式は焼かず、目印を据えるチャンクが素材として読むだけである。
const 目印の柱: 原型の識別 = 原型の識別::生成する("fox_tour_destination_marker", crate::ソースアセットの相対パス::目印の柱);

/// 巡る3つの目的地。1つ目は出発地点の真北にあり、2つ目と3つ目は向きを変えないと着かない位置に置く。
const 巡る目的地の一覧: &[固定物の据え付け宣言] = &[目的地(0, -15), 目的地(-40, -45), 目的地(45, -70)];

/// 目的地1つの宣言。据える原型はどれも同じ目印の柱である。
const fn 目的地(東メートル: i32, 南メートル: i32) -> 固定物の据え付け宣言 {
    固定物の据え付け宣言 {
        原型の安定id: 目印の柱.安定id(),
        東メートル,
        南メートル,
    }
}

pub(super) fn チャンクのソース種別(散布: 散布の焼き方) -> ソース種別 {
    ソース種別::固定物を据えた高さ格子 {
        据え付け一覧: 巡る目的地の一覧,
        散布一覧: 散布.適用する(散らす原型の一覧),
        部品で組む散布一覧: 散布.適用する(散らす部品で組んだ木の一覧),
    }
}

/// この世界が地面へ散らす原型の一覧。焼き方の指定がこの文字列を鍵へ入れるため、宣言を変えた実行は
/// 材料が同じでもチャンクを焼き直す。
pub(super) fn 散布の宣言一覧() -> &'static [原型と置き方の宣言] {
    散らす原型の一覧
}

/// この世界がチャンク以外に焼くもの。キツネだけが実行時形式まで焼かれ、目印の原型はチャンクが素材として読むだけである。
pub(super) fn 場所巡りの世界のアセット定義一覧を組む() -> Vec<アセット定義> {
    let mut 定義一覧 = vec![キツネのシーン定義を作る(キツネのシーン.安定id()), ソース専用定義(目印の柱.安定id(), 目印の柱.ソース相対パス())];
    定義一覧.extend(散らす原型の一覧.iter().map(|宣言| 外部ソース専用定義(宣言.原型の識別.安定id(), 宣言.原型の識別.ソース相対パス())));
    // 部品で組んで散らす種は、規則が指す部品を1件ずつ素材として読む。規則の種類が部品の一覧を答えるため、
    // 宣言の側が部品を書き足す必要は無い。
    for 宣言 in 散らす部品で組んだ木の一覧 {
        定義一覧.extend(部品のアセット定義一覧を作る(宣言.規則の種類.部品一覧()));
    }
    定義一覧
}
