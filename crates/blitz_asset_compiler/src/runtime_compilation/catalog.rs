...
/// ソースの形式と、形式ごとに要る付随の宣言は`source_kind`が持つ。宣言の書き手が同じ文字列で参照できるよう、ここから見せ直す。
...
impl 実行時アセットのコンパイル {
    ...
    pub(super) fn アセット宣言をカタログへ写す(&mut self) -> Result<Vec<コンパイル対象>, String> {
        ...
        for 定義 in self.世界.アセット定義一覧() {
            ...
            if 定義.実行時へ焼く {
                対象一覧.push(コンパイル対象::生成する(id, 原点チャンク, 定義.種別, &self.出力ルート, 台帳での扱い::毎回焼く));
            }
        }
        Ok(対象一覧)
    }
    ...
    fn 宣言のソースを開ける形で参照する(&self, 定義: &アセット定義, 外部ルート: &source_location::外部ソースルート) -> Result<Option<PathBuf>, String> {
