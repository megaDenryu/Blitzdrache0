...
pub use building_outline_catalog::{
    ベイ構造, 差し替えの候補, 建物の入口方向, 建物の外接箱, 建物外形カタログ, 建物外形カタログのファイル, 建物外形カタログの組み立て係, 建物外形カタログエラー, 建物定義, 建物定義ID, 建物定義の用途, 建物定義の識別子一覧, 空の建物外形カタログを作る,
};
...
/// 骨格方式の役割ごとの部品の識別子と部品の一覧。エディターチャンクの焼き込みが格子から手順を作るために使う。
