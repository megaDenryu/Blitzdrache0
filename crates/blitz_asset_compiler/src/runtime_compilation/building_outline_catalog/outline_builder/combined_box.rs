...
pub(super) fn 建物全体の箱を作る(材料: &建物定義の材料, 部品一覧: &[部品の外形], 配置表: &部品ごとの配置表) -> Result<群ローカルの箱, 建物外形カタログエラー> {
    let mut 全体: Option<群ローカルの箱> = None;
    for 据えた in 配置表.据えた順() {
        let 部品 = 部品一覧.iter().find(|部品| &部品.識別子 == 据えた.識別子()).ok_or_else(|| 建物外形カタログエラー::据えた部品の外形が無い {
            建物定義: 材料.識別子の文字列().to_string(),
            部品: 据えた.識別子().文字列().to_string(),
        })?;
