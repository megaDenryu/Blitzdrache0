...
pub(super) fn 部品のパス一覧を参照する(外部ルート: &外部ソースルート, 材料: &建物定義の材料) -> Result<Vec<PathBuf>, 建物外形カタログエラー> {
    材料
        .由来
        .部品一覧()
        .iter()
        .map(|部品| {
            外部ルート.相対パスが指すソースを参照する(部品.ソース相対パス()).map_err(|原因| 建物外形カタログエラー::部品ソースを参照できない {
                建物定義: 材料.識別子の文字列().to_string(),
                原因,
            })
        })
        .collect()
...
pub(super) fn 部品の外形一覧を読む(パス一覧: &[PathBuf], 材料: &建物定義の材料) -> Result<Vec<部品の外形>, 建物外形カタログエラー> {
    パス一覧
        .iter()
        .map(|パス| {
