...
//! ファイル名の文字列と、それらの書き出しと読み戻しを持つ。
//!
//! 文字列を型の中へ閉じるのは、拡張子やカタログの名前の写しが呼び出し側へ散ると、書く側と読む側で食い違ったときに
...
/// 実行時形式のアセット1件のファイル名が終わる文字列。**この文字列の正本はここ1つである。**
/// 公開するのは、焼き上がりの実在を確かめる検収の側が同じ文字列を持つと二重台帳になるためである。
...
impl 実行時形式の出力ルート {
    ...
    fn 直下へ書き込む(&self, ファイル名: &str, バイト列: &[u8]) -> Result<PathBuf, アセット配置エラー> {
        ...
        crate::atomic_file_write::一時ファイル経由で書き込む(&パス, バイト列).map_err(|誤り| アセット配置エラー::ファイルへ書き出せなかった {
            パス: パス.clone(), 事由: 誤り.to_string()
        })?;
        Ok(パス)
    }
}
