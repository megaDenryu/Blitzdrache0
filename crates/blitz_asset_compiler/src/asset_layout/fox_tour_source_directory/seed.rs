...
impl 場所巡りの世界のソースディレクトリ {
    ...
    /// 生成に使った乱数の種を書いたファイルの中身。書き出す側と同じ文字列のファイルを読むため、読み手は名前を持たない。
    pub fn 生成に使った乱数の種を読む(&self) -> Result<マップ生成の乱数の種, アセット配置エラー> {
        let パス = self.0.直下のファイルのパス(種を書き出すファイル名);
        let 文字列 = self.0.直下のファイルを文字列として読む(種を書き出すファイル名)?;
        文字列.trim().parse::<u32>().map(マップ生成の乱数の種::生成する).map_err(|_| アセット配置エラー::乱数の種が不正 { パス, 文字列 })
    }
}
