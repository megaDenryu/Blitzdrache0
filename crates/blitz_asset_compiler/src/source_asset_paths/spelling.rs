//! ソースアセットの文字列の検査。受け取るのは文字列、返すのはその条件に当たるかどうかである。
//! 拒む理由は条件ごとに違うため、条件ごとの関数に分けて、理由は呼び出す側の`生成する`に書く。
//!
//! `const fn`にするのは、文字列がすべてコンパイル時の定数であり、破れをコンパイルの失敗として出すためである。
//! 実行時まで持ち越すと、不正な文字列を持つ宣言が「ファイルが見つからない」という遠い場所の失敗になる。
//!
...
/// 成分の切れ目を見ずにまとめるのは、ソースアセットの文字列に点を2つ続ける正当な用途が1つも無いためである。
pub(super) const fn 連なる点を含むか(文字列: &str) -> bool {
    連なる同じバイトを含むか(文字列, 点)
}

/// 今の場所を指す文字列か。ファイルを1つも指さない文字列である。
