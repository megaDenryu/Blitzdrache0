//! 複数の世界または開発用入口が共有するソースアセットの文字列の正本。
//!
//! 文字列をこの1つのファイルへ集めるのは、同じファイルの文字列を宣言する側と複製する側が別々に持つと、
//! 片方だけを直した食い違いがファイルを開く段まで出ないためである。
//! ファイル名と、そのファイル名を含む相対パスを同じファイルへ並べるのは、文字列の正本を1箇所に保つためである。
//! smokeの検収は複製のためにファイル名だけを、宣言はソースルートからの相対パスを読む。
//!
//! glTFの文書と対になる共有バッファ(`.bin`)もここが持つ。あれは生成物であり追跡しないため、揃っているかを見る側が
//! 文字列を必要とする。生成する側と見る側が別々に文字列を持つと、片方だけを直した食い違いが「揃わないまま生成し続ける」形で出る。
