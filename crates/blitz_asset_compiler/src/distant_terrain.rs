...
pub fn 遠景アセットをコンパイルする(ソース一覧: &[チャンクの高さ格子ソース], 安全幅: メートル) -> Result<コンパイル済み遠景, 遠景コンパイルエラー> {
...
fn 高さ場を作る(ソース一覧: &[チャンクの高さ格子ソース]) -> Result<(blitz_engine::height_field::高さ場, Vec<PathBuf>), 遠景コンパイルエラー> {
