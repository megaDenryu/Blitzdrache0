...
//! JSONの直列化へ接して欄の名前と文字列を持ち、領域の型は接合点の宣言へ接して指示を組む。
//!
//! 「無し」を欄の不在で表さず`入れない`という選択肢で表すのは、壁を消した保存物と壁の欄を書き忘れた保存物を
//! 読む側が区別できるようにするためである。
        ...
        match self {
            ...
            Self::煙突を立てる { 段数 } => {
                ...
                Ok(壁の外面へ付ける飾り::煙突を立てる {
                    段数: 煙突の段数::生成する(段数).map_err(|原因| 欄が不正なエラーを作る(建物定義, "煙突の段数", &原因.to_string()))?,
                })
            }
        }
    }
...
pub enum はめ口ソース {
    入れない,
    壁を入れる { 壁の種類: 壁の種類ソース, 外面の飾り: 壁の外面の飾りソース },
}
...
impl はめ口ソース {
    pub(super) fn 領域の宣言へ解く(self, 建物定義: &str) -> Result<はめ口の値, 建物の格子のソースエラー> {
        match self {
            ...
            Self::壁を入れる { 壁の種類, 外面の飾り } => Ok(はめ口の値::壁を入れる {
                種類: 壁の種類.領域の宣言へ解く(),
