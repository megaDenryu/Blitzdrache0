...
//!
//! ファイル名の文字列を置き場(`directory`)でなくここが持たないのは、置き場が建物ごとのディレクトリを
//! 数え上げる責務も持ち、文字列が2つの導出に要るためである。文字列の正本は置き場にある。
...
impl 建物の格子のファイル {
    ...
    pub fn 読んで最新の形にする(&self) -> Result<建物の格子ソース, 建物の格子のソースエラー> {
        let 本文 = std::fs::read_to_string(&self.パス).map_err(|原因| 建物の格子のソースエラー::ファイルを読めない { パス: self.パスの文字列(), 原因 })?;
    ...
    pub fn 書き出す(&self, ソース: &建物の格子ソース) -> Result<(), 建物の格子のソースエラー> {
        let 建物定義 = ソース.建物定義ID.文字列().to_string();
