...
impl チャンクの矩形 {
    ...
    pub(super) fn 座標一覧から求める(座標一覧: &[チャンク座標]) -> Result<Self, 高さ場コンパイルエラー> {
        ...
        for 座標 in 残り {
            if !既出.insert(*座標) {
                return Err(高さ場コンパイルエラー::チャンク座標が重複している { 東: 座標.x(), 南: 座標.z() });
            }
    ...
    pub(super) fn 東方向のチャンク数(&self) -> Result<u32, 高さ場コンパイルエラー> {
        最小と最大からチャンク数を求める(self.最小.x(), self.最大.x())
    }
...
fn 最小と最大からチャンク数を求める(最小: i32, 最大: i32) -> Result<u32, 高さ場コンパイルエラー> {
    最大.checked_sub(最小).and_then(|差| u32::try_from(差).ok()).and_then(|差| 差.checked_add(1)).ok_or(高さ場コンパイルエラー::格子点数が表現できない)
}
