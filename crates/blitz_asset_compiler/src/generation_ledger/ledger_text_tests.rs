//! 生成台帳の本文の書式そのものの回帰検査。組み立てた文字列を固定の期待値と1文字ずつ突き合わせる。
//!
...
/// 挿入の順を東の順でも南の順でもない並びにして、文字列の並びが挿入の順でなく座標の順で決まることを確かめる。
...
/// 種を持たない台帳の文字列も固定する。欄を空にせず`none`と書くのは、種の欄が消えると行の位置が動くためである。
#[test]
...
/// 固定した文字列が今の解析でそのまま読めることも確かめる。書式だけを変えて解析を直し忘れる退行を止める。
#[test]
