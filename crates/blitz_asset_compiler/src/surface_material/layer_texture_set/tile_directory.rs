...
//!
//! 文字列(ディレクトリ名と拡張子)をこの型の中へ閉じるのは、置き場を移したときに触る場所を1つにするためである。
...
impl 地表層タイルの置き場 {
    ...
    pub fn 表示用の表記(&self) -> std::path::Display<'_> {
        self.0.display()
    }

    pub(super) fn 材質のタイル画像のパス(&self, 材質名: &エンジン材質名) -> PathBuf {
        self.0.join(format!("{}.{タイル画像の拡張子}", 材質名.文字列()))
    }

    /// 材質1つ分のタイル画像のバイト列と、依存一覧へ載せるパスを返す。
    pub(super) fn 材質のタイル画像を読む(&self, 材質名: &エンジン材質名) -> Result<(Vec<u8>, PathBuf), 地表層テクスチャ集コンパイルエラー> {
        ...
        let バイト列 = std::fs::read(&パス).map_err(|原因| 地表層テクスチャ集コンパイルエラー::タイル画像の読込に失敗 {
            材質名: 材質名.to_string(),
            原因: 原因.to_string(),
        })?;
        Ok((バイト列, パス))
