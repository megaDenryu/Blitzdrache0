...
pub(super) fn メッシュデータを取り出す(
    ...
    for プリミティブ in メッシュ.primitives() {
        ...
        let 頂点オフセット = u32::try_from(未確定頂点一覧.len())
            .map_err(|誤り| アセットコンパイルエラー::解析失敗(format!("頂点オフセットがu32の範囲を超えている: {誤り}")))?;
        インデックス一覧.extend(プリミティブデータ.インデックス一覧.into_iter().map(|添字| 添字 + 頂点オフセット));
