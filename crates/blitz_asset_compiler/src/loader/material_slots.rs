...
pub(super) fn 解決する<'文書>(文書: &開いた文書, メッシュ列: impl Iterator<Item = gltf::Mesh<'文書>>, 方針: テクスチャ格納の方式) -> Result<材質スロットの解決結果, アセットコンパイルエラー> {
    ...
    for メッシュ in &メッシュ一覧 {
        for プリミティブ in メッシュ.primitives() {
            let スロット = 語彙.スロットを参照する(&プリミティブ).ok_or(アセットコンパイルエラー::材質スロット未登録)?;
