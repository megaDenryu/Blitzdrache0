...
pub(super) fn プリミティブから取り出す(文書: &開いた文書, プリミティブ: &gltf::Primitive<'_>) -> Result<未確定メッシュデータ, アセットコンパイルエラー> {
    ...
    let インデックス一覧: Vec<u32> = match 読み取り器.read_indices() {
        ...
        None => {
            let 頂点数u32 = u32::try_from(頂点数).map_err(|誤り| アセットコンパイルエラー::解析失敗(format!("頂点数がu32の範囲を超えている: {誤り}")))?;
            (0..頂点数u32).collect()
        }
    };
    ...
    Ok(未確定メッシュデータ { 頂点一覧, インデックス一覧 })
}

fn 頂点一覧を組み立てる(位置一覧: Vec<[f32; 3]>, 法線一覧: Option<Vec<[f32; 3]>>, 接線一覧: Option<Vec<[f32; 4]>>, uv一覧: Vec<[f32; 2]>) -> Vec<未確定頂点属性> {
    let 頂点数 = 位置一覧.len();
