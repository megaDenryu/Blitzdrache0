...
/// `材質スロット数`をプリミティブ数と並べると、材質の境界が何本残ったかが読める。頂点を1つも読めなかったときの`境界箱`は`None`である。
pub struct 契約検査概要 {
    pub(super) メッシュ数: usize,
    pub(super) プリミティブ数: usize,
    pub(super) 材質スロット数: usize, // 先頭メッシュが参照する相異なる材質の数
    pub(super) 頂点数: usize,
    pub(super) インデックス数: usize,
    pub(super) 境界箱: Option<([f32; 3], [f32; 3])>, // 先頭メッシュの頂点位置の最小と最大
}
