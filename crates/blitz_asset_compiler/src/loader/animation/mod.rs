...
pub(super) fn アニメーション一覧を取り出す(文書: &開いた文書, 関節解決: &関節解決, ジョイント数: usize) -> Result<Vec<アニメーションクリップ>, アセットコンパイルエラー> {
    文書
        .document
        .animations()
        .enumerate()
        .map(|(添字, アニメーション)| {
            ...
            let ジョイントチャンネル一覧 = channel_group::ジョイントチャンネル一覧を取り出す(文書, &アニメーション, 関節解決, ジョイント数)?;
