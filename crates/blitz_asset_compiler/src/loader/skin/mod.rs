...
use blitz_engine::{ジョイント, スキンデータ, 静的TRS};
...
pub(super) fn スキンを取り出す(文書: &開いた文書, 対象メッシュ添字: usize) -> Result<Option<(スキンデータ, 関節解決)>, アセットコンパイルエラー> {
    ...
    let グローバルノードから新添字 = 旧グローバル一覧.iter().enumerate().map(|(旧添字, &グローバル)| (グローバル, 並べ替え.旧から新[旧添字])).collect();
...
fn メッシュを持つノードのスキン(文書: &開いた文書, 対象メッシュ添字: usize) -> Option<gltf::Skin<'_>> {
    文書.document.nodes().find_map(|ノード| {
        ...
        if ノードのメッシュ添字 == 対象メッシュ添字 { ノード.skin() } else { None }
    })
}
