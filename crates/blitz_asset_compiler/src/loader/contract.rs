...
//!
//! 検査するのは`ソースシーンを読み込む`の契約(1ファイル1メッシュ1マテリアル)である。植生の原型は複数メッシュをLODの段として読む別の契約を持ち、
...
pub fn 入力契約を検査する(パス: &Path) -> 契約検査結果 {
    ...
    指摘一覧.extend(material_scan::検査する(&文書));
    指摘一覧.extend(material_drop_scan::検査する(&文書.document));
    指摘一覧.extend(texture_scan::検査する(&文書.document));
