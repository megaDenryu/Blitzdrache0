...
//!
//! 検査する契約は3つある。`ソースシーンを読み込む`の契約(1ファイル1メッシュ、そのメッシュは複数の材質スロットを持てる)と、
//! `原型ソースを読み込む`の契約(複数のメッシュを詳細段の列として読み、全段が1つのマテリアルを共有する)と、
//! `部品の原型ソースを読み込む`の契約(段の列として読み、段ごとに複数の材質スロットを許し、接合点を段0のノードから読む)である。
//! どれで検査するかは呼び出し側が`検査する契約`で渡し、文書の形から推し量らない。
//!
