...
pub fn チャンク目録ソースを読み込む(パス: &Path) -> Result<チャンク目録ソース, アセットコンパイルエラー> {
    let 内容 = std::fs::read_to_string(パス).map_err(|誤り| アセットコンパイルエラー::ファイル読込失敗(format!("{}: {誤り}", パス.display())))?;
    let mut 行一覧 = 内容.lines().enumerate();
    let (_, 宣言行) = 行一覧.next().ok_or_else(|| チャンク目録コンパイルエラー::形式宣言不正("形式宣言の行が無い".to_string()))?;
