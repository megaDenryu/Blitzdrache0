...
#[test]
fn 形式版2は隣の重み格子を読む() {
    let ソース = ソースを置いて読む("version2", r#"{"形式版":2,"高さ格子":"地形.heightgrid","地表材質の重み格子":"材質.surfaceweights","建物配置一覧":[]}"#, true);
    let 格子 = ソース.地表材質の重み格子を得る(一辺の標本数).unwrap();
...
fn 素材の相対パスが置き場の外を指すと拒む() {
    ...
    let ソースパス = 置き場.join("チャンク.json");
    std::fs::write(&ソースパス, r#"{"形式版":2,"高さ格子":"地形.heightgrid","地表材質の重み格子":"../外.surfaceweights","建物配置一覧":[]}"#).unwrap();
    assert!(エディターチャンクソース::ファイルから読む(&ソースパス).is_err());
}
