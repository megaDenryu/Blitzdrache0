...
//! 重複を正準化した文字列で判定するのは、下流の種・描画・物理が同じ文字列で同じ配置を指すためである。
//!
...
pub(super) fn 建物配置一覧を検証する(一覧: &[建物配置ソース], ファイル: &エディターチャンクソースのファイル<'_>) -> Result<(), アセットコンパイルエラー> {
    let mut 出会った配置識別子 = HashSet::new();
    for 配置 in 一覧 {
        if !出会った配置識別子.insert(配置.配置識別子.文字列()) {
