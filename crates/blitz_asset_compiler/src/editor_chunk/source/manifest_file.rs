...
impl<'パス> エディターチャンクソースのファイル<'パス> {
    ...
    /// 本文を版の型として解析する。失敗の文字列をこの1箇所へ集めるため、版ごとに書き分けない。
    ...
    /// 本文が名乗る形式版を読む。どの版の型で解析するかを決めるのに先立って要る、ただ1つの欄である。
    pub(super) fn 形式版を読む(&self, 本文: &str) -> Result<u32, アセットコンパイルエラー> {
        let 宣言: 形式版の宣言 = self.版の型として解析する(本文)?;
        Ok(宣言.形式版)
    }
