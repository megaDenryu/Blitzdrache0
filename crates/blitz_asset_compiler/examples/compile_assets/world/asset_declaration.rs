...
/// 地形チャンク1つが持つ個体数。物量の計測は第5段の`ow4-bench`が別の世界で行うため、
/// ここは束の追加と解除で状態が生まれて消えることを見るのに足りる軽さに留める。
const 地形同居の個体数: usize = 64;
...
/// 地形チャンクが焼く同居植生の宣言。地形世界のチャンクだけがこれを持つ。
pub(super) fn 地形の同居植生() -> 同居植生宣言 {
    同居植生宣言 {
        原型の安定id: 地形世界の植生原型,
        個体数: 地形同居の個体数,
    }
...
fn ソース専用定義(名前: &'static str, 相対パス: &'static str) -> アセット定義 {
    アセット定義 {
        名前,
        相対パス,
        必須: true,
