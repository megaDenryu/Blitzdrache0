//! 目視見本の世界が焼くものの宣言。担当するのは、材質見本の立体と、地面へ同居させる小物の種と個体数と並べ方を1箇所へ集めることだけである。
...
/// 材質見本の立体を指す安定ID。地面のチャンクが素材として読むだけであり、実行時形式は作らない。
...
/// この世界がチャンク以外に焼くもの。材質見本の立体も小物の原型も、地面のチャンクが素材として読むだけである。
pub(super) fn 一覧() -> Vec<アセット定義> {
    let mut 定義一覧 = vec![ソース専用定義(材質見本の立体, 材質見本の立体のソース)];
