//! 部品で建てた家並みの世界が焼くものの宣言。担当するのは、カタログへ載せる部品6件と、何軒建てるかと、
//! 宣言をコンパイラが受け取る指定へ写すことである。組み立て規則そのものは`house_rule`が持つ。
//!
//! 煙突を宣言へ入れないのは、種別と寸法の是正が本体のアセット置き場へまだ再収蔵されていないためである
//! (参照: `_doc/設計/部品カタログと接合点.md`「実データが暴いた契約の穴」穴2)。
//!
//! 10軒と100軒の2つの規模を持つのは、段4の実証が「発行数が軒数に依らない」ことだからである。
//! 1つの規模だけでは、発行数がたまたまその軒数と一致しているのか軒数に依らないのかを見分けられない。

mod house_rule;

use blitz_asset_compiler::{家並みの指定, 家並みの軒数};
...
/// 家並みの規模。同じ地面と同じ規則で軒数だけが違う2つの世界を、この枝が分ける。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum 家並みの規模 {
    十軒,
    百軒,
}
...
pub(super) fn チャンクのソース種別(規模: 家並みの規模) -> ソース種別 {
    ソース種別::部品で建てた家並み(家並みの宣言 {
        部品一覧: 家の部品一覧,
        軒数: 規模.軒数(),
    })
}

/// 宣言が持つ安定IDの綴りを検証済みのアセットIDへ写し、規則と軒数を添えて指定を組む。
pub(crate) fn 家並みの指定を作る(宣言: 家並みの宣言) -> Result<家並みの指定, String> {
    let 安定id一覧 = 宣言
        .部品一覧
        .iter()
        .map(|識別| アセットID::生成する(識別.安定id()).map_err(|誤り| 誤り.to_string()))
        .collect::<Result<Vec<アセットID>, String>>()?;
    let 軒数 = 家並みの軒数::生成する(宣言.軒数).map_err(|誤り| 誤り.to_string())?;
    家並みの指定::生成する(安定id一覧, house_rule::酒場宿屋の規則()?, 軒数, house_rule::家並みの種()).map_err(|誤り| 誤り.to_string())
}
