...
use blitz_asset_compiler::{
    ソースの高さ格子の重なり帯が一致することを確かめる, ソースルート, テクスチャ格納方針, 実行時形式の出力ルート
};
...
fn 実行する() -> Result<(), String> {
    ...
    let ソースルート = ソースルート::生成する(PathBuf::from(ソースルート文字列));
    let 出力ルート = 実行時形式の出力ルート::作る(PathBuf::from(出力ルート文字列))?;
    ...
    let 焼き上がり = chunk_ledger::台帳を見ながら対象一覧を焼く(&出力ルート, &カタログ, &対象一覧, 焼き方, &mut 実行時カタログ)?;
    ...
    // 注意: 生成台帳は出力一式の最後に確定させる。ここより前で書くと、カタログか目録の書き出しに失敗した実行が
    // 「新しい台帳と古いカタログ」を残し、次の実行がチャンクを据え置きながら古いカタログの項目を写して食い違う。
    焼き上がり
        .書き出しを待つ生成台帳
        .出力ルートへ書き出す(出力ルート.台帳の置き場())
...
fn カタログを書き出す(出力ルート: &実行時形式の出力ルート, カタログ: &カタログ) -> Result<(), String> {
    let バイト列 = カタログを実行時形式へ格納する(カタログ).map_err(|誤り| 誤り.to_string())?;
    出力ルート.実行時カタログを書き出す(&バイト列)
}
