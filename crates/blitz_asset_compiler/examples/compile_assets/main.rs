...
use blitz_asset_compiler::{ソースの高さ格子の重なり帯が一致することを確かめる, テクスチャ格納方針};
...
fn 実行する() -> Result<(), String> {
    ...
    if 世界.チャンクのソース種別(同居植生個体数).地面が高さ格子で書かれているか() {
        ソースの高さ格子の重なり帯が一致することを確かめる(&取り込み結果.チャンクごとのソース一覧).map_err(|誤り| 誤り.to_string())?;
    }
    let mut 実行時カタログ = カタログ::空を作る();
    let 焼き方 = chunk_ledger::焼き方の指定 {
        世界名,
        同居植生個体数,
        テクスチャ格納方針: 方針,
    };
    let 勘定 = chunk_ledger::台帳を見ながら対象一覧を焼く(出力ルート, &カタログ, &対象一覧, 焼き方, &mut 実行時カタログ)?;
