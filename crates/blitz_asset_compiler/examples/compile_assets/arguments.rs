...
use blitz_asset_compiler::{ソースルート, テクスチャ格納方針, 散布の焼き方, 遠景地形の焼き方};
...
pub(super) struct 焼く世界の指定 {
    pub(super) ソースルート: ソースルート,
    pub(super) 出力ルートのパス: PathBuf,
    pub(super) 世界: 対象世界,
    /// プロセス境界で世界を指した綴り。増分の鍵の焼き方の指定へそのまま載せる。
    ...
    /// 地面へ散らす原型を焼くか。既定は焼くであり、絵の対照を採る検収だけが焼かない側を選ぶ。
    pub(super) 散布: 散布の焼き方,
}
...
pub(super) fn 引数一覧から焼く世界の指定を読む(全引数一覧: &[String]) -> Result<焼く世界の指定, String> {
    ...
    Ok(焼く世界の指定 {
        ソースルート: ソースルート::生成する(PathBuf::from(ソースルート文字列)),
        出力ルートのパス: PathBuf::from(出力ルート文字列),
        世界: 対象世界::引数名から解析する(世界名)?,
        世界名: 世界名.clone(),
        同居植生個体数,
