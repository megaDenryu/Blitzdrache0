...
//! テクスチャ格納の方式の文字列の解析は`texture_policy_argument`が持つ。

use std::path::PathBuf;

use blitz_asset_compiler::{ソースルート, テクスチャ格納の方式, 対象世界, 散布の焼き方, 焼く世界の指定, 遠景地形の焼き方};

use super::texture_policy_argument;

/// この入口が当てはめる既定のテクスチャ格納の方式。既定を当てはめるのはこの1箇所だけであり、
/// コンパイルのライブラリ関数は方針を必須の引数で受け取る。ブロック圧縮を渡すのは`cargo xtask texture-compression`だけである
/// (参照: `_doc/設計/テクスチャのブロック圧縮と縮小段生成.md`「判断i」)。
const 既定のテクスチャ格納の方式: テクスチャ格納の方式 = テクスチャ格納の方式::全てRGBA8;
...
pub(super) fn 引数一覧から焼く世界の指定を読む(全引数一覧: &[String]) -> Result<焼く世界の指定, String> {
    ...
    let 地表層テクスチャ集 = 世界.地表層テクスチャ集の焼き方を選ぶ();
    Ok(
        焼く世界の指定::組み立てを始める(ソースルート::生成する(PathBuf::from(ソースルート文字列)), PathBuf::from(出力ルート文字列), 世界, 世界名.clone()).焼き方を確定する(
            同居植生個体数,
            指定された方針.unwrap_or(既定のテクスチャ格納の方式),
            遠景地形,
            散布,
            地表層テクスチャ集,
        ),
