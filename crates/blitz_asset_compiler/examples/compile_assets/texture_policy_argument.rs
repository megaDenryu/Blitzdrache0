//! プロセス境界でテクスチャ格納の方式を指す文字列の台帳と、引数一覧からその指定を取り出す解析。
//! 文字列はxtask側(`xtask/src/compile_assets/texture_policy_name.rs`)にも同じものがあり、2つの実行ファイルの
//! 引数の受け渡しがこの文字列で一致していることを、知らない文字列を失敗にすることで確かめる。
//!
//! 指定を位置引数でなく選択肢の文字列で受けるのは、既にある位置引数(ソースルート・出力ルート・世界名・同居植生個体数)の
//! どれとも紛れないようにするためである。指定が無いときの既定を当てはめるのは呼び出し元の`main`1箇所だけである。

use blitz_asset_compiler::テクスチャ格納の方式;

/// 方針の指定を導く選択肢の文字列。この文字列の直後の1語が方針の名前である。
