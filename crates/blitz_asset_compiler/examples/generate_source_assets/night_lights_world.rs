//! 夜の多光源の検収世界の定義と、そのソースアセットの書き出し。世界そのものの決め事(広がり・一辺長・格子の細かさ)を
//! このモジュールが持ち、地面の形は`height`が持つ。
//!
//! 焼くのは地面と、影の形で灯を見分けるための遮蔽物2件である。布は1つも置かない。布を置かないのは、
//! 布の画素が実行のたびに揺れて読み戻しの決定性の判定が成立しなくなるためである
//! (参照: `_doc/設計/クラスタ多光源と点光源の影.md`「検収戦略(判断i)」)。点光源はblitz_appの世界の宣言が置く。
//! 外部のアセットリポジトリを1件も引かないのは、この世界を持たない環境でも夜の多光源の検収が動くようにするためである。
//! 遮蔽物を手続きで作るのもこの理由による。遮蔽物の形と色は`obstacle`が持つ。

mod height;
mod obstacle;
mod obstacle_gltf_json;
mod obstacle_shape;

use std::path::Path;

use blitz_asset_compiler::{高さ格子を切り出す, 高さ格子を格納する, 高さ格子諸元};
use blitz_engine::チャンク座標;

use crate::directory_source::{目録ソースを作る, 目録項目};

/// 街灯の並びは1チャンクの内側へ収まるため、世界は原点チャンク1つだけを持つ。
const チャンクのX: i32 = 0;
const チャンクのZ: i32 = 0;
const 一辺メートル: f32 = 100.0;

/// 地形の世界と同じ細かさにして、段の作られ方と接地の求め方をそのまま流用できるようにする。
const 辺分割数: u16 = 64;

/// 隣接チャンクと重ねる縁の幅。中央差分で法線を求めるのに前後1点あれば足りる。
const 重なり幅: u8 = 1;

/// 夜の多光源のシーンを指す安定ID。`terrain`で始めることが、この世界の空方針と間接照明方針を天空の遠方環境にし、
/// 一日内秒から方向光と環境光を導く経路へ載せる。参照: `crates/blitz_app/src/app/time_of_day/scene_policy.rs`
const シーン名: &str = "terrain_night_lights";

const 格子ファイル名: &str = "terrain_night_lights.heightgrid";
const 目録ソースファイル名: &str = "chunk_directory.txt";

pub(crate) fn 書き出す(出力先ディレクトリ: &Path) -> Result<(), String> {
    地面を書き出す(出力先ディレクトリ)?;
    obstacle::遮蔽物2件を書き出す(出力先ディレクトリ)
}

fn 地面を書き出す(出力先ディレクトリ: &Path) -> Result<(), String> {
    let 諸元 = 高さ格子諸元::生成する(辺分割数, 重なり幅, 一辺メートル).map_err(|誤り| 誤り.to_string())?;
    let 座標 = チャンク座標::生成する(チャンクのX, チャンクのZ);
    let 格子 = 高さ格子を切り出す(諸元, 座標, height::大域高さを求める).map_err(|誤り| 誤り.to_string())?;
    書き込む(&出力先ディレクトリ.join(格子ファイル名), &高さ格子を格納する(&格子))?;
    let 項目一覧 = vec![目録項目 {
        座標,
        アセット識別子: シーン名.to_string(),
        ソース相対パス: 格子ファイル名.to_string(),
    }];
    書き込む(&出力先ディレクトリ.join(目録ソースファイル名), 目録ソースを作る(&項目一覧).as_bytes())
}

fn 書き込む(パス: &Path, バイト列: &[u8]) -> Result<(), String> {
    std::fs::write(パス, バイト列).map_err(|誤り| format!("{}: {誤り}", パス.display()))
}
