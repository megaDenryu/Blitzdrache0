...
pub(crate) fn 場所巡りの世界のソース一式を書き出す(ソースルート: &ソースルート, 種: マップ生成の乱数の種, 広がり: 世界の広がり) -> Result<焼き直しの勘定, String> {
    let 書き出し = 場所巡りの世界のソース書き出し::始める(ソースルート, 種, 広がり)?;
    let 置き場の文字列 = 書き出し.表示用の表記().to_string();
    let 勘定 = 書き出し.ソース一式を書き出して台帳を確定する()?;
    println!("[generate_source_assets] {置き場の文字列}へ種{}から生成完了、{}", 種.値(), 勘定.報告の行を作る());
    Ok(勘定)
}
