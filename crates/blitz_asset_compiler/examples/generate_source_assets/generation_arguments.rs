...
pub(crate) fn 引数一覧から書き出す対象を読む(引数一覧: &[String]) -> Result<書き出す対象, String> {
    ...
    let mut 読んだ種 = None;
    let mut 東西チャンク数 = None;
    let mut 南北チャンク数 = None;
    ...
    loop {
        ...
        if 綴り == 種の選択肢の綴り {
            ...
            return Err(format!(
                "知らない引数である: {綴り}(選べるのは種・ソースルート・東西チャンク数・南北チャンク数である)"
            ));
        }
        残りの引数 = 続き;
    }
...
fn チャンク数を解析する(選択肢: &str, 値: &str) -> Result<u16, String> {
    値.parse::<u16>()
        .map_err(|誤り| format!("{選択肢}の値を16ビットの非負整数として読めない({値}): {誤り}"))
}

fn 広がりを組み立てる(東西: Option<u16>, 南北: Option<u16>) -> Result<世界の広がり, String> {
    match (東西, 南北) {
