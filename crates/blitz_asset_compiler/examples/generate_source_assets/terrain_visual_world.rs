...
fn 材質見本の立体を書き出す(出力先ディレクトリ: &Path) -> Result<(), String> {
    ...
    書き込む(&出力先ディレクトリ.join(立体の文書ファイル名), gltf_json::材質見本の立体のgltf文書を組み立てる(&連結, 立体のバイナリファイル名).as_bytes())
}

fn 遠景地面を書き出す(出力先ディレクトリ: &Path) -> Result<(), String> {
    書き込む(&出力先ディレクトリ.join(遠景地面のバイナリファイル名), &distant_ground::バッファバイト列を作る())?;
    書き込む(&出力先ディレクトリ.join(遠景地面の文書ファイル名), distant_ground::遠景地面のgltf文書を組み立てる(遠景地面のバイナリファイル名).as_bytes())
}
