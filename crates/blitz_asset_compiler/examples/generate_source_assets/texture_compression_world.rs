...
fn 対照の素材を書き出す(出力先ディレクトリ: &Path) -> Result<(), String> {
    ...
    画像を保存する(&出力先ディレクトリ.join(グラデーション画像ファイル名), &グラデーション)?;
    let 雑音 = image::RgbaImage::from_fn(一辺, 一辺, noise_texture::決定的な雑音の画素を求める);
    画像を保存する(&出力先ディレクトリ.join(雑音画像ファイル名), &雑音)
}

fn 板を書き出す(出力先ディレクトリ: &Path) -> Result<(), String> {
    書き込む(&出力先ディレクトリ.join(板のバイナリファイル名), &geometry::バッファバイト列を作る())?;
    書き込む(
        &出力先ディレクトリ.join(板の文書ファイル名),
        gltf_json::文書を作る(板のバイナリファイル名, グラデーション画像ファイル名, 雑音画像ファイル名).as_bytes(),
    )
}
...
fn 画像を保存する(パス: &Path, 画像: &image::RgbaImage) -> Result<(), String> {
