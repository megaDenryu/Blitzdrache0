...
fn 原型を書き出す(出力先ディレクトリ: &Path, 諸元一覧: &[直方体諸元], バッファファイル名: &str, 文書ファイル名: &str) -> Result<(), String> {
    書き込む(&出力先ディレクトリ.join(バッファファイル名), &geometry::バッファバイト列を作る(諸元一覧))?;
    書き込む(&出力先ディレクトリ.join(文書ファイル名), gltf_json::文書を作る(諸元一覧, バッファファイル名, geometry::直方体の量).as_bytes())
}
