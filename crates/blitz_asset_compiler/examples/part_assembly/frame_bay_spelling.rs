//! 骨格方式の部品と、面に属さない接合点の文字列の正本。Blender側が骨格1件と壁3種と切妻屋根と床板へ与えた
//! 名前と、壁の外面へ付ける飾りとして一体型から流用する出窓の名前を、この1箇所だけが持つ。
//! 骨格の側面と2つの口の対応は`frame_bay_face`が持つ。
//!
//! 手順(`frame_recipe`)から分けるのは、文字列が手順の数だけ写ると、Blender側が名前を変えたときに
...
pub(super) fn 壁の外面の飾り取付の接合点名() -> Result<接合点名, String> {
    接合点名を作る("外面の飾り取付")
}

pub(super) fn 出窓の壁面差込口の接合点名() -> Result<接合点名, String> {
    接合点名を作る("壁面差込口")
}
