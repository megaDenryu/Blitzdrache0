...
impl<'a> クレート構文検査<'a> {
    ...
    pub fn 型の定義ブロック(&self, 型名: &str) -> String {
        for ソース in self.ソース一覧 {
            ...
            for (i, 行) in 行一覧.iter().enumerate() {
                let 行 = 行.trim();
                if 行.contains(&format!("struct {型名}")) || 行.contains(&format!("enum {型名}")) {
                    宣言 = Some(i);
                    break;
                }
            }
    ...
    pub fn dto純粋性違反一覧(&self, 型名: &str) -> Vec<String> {
        ...
        let mut 違反 = Vec::new();
        if !定義.contains("Clone") {
            違反.push(format!("MDTO `{型名}` は Clone を導出していません"));
        }
        if 定義.contains('&') {
            違反.push(format!("MDTO `{型名}` は参照(&)を持てません"));
        }
        if 定義.contains("*const") || 定義.contains("*mut") {
            違反.push(format!("MDTO `{型名}` は生ポインタを持てません"));
        }
        for 型 in ["RefCell", "Mutex", "RwLock", "Cell", "Atomic"] {
            if 定義.contains(型) {
                違反.push(format!("MDTO `{型名}` は内部可変性 `{型}` を持てません"));
            }
        }
        違反
    }
