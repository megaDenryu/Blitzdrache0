...
use blitz_design::{MDTO, M入力, M解釈関数, 解釈関数として確認する};
...
// 歩行入力を解釈する の型引数が M入力 と Mコマンド を満たすことをコンパイラに確かめさせる。
const _: M解釈関数<キーボード歩行入力, 旅行者の意図> = 解釈関数として確認する(キーボード歩行入力::歩行入力を解釈する);
