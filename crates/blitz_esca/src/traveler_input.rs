...
//!
//! デバイス境界の生入力(Mデバイス生入力: キーボード歩行入力)から、ゲーム内のコマンド(Mコマンド: 旅行者の意図)を
//! 導出する純粋計算(M解釈関数)を、入力自身のメソッドとして持つ。

use blitz_design::{Mデバイス生入力, M不変データ, M解釈関数};
...
// 歩行入力を解釈する が解釈の役割(Mデバイス生入力 から Mコマンド)を満たすことをコンパイラに確かめさせる。
