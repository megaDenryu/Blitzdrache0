...
// 逆長さと長さの積は無次元量になり、消散係数と経路長からこの積で得るのが光学的深さである。
// その積をここで演算子として提供しないのは、無次元の光学的深さを表す値オブジェクトがまだ無く、
// 積の結果が生のf32へ落ちるためである。意味を持つ量を単位の無い数値として持ち回れる入口は作らない。
// 光学的深さを値として扱う段でその型を作り、そのときにこの積を戻す。
