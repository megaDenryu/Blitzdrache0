...
impl<入力空間, 出力空間> 変換<入力空間, 出力空間> {
    ...
    pub fn gpu境界用の列優先配列(&self) -> [[f32; 4]; 4] {
        self.内部.to_cols_array_2d()
    }

    /// 出力空間から入力空間へ戻す変換。ビュー射影の逆でクリップ座標から視線の向きを復元する用途に使う。
    ///
    /// 注意: 正則でない変換に対しては成分が非有限になる。呼び出し元は逆が存在することを保証するか、
    /// 生値へ出す境界で有限性を確かめること(ビューと透視投影の合成は常に正則である)。
    pub fn 逆変換する(&self) -> 変換<出力空間, 入力空間> {
        変換::内部から生成する(self.内部.inverse())
    }
