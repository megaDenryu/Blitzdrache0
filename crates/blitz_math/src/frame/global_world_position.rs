...
impl 大域ワールド位置 {
    ...
    /// 描画の基準原点を引いてf32のワールド位置へ狭める。動く個体の変換をその対象の基準原点からの相対で表すときに使う。
    /// カメラ相対位置と別の型を返すのは、引く相手がカメラ大域原点ではなく描画の基準原点であり、2つを取り違えると
    /// 個体がカメラの動きに引きずられて動くためである。
    pub fn 基準原点からの相対位置へ狭める(self, 基準原点: Self) -> Result<位置<ワールド>, 座標変換エラー> {
        let 差分 = self.f32へ狭めた差分を求める(基準原点)?;
        Ok(位置::生成する(
            メートル::生成する(差分.x),
            メートル::生成する(差分.y),
            メートル::生成する(差分.z),
