//! 空間に属する単位方向。長さが1であることを不変条件に持つ。位置と別の型にするのは、方向が単位を持たない量であり、
//! メートルの位置と足しても意味が定まらないためである。生の`[f32; 3]`で持つと、姿勢を組み立てるたびに検査を書き直すことになる。
