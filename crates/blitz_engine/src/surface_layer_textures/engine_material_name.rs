...
//! 名前として成立しない文字列を持てなくすることと、名前を運ぶ経路から生の文字列を消すことである。
//!
//! 裸の`String`で運ばないのは、材質名・ファイル名・表示名が同じ型になり、取り違えても型が通るためである。
//! 生の文字列へ戻るのはファイル名の組み立てと直列化の境界だけであり、その口を`文字列`に限る。
...
impl エンジン材質名 {
    pub fn 生成する(文字列: &str) -> Result<Self, 地表層テクスチャ集エラー> {
        if 文字列.trim().is_empty() {
            return Err(地表層テクスチャ集エラー::材質名が空);
        }
        Ok(Self(文字列.to_string()))
    }
