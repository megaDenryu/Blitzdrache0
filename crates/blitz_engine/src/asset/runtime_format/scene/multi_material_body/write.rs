//! シーンを版4または版5の内容へ決定的な順序で書く工程。受け取るのはシーンとテクスチャ1件を書く工程、返すのは内容のバイト列である。
//! 版4の並びを書く工程はブロック圧縮のテクスチャを表せないため、その組み合わせは工程の側が型付きエラーで拒む。
...
use super::super::write_element::テクスチャを書く工程;
...
pub(in crate::asset::runtime_format::scene) fn 内容を書く(
    ...
    for 対象 in シーン.描画対象一覧() {
        ...
        描画対象を書く(&mut 出力, 対象, ジョイント数, テクスチャを書く)?;
    }
...
fn 描画対象を書く(
    ...
    write_element::材質集合を書く(出力, 対象.材質集合(), テクスチャを書く)
}
