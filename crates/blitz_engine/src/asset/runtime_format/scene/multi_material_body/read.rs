...
pub(in crate::asset::runtime_format::scene) fn マルチマテリアル本体を読む(内容: &[u8], 読み方: 版ごとの要素の読み方) -> Result<シーンデータ, アセット実行時形式エラー> {
    ...
    read_element::頂点属性を検査する(std::iter::once(&先頭).chain(&残り).map(|対象| 対象.形状().スキン頂点属性一覧()), スキン.as_ref())?;
    let アニメーション一覧 = read_element::アニメーション一覧を読む(&mut 入力, スキン.as_ref())?;
    let 静的物理形状 = (読み方.静的物理形状の節を読む処理())(&mut 入力)?;
    入力.完了を検査する()?;
    Ok(シーンデータ::生成する(先頭, 残り, Vec::new(), スキン, アニメーション一覧).静的物理形状を添える(静的物理形状))
}

fn 描画対象を読む(入力: &mut 読取位置<'_>, 読み方: 版ごとの要素の読み方) -> Result<描画対象データ, アセット実行時形式エラー> {
    ...
    Ok(描画対象データ::生成する(識別子, 所有チャンク, ローカルからワールド, 形状, 材質集合))
}
