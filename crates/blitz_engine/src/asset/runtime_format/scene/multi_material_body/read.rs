//! 版4と版5の内容を境界検査し、シーンへ復元する工程。受け取るのは内容のバイト列とテクスチャ1件を読む工程、返すのはシーンである。
...
use super::super::read_element::テクスチャを読む工程;
...
fn 描画対象を読む(
    ...
    let 形状 = 形状を読む(入力)?;
    let 材質集合 = read_element::材質集合を読む(入力, テクスチャを読む)?;
