...
impl<'a> 読取位置<'a> {
    ...
    pub(crate) fn f32(&mut self) -> Result<f32, アセット実行時形式エラー> {
        ...
        if 値.is_finite() { Ok(値) } else { Err(アセット実行時形式エラー::非有限小数) }
    }
    ...
    /// 積を求めてからこれを呼ぶ。
    pub(crate) fn 宣言件数を確保前に検査する(&self, 宣言件数: usize, 最小要素長: usize) -> Result<(), アセット実行時形式エラー> {
        let 最小長 = 宣言件数.checked_mul(最小要素長).ok_or(アセット実行時形式エラー::長さ表現不能)?;
        if 最小長 > self.0.len() {
            return Err(アセット実行時形式エラー::件数過大 {
                宣言件数, 残りバイト数: self.0.len()
            });
        }
        Ok(())
    }
    ...
    pub(crate) fn 完了を検査する(self) -> Result<(), アセット実行時形式エラー> {
        if self.0.is_empty() { Ok(()) } else { Err(アセット実行時形式エラー::未読末尾(self.0.len())) }
    }
