...
pub(super) struct シーン版2 {
    先頭の描画対象: 描画対象版2,
    残りの描画対象一覧: Vec<描画対象版2>,
    スキン: Option<スキンデータ>,
    アニメーション一覧: Vec<アニメーションクリップ>,
}

pub(super) struct 描画対象版2 {
    識別子: 描画対象ID,
    所有チャンク: チャンク座標,
...
impl シーン版2 {
    ...
    pub(super) fn 最新へ変換する(self) -> シーンデータ {
        シーンデータ::生成する(
            ...
            self.残りの描画対象一覧.into_iter().map(描画対象版2::最新へ変換する).collect(),
            Vec::new(),
            self.スキン,
            self.アニメーション一覧,
        )
...
impl 描画対象版2 {
    fn 最新へ変換する(self) -> 描画対象データ {
        描画対象データ::生成する(self.識別子, self.所有チャンク, self.ローカルからワールド, self.形状.最新へ変換する(), 材質集合::単一材質から生成する(self.マテリアル))
    }
}
