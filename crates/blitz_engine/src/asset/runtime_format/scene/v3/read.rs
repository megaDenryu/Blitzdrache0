...
pub(in crate::asset::runtime_format::scene) fn シーン内容を読む(内容: &[u8]) -> Result<シーン版3, アセット実行時形式エラー> {
...
fn 描画対象を読む(入力: &mut 読取位置<'_>) -> Result<描画対象版3, アセット実行時形式エラー> {
    Ok(描画対象版3 {
...
fn 形状を読む(入力: &mut 読取位置<'_>) -> Result<描画形状, アセット実行時形式エラー> {
    match 入力.u8()? {
        ...
        地形LODメッシュ群の判別値 => {
            ...
            Ok(描画形状::地形LODメッシュ群(地形LODメッシュ群::生成する(最詳細段, より粗い段一覧)?))
        }
...
fn メッシュ列を読む(入力: &mut 読取位置<'_>, 空段の失敗: アセット実行時形式エラー) -> Result<(メッシュデータ, Vec<メッシュデータ>), アセット実行時形式エラー> {
