...
pub(in crate::asset::runtime_format::scene) fn 配置列を書く(出力: &mut 書込先, 配置一覧: &[個体配置]) -> Result<(), アセット実行時形式エラー> {
    出力.件数(配置一覧.len())?;
    for 配置 in 配置一覧 {
...
pub(in crate::asset::runtime_format::scene) fn 境界を書く(出力: &mut 書込先, 境界: 群境界) -> Result<(), アセット実行時形式エラー> {
    let 包囲領域 = 境界.群の包囲領域();
