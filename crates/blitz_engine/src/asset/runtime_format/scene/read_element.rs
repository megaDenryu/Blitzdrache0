...
/// 版4のメッシュ1つ分の最小バイト数。旧版の61へプリミティブ数4とプリミティブ1件16を足した合計である。
pub(super) const 版4のメッシュ最小長: usize = メッシュ最小長 + 4 + mesh_primitive::プリミティブ長;
...
/// 版4で、描画対象のうち形状より前と後の最小バイト数。識別子8 + 所有チャンク8 + 行列64 + 材質数4 + 材質1件39の合計である。
pub(super) const 版4の形状以外の描画対象長: usize = 8 + 8 + 64 + 4 + material_set::材質最小長;
