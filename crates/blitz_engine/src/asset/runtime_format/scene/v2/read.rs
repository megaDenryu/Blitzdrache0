...
pub(in crate::asset::runtime_format::scene) fn シーン内容を読む(内容: &[u8]) -> Result<シーン版2, アセット実行時形式エラー> {
    ...
    read_element::頂点属性を検査する(std::iter::once(&先頭の描画対象).chain(&残りの描画対象一覧).map(|対象| 対象.形状.スキン頂点属性一覧()), スキン.as_ref())?;
...
fn 描画対象を読む(入力: &mut 読取位置<'_>) -> Result<描画対象版2, アセット実行時形式エラー> {
    Ok(描画対象版2 {
...
fn 形状を読む(入力: &mut 読取位置<'_>) -> Result<形状版2, アセット実行時形式エラー> {
    match 入力.u8()? {
        ...
        地形LODメッシュ群の判別値 => {
            let (最詳細段, より粗い段一覧) = read_element::メッシュ列を読む(入力, read_element::メッシュ最小長, アセット実行時形式エラー::地形LOD段なし, read_element::旧版のメッシュを読む)?;
            Ok(形状版2::地形LODメッシュ群(地形LODメッシュ群::生成する(最詳細段, より粗い段一覧)?))
        }
