//! 版2で焼かれたシーンが最新版の型へ変換されて読めることの検査。版2の書き出しは検査だけが持つため、材料もここで作る。
...
fn 版2バイト列を作る() -> Vec<u8> {
    ...
    実行時アセットを格納する(アセット形式版::V2, 実行時アセット種別::シーン, &内容).unwrap()
}

#[test]
fn 版2のシーンを最新版へ変換して読む() {
    let 読み取り = 実行時形式からシーンを読む(&版2バイト列を作る()).unwrap();
    assert_eq!(読み取り, 静的シーンを作る(vec![10, 20, 30, 255], 0.2));
    assert!(matches!(読み取り.先頭の描画対象().形状(), 描画形状::通常メッシュ(_)));
}

#[test]
fn 版2と版3で先頭のヘッダー形式版が異なる() {
    let 版2 = 版2バイト列を作る();
    let 版3 = crate::asset::シーンを実行時形式へ格納する(&静的シーンを作る(vec![10, 20, 30, 255], 0.2)).unwrap();
