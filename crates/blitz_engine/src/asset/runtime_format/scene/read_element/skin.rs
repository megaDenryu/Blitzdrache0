...
pub(in crate::asset::runtime_format::scene) fn 読む(入力: &mut 読取位置<'_>) -> Result<Option<スキンデータ>, アセット実行時形式エラー> {
    match 入力.u8()? {
        0 => Ok(None),
        1 => {
...
fn ジョイントを読む(入力: &mut 読取位置<'_>, ジョイント添字: usize) -> Result<ジョイント, アセット実行時形式エラー> {
    let 親添字 = match 入力.u8()? {
        0 => None,
        1 => Some(入力.usize()?),
...
fn 静的trsを読む(入力: &mut 読取位置<'_>) -> Result<静的TRS, アセット実行時形式エラー> {
    Ok(静的TRS {
...
pub(in crate::asset::runtime_format::scene) fn 頂点属性を検査する<'a>(
    描画対象ごとの属性一覧: impl Iterator<Item = Option<&'a Vec<スキン頂点属性>>>, スキン: Option<&スキンデータ>
) -> Result<(), アセット実行時形式エラー> {
