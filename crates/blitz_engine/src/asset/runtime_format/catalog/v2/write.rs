//! 版2のカタログバイト列を組み立てる。旧版を読んで最新版へ変換する経路を検査するには版2の実物が要るため、その材料をここが作る。
...
pub(in crate::asset::runtime_format::catalog) fn カタログ内容を書く(カタログ: &カタログ) -> Result<Vec<u8>, アセット実行時形式エラー> {
