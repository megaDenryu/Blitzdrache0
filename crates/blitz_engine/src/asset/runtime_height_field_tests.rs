...
use super::runtime_format::{
    アセット実行時形式エラー, アセット形式版, カタログを実行時形式へ格納する, 実行時アセットを格納する, 実行時アセット種別, 実行時形式から高さ場を読む, 高さ場を実行時形式へ格納する, 高さ場実行時形式エラー
};
...
fn 別種別のアセットを高さ場として読むと拒む() {
    ...
    assert!(matches!(
        ...
        Err(高さ場実行時形式エラー::共通形式の失敗(アセット実行時形式エラー::アセット種別不一致 { 期待: 4, .. }))
    ));
}
...
fn 標本の実体より大きい格子点数を確保前に拒む() {
    ...
    assert!(matches!(実行時形式から高さ場を読む(&バイト列), Err(高さ場実行時形式エラー::共通形式の失敗(アセット実行時形式エラー::件数過大 { .. }))));
}
