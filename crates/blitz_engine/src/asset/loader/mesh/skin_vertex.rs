...
fn 新添字組へ変換する(
    ...
    for (出力, &旧添字) in 新添字組.iter_mut().zip(旧添字組.iter()) {
        ...
        let 新添字usize = *関節解決.旧添字から新添字.get(旧添字usize).ok_or_else(|| {
            アセットエラー::解析失敗(format!("JOINTS_0の参照添字{旧添字usize}がジョイント数を超えている"))
        })?;
        *出力 = u16::try_from(新添字usize).map_err(|誤り| {
            アセットエラー::解析失敗(format!("ジョイント添字がu16の範囲を超えている: {誤り}"))
        })?;
    }
    Ok(スキン頂点属性 {
        ジョイント: 新添字組,
