...
pub(super) fn プリミティブのスキン属性を取り出す(
    ...
    関節解決: &関節解決,
    頂点数: usize,
) -> Result<Vec<スキン頂点属性>, アセットエラー> {
    let 読み取り器 = プリミティブ.reader(|バッファ| 文書.バッファ一覧.get(バッファ.index()).map(Vec::as_slice));

    let 旧ジョイント一覧: Vec<[u16; 4]> = 読み取り器.read_joints(0).ok_or(アセットエラー::スキン頂点属性なし)?.into_u16().collect();
    let ウェイト一覧: Vec<[f32; 4]> = 読み取り器.read_weights(0).ok_or(アセットエラー::スキン頂点属性なし)?.into_f32().collect();

    if 旧ジョイント一覧.len() != 頂点数 || ウェイト一覧.len() != 頂点数 {
        return Err(アセットエラー::解析失敗(
...
fn 新添字組へ変換する(
    旧添字組: [u16; 4], ウェイト: [f32; 4], 関節解決: &関節解決
) -> Result<スキン頂点属性, アセットエラー> {
    let mut 新添字組 = [0u16; 4];
    for (出力, &旧添字) in 新添字組.iter_mut().zip(旧添字組.iter()) {
        ...
        let 新添字usize = *関節解決
            .旧添字から新添字
            .get(旧添字usize)
            .ok_or_else(|| アセットエラー::解析失敗(format!("JOINTS_0の参照添字{旧添字usize}がジョイント数を超えている")))?;
        *出力 = u16::try_from(新添字usize)
            .map_err(|誤り| アセットエラー::解析失敗(format!("ジョイント添字がu16の範囲を超えている: {誤り}")))?;
    }
    Ok(スキン頂点属性 {
        ジョイント: 新添字組,
        ウェイト,
