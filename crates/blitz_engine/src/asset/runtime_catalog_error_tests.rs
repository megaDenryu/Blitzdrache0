use super::{アセット実行時形式エラー, アセット形式版, 実行時アセットを格納する, 実行時アセット種別, 実行時形式からカタログを読む};
...
fn カタログの重複idを型付きエラーにする() {
    ...
    項目を書く(&mut 内容, b"fox");
    項目を書く(&mut 内容, b"fox");
    let バイト列 = 試験用カタログを版1で格納する(&内容);
    assert!(matches!(実行時形式からカタログを読む(&バイト列), Err(アセット実行時形式エラー::カタログID重複)));
}
...
fn カタログの不正utf8を型付きエラーにする() {
    ...
    項目を書く(&mut 内容, &[0xff]);
    let バイト列 = 試験用カタログを版1で格納する(&内容);
    assert!(matches!(実行時形式からカタログを読む(&バイト列), Err(アセット実行時形式エラー::不正な文字列)));
}
...
fn カタログの過大件数を確保前に拒否する() {
    ...
    let バイト列 = 試験用カタログを版1で格納する(&内容);
    assert!(matches!(実行時形式からカタログを読む(&バイト列), Err(アセット実行時形式エラー::件数過大 { .. })));
}
...
fn 試験用カタログを版1で格納する(内容: &[u8]) -> Vec<u8> {
    match 実行時アセットを格納する(アセット形式版::V1, 実行時アセット種別::カタログ, 内容) {
        Ok(バイト列) => バイト列,
