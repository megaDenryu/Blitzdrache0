...
//! `準備済みシーン`は準備済みチャンクのCPUデータであり、台帳が準備済みとして数えるRAMの実体である。読込完了での投入と不要時の破棄は`prepared_data`が、GPU転送のための取り出しは`gpu_handoff`が所有する。
//! `静的物理形状の保管`は読込済みチャンクの静的物理形状であり、読込完了で載り、CPUデータの破棄とGPU資源の解除とリセットで消える。描画へ渡すシーンからは読込完了の時点で切り離すため、同じ形状が2箇所に在る間は無い。
//! `観測済み読込量`は予算候補の見積を押し上げ、見積の過小によるスラッシングを止める。`優位計数`は近い未収容チャンクのための退避が同じ座標について何フレーム続けて優位だったかを数え、境界の往復で退避が起きないようにする。
//! `世代`はリセットのたびに進み、投入した読込と返ってきた完了が同じ値を持つかでリセット前の完了を退ける。
//! `読込済みチャンクの形の版`は`静的物理形状の保管`自身が持ち、保管を変える操作のたびに保管の中で進む。調停はそれを読んで貸すだけであり、前進を書き忘れる余地を持たない。
...
pub struct ストリーミング調停 {
    ...
    読込器: チャンク読込器,
    先読み半径: u8,
    準備済みシーン: HashMap<チャンク座標, シーンデータ>,  // 準備済みチャンクのCPUデータ
    静的物理形状の保管: チャンクごとの静的物理形状の保管, // 読込済みチャンクの静的物理形状
    観測済み読込量: 観測済み読込量,                       // 読込で得た読込量の観測
    優位計数: 退避優位計数,                               // 退避が優位だったフレーム数の計数
    世代: 世代状態,                                       // リセットのたびに進む世代
    転送量: ストリーミング転送量,
}
...
impl ストリーミング調停 {
    ...
    pub fn 起動する(設定: ストリーミング調停設定, 目録: チャンク目録) -> Result<Self, ストリーミング調停エラー> {
        let チャンク一辺 = 目録.一辺();
        Ok(Self {
            格子: チャンク格子::生成する(チャンク一辺.大域メートルへ変換する())?,
            目録,
            ...
            先読み半径: 設定.先読み半径,
            準備済みシーン: HashMap::new(),
            静的物理形状の保管: チャンクごとの静的物理形状の保管::空を作る(),
