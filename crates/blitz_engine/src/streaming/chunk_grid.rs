...
impl チャンク格子 {
    ...
    pub fn 必要集合を計算する(self, プレイヤー位置: 大域ワールド位置, 先読み半径: u8) -> Result<Vec<チャンク要求>, チャンク格子エラー> {
        let 中心 = self.所属座標を求める(プレイヤー位置)?;
        範囲を検査する(中心, 先読み半径)?;
    ...
    /// 大域位置が属するチャンク座標。必要集合の中心であり、退避で遠さを測る基準でもある。
    pub fn 所属座標を求める(self, 位置: 大域ワールド位置) -> Result<チャンク座標, チャンク格子エラー> {
    ...
    for z差 in -半径..=半径 {
        for x差 in -半径..=半径 {
            if x差.abs().max(z差.abs()) == 半径 {
                出力.push(チャンク要求::生成する(チャンク座標::生成する(中心.x() + x差, 中心.z() + z差), 距離));
            }
        }
    }
}
