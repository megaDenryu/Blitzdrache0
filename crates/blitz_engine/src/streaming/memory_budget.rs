...
impl ストリーミング予算 {
    ...
    pub fn 適用する(self, 現在使用量: ストリーミングメモリ量, 候補一覧: &[チャンク予算候補]) -> Result<ストリーミング予算結果, ストリーミング予算エラー> {
...
fn 結果を作る(
    候補一覧: &[チャンク予算候補], 除外開始: usize, 使用量: ストリーミングメモリ量, 上限: ストリーミングメモリ量, 収容一覧: Vec<チャンク予算候補>
) -> ストリーミング予算結果 {
