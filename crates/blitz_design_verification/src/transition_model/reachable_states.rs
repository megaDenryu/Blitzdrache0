...
impl 到達可能な状態一覧 {
    ...
    pub(super) fn 幅優先で組む(初期状態一覧: &[設計概念への参照], 遷移一覧: &[遷移]) -> Self {
        let mut 済み: HashSet<設計概念への参照> = 初期状態一覧.iter().cloned().collect();
        let mut 待ち: VecDeque<設計概念への参照> = 初期状態一覧.iter().cloned().collect();
    ...
    pub fn 含むか(&self, 状態: &設計概念への参照) -> bool {
