...
describe('実サーバー接続のbigint往復', () => {
    ...
    it('チャンク構造のPUTで2^53超えbigintが生数値文字列として送信され、GETで完全復元されること', async () => {
        ...
        const チャンク: チャンク構造 = {
            ...
            散布の個体一覧: [],
            見下ろし図の下書き: { 等高線一覧: [], 大升の一辺の升目数: 8, 大升の塗り一覧: [] },
        }
        ...
        // GET受信テスト
        const 生サーバーJSON = `{"道路一覧":[{"制御点列":[],"全幅メートル":8,"散布除外バッファメートル":14,"細分割数":80}],"建物一覧":[],"散布":{"最小間隔メートル":5.5,"乱数の種":18446744073709551615},"散布の個体一覧":[],"見下ろし図の下書き":{"等高線一覧":[],"大升の一辺の升目数":8,"大升の塗り一覧":[]}}`
