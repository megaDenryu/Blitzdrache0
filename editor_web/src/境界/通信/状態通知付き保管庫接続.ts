...
export class 状態通知付き保管庫接続 implements プロジェクト保管庫接続, 建物外形カタログ接続, 建物の格子接続, 楽曲接続 {
    ...
    public 楽曲一覧を読む(): Promise<読込結果<楽曲ID[]>> {
        return this._補助の口.楽曲一覧を読む()
    }

    public 楽曲を読む(楽曲ID: 楽曲ID): Promise<読込結果<楽曲>> {
        return this._補助の口.楽曲を読む(楽曲ID)
    }

    public 楽曲を保存する(楽曲: 楽曲): Promise<保存結果> {
        return this._補助の口.楽曲を保存する(楽曲)
    }
}
