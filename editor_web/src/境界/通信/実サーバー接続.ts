...
import { 大域世界パスを組み立てる, チャンクパスを組み立てる, 書き出しパスを組み立てる, 建物外形カタログパスを組み立てる } from './実サーバー接続の経路.ts'

// crates/editor_server（127.0.0.1:7901）への実通信を行う境界実装。
// ブラウザではViteプロキシ越しの相対/api経路を用い、Nodeヘッドレスでは基底URLを受け取って接続する。
export class 実サーバー接続 implements プロジェクト保管庫接続, ソースアセット書き出し接続, 建物外形カタログ接続 {
