...
describe('ヘッドレス編集コマンド一括適用のテスト', () => {
    ...
    it('等高線3本から高さ場を生成し大升で岩を塗って地形を生成するコマンド列のJSONがヘッドレス経路で適用できること', async () => {
        const 偽保管庫 = new 偽保管庫接続()
        ...
        const コマンドJSON = JSON.stringify([
            ...
            { 種類: '大升を塗る', 値: { ...座標, 大升の一辺の升目数: 8, 塗り一覧: [{ 列: 8, 行: 8, 高さメートル: null, 層: '岩' }] } },
            { 種類: '大升から地形を生成する', 値: 座標 },
        ])
        ...
        assert.deepStrictEqual(保存チャンク.見下ろし図の下書き.大升の塗り一覧, [{ 列: 8, 行: 8, 高さメートル: null, 層: '岩' }])
        const 保存高さ = 偽保管庫.保存されたチャンク高さマップ.get('0,0')
        ...
        assert.strictEqual(材質[((68 * 解像度) + 68) * 4 + 2], 255, '塗った大升(8,8)の中心は岩100%になるべき')
    })
})
