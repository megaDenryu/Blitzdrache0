...
describe('ブラウザなしの編集コマンド一括適用のテスト', () => {
    it('偽保管庫接続へ初期状態から14の選択肢にわたるコマンドを適用し、大域高さ格子を書かずにチャンクが保存されること', async () => {
    ...
    it('保管庫に既存の大域構造がある場合、ブラウザなしの保存で既存大域構造が上書きされないこと', async () => {
        const 偽保管庫 = new 偽保管庫接続()
    ...
    it('等高線3本から高さ場を生成し粗マスで岩を塗って地形を生成するコマンド列のJSONがブラウザなしの経路で適用できること', async () => {
        const 偽保管庫 = new 偽保管庫接続()
