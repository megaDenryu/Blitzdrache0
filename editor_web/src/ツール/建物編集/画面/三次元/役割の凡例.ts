...
import { 全ての部品の役割, type 部品の役割 } from '../../編集モデル/index.ts'
import { 役割の識別色を引く } from './役割の識別色.ts'
import { 凡例の1件, 凡例の並び, 凡例の色見本 } from './三次元表示のスタイル.css.ts'

// どの色がどの役割かを並べる凡例。色だけを出して意味を人の記憶に頼らせると、識別色の表示が読めない。
export function 役割の凡例を作る(): DivC {
    return div({ class: 凡例の並び }).childs(全ての部品の役割.map(この役割の凡例を作る))
}

function この役割の凡例を作る(役割: 部品の役割): DivC {
