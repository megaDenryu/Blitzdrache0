...
class 偽保管庫 implements プロジェクト保管庫接続 {
    ...
    public async チャンクの構造を読む(): Promise<読込結果<チャンク構造>> {
        return 読込無し()
    }
    ...
    public async チャンクの材質重みを読む(): Promise<読込結果<ArrayBufferLike>> {
        return 読込無し()
    }
...
describe('ワールド永続化の大域世界構造整合テスト', () => {
    it('保存済みの大域世界構造があるとき、その区画割りに基づく頂点数のチャンクが生成されること', async () => {
        ...
        const 期待頂点数 = 65
        assert.strictEqual(チャンク.高さ場.解像度, 期待頂点数, '保存済み区画割りの解像度+1になること')
        assert.strictEqual(チャンク.高さ場.格子データ.length, 期待頂点数 * 期待頂点数)
    })

    it('大域世界構造が無いときは既定値の直書きへフォールバックすること', async () => {
        const 保管庫 = new 偽保管庫()
        const 座標: チャンク座標 = { x: 0, z: 0 }
        const { 状態, 結果種別 } = await ワールド状態を保管庫から読み込む(保管庫, 座標)
        assert.strictEqual(結果種別, '初期生成')

        const チャンク = 状態.チャンクを取得する(座標)
        const 既定頂点数 = 129
        assert.strictEqual(チャンク.高さ場.解像度, 既定頂点数, '既定解像度128+1になること')
    })

    it('チャンク保存時に大域構造と大域高さ格子がチャンク格子より先に保存されること', async () => {
        const 保管庫 = new 偽保管庫()
        const 座標: チャンク座標 = { x: 0, z: 0 }
        const { 状態 } = await ワールド状態を保管庫から読み込む(保管庫, 座標)

        await ワールド状態を保管庫へ保存する(保管庫, 状態)
        assert.deepStrictEqual(保管庫.保存呼出順序, [
            '大域世界構造',
            '大域世界高さ格子',
            'チャンク構造',
            'チャンク高さ格子',
            'チャンク材質重み',
        ], '大域世界構造と高さ格子が先に保存されること')
    })
})
