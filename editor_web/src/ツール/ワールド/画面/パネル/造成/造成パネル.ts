...
export class 造成パネル extends LV2HtmlComponentBase implements I配線可能<I造成パネル配線> {
    ...
    private readonly _ボタンマップ: Map<造成筆致種別, ButtonC> = new Map<造成筆致種別, ButtonC>()
    ...
    public 筆致種別を更新する(選択種別: 造成筆致種別): void {
        for (const [種別, ボタン] of this._ボタンマップ.entries()) {
            ボタン.setAttribute('data-selected', 種別 === 選択種別 ? 'true' : 'false')
        }
    }
        ...
        return (
            div({ class: styles.パネル }).childs([
                div({ class: styles.見出し行 }).childs([
                    span({ text: '地形ブラシ設定' }),
                    span({ class: styles.補助テキスト, text: '[Shift: 削り / 平滑化]' })]),
                div({ class: styles.ボタングループ }).childs(
                    筆致種別一覧.map(({ 種別, ラベル }) =>
                        button({
                            class: styles.ブラシ種別ボタン,
                            text: ラベル,
                        })
                            .setAttribute('data-selected', 種別 === 初期種別 ? 'true' : 'false')
                            .tap((btn: ButtonC) => {
