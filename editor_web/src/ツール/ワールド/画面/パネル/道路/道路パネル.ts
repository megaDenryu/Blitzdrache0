...
export class 道路パネル extends LV2HtmlComponentBase implements I配線可能<I道路パネル配線> {
    ...
    public constructor(初期全幅: number, 初期バッファ: number, 初期細分割: number) {
        ...
        this._削除ボタン = button({ class: styles.削除ボタン, text: 'ノード削除', disabled: true })
    ...
    private _ルートを構築する(): DivC {
        return (
            div({ class: styles.パネル }).childs([
                span({ class: styles.見出し, text: '道路 & パラメータ' }),
                this._全幅スライダー,
                this._除外バッファスライダー,
                this._細分割スライダー,
                div({ class: styles.アクション区画 }).childs([
                    button({ class: styles.切土盛土ボタン, text: '道路に合わせて地形を切土・盛土' })
                        .onClick(() => this._配線.先.on道路切土盛土()),
                    div({ class: styles.行ボタン群 }).childs([
                        this._削除ボタン
                            .onClick(() => this._配線.先.on選択ノード削除()),
                        button({ class: styles.副ボタン, text: 'リセット' })
