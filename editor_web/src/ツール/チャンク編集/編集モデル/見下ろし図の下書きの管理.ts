import type { 等高線, 大升の塗り, 見下ろし図の下書き } from '../../../生成/編集資源契約.ts'

// 1チャンクの見下ろし図の下書き(等高線一覧と大升の塗り)を保持する。高さ場と材質重みの正本ではなく、
// 正本を生成するための入力であり、チャンク構造のJSONへ併存して保存する。
// 参照: `_doc/設計/見下ろし図による地形編集.md`「判断1」「判断4」。
export class 見下ろし図の下書きの管理 {
    private 等高線一覧: Array<等高線>
    private 大升の一辺の升目数: number
    private readonly 大升の塗りマップ: Map<string, 大升の塗り>

    public constructor(初期下書き: 見下ろし図の下書き) {
        this.等高線一覧 = 初期下書き.等高線一覧.map(等高線を複製する)
        this.大升の一辺の升目数 = 初期下書き.大升の一辺の升目数
        this.大升の塗りマップ = new Map<string, 大升の塗り>()
        this.大升を塗る(初期下書き.大升の一辺の升目数, 初期下書き.大升の塗り一覧)
    }
    ...
    public 大升の一辺の升目数を取得する(): number {
        return this.大升の一辺の升目数
    }

    // 列・行の昇順で返し、同じ塗りの集まりが同じ並びになるようにする(保存物の差分と決定性の試験のため)。
    public 大升の塗り一覧を取得する(): Array<大升の塗り> {
        return Array.from(this.大升の塗りマップ.values())
            .map((塗り) => ({ ...塗り }))
    ...
    // 高さも層もnullの項目はその大升の塗りを消す。
    public 大升を塗る(一辺の升目数: number, 塗り一覧: ReadonlyArray<大升の塗り>): void {
        if (!Number.isInteger(一辺の升目数) || 一辺の升目数 < 1) {
            throw new Error(`大升の一辺の升目数は正の整数でなければならない: ${一辺の升目数}`)
        }
        if (一辺の升目数 !== this.大升の一辺の升目数) {
            this.大升の塗りマップ.clear()
            this.大升の一辺の升目数 = 一辺の升目数
        }
        for (const 塗り of 塗り一覧) {
            const キー = 大升のキー(塗り.列, 塗り.行)
            if (塗り.高さメートル === null && 塗り.層 === null) {
                this.大升の塗りマップ.delete(キー)
            } else {
                this.大升の塗りマップ.set(キー, { ...塗り })
            }
        }
    }

    public 大升の塗り全体を置き換える(一辺の升目数: number, 塗り一覧: ReadonlyArray<大升の塗り>): void {
        this.大升の塗りマップ.clear()
        this.大升の一辺の升目数 = 一辺の升目数
        this.大升を塗る(一辺の升目数, 塗り一覧)
    }

    public 契約の形で取り出す(): 見下ろし図の下書き {
        return {
            等高線一覧: this.等高線一覧を取得する(),
            大升の一辺の升目数: this.大升の一辺の升目数,
            大升の塗り一覧: this.大升の塗り一覧を取得する(),
        }
    }
...
function 大升のキー(列: number, 行: number): string {
    return `${列},${行}`
}
