...
import { 高さの一致とみなす許容差メートル, 等高線の拘束衝突 } from './等高線の拘束衝突.ts'
...
// 同じ格子点へ異なる等高線の高さが来たら、その差が高さの一致とみなす許容差メートルを超える限り
// `等高線の拘束衝突`を投げて生成を拒否する(見下ろし図の設計正本の判断3)。線どうしが実際に
// 交差する場合だけでなく、交差しなくても格子への丸めで別の等高線が同じ格子点へ落ちる場合にも起こる。
...
export function 等高線を格子点へ固定する(
    ...
    const 固定した等高線の添字 = new Int32Array(解像度 * 解像度)
    let 現在の等高線添字 = -1
    const 格子点を固定する = (位置: 平面の位置, 高さ: number): void => {
        ...
        const 添字 = gz * 解像度 + gx
        const 既存の高さ = 固定の高さ[添字] ?? 0
        const 既存の等高線添字 = 固定した等高線の添字[添字] ?? -1
        if (固定の印[添字] === 1 && 既存の等高線添字 !== 現在の等高線添字) {
            if (Math.abs(既存の高さ - 高さ) > 高さの一致とみなす許容差メートル) {
                throw new 等高線の拘束衝突(既存の等高線添字, 現在の等高線添字, gx, gz, 既存の高さ, 高さ)
            }
        }
        固定の印[添字] = 1
        固定の高さ[添字] = 高さ
        固定した等高線の添字[添字] = 現在の等高線添字
    }
    等高線一覧.forEach((線, 添字) => {
        現在の等高線添字 = 添字
