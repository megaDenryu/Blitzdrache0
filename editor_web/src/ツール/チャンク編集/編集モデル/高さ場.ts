import type { 位置3次元, 造成筆致, 等高線 } from '../../../生成/編集資源契約.ts'
import { 等高線から高さ場を生成する as 等高線から高さ場を計算する } from './等高線から高さ場を生成する計算.ts'
import { 高さ場から等高線を導く as 高さ場から等高線を計算する } from './高さ場から等高線を導く計算.ts'
...
export class 高さ場 {
    ...
    // 等高線一覧を拘束にして格子データを丸ごと作り直す(見下ろし図の設計正本の判断3)。
    public 等高線から生成し直す(等高線一覧: ReadonlyArray<等高線>): void {
        this.格子データを置き換える(等高線から高さ場を計算する(等高線一覧, this.解像度, this.一辺のメートル, this.格子データ))
    }

    // 一定間隔の等高線一覧を格子データから導く(見下ろし図の設計正本の判断5)。
    public 等高線を導く(間隔メートル: number): Array<等高線> {
        return 高さ場から等高線を計算する(間隔メートル, this.解像度, this.一辺のメートル, this.格子データ)
    }
