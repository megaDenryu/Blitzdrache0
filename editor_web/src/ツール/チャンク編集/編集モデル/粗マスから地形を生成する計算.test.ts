...
import { 粗マスから地形を生成する } from './粗マスから地形を生成する計算.ts'
import { 高さ場から粗マスを導く } from './高さ場から粗マスを導く計算.ts'
...
describe('粗マスから地形を生成する計算', () => {
    it('粗マスの塗りが塗っていない粗マスの格子点を変えないこと', () => {
        const 元の高さ = 起伏のある格子()
        const 元の材質 = 草だけの材質()
        const 結果 = 粗マスから地形を生成する([{ 列: 1, 行: 1, 高さメートル: 20, 層: '岩' }], 一辺の升目数, 解像度, 元の高さ, 元の材質)
        for (let gz = 24; gz < 解像度; gz++) {
            for (let gx = 24; gx < 解像度; gx++) {
                const 添字 = gz * 解像度 + gx
                assert.strictEqual(結果.高さ[添字], 元の高さ[添字], `塗っていない粗マス(3,3)の格子点(${gx},${gz})の高さは変わらないべき`)
                for (let 層 = 0; 層 < 4; 層++) {
        ...
        assert.ok(Math.abs((結果.高さ[塗った中心] ?? 0) - 20) < 1e-3, '塗った粗マスの中心は塗りの高さになるべき')
        assert.strictEqual(結果.材質[塗った中心 * 4 + 2], 255, '塗った粗マスの中心は岩100%になるべき')
    })

    it('粗マスの層の境界で重みの合計が255を保つこと', () => {
        const 結果 = 粗マスから地形を生成する(
    ...
    it('一辺が升目数を割り切らないときは拒否されること', () => {
        assert.throws(() => 粗マスから地形を生成する([], 7, 解像度, 起伏のある格子(), 草だけの材質()), /割り切らない/)
    })
})

describe('高さ場から粗マスを導く計算', () => {
    it('粗マスごとに格子点の平均高さと重み合計が最大の層になること', () => {
        ...
        const 塗り一覧 = 高さ場から粗マスを導く(一辺の升目数, 解像度, 高さ, 材質)
        assert.strictEqual(塗り一覧.length, 16)
        const 岩の粗マス = 塗り一覧.find((塗り) => 塗り.列 === 1 && 塗り.行 === 1)
        assert.deepStrictEqual(岩の粗マス, { 列: 1, 行: 1, 高さメートル: 7, 層: '岩' })
