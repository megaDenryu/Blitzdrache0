...
describe('等高線から高さ場を生成する計算', () => {
    ...
    it('外周ちょうどを通る等高線でも外周の格子点が変わらないこと', () => {
        const 元 = 起伏のある格子()
        const 結果 = 等高線から高さ場を生成する([正方形の等高線(一辺 / 2, 20)], 解像度, 一辺, 元)
        for (let i = 0; i < 解像度; i++) {
            for (const 添字 of [格子点(i, 0), 格子点(i, 解像度 - 1), 格子点(0, i), 格子点(解像度 - 1, i)]) {
                assert.strictEqual(結果[添字], 元[添字], `外周(${添字})は変わらないべき`)
            }
    ...
    it('外周の外側から丸め込まれる等高線でも外周の格子点が変わらないこと', () => {
        const 元 = 起伏のある格子()
        const 結果 = 等高線から高さ場を生成する([正方形の等高線(一辺 / 2 + 2, 20)], 解像度, 一辺, 元)
        for (let i = 0; i < 解像度; i++) {
            for (const 添字 of [格子点(i, 0), 格子点(i, 解像度 - 1), 格子点(0, i), 格子点(解像度 - 1, i)]) {
                assert.strictEqual(結果[添字], 元[添字], `外周(${添字})は変わらないべき`)
            }
