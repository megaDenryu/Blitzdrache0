...
import { モードヒント写像, type 編集モード } from '../パネル/モード切替/モード定義.ts'
import { 棚の案内文 } from '../パネル/共通/棚.css.ts'
...
export class モードの案内札 extends DivC {
    public constructor(初期モード: 編集モード) {
        super({ class: 棚の案内文 })
        this.モードを更新する(初期モード)
    }

    public モードを更新する(モード: 編集モード): this {
        const 文言 = `${モードヒント写像[モード]} ${視点の案内}`
