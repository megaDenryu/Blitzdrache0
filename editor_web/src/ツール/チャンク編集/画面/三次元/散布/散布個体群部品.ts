...
import type { 散布の個体 } from '../../../../../生成/編集資源契約.ts'
...
export class 散布個体群部品 extends 個体群部品<ジオメトリ包み, MeshStandardMaterial> {
    ...
    public 散布の個体を更新する(個体一覧: ReadonlyArray<散布の個体>, 高さ場モデル: 高さ場): void {
        const 表示個数 = Math.min(個体一覧.length, this.容量)

        for (let i = 0; i < 表示個数; i++) {
            const 個体 = 個体一覧[i]
            if (個体 === undefined) continue
            const 東 = 個体.チャンク中心からの東メートル
            const 南 = 個体.チャンク中心からの南メートル
            const y = 高さ場モデル.標本高さを取得する(東, 南)
            const 法線 = 高さ場モデル.標本法線を取得する(東, 南)

            this._法線作業.set(法線.x, 法線.y, 法線.z).normalize()
            this._作業用位置.set(東, y, 南)
            this._作業用回転.setFromUnitVectors(this._上方向, this._法線作業)
            const スケール = 0.8 + (Math.sin(東 * 5.0) * 0.5 + 0.5) * 0.5
    ...
    // テーマ切替時に植生の色を差し替える(参照: 工房テーマ/夜間テーマの植生色)。
    public 植生色を更新する(植生色: number): void {
        this.材質情報.color.set(植生色)
    }
}
