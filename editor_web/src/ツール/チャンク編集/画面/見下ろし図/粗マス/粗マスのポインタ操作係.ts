...
import { 同じ粗マスか, type 粗マスの格子, type 粗マスの番地 } from './粗マスの座標変換.ts'

export interface I粗マスの操作の相手 {
    readonly 格子を読む: () => 粗マスの格子 | null
    readonly 粗マスを塗る: (一辺の升目数: number, 塗り一覧: readonly 粗マスの塗り[]) => void
    readonly 粗マスを選択する: (番地: 粗マスの番地) => void
    readonly 再描画する: () => void
}

// 左ボタンを押してから離すまでの画素での移動がこの値以内なら「クリック」とみなし1升を選ぶ。
// 超えれば通った粗マスを塗る従来の操作になる(エディター基盤の判断13、左クリック=選択・左ドラッグ=配置)。
const 選択とみなす移動画素の上限 = 4

// 粗マスモードの左ボタン。押してから離すまでに通った粗マスが1つだけで、かつ画素での移動が小さいときは
// その1升を選ぶ。それ以外は従来どおり、通った粗マスを集めて離したときに1つの`粗マスを塗る`へまとめる
// (設計正本の判断2、判断7)。同じ粗マスは1回だけ数える。置く高さ・層・塗りを消すかは右サイドバーの設定に従う。
export class 粗マスのポインタ操作係 {
    private readonly _通った番地一覧: 粗マスの番地[] = []
    private _押下中: boolean = false
    private _押した画素: { readonly x: number; readonly y: number } | null = null

    public constructor(
        private readonly _状態: 見下ろし図の編集状態,
        private readonly _相手: I粗マスの操作の相手,
    ) {}

    public get 通った番地一覧(): readonly 粗マスの番地[] {
    ...
    public 離された(原初事象: PointerEvent): void {
        ...
        if (唯一の番地 !== undefined && 移動画素 <= 選択とみなす移動画素の上限) {
            this._相手.粗マスを選択する(唯一の番地)
            this._相手.再描画する()
            return
        }
        const 塗り一覧 = 通った番地一覧.map((番地) => this._塗りを組む(番地))
        // 高さも層も置かず消しもしない設定では、塗りを消す項目を積んでしまうため何もしない。
        if (塗り一覧.length > 0 && (this._状態.粗マスの塗りを消すか || this._状態.粗マスに高さを置くか || this._状態.粗マスに層を置くか)) {
            this._相手.粗マスを塗る(this._状態.粗マスの一辺の升目数, 塗り一覧)
        }
        this._相手.再描画する()
    }
    ...
    private _通る(位置: ワールドXZ): void {
        ...
        if (格子 === null) return
        const 番地 = 格子.ワールドから粗マスへ(位置)
        if (番地 === null || this._通った番地一覧.some((既) => 同じ粗マスか(既, 番地))) return
        this._通った番地一覧.push(番地)
        this._相手.再描画する()
    }

    // 高さも層も無い項目は塗りを消す意味になる(設計正本の判断2の表)。
    private _塗りを組む(番地: 粗マスの番地): 粗マスの塗り {
        if (this._状態.粗マスの塗りを消すか) return { 列: 番地.列, 行: 番地.行, 高さメートル: null, 層: null }
        return {
            列: 番地.列,
            行: 番地.行,
            高さメートル: this._状態.粗マスに高さを置くか ? this._状態.粗マスに置く高さメートル : null,
            層: this._状態.粗マスに層を置くか ? this._状態.粗マスに置く層 : null,
        }
    }
}
