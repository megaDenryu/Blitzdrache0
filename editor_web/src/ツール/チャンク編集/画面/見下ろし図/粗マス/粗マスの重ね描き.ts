...
export function 粗マスを重ね描きする(キャンバスへ描く口: CanvasRenderingContext2D, 視点: 見下ろし図の視点, 入力: 粗マスの重ね描きの入力): void {
    if (入力.格子線を描くか) 格子線を描く(キャンバスへ描く口, 視点, 入力.塗っている途中の格子)
    for (const 塗り of 入力.塗り一覧) {
        const { 左上, 一辺 } = 粗マスの画素の矩形(視点, 入力.格子, 塗り)
        if (塗り.層 !== null) {
            キャンバスへ描く口.globalAlpha = 重ね描きの寸法.粗マスの塗りの不透明度
            キャンバスへ描く口.fillStyle = 入力.材質色[塗り.層]
            キャンバスへ描く口.fillRect(左上.x, 左上.y, 一辺, 一辺)
            キャンバスへ描く口.globalAlpha = 1
        }
        キャンバスへ描く口.strokeStyle = 重ね描きの配色.粗マスの塗りの縁
        キャンバスへ描く口.lineWidth = 重ね描きの寸法.粗マスの格子線の太さ画素
        キャンバスへ描く口.strokeRect(左上.x, 左上.y, 一辺, 一辺)
        if (塗り.高さメートル !== null) {
            const 文 = 高さを図の文字にする(塗り.高さメートル)
            if (高さの文字が収まるか(キャンバスへ描く口, 文, 一辺 * 重ね描きの寸法.高さの文字を描く升の幅の割合)) {
                高さの文字を描く(キャンバスへ描く口, 文, 左上.x + 一辺 / 2, 左上.y + 一辺 / 2)
            }
        }
    }
    for (const 番地 of 入力.塗っている途中の番地一覧) {
        const { 左上, 一辺 } = 粗マスの画素の矩形(視点, 入力.塗っている途中の格子, 番地)
        キャンバスへ描く口.strokeStyle = 重ね描きの配色.描いている途中の等高線
        キャンバスへ描く口.lineWidth = 重ね描きの寸法.等高線の太さ画素
        キャンバスへ描く口.strokeRect(左上.x, 左上.y, 一辺, 一辺)
    }
    if (入力.選択中の番地 !== null) {
        const { 左上, 一辺 } = 粗マスの画素の矩形(視点, 入力.格子, 入力.選択中の番地)
        キャンバスへ描く口.strokeStyle = 重ね描きの配色.選択中の粗マスの枠
        キャンバスへ描く口.lineWidth = 重ね描きの寸法.選択中の粗マスの枠の太さ画素
        キャンバスへ描く口.strokeRect(左上.x, 左上.y, 一辺, 一辺)
    }
}
...
function 格子線を描く(キャンバスへ描く口: CanvasRenderingContext2D, 視点: 見下ろし図の視点, 格子: 粗マスの格子): void {
    const 数 = 格子.一辺に並ぶ粗マスの数()
    const 半分 = 格子.一辺のメートル / 2
    const 一辺 = 格子.粗マスの一辺のメートル()
    キャンバスへ描く口.strokeStyle = 重ね描きの配色.粗マスの格子線
    キャンバスへ描く口.lineWidth = 重ね描きの寸法.粗マスの格子線の太さ画素
    キャンバスへ描く口.beginPath()
    for (let i = 1; i < 数; i++) {
        const 位置 = -半分 + i * 一辺
        ...
        キャンバスへ描く口.moveTo(縦の上.x, 縦の上.y)
        キャンバスへ描く口.lineTo(縦の下.x, 縦の下.y)
        キャンバスへ描く口.moveTo(横の左.x, 横の左.y)
        キャンバスへ描く口.lineTo(横の右.x, 横の右.y)
    }
    キャンバスへ描く口.stroke()
}
