...
import { 始点で閉じるか, 辺の当たりを探す, 頂点の当たりを探す } from './等高線の当たり判定.ts'
import { 頂点のドラッグ } from './頂点のドラッグ.ts'
import { 辺のドラッグ } from './辺のドラッグ.ts'
...
// 描いていないときは、頂点の近くを押すとドラッグで動かし、線分の近くを押すとドラッグで両端の頂点を一緒に動かし、
// 動かさずに離せば選ぶだけにする。当たりの優先順位は頂点、次に線分である。
...
export class 等高線のポインタ操作係 {
    ...
    private readonly _辺ドラッグ: 辺のドラッグ = new 辺のドラッグ()
    ...
    public 押された(位置: ワールドXZ, 原初事象: PointerEvent): void {
        ...
        const 辺 = 辺の当たりを探す(一覧, 位置, 半径)
        if (辺 !== null) {
            const 線 = 一覧[辺.線の添字]
            const 視点 = this._相手.視点を読む()
            if (線 !== undefined && 視点 !== null) this._辺ドラッグ.始める(辺, 線, 位置, 視点.画素毎メートル)
            this._状態.選択中の等高線の添字 = 辺.線の添字
        } else {
    ...
    public 動かされた(位置: ワールドXZ, 押下中: boolean): void {
        if (!押下中) return
        if (this._ドラッグ.動かす(位置) || this._辺ドラッグ.動かす(位置)) this._相手.再描画する()
    }

    public 離された(): void {
        const 頂点の結果 = this._ドラッグ.終える()
        if (頂点の結果 !== null) {
            if (頂点の結果.動いた) this._相手.等高線を変更する(頂点の結果.線の添字, 頂点の結果.線)
            this._相手.再描画する()
            return
        }
        const 辺の結果 = this._辺ドラッグ.終える()
        if (辺の結果 === null) return
        if (辺の結果.動いた) this._相手.等高線を変更する(辺の結果.線の添字, 辺の結果.線)
        this._相手.再描画する()
    }
    ...
    public 重ね描きの入力を作る(): 等高線の重ね描きの入力 {
        return {
            等高線一覧: this._辺ドラッグ.写しで置き換える(this._ドラッグ.写しで置き換える(this._相手.等高線一覧を読む())),
