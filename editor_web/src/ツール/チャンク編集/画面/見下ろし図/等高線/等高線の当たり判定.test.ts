...
import { 始点で閉じるか, 点と線分の距離, 線分の列を作る, 辺の当たりを探す, 頂点の当たりを探す } from './等高線の当たり判定.ts'
...
describe('辺の当たりを探す', () => {
    it('線分の近くをクリックするとその線の添字と両端の頂点添字を返すこと', () => {
        assert.deepStrictEqual(辺の当たりを探す(一覧, { x: 5, z: 0.5 }, 1), { 線の添字: 0, 頂点の添字甲: 0, 頂点の添字乙: 1 })
    })

    it('閉じた線の戻りの線分にも当たり、末尾と先頭の頂点添字を返すこと', () => {
        assert.deepStrictEqual(辺の当たりを探す(一覧, { x: 55, z: 55.5 }, 1), { 線の添字: 1, 頂点の添字甲: 2, 頂点の添字乙: 0 })
    })

    it('どの辺からも遠いときは当たらないこと', () => {
        assert.strictEqual(辺の当たりを探す(一覧, { x: 30, z: 30 }, 1), null)
    })
})
