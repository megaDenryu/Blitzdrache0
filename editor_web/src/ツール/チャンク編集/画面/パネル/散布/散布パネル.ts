...
import { パネル, 見出し行, 本数ラベル, 再ベイクボタン } from './スタイル.css.ts'
...
class 散布本数ラベル extends SpanC {
    public constructor(初期本数: number) {
        super({ class: 本数ラベル, text: `${初期本数} 本` })
    }

    public 本数を更新する(本数: number): this {
        this.setTextContent(`${本数} 本`)
        return this
    }
}
...
export class 散布パネル extends LV2HtmlComponentBase implements I配線可能<I散布パネル配線> {
    ...
    public 本数を更新する(本数: number): void {
        this._本数表示.本数を更新する(本数)
    }

    public override delete(): void {
        this._間隔スライダー.delete()
        super.delete()
    }
