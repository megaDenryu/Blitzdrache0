...
// チャンクぜんたいに効く操作・表示面の切替)を置き、その下の残りをすべて地形の三次元と見下ろし図が占める枠へ渡す。
// 枠には両方を置き、片方だけを表示する(見下ろし図の設計正本の判断6)。
...
export class チャンク編集画面 extends LV2部品集約Base<チャンク編集画面部品> {
    ...
    public 寸法を合わせる(ピクセル比: number = 1): void {
        ...
        this.部品.見下ろし図.いまの枠の大きさへ合わせる(ピクセル比)
    }
    ...
    public 三次元の配色を設定する(配色: 三次元の配色): void {
        ...
        this.部品.見下ろし図.配色を設定する({ 標高低色: 配色.標高低色, 標高中色: 配色.標高中色, 標高高色: 配色.標高高色 })
    }
    ...
    protected _ルートを構築する(部品: チャンク編集画面部品): DivC {
        return (
            div({ class: 画面ルート }).childs([
                ...
                部品.お知らせ,
                div({ class: 三次元の枠 }).childs([部品.三次元ビュー, 部品.見下ろし図])])
        )
    }
