...
export class 大域ヒントパネル extends LV2HtmlComponentBase {
    ...
    public モードを更新する(モード: 大域編集モード): void {
        ...
        switch (モード) {
            ...
            case '広域道路作成':
                this._テキスト要素.childs([
                    span({ class: 強調, text: '左クリックで対象の道の終わりへ点を追加' }),
                    span({ text: ' (チャンク境界を跨いで配置可能)。既存の点をクリックするとそこから道が分かれ、地形のダブルクリック(または道路パネルの「新しい道」)で別の道を始めます。※右ドラッグでカメラ回転、中ドラッグでカメラ平行移動' }),
                ])
                break
            case '広域道路編集':
                this._テキスト要素.childs([
                    span({ class: 強調, text: '点を左ドラッグで移動、クリックで選択' }),
                    span({ text: '(選んだ点の道が対象の道になります)。道の上をクリックすると、その場所へ点を割り込ませます。選んだ点は道路パネルの「選択点を削除」、道ごとなら「この道を削除」で消せます。※右ドラッグでカメラ回転、中ドラッグでカメラ平行移動' }),
                ])
                break
        }
    }
