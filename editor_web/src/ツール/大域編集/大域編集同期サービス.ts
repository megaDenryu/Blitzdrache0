...
import { 居なくなった道路点の選択を外す } from '../チャンク編集/選択中の道路点の後始末.ts'
...
export class 大域編集同期サービス {
    ...
    public 道路を同期する(): void {
        居なくなった道路点の選択を外す(this._UI状態, this._モデル.広域道路.制御点列.length)
        this._部品.三次元ビュー.道路帯.更新する(this._モデル.広域道路, this._モデル.大域高さ場)
        this._部品.三次元ビュー.道路点マーカー.更新する(
            this._モデル.広域道路,
            this._UI状態.選択中の道路点の添字,
            this._UI状態.つかんでいる道路点の添字,
        )
    }
    ...
    public UIを同期する(): void {
        ...
        this._部品.インスペクター.部品.道路.選択中の道路点があるか設定する(this._UI状態.選択中の道路点の添字 !== null)
    }
}
