...
describe('演奏の範囲', () => {
    it('パターンの繰り返しを選ぶと、曲構成があっても使わないこと', () => {
        ...
        assert.strictEqual(演奏が1周する長さ('パターンの繰り返し', 楽曲).数値(), パターンのステップ数)
    })

    it('曲構成が空のときは、曲構成のとおりを選んでも選択中のパターンだけを繰り返すこと', () => {
        const 楽曲 = 初期楽曲を生成する('song-1', '見本')
        assert.deepEqual(楽曲.曲構成, [])
        assert.strictEqual(演奏が1周する長さ('曲構成のとおり', 楽曲).数値(), パターンのステップ数)
        const 曲構成 = 演奏に使う曲構成('曲構成のとおり', 楽曲)
        for (const 通しステップ of [0, 5, 31, 32, 33, 96]) {
            const 位置 = 曲のステップ位置を解決する(曲構成, 通しステップ, 'pattern-1')
            assert.strictEqual(位置.パターンの名乗り, 'pattern-1')
            assert.strictEqual(位置.パターン内ステップ, 通しステップ % パターンのステップ数)
        }
    })
    ...
    it('曲構成のとおりを選ぶと、節の繰り返し回数ぶんの長さになること', () => {
        ...
        assert.strictEqual(演奏が1周する長さ('曲構成のとおり', 楽曲).数値(), パターンのステップ数 * 5)
    })
})
