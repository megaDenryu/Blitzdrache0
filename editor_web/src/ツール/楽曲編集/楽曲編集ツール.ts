    ...
    private async _起動時に楽曲を読み込む(楽曲ID: 楽曲ID): Promise<void> {
        try {
            ...
            if (結果.種別 === '成功') {
                ...
                this.画面.インスペクター.永続化.状態文言を更新する('読込完了(起動時)', false)
            } else if (結果.種別 === '無し') {
                this.画面.インスペクター.永続化.状態文言を更新する('未保存: 初期楽曲を表示中', false)
            } else {
                this.画面.インスペクター.永続化.状態文言を更新する(`起動時読込失敗: ${結果.エラー.種別} ${結果.エラー.説明}`, true)
            }
        } catch (原因: unknown) {
            const メッセージ = 原因 instanceof Error ? 原因.message : String(原因)
            this.画面.インスペクター.永続化.状態文言を更新する(`起動時読込失敗: ${メッセージ}`, true)
        }
        this.同期.再構築する()
