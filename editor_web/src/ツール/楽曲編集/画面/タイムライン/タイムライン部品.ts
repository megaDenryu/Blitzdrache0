...
export class タイムライン部品 extends LV2HtmlComponentBase implements I配線可能<Iタイムライン配線> {
    ...
    public 再生中のカード位置を求める(位置: 再生位置 | null, 範囲: 演奏の範囲): カード位置 | null {
        return 再生中のカードを求める(this._カード列, 位置, 範囲)
    }
    ...
    public 再生位置を示す(再生中のカード: カード位置 | null, 繰り返し中か: boolean): void {
        const 見つかった添字 = 再生中のカード === null
            ? -1
            : this._カード部品一覧.findIndex((カード) => カード位置は同じか(カード.位置, 再生中のカード))
        const 新しい添字 = 見つかった添字 === -1 ? null : 見つかった添字
        if (新しい添字 !== this._再生中の添字) {
            if (this._再生中の添字 !== null) this._カード部品一覧[this._再生中の添字]?.再生中の印を示す(false)
            if (新しい添字 !== null) this._カード部品一覧[新しい添字]?.再生中の印を示す(true)
            this._再生中の添字 = 新しい添字
        }
        if (繰り返し中か !== this._繰り返し中か) {
            this._繰り返し中か = 繰り返し中か
            if (this._選択中の添字 !== null) this._カード部品一覧[this._選択中の添字]?.繰り返し中の印を示す(繰り返し中か)
        }
    }
