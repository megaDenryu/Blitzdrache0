...
// 演奏の操作帯と同じ行へ置いて名前のためだけの行を無くすため、格子の上の固定の行が持つ
// (参照: `_doc/設計/楽曲エディター.md`「層への写像」の画面の行)。
// 同じ値を2箇所で変えられる形にしないため、右サイドバーの設定側にはこの欄を置かない。
// 保存・読み込みの押しどころと状態文言は永続化の操作列(チャンク編集と共有する型)へ委ね、
// この行の右端に子として並べる。文書ぜんたいに効き、スクロールしても消えてはならない操作であるため
// 右サイドバーへは置かない(エディター制作スキル「固定とスクロールの規律」)。
export class 楽曲名の欄 extends LV2HtmlComponentBase implements I配線可能<I楽曲名の欄配線> {
    ...
    public readonly 永続化: 永続化の操作列 = 永続化の操作列.名前の行に置く形で作る()
    ...
    // この間に演奏や打ち込みで画面が作り直されると、打った文字が正本の文字列へ戻ってしまう。
