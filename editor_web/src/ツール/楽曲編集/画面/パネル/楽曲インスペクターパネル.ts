...
// 右サイドバーへ出す楽曲の設定。パターン・曲構成・ミキサー・トラック設定・コード進行を収める。
// 保存と読み込みの押しどころは楽曲名の欄が持つため、ここには含めない。文書ぜんたいに効く操作を、
// スクロールで隠れうる右サイドバーでなく上部の固定の行へ置くためである(参照: `_doc/設計/楽曲エディター.md`「層への写像」の画面の行)。
// 中央には編集の対象そのもの(打ち込みの格子)だけを残すため、設定はここへ集める(参照: `_doc/設計/ゲーム開発用エディター基盤.md`「判断14」)。
