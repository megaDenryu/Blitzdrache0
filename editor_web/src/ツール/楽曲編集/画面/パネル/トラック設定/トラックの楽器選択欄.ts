...
export class トラックの楽器選択欄 extends SelectC {
    ...
    // 選ばれた文字列はこの欄が出した選択肢のものしか来ないため、種類に合わない文字列は配線の誤りとして失敗させる。
    public 選ばれた楽器(): 楽器 {
        const 文字列 = this.getValue()
        for (const 楽器名 of トラックの種類から選べる楽器一覧を取得する(this._トラックの種類)) {
            if (楽器名 === 文字列 && トラックの楽器は妥当か(this._トラックの種類, 楽器名)) {
                return 楽器名
            }
        }
        throw new Error(`${this._トラックの種類}トラックで選べない楽器が選ばれています: ${文字列}`)
    }
}
