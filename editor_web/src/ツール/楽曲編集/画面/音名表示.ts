...
// ピッチクラス（0〜11）の音名表記（例: C, A#）を返す。
export function ピッチクラスの音名表記(ピッチクラス: number): string {
    const 音名 = 音名一覧[ピッチクラス]
    if (音名 === undefined) {
        throw new Error(`ピッチクラスとして扱えない値です: ${ピッチクラス}`)
    }
    return 音名
}
...
export function 音高番号の音名表記(音高番号: number): string {
    const ピッチクラス = ((音高番号 % 一オクターブの半音数) + 一オクターブの半音数) % 一オクターブの半音数
    const オクターブ = Math.floor(音高番号 / 一オクターブの半音数) - 1
    return `${ピッチクラスの音名表記(ピッチクラス)}${オクターブ}`
}
