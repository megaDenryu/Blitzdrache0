...
// 設定はインスペクターへ渡して右サイドバーへ出す(参照: `_doc/設計/ゲーム開発用エディター基盤.md`「判断14」)。
...
export class 楽曲編集画面 extends LV2HtmlComponentBase {
    ...
    public 表示を更新する(
        ...
        const 強調する小節 = 選択中のカード === null ? null : 選択中のカード.小節の番号

        if (パターン === undefined) {
            this.進行の帯.表示を更新する([], 0)
            this.トラック領域.表示を更新する(楽曲, undefined, ドラッグ見込み, 強調する小節)
        } else {
            const パターン和音一覧 = コード進行参照から和音一覧を解決する(パターン.進行の参照, 楽曲.独自進行一覧)
            this.進行の帯.表示を更新する(パターン和音一覧, パターンのステップ数を求める(パターン))
            this.トラック領域.表示を更新する(楽曲, パターン, ドラッグ見込み, 強調する小節)
        }
