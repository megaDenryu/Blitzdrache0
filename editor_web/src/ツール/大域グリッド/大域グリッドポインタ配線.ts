...
import type { 大域グリッド操作サービス } from './大域グリッド操作サービス.ts'
import type { 大域グリッド同期サービス } from './大域グリッド同期サービス.ts'
import { 大域グリッド筆致ハンドラ } from './大域グリッド筆致ハンドラ.ts'
import { 大域グリッドクリックハンドラ } from './大域グリッドクリックハンドラ.ts'
...
export function 大域ポインタとキー入力を配線する(
    ...
    const 筆致 = new 大域グリッド筆致ハンドラ(モデル, 状態, 部品, 同期)
    const クリック = new 大域グリッドクリックハンドラ(状態, 部品, 操作, 同期)
    const ビュー = 部品.三次元ビュー
