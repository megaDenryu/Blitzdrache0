...
export class タブ開閉サービス {
    ...
    public 大域世界を開く(): void {
        this.タブを開くか選ぶ(タブ識別子.大域世界().文字列(), 大域世界表示名, () => {
    ...
    public チャンクを開く(座標: チャンク座標): void {
        this.タブを開くか選ぶ(タブ識別子.チャンクから生成する(座標).文字列(), チャンク表示名を生成する(座標), () => {
    ...
    public 建物を開く(建物定義ID: 建物定義ID, 表示名: string): void {
        const 文字列 = タブ識別子.建物から生成する(建物定義ID).文字列()
        this.タブを開くか選ぶ(文字列, 表示名, () => this.建物編集ツールを作る(建物定義ID, 表示名, 文字列))
    }

    // 楽曲1件の編集タブを開く。
    public 楽曲を開く(楽曲ID: 楽曲ID, 表示名: string): void {
        const 文字列 = タブ識別子.楽曲から生成する(楽曲ID).文字列()
        this.タブを開くか選ぶ(文字列, 表示名, () => this.楽曲編集ツールを作る(楽曲ID, 表示名, 文字列))
    }

    public マテリアルを開く(): void {
        this.タブを開くか選ぶ(タブ識別子.マテリアル().文字列(), 'マテリアル', () => new マテリアル台帳ツール(this._保管庫))
    }

    public 使い方を開く(): void {
        const 文字列 = タブ識別子.使い方().文字列()
        this.タブを開くか選ぶ(文字列, 文字列, () => new 使い方タブ())
    }
    ...
    public タブ選択時処理(タブID: string): void {
        ...
        this._登録簿.前面のタブに合わせて選択表示する(タブ識別子.文字列から復元する(タブID))
    }
    ...
    private 楽曲編集ツールを作る(楽曲ID: 楽曲ID, 表示名: string, 文字列: string): 楽曲編集ツール {
        const ツール = new 楽曲編集ツール(楽曲ID, 表示名, this._保管庫)
        ツール.表示名の知らせの口.配線する(new タブ見出しを文書の表示名へ追随させる(this._シェル, 文字列))
        return ツール
    }

    // 建物の表示名も楽曲と同じく中央の欄で書き換えられるため、同じ届け先へ結ぶ。
    private 建物編集ツールを作る(建物定義ID: 建物定義ID, 表示名: string, 文字列: string): 建物編集ツール {
    ...
    private タブを開くか選ぶ(文字列: string, 表示名: string, ツールを作る: () => 実行可能ツール & HtmlComponentBase): void {
        if (this._シェル.タブが存在するか(文字列)) {
            this._シェル.タブを選択する(文字列)
            return
        }
        const ツール = ツールを作る()
        this.タブ管理.ツールを登録する(文字列, ツール)
        this._シェル.タブを追加する(文字列, 表示名, ツール)
    }
}
