...
export function 道の作り方と直し方の節を組み立てる(): DivC {
    return div({ class: セクション }).childs([
        ...
        div({ class: 手順リスト }).childs([
            p({ text: '1. 道作成モード(大域編集では広域道路作成モード)で地形をクリックすると、道の終わりに点が足される。' }),
            p({ text: '2. 道編集モード(大域編集では広域道路編集モード)へ切り替えると、点が地表に伏せた輪のマーカーで表示される。マーカーは画面上での大きさがそろうよう、カメラから遠いほど大きく描かれる。' }),
            p({ text: '3. マーカーを左ドラッグすると、その点が地形の上を追いかけて動く。ボタンを離した位置で確定し、Ctrl+Z1回でドラッグ前の位置へ戻る。' }),
            p({ text: '4. 道の面をクリックすると、そこにいちばん近い区間へ点が割り込む。曲がりを増やしたいときに使う。' }),
            p({ text: '5. マーカーをクリックすると点が選ばれ、道路パネルの「選択点を削除」で消せる。' }),
            p({ text: '6. マーカーは道作成モードと道編集モードのときだけ表示される。ほかのモードでは触れないため隠してある。' }),
        ]),
    ])
}
