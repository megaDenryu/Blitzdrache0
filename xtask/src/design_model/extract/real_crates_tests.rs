...
//! 実物のどの綴りも抽出器の保証範囲の外へ落ちないことを確かめる。
//! 実物で確かめるのは、受理する正規形を狭く書き下した条件が実物の綴りを落とさないことを、文字列の模型では保証できないためである。
...
fn 実物のタプル構造体が包む型へ保持する関係が出る() {
    let 結末 = 実物のcratesから結末を組む();
    let 表記一覧: Vec<String> = 結末.グラフ.関係一覧().iter().map(|関係| 関係.表記()).collect();
    for 期待 in ["blitz_esca::elapsed_time::経過時間 保持する 秒", "blitz_esca::tests::behavior_naming::現在地を名指すマスの一辺 保持する メートル"] {
