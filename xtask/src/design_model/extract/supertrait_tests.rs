...
use super::test_support::{原文から設計関係グラフと抽出の欠けを組む, 抽出できなかった理由の説明一覧, 関係の表記一覧};
...
fn 正本のトレイトの宣言の上位トレイトから下位型である関係が出る() {
    ...
    let 表記一覧 = 関係の表記一覧(&原文から設計関係グラフと抽出の欠けを組む(&[(正本のパス, 原文)]));
...
fn 上位トレイトを持たないマーカーの宣言からは関係が出ない() {
    let 表記一覧 = 関係の表記一覧(&原文から設計関係グラフと抽出の欠けを組む(&[(正本のパス, "pub trait M結果 {}\n")]));
...
fn 正本の外でマーカーを継ぐトレイトの宣言からも関係が出る() {
    ...
    let 表記一覧 = 関係の表記一覧(&原文から設計関係グラフと抽出の欠けを組む(&[("crates/blitz_esca/src/ontology.rs", 原文)]));
    assert_eq!(表記一覧, vec!["blitz_esca::ontology::M旅程 下位型である blitz_design::marker::M状態".to_string()]);
}

#[test]
fn 正本の外でマーカーを継がないトレイトの宣言からは関係が出ない() {
    let 表記一覧 = 関係の表記一覧(&原文から設計関係グラフと抽出の欠けを組む(&[("crates/blitz_esca/src/ontology.rs", "pub trait 表示器: Clone {}\n")]));
