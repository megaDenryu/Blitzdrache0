...
fn where節を持つトレイトの宣言は関係の欠落として数える() {
    let 結末 = 原文から結末を組む(&[(正本のパス, "pub trait M旅程: M状態 where Self: Sized {}\n")]);
    assert!(!関係の表記一覧(&結末).iter().any(|表記| 表記.contains("下位型である")), "{:?}", 関係の表記一覧(&結末));
    let 欠落 = 結末.関係を落とした抽出の欠落へ写す();
    assert_eq!(欠落.合計の件数(), 1, "{}", 欠落.説明());
    assert!(抽出できなかった理由の説明一覧(&結末).iter().any(|説明| 説明.contains("`where` 節")), "{:?}", 抽出できなかった理由の説明一覧(&結末));
}

#[test]
fn 宣言が1行に収まらないトレイトは関係の欠落として数える() {
    let 結末 = 原文から結末を組む(&[(正本のパス, "pub trait M旅程:\n    M状態\n{\n}\n")]);
    assert!(!関係の表記一覧(&結末).iter().any(|表記| 表記.contains("下位型である")), "{:?}", 関係の表記一覧(&結末));
    let 欠落 = 結末.関係を落とした抽出の欠落へ写す();
    assert_eq!(欠落.合計の件数(), 1, "{}", 欠落.説明());
    assert!(抽出できなかった理由の説明一覧(&結末).iter().any(|説明| 説明.contains("1行に収まっておらず")), "{:?}", 抽出できなかった理由の説明一覧(&結末));
}
