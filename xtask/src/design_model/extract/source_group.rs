...
impl 抽出対象のソース群 {
    ...
    pub fn 名指す型の識別子を求める(&self, 名指す型: &表記が名指す型, 参照元のパス: &Path, 参照元の行番号: usize) -> (設計概念の識別子, Option<抽出できなかった行>) {
        let 問い = 型の在り処の問い {
            型名: 名指す型.定義を探す名前(),
            参照元のパス,
            参照元の行番号,
        };
        let (識別子, 解決できなかった行) = self.型の識別子を求める(問い);
        (設計概念の識別子::Rustの項目として生成する(&識別子.モジュールパス, &名指す型.同一性の名前()), 解決できなかった行)
    }
}
