...
fn タプル構造体の位置で並べたフィールドから保持する関係が出る() {
    ...
    assert!(!結末.関係を落とした抽出の欠落へ写す().在るか(), "{}", 結末.関係を落とした抽出の欠落へ写す().説明());
}

#[test]
fn 関数の役割のnewtypeの宣言は明示して外し関係の欠落に数えない() {
    let 原文 = "pub struct M射影関数<状態: M状態, 射影DTO: M不変データ>(fn(&状態) -> 射影DTO);\n";
    let 結末 = 原文から結末を組む(&[("crates/blitz_design/src/function_role.rs", 原文)]);
    assert!(!関係の表記一覧(&結末).iter().any(|表記| 表記.contains("保持する")), "{:?}", 関係の表記一覧(&結末));
    assert_eq!(抽出できなかった理由の説明一覧(&結末).len(), 1, "{:?}", 抽出できなかった理由の説明一覧(&結末));
    assert!(!結末.関係を落とした抽出の欠落へ写す().在るか(), "{}", 結末.関係を落とした抽出の欠落へ写す().説明());
}
...
fn 保証範囲の外のフィールドの型は関係の欠落として数える() {
    ...
    assert!(!関係の表記一覧(&結末).iter().any(|表記| 表記.contains("保持する")), "{:?}", 関係の表記一覧(&結末));
    let 欠落 = 結末.関係を落とした抽出の欠落へ写す();
    assert_eq!(欠落.合計の件数(), 1, "{}", 欠落.説明());
    assert!(抽出できなかった理由の説明一覧(&結末).iter().any(|説明| 説明.contains("Rustのパスとして読めない")), "{:?}", 抽出できなかった理由の説明一覧(&結末));
}

#[test]
fn 保証範囲の外の本体の行は関係の欠落として数える() {
    let 結末 = 原文から結末を組む(&[(旅行者のパス, "pub enum 旅行者の意図 {\n    静止 = 1,\n}\n")]);
    let 欠落 = 結末.関係を落とした抽出の欠落へ写す();
    assert_eq!(欠落.合計の件数(), 1, "{}", 欠落.説明());
    assert!(
        抽出できなかった理由の説明一覧(&結末).iter().any(|説明| 説明.contains("フィールドの記述でも属性でも選択肢の名前でもない")),
        "{:?}",
        抽出できなかった理由の説明一覧(&結末)
    );
}
...
fn 属性の行と選択肢の名前だけの行は欠落にも抽出できなかった行にも数えない() {
    let 結末 = 原文から結末を組む(&[(旅行者のパス, "pub enum 歩行の失敗 {\n    #[error(\"速さが負である\")]\n    速さが負である,\n}\n")]);
    assert!(!結末.関係を落とした抽出の欠落へ写す().在るか(), "{}", 結末.関係を落とした抽出の欠落へ写す().説明());
    assert!(抽出できなかった理由の説明一覧(&結末).is_empty(), "{:?}", 抽出できなかった理由の説明一覧(&結末));
}
