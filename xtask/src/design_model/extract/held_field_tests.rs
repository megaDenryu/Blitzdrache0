...
fn 宣言の型引数と一致するフィールドの型からは関係を出さず抽出できなかった行として数える() {
    ...
    let 結果 = 原文から結果を組む(&[("crates/blitz_esca/src/ontology.rs", 原文)]);
    let 表記一覧 = 関係の表記一覧(&結果);
...
fn マーカーを1つも実装しない型も適用範囲のクレートなら節点になる() {
    let 表記一覧 = 概念の表記一覧(&原文から結果を組む(&[("crates/blitz_esca/src/walking_direction.rs", "pub struct 歩行方向 {\n    東: f32,\n}\n")]));
    assert!(表記一覧.contains(&"blitz_esca::walking_direction::歩行方向".to_string()), "{表記一覧:?}");
}

#[test]
fn 適用範囲の外のクレートの型は節点にならない() {
    let 表記一覧 = 概念の表記一覧(&原文から結果を組む(&[("crates/blitz_render/src/draw_bundle.rs", "pub struct 描画束 {\n    数: u32,\n}\n")]));
...
fn 関数の役割の型自身の宣言は明示して外し関係の欠落に数えない() {
    ...
    let 結果 = 原文から結果を組む(&[("crates/blitz_design/src/function_role.rs", 原文)]);
...
fn 保証範囲の外のフィールドの型は関係の欠落として数える() {
    let 結果 = 原文から結果を組む(&[(旅行者のパス, "pub struct 旅行者 {\n    足跡: [メートル; 3],\n}\n")]);
    assert!(!関係の表記一覧(&結果).iter().any(|表記| 表記.contains("保持する")), "{:?}", 関係の表記一覧(&結果));
...
fn 保証範囲の外の本体の行は関係の欠落として数える() {
    let 結果 = 原文から結果を組む(&[(旅行者のパス, "pub enum 旅行者の意図 {\n    静止 = 1,\n}\n")]);
    let 欠落 = 結果.関係を落とした抽出の欠落へ写す();
    assert_eq!(欠落.合計の件数(), 1, "{}", 欠落.説明());
    assert!(
...
fn 属性の行と選択肢の名前だけの行は欠落にも抽出できなかった行にも数えない() {
    let 結果 = 原文から結果を組む(&[(旅行者のパス, "pub enum 歩行の失敗 {\n    #[error(\"速さが負である\")]\n    速さが負である,\n}\n")]);
    assert!(!結果.関係を落とした抽出の欠落へ写す().在るか(), "{}", 結果.関係を落とした抽出の欠落へ写す().説明());
