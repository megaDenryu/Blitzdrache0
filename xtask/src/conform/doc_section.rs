...
//! 対象は `_doc/` 配下の .md と README.md から、ログ型を除いたもの。ログ型は
//! `_doc/開発スレッド/`(逐語収録の追記専用)・`_doc/ブレスト/`(壁打ちと外部レビューの
//! 持ち込み素材。改稿しない)・`_doc/ゲーム開発用エディター計画/`(モックアップと書きかけの
//! 持ち込み素材)・`_doc/検査報告/`(ローラー検査で走らせた検査員の報告全文の追記専用)の
//! 4つである。前の3つは過去の発言や外部の文章を原文のまま持つため、`_doc/検査報告/`は
//! 検査員が他の文書の節の名前や検査した識別子の文字列を鉤括弧のまま引くため、
//! どちらも同一文書内の参照として判定できない。
...
const ログ型ディレクトリ一覧: [&str; 4] = ["開発スレッド", "ブレスト", "ゲーム開発用エディター計画", "検査報告"];

fn 対象文書か(パス: &Path) -> bool {
    !パス.components().any(|部品| matches!(部品, Component::Normal(名前) if ログ型ディレクトリ一覧.iter().any(|ログ型| 名前 == *ログ型)))
}
    ...
    for (行番号, 行) in 内容.lines().enumerate() {
        ...
        for 節名 in 節名を抽出する(行) {
            if !見出し一覧.iter().any(|見出し| 見出し.contains(&節名)) {
                違反一覧.push(違反::行単位(パス.to_path_buf(), 行番号 + 1, format!("同一文書内に実在しない節への参照: {節名}")));
            }
        }
    }
    違反一覧
...
mod tests {
    ...
    fn ログ型ディレクトリは対象外() {
        ...
        assert!(!対象文書か(Path::new("_doc/ブレスト/x.md")));
        assert!(!対象文書か(Path::new("_doc/ゲーム開発用エディター計画/x.md")));
        assert!(!対象文書か(Path::new("_doc/検査報告/x.md")));
