...
/// 材質のシェーダーが読んでよいのは影・ヘッダ・方向光レコード列の3つであり、局所光レコード列とクラスタの2本は局所光の問い合わせの内側だけが読む。
/// 遠方環境は拡散照度・鏡面畳込み・反射率積分表、局所可視度は画素ごとの可視度の画像である。
///
/// 正本を4つに分けるのは、どれも「その番号を宣言してよい唯一のモジュール」が別だからである。
/// とりわけ局所光とクラスタの3つを`local_light_binding.slang`へ分けているのは、材質のシェーダーがその名前を見られないことが先行裁定の実効だからであり、この表がその分割に追随していないと検査が素通りする。
