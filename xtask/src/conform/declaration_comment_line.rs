...
//! 説明の注釈の行数の検査は、呼び出し元が当てるかどうかを選ぶ。Graphiteの生成物には当てない(2026-09-26のオーナー裁定。Issue #187)。
//! 生成物の説明の注釈は生成器が書き、手編集を禁じられていて実装者が直せないためである。コメントだけの行の検査は生成物にも当てる。
...
/// 宣言の前へ2行以上積まれた説明の注釈を違反にするかどうか。コメントだけの行の検査はこの指定に依らず当てる。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum 説明の注釈の行数の検査 {
    当てる,
    当てない,
}

pub fn 宣言の間のコメント行を検査する(パス: &Path, 内容: &str, 説明の注釈の行数: 説明の注釈の行数の検査) -> Vec<違反> {
    let 内訳一覧 = 行ごとの内訳(内容);
    ...
    for (添字, コード行) in コード行一覧.iter().enumerate() {
        ...
        if let Some(文言) = 宣言の前のコメントの違反の文言を求める(&内訳一覧, 添字, 説明の注釈の行数) {
...
fn 宣言の前のコメントの違反の文言を求める(内訳一覧: &[行の内訳], 添字: usize, 説明の注釈の行数: 説明の注釈の行数の検査) -> Option<String> {
    if !内訳一覧.get(添字).is_some_and(行の内訳::説明の注釈だけの行か) {
        return Some("宣言の間にコメントだけの行が在る。説明は宣言と同じ行の末尾へ書く".to_string());
    }
    if 説明の注釈の行数 == 説明の注釈の行数の検査::当てない {
        return None;
    }
