...
//! 面の曲げ拘束の退化の下限2つ(共有する辺の長さ・三角形の面積ベクトルの長さ)は、CPU正本(`blitz_sim::xpbd`)とGPUの写し(`shaders/xpbd_bending_projection.slang`)を突き合わせる。
//! ずれると同じ形を片側だけ退化として扱う。突き合わせはどちらも同じ次元の裸の数で行う。CPU正本は単位の型の定数の材料として裸の数の定数を名前付きで置き、
//! GPUの写しは辺を長さの2乗でなく長さで比べる形に揃えた(2乗の綴り1e-12と長さの綴り1e-6を検査が読み替える形は採らない)。
