//! シェーダーの原文の形の台帳。どのファイルにどの文字列が在るべきで、どの文字列が在ってはならないかを、守るものの説明と共に並べる。
...
//! 文字列で固定するのは、GPUの読み戻しでこの経路を通す題材は静止角がちょうど半周のときに2つの三角形が重なり、
//! 畳みでなくatan2の分選択肢の丸めを測ることになるためである(実測でこの形を固定できない)。

/// 1ファイルの原文の形の契約。在るべき文字列は全部がコメントでない行に在ること、在ってはならない文字列は1つも無いことを求める。
