...
//!
//! ここに集めるのは、依存の白リスト・依存の機能一覧の台帳・文書の節参照・正本と写しが同じ値を持つべき定数の一致・束縛番号の正本と写しの一致・報告の行の綴りの契約・ファイル名らしい綴りの重複・材質見本の宣言の写しの一致・シェーダー定数の宣言箇所・照明問い合わせのセットの宣言箇所・廃止した語である。
...
use super::{
    ...
    shader_uniform_alias, single_lighting_slot_write, wording_contract, workspace_dependency_features,
};
