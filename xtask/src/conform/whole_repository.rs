...
//!
//! ここに集めるのは、依存の白リスト・文書の節参照・正本と写しが同じ値を持つべき定数の一致・報告の行の綴りの契約・ファイル名らしい綴りの重複・材質見本の宣言の写しの一致・シェーダー定数の宣言箇所・照明問い合わせのセットの宣言箇所・廃止した語である。
...
use super::{
    ...
    removed_slot_material_set, removed_view_pass_lighting, sample_bodies_consistency, shader_constant, shader_uniform_alias,
...
pub fn 集める() -> Result<Vec<違反>, String> {
    ...
    違反一覧.extend(shader_constant::全定数を検査する().map_err(|誤り| format!("定数の写し検査に失敗した: {誤り}"))?);
    違反一覧.extend(duplicate_file_literal::全ファイルを検査する().map_err(|誤り| format!("ファイル名らしい綴りの重複の検査に失敗した: {誤り}"))?);
    違反一覧.extend(wording_contract::全綴りを検査する().map_err(|誤り| format!("報告の行の綴りの契約の検査に失敗した: {誤り}"))?);
    違反一覧.extend(sample_bodies_consistency::全宣言を検査する().map_err(|誤り| format!("材質見本の宣言の写し検査に失敗した: {誤り}"))?);
