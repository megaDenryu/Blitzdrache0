...
//!
//! ここに集めるのは、依存の白リスト・依存の機能一覧の台帳・文書の節参照・正本と写しが同じ値を持つべき定数の一致・束縛番号の正本と写しの一致・シェーダーの原文に在るべき綴りと在ってはならない綴り・報告の行の綴りの契約・ファイル名らしい綴りの重複・材質見本の宣言の写しの一致・シェーダー定数の宣言箇所・照明問い合わせのセットの宣言箇所・廃止した語・型ごとの分量の台帳・検証の出力が載る木を指す綴りの置き場・設計オントロジーの法則である。
//! どれも複数のファイルを突き合わせて初めて判定でき、走査中の1ファイルからは答えが出ない。

use super::error::規約検査の破れ;
use super::violation::違反;
use super::{
    dependency_whitelist, depth_contract, design_ontology, doc_section, duplicate_file_literal, free_function_whole_type, lighting_query_declaration, reload_without_device_wait, removed_object_uniform, removed_slot_material_set,
...
pub fn 複数ファイルを横断する検査の違反一覧を集める() -> Result<Vec<違反>, 規約検査の破れ> {
    ...
    違反一覧.extend(design_ontology::全ファイルを検査する()?);
    Ok(違反一覧)
}
