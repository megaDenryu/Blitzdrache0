...
fn 内容から違反を集める(パス: &std::path::Path, 内容: &str) -> Vec<違反> {
    ...
    for 行 in 依存節の行一覧を切り出す(内容) {
        match 依存節の行::読み取る(行) {
            依存節の行::宣言でない => {}
            依存節の行::一行で閉じていない { 名前 } => 違反一覧.push(違反::ファイル単位(パス.to_path_buf(), format!("依存{名前}の宣言が1行で閉じていない。機能一覧を読み取れないため、1行で書く"))),
            依存節の行::宣言(宣言) => {
                見つけた依存名一覧.push(宣言.名前().to_string());
                match 機能台帳.iter().find(|項| 項.依存名 == 宣言.名前()) {
                    Some(項) => 違反一覧.extend(宣言.台帳と食い違う点一覧(項).into_iter().map(|説明| 違反::ファイル単位(パス.to_path_buf(), 説明))),
                    None => 違反一覧.push(違反::ファイル単位(パス.to_path_buf(), format!("機能台帳に未登録の依存: {}", 宣言.名前()))),
                }
            }
        }
    }
    for 項 in 機能台帳.iter().filter(|項| !見つけた依存名一覧.iter().any(|名前| 名前 == 項.依存名)) {
        違反一覧.push(違反::ファイル単位(パス.to_path_buf(), format!("機能台帳にあるのに宣言が無い依存: {}。依存を消したなら台帳からも消す", 項.依存名)));
    }
    違反一覧
}

fn 依存節の行一覧を切り出す(内容: &str) -> impl Iterator<Item = &str> {
    内容.lines().skip_while(|行| 行.trim() != 依存節の見出し).skip(1).take_while(|行| !行.trim().starts_with('['))
}
