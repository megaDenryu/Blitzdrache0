//! 機能一覧の文字列の読み取り。受け取るのは依存の宣言の右辺の文字列、返すのは明示された機能の名前を宣言の順に並べた一覧である。
//!
//! 分けているのは、`default-features = false` の文字列が `features = ` を部分文字列として含み、素朴に探すと
//! 既定機能の宣言を機能一覧と読み違えるためである。この読み違えは検査を素通りさせるだけで何の症状も出さない。
//! 何が違反かを持つ側から、文字列の解き方を持つ側を分ける。

/// 機能一覧の宣言の始まり。角括弧まで含めるのは、`default-features = false` と文字列を分けるためである。
