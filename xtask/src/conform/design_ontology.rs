...
//! 警告(終了コードを変えない報告)にする規則: 設計解釈マーカーを実装した型の実装(固有の `impl`・トレイトの実装・`unsafe impl`・設計解釈マーカーの実装)は、その型の定義と同じファイルに置く(`marker_impl_placement.rs`)。設計解釈マーカーが目に入らない場所で実装を書こうとしている書き手に気づかせるためであり、同じクレートの実装の対象の名前で照らす近似である。
