...
//! 自己変更の禁止の探索は、実装の対象の型をパスの修飾・明示の取り込み・glob の取り込みから定義へ結び付け、取り込み元の `use`(再公開を含む)を1段たどる(`implementation_binding.rs`)。型名が一致するのに結び付けられない実装と、`use ... as`・`type` の別名を通した実装は、自己変更の根拠を持てば違反にする。
//! 型引数の境界が入れ子の `<` を含むジェネリックな `impl`(`impl<T: Into<Vec<u8>>> 型<T>`)は、山括弧の対応を数えて型引数を分けるため読む(`line_matching.rs` の `先頭の型引数を分ける`)。
//! 入れ子の波括弧の `use`、2段以上の再公開、フィールドの型の中に間接的に含まれる内部可変性、rustfmt が整形しない書き方(`#[rustfmt::skip]` の中で見出しを複数行へ崩した形)は保証範囲の外である。
