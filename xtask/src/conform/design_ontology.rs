...
//!
//! 検査する規則: `Mコマンド` は enum である。`M規則` は struct である。`M結果` を実装する型は enum である(正本 `blitz_design` の `marker.rs` が持つ完全に修飾した `std::result::Result` への包括の実装だけが定義をたどらない特例)。`M不変データ` またはデータの役割(`Mコマンド`・`Mイベント`・`M規則`・`M状態`・`M解釈前の入力`・`M生入力`・`M中間入力`・`M関数への入力`・`M観測`)を実装する型の定義は
...
//! 実装の位置のモジュールの直下に同名の定義が複数あるときと、同じファイルの局所か別の `mod` の定義しか無く `use` も無いときは、一意に決まらないとして違反にする。同じ型が同じ軸の排他の分類(`M不変エンティティ` と `M可変エンティティ`、`MParameter` と `MOptions`、`M生入力` と `M中間入力`)を同時に名乗ることは違反にする。`M解釈前の入力` を実装する型が `M生入力` と `M中間入力` のどちらも名乗らないことは違反にする。
...
mod 生成物の中の実装の置き場の試験;
#[path = "design_ontology/設計解釈マーカーの一覧.rs"]
