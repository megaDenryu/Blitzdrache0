...
//!
//! 検査する規則: `Mコマンド` は enum である。`M規則` は struct である。`MDTO`・`Mイベント`・`M規則` の定義は参照・生ポインタ・内部可変性を持たない。
//! `Mコマンド`・`Mイベント`・`M規則` の固有の `impl` は `&mut self` メソッドを持たない。定義が見つからない実装は違反にする。
//! 関数役割の宣言(`const _: M射影関数<...> = ...` 等)の右辺は対応する確認関数の呼び出しで始まる。
//! 検査しない規則(コンパイラが強制する): `M状態` と `M入力` の `MDTO` 実装、`MDTO` と `Mイベント` の `Clone`、確認関数を通した宣言の型引数の境界、`遷移結果` の型引数の境界。
//! 保証範囲: 検査は Rust の型意味論でなく構文パターン(`impl トレイト for 型` の行と `struct`/`enum` の定義ブロック)に対して行う。同名の型が複数のクレートにあるときは
//! 実装と同じファイルの定義を採り、決まらなければ違反にする。固有の `impl` の探索は採った定義と同じクレートのファイルだけを見る。
//! ジェネリックな `impl` や、フィールドの型の中に間接的に含まれる内部可変性は保証範囲の外である。

mod function_role_declaration;
#[cfg(test)]
