...
pub(super) const 接点一覧: [接点; 10] = [
    接点 {
        ...
        期待する文字列: "depth_compare_op(vk::CompareOp::GREATER)",
    },
    接点 {
        契約: &カメラ深度,
        項目: "色描画の近い比較",
        パス: "crates/blitz_render/src/vulkan/pipeline/color_pass_depth.rs",
        期待する文字列: "Self::より近いものを描いて書く => vk::CompareOp::GREATER",
    },
    接点 {
        契約: &カメラ深度,
        項目: "色描画の同値込み比較",
        パス: "crates/blitz_render/src/vulkan/pipeline/color_pass_depth.rs",
        期待する文字列: "Self::近いか同値を描いて書く => vk::CompareOp::GREATER_OR_EQUAL",
    },
    接点 {
        契約: &カメラ深度,
        項目: "色描画の等値比較",
        パス: "crates/blitz_render/src/vulkan/pipeline/color_pass_depth.rs",
        期待する文字列: "Self::等値だけを描いて書かない => vk::CompareOp::EQUAL",
    },
    接点 {
        契約: &カメラ深度,
        項目: "粒子の比較",
        パス: "crates/blitz_render/src/vulkan/particles/draw_pipeline/assemble.rs",
        期待する文字列: "depth_compare_op(vk::CompareOp::GREATER_OR_EQUAL)",
    },
    接点 {
        契約: &カメラ深度,
        項目: "空の比較",
        パス: "crates/blitz_render/src/vulkan/pipeline/sky_pipeline/assemble.rs",
        期待する文字列: "depth_compare_op(vk::CompareOp::EQUAL)",
    },
    接点 {
        契約: &カメラ深度,
        項目: "CPU奥行き復元",
    ...
    接点 {
        ...
        期待する文字列: "projection.nearClip + depth * (projection.farClip - projection.nearClip)",
    },
    接点 {
        契約: &カメラ深度,
        項目: "時間再構成の奥行き復元",
        パス: "shaders/temporal_reconstruction_setting.slang",
        期待する文字列: "setting.nearClip + depth * (setting.farClip - setting.nearClip)",
    },
    接点 {
        契約: &カメラ深度,
        項目: "空の逆投影に使う遠面",
        パス: "shaders/sky_frame.slang",
        期待する文字列: "float4(ndc, 0.0, 1.0)",
    },
];
