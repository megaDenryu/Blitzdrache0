...
//! 与えると答えるのは、実装が可変参照か可変の借用を対象にするか名前が境界に当たったときと、受け手か引数が自分の型への可変参照であるときと、関数の署名の境界の可変の借用を与えるトレイトの型引数が `Self` か型名を指すとき(`fn 変える(mut 相手: impl DerefMut<Target = Self>)`)である。
...
pub struct 自己変更を問う対象<'a> {
    pub 型名: &'a str,
    pub 可変参照を対象にするか: bool, // 対象の表記に可変参照か可変の借用を与えるトレイトが現れるか、名前が境界に当たったとき真。真なら値で受ける `self` の関数も自己変更を与える
