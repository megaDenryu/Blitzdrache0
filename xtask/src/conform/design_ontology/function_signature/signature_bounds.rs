//! 関数の署名の境界を読んだ値と、名前がそこに現れるかの問い。関数の署名の境界とは、関数の型引数の並びの境界と、関数の `where` 句と、引数の `impl Trait` と `dyn Trait` の3か所のことである。
...
//! 引数の `dyn Trait`(`Box<dyn DerefMut<Target = Self>>`)も `impl Trait` と同じ働きをするため、同じく照らす。
...
// 引数の型の中でトレイトを書く位置を始める予約語。
const トレイトを書く位置を始める予約語一覧: [&str; 2] = ["impl", "dyn"];

/// 関数の署名の境界。型引数の並びの境界と `where` 句と、引数の `impl Trait` と `dyn Trait` の表記を持つ。
pub struct 関数の署名の境界 {
    型引数と境界の句: 実装の境界,
    引数の型の中のトレイトの境界: String, // 引数の型の中の `impl` か `dyn` から、その要素の終わりまでの表記を空白で繋いだもの
}

impl 関数の署名の境界 {
    pub(super) fn 読む(名前と型引数: &str, 引数一覧: &[String], 引数の並びより後ろ: &str) -> Self {
        let 型引数 = 名前と型引数.find('<').map_or("", |位置| 先頭の型引数を分ける(&名前と型引数[位置..]).0);
        let 引数の型の中のトレイトの境界 = 引数一覧
            .iter()
            .flat_map(|引数| {
                トレイトを書く位置を始める予約語一覧
                    .into_iter()
                    .flat_map(move |予約語| 識別子として現れる位置一覧(引数, 予約語).into_iter().map(move |位置| &引数[位置..位置 + 要素の終わり(&引数[位置..])]))
            })
            .collect::<Vec<_>>()
            .join(" ");
        Self {
    ...
    /// 3か所の可変の借用を与えるトレイトの型引数が、名前を指すか(`U: DerefMut<Target = 規則>`・`impl AsMut<Self>`・`Box<dyn DerefMut<Target = Self>>`)。
...
mod tests {
    ...
    #[test]
    fn 型引数の並びの境界とwhere句と引数のimplとdynの境界を読み戻り値の型は読まない() {
