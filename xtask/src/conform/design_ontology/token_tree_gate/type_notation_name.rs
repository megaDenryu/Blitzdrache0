//! 字句の木の型の表記から、実装の対象の型の中身とパスの最後の名前を読む純粋な関数。受け取るのは型の表記の字句、返すのは外した後の字句か、1つの識別子で書ける名前か名前で読めないときの無しである。
//! 実装の対象の型は、1つの型を囲む丸括弧と、参照(`&`・`&'a`・`&mut`)と、`Pin<&mut ..>` を、読み口(`impl_syntax/target_type.rs`)と同じ順で外してから表記と名前を読む。外し方は字句の木の群と記号で読み、読み口の文字列の読みを流用しない。
//! 可変参照を対象にするかは、外す前の字句のどの深さにでも `&`(寿命を挟んでよい)と `mut` の並びがあるかで答える。群の中へは入れ子ごと入り、`Option<&mut 規則>` と `(&mut 規則, u8)` も数える。
...
/// 対象の型の字句から、囲む丸括弧と参照と `Pin<&mut ..>` を外した型の字句と、どの深さにでも可変参照(`&mut X`・`&'a mut X`・`Pin<&mut X>`・`Option<&mut X>`)が現れるか。
pub fn 対象の型の中身(字句一覧: &[TokenTree]) -> (Vec<TokenTree>, bool) {
    let 外した = 丸括弧を外す(字句一覧);
    let 参照先: &[TokenTree] = match 参照を外す(&外した).or_else(|| pinの中身(&外した).and_then(|中身| 参照を外す(中身).filter(|(_, 可変か)| *可変か))) {
        Some((参照先, _)) => 参照先,
        None => &外した,
    };
    (丸括弧を外す(参照先), 可変参照を含むか(字句一覧))
}

// 字句の並びのどの深さにでも、可変参照の型の書き出し(`&` と、寿命を挟んで `mut`)があるか。群の中へは入れ子ごと入る。
fn 可変参照を含むか(字句一覧: &[TokenTree]) -> bool {
    (0..字句一覧.len()).any(|位置| 参照を外す(&字句一覧[位置..]).is_some_and(|(_, 可変か)| 可変か)) || 字句一覧.iter().any(|字句| matches!(字句, TokenTree::Group(群) if 可変参照を含むか(&群.stream().into_iter().collect::<Vec<_>>())))
}
