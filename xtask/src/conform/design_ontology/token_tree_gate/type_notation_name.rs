...
/// 対象の型の字句から、囲む丸括弧と参照と `Pin<&mut ..>` を外した型の字句と、可変参照(`&mut X`・`&'a mut X`・`Pin<&mut X>`)を対象にするか。
pub fn 対象の型の中身(字句一覧: &[TokenTree]) -> (Vec<TokenTree>, bool) {
    let 外した = 丸括弧を外す(字句一覧);
    let (参照先, 可変参照か): (&[TokenTree], bool) = match 参照を外す(&外した).or_else(|| pinの中身(&外した).and_then(|中身| 参照を外す(中身).filter(|(_, 可変か)| *可変か))) {
        Some((参照先, 可変か)) => (参照先, 可変か),
        None => (&外した, false),
    };
    (丸括弧を外す(参照先), 可変参照か)
}
