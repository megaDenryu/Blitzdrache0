...
//! バイト順マークを除いた先頭が `#!` で3文字目が `[` でないファイルは、数えずにシバンで始まると答える。rustc と検査器では `#!` の行の読み方が分かれうるためである。
//! rustc は `#!/usr/bin/env run` を読み飛ばし、`#! [..]` と、`#!` の後ろで改行した `[..]` を内側の属性と読むが、proc-macro2 は `#!` の行を読み飛ばさず、読み口はどちらの読み方もしない。そのため `#!` の直後に空白や改行を置かず `#![` と書くことを求めて違反にする(`readable_form_assertion.rs`)。
