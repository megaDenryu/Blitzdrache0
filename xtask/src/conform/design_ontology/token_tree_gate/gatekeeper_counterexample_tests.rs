...
//! 反例は、識別子の文字が `is_alphanumeric` でない名前のマクロの呼び出し(`通す·!`)・コメントや改行を挟んだ予約語(`impl/**/丙`・`impl` の直後の改行・`use` の直後の改行)・同じ行の2つ目の実装・行の途中の `macro_rules!`・`$` の後ろの実装・マクロの引数の前置きの後ろの宣言・式の中の実装である。
