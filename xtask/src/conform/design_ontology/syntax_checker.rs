...
impl クレート構文検査 {
        ...
        for トレイト in [オントロジートレイト::MDTO].into_iter().chain(オントロジートレイト::データの役割一覧()) {
            for 型 in self.トレイト実装型一覧(トレイト) {
                let 定義のパス = match self.型の定義ブロック(&型) {
                    ...
                    定義ブロックの結果::見つからない | 定義ブロックの結果::複数ある | 定義ブロックの結果::別名で解決できない => 型.パス.clone(),
                };
                if 済み.insert((モジュールパス(&定義のパス), 型.型名.clone())) {
                    対象一覧.push((トレイト, 型));
                }
            }
        }
    ...
    pub fn 型種別を満たしているか(&self, 種別: Rust型種別, 型: &トレイト実装型) -> bool {
        ...
        match self.型の定義ブロック(型) {
            ...
            定義ブロックの結果::見つからない | 定義ブロックの結果::複数ある | 定義ブロックの結果::別名で解決できない => false,
        }
    }
    ...
    pub fn dto純粋性違反一覧(&self, 型: &トレイト実装型) -> Vec<String> {
        let 定義 = match self.型の定義ブロック(型) {
            ...
            定義ブロックの結果::見つからない | 定義ブロックの結果::複数ある | 定義ブロックの結果::別名で解決できない => return Vec::new(),
        };
        let mut 違反 = Vec::new();
        if 定義.contains('&') {
