...
impl クレート構文検査 {
    ...
    /// 型名は対象の型のパスの最後の名前であり、名前の閉包の種と同じ名前である。先頭の識別子で読むと `::jibun::規則` の名前を空と読んで種から落とすためである。
    /// 名前を1つの識別子で読めない実装(`[型]`・`(型, u8)`)は定義をたどれないため集めず、`impl_syntax/target_form.rs` がその実装を違反にする。
