...
//! `::blitz_design::` の絶対パス・`blitz_design :: MDTO` のようなパスの中の空白)で書くと認識できない実装が書け、純粋データ規約などの法則を黙って迂回できるため、
...
impl クレート構文検査 {
    ...
    pub fn 設計マーカーを別名で取り込んでいないこと(self) -> Self {
        self.行単位の違反を足す(
            |_, 行一覧| blitz_designの別名の行一覧(行一覧),
            "設計オントロジー: 設計マーカーは `impl MDTO for 型` または `impl blitz_design::MDTO for 型` の形だけで実装する。別名で取り込むと conform が実装を認識できない",
        )
    }

    /// 設計マーカーの名前を含むトレイトの実装の行が、`impl マーカー名 for 型` または `impl blitz_design::マーカー名 for 型` のどちらかであること。
    pub fn 設計マーカーの実装が正規形であること(self) -> Self {
        self.行単位の違反を足す(
            |_, 行一覧| 正規形でないマーカー実装の行一覧(行一覧),
            "設計オントロジー: 設計マーカーの実装は `impl マーカー名 for 型` または `impl blitz_design::マーカー名 for 型` の形だけで書く。再公開・絶対パス・パスの中の空白のどれも、構文検査が実装として認識できない",
        )
    }

    /// `blitz_design` の外のファイルが `blitz_design` を公開の `use` で再公開していないこと。`blitz_design` 自身の `lib.rs` が自分のモジュールを再公開するのは対象でない。
    pub fn 設計マーカーを再公開していないこと(self) -> Self {
        self.行単位の違反を足す(
            |パス, 行一覧| if blitz_design自身のファイルか(パス) { Vec::new() } else { blitz_designの再公開の行一覧(行一覧) },
            "設計オントロジー: 設計マーカーを再公開すると、実装の行が正規形でなくなり構文検査が実装を認識できない。`blitz_design` から直接取り込む",
        )
    }

    /// 設計マーカーの実装がファイルの中の `mod 名 { ... }` の中に無いこと。宣言だけの `mod 名;` は対象でない。
    pub fn 設計マーカーの実装が波括弧付きのモジュールの中に無いこと(self) -> Self {
        self.行単位の違反を足す(
            |_, 行一覧| 波括弧付きのモジュールの中のマーカー実装の行一覧(行一覧),
            "設計オントロジー: 設計マーカーの実装をファイルの中の `mod` の中へ置かない(構文検査は型の定義をファイル単位で探すため、同じファイルの別の `mod` の同名の型と取り違える)",
        )
    }
...
fn 正規形でないマーカー実装の行一覧(行一覧: &[String]) -> Vec<usize> {
    let 名前一覧: Vec<&str> = [オントロジートレイト::MDTO].into_iter().chain(オントロジートレイト::データの役割一覧()).map(オントロジートレイト::名前).collect();
