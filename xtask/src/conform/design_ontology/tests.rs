...
/// 正規形の説明関数だけを連ねた違反の説明の一覧。正規形の検査は、`use … as` の別名を意図して置く他の試験のソースの断片に当たるため、上の入口へ混ぜず、この専用の入口で確かめる。
/// 走査範囲の外のトレイトの台帳の検査も同じ理由で上の入口へ混ぜない(他の試験の断片が `impl std::fmt::Display for 規則` のように走査範囲の外のトレイトを実装するためである)。その検査は `external_trait_ledger_tests.rs` が専用の入口で確かめる。
...
/// 台帳を与えた自己変更の禁止の説明関数だけを連ねた違反の説明の一覧。名前が当たった別の型の実装を台帳で除くことと、台帳の行の陳腐化を試験が確かめるために使う。
pub(super) fn 台帳を与えた自己変更の違反の説明一覧(ソース一覧: Vec<(PathBuf, Vec<String>)>, 台帳: &名前が当たった別の型の実装の台帳) -> Vec<String> {
    クレート構文検査::生成する(ソース一覧).すべての不変データが可変参照メソッドを持たないこと(台帳).違反一覧().into_iter().map(|違反| 違反.説明).collect()
}
