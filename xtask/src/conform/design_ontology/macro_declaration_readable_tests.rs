//! マクロの本体の中の宣言の読み口の試験。`macro_rules!` の本体はトークン木であるため、行の途中の `impl`・`type`・`use` が例外なしに読み口の読まなかった予約語として違反になることと、
...
#[test]
fn macro_rulesの本体に書いたimplとtypeとuseを読み口が読まなかった予約語として違反にする() {
    let ソース一覧 = vec![原文(
        "crates/a/src/x.rs",
    ...
    for (説明, 予約語) in 説明一覧.iter().zip(["impl", "impl", "type", "use"]) {
        assert!(説明.contains(&format!("項目を始めうる `{予約語}` が1個あるが、検査器の読み口が読んだ宣言は0個である")), "{説明}");
    }
}
