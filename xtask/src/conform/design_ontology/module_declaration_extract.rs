...
//! 波括弧の深さはコードだけの行の開き括弧と閉じ括弧を数えて持ち回る。宣言が別の波括弧の内側に置かれているかを一致の検査へ渡すためであり、コメントの中の `{` と文字列リテラルの中の `{` はここへ入らない。
...
impl モジュール宣言の抽出 {
    ...
    pub fn 宣言一覧(&self) -> Vec<モジュール宣言> {
        ...
        let mut 深さ = 0usize;
        for (添字, 行) in self.行一覧.iter().enumerate() {
            if let Some((モジュール名, 本体の形)) = 行の中のmodの宣言(行) {
                let 囲み = 宣言を囲む波括弧::波括弧の深さから求める(深さ);
                宣言一覧.push(self.宣言にする(添字, モジュール名, 本体の形, 属性の対応表.get(&添字).copied(), 囲み));
            }
            深さ = 深さ.saturating_add(行.matches('{').count()).saturating_sub(行.matches('}').count());
        }
        宣言一覧
    }
    ...
    fn 宣言にする(&self, 添字: usize, モジュール名: String, 本体の形: 本体の形, 属性の行番号: Option<usize>, 宣言を囲む波括弧: 宣言を囲む波括弧) -> モジュール宣言 {
        モジュール宣言 {
