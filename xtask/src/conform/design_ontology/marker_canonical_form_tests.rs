//! 設計解釈マーカーの実装の形を正規形へ固定する検査の試験のうち、別名を1度も使わずに正規形を迂回する書き方(再公開の経由・絶対パス・パスの中の空白)と、`blitz_design` の再公開を確かめるもの。
//! 別名を使わずに正規形を迂回する3つの書き方(再公開の経由・絶対パス・パスの中の空白)を、そのままの形で違反にすることを固定する。

use super::marker_form_tests::正規形でない実装の違反;
use super::tests::{ソース, 全部の説明関数を連ねた違反の説明一覧};

const 再公開の違反: &str = "設計解釈マーカーを再公開すると、実装の行が正規形でなくなり構文検査が実装を認識できない。`blitz_design` から直接取り込む";
...
fn 構文解析_再公開したパスを経由したマーカーの実装は違反になる() {
    let ソース一覧 = vec![ソース(
        "crates/a/src/x.rs",
        "pub struct 借り;
impl design_alias::M不変データ for 借り {}
",
    )];
...
fn 構文解析_絶対パスのマーカーの実装は違反になる() {
    let ソース一覧 = vec![ソース(
        "crates/a/src/x.rs",
        "pub struct 借り;
impl ::blitz_design::M不変データ for 借り {}
",
    )];
    let 説明一覧 = 全部の説明関数を連ねた違反の説明一覧(ソース一覧);
    assert_eq!(説明一覧.len(), 1);
