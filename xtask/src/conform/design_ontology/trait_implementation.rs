//! 設計解釈マーカーの実装(`impl マーカー for 型`)の1件。どの設計解釈マーカーの実装かと、構文検査が違反の報告に使う位置(パスと行番号)と、対象の型名(定義をたどる鍵)と対象の型の表記そのもの(パスの修飾と型引数を含む)を持ち、
//! 正本の包括の実装かという判定と、実装の見出しの行の位置のモジュール(`module_path/enclosing_module.rs`)と、対象の型の定義を探すための問い(`type_definition.rs`)を自分で答える。
...
pub struct 設計解釈マーカーの実装 {
    pub マーカー: 設計解釈マーカー, // どの設計解釈マーカーの実装か
...
impl 設計解釈マーカーの実装 {
    ...
    /// 対象の型の定義を探すための問い。型名と、それを書いている実装の見出しの行の位置を渡す。
    pub fn 型の在り処の問い(&self) -> 型の在り処の問い<'_> {
        型の在り処の問い {
            型名: &self.型名,
            参照元のパス: &self.パス,
            参照元の行番号: self.行番号,
