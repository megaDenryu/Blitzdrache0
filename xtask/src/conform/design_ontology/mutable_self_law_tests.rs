...
fn 構文解析_イベントを実装する型の固有のimplの可変参照メソッドは違反になる() {
    ...
    let 説明一覧 = 全部の説明関数を連ねた違反の説明一覧(ソース一覧);
    assert_eq!(説明一覧.len(), 1);
    assert!(説明一覧[0].contains("M不変データ `出来事` は自分の型への可変参照を受け手か引数に持つ関数を持てません"));
}
...
fn 構文解析_不変データだけを実装する型の固有のimplの可変参照メソッドは違反になる() {
    ...
    let 説明一覧 = 全部の説明関数を連ねた違反の説明一覧(ソース一覧);
    assert_eq!(説明一覧.len(), 1);
    assert!(説明一覧[0].contains("M不変データ `変位` は自分の型への可変参照を受け手か引数に持つ関数を持てません"));
}
...
fn 構文解析_状態を実装する型の固有のimplの可変参照メソッドは違反になる() {
    ...
    let 説明一覧 = 全部の説明関数を連ねた違反の説明一覧(ソース一覧);
    assert_eq!(説明一覧.len(), 1);
    assert!(説明一覧[0].contains("M不変データ `位置` は自分の型への可変参照を受け手か引数に持つ関数を持てません"));
}
...
",
    ...
    let 説明一覧 = 全部の説明関数を連ねた違反の説明一覧(ソース一覧);
    assert_eq!(説明一覧.len(), 1);
    assert!(説明一覧[0].contains("M不変データ `不変値` は自分の型への可変参照を受け手か引数に持つ関数を持てません"));
}
