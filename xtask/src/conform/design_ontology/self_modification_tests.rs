...
//! トレイトの宣言の関数と宣言を読める範囲・トレイトの同一性・波括弧付きのモジュールの中の `use`・関数の本体などの局所の `use` と項目・マクロの呼び出しの中の `use` と項目・cfg で切り替わる同名の候補・glob の取り込みを隠す取り込み・トレイトの宣言の本体のマクロ・prelude と依存クレートの名前を名乗るトレイト・マクロを通した自己変更・対象の型を決められない実装と台帳を、観点ごとのファイルに分けて置く。

mod conditional_candidate_tests;
mod const_generic_impl_tests;
mod declaration_form_tests;
mod definition_position_tests;
mod glob_hiding_tests;
