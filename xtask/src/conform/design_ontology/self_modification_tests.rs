...
//! トレイトの宣言の関数と宣言を読める範囲・トレイトの同一性・波括弧付きのモジュールの中の `use`・関数の本体などの局所の `use` と項目・cfg で切り替わる同名の候補・トレイトの宣言の本体のマクロ・prelude と依存クレートの名前を名乗るトレイト・マクロを通した自己変更・対象の型を決められない実装と台帳を、観点ごとのファイルに分けて置く。
