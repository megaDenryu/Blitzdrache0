...
//! 名前の閉包(`use … as` と `type` の別名の辺)と実装の在り処を問わないこと・名前が当たった別の型の実装の台帳・トレイトの宣言の関数と宣言の本体のマクロ・マクロを通した自己変更・対象の型を決められない実装と台帳・包んだ可変参照と型の表記の中のマクロと型引数の境界の名前の反例を、観点ごとのファイルに分けて置く。
