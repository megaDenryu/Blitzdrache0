//! 設計解釈マーカーを実装した型の実装を定義と同じファイルに置く規則の試験。この規則が違反でなく警告の一覧を返すことと、警告がその型の実装した設計解釈マーカーを名指すことと、名前で照らす範囲(同じクレートの実装だけ・全称の実装を除く)を固定する。
...
fn 別のファイルの設計解釈マーカーの実装は警告になり違反の一覧へは入らない() {
    ...
    assert!(
        説明一覧[0].contains("設計解釈マーカー(`M不変データ`・`M規則`)を実装した型 `規則` の実装が、定義のファイル(crates/a/src/x.rs)の外にある"),
        "{説明一覧:?}"
    );
