...
//! 識別子の境界とは、その現れの直前の文字と直後の文字がどちらも識別子の文字(英数字・下線・非ASCIIの文字)でない位置のことである。ただし生の識別子の接頭辞 `r#` の直後は境界に数えない。識別子の文字の判定 `識別子の文字か` は、設計オントロジーの検査器の全体がこの1つを使う。
//! 単純な部分文字列の一致にしないのは、日本語の識別子に語の区切りが無く、`歩数を動かす領域` が `歩数` で当たるためである。
...
pub fn 識別子として現れる位置一覧(表記: &str, 名前: &str) -> Vec<usize> {
    ...
    表記
        .match_indices(名前)
        .filter(|(位置, _)| {
            let 前が境界か = 表記[..*位置].chars().next_back().is_none_or(|文字| !識別子の文字か(文字)) && !生の識別子の接頭辞で終わるか(&表記[..*位置]);
...
/// 上位集合で読み違えないのは、コードだけの行に残る非ASCIIの文字が識別子の中にしか現れないためである。rustc は全角の空白と全角の括弧を拒む。
...
#[cfg(test)]
mod tests {
    use super::識別子として現れる位置一覧;

    // 表記の中に名前が識別子の境界で1回でも現れるか。
    fn 識別子として現れるか(表記: &str, 名前: &str) -> bool {
    ...
    #[test]
    fn 生の識別子の接頭辞の後ろでは当たらず属性の後ろでは当たる() {
        assert!(!識別子として現れるか("impl 変更 for r#where {", "where") && !識別子として現れるか("r#for", "for"));
        assert!(識別子として現れるか("#[cfg(x)] where", "where") && 識別子として現れるか("甲r#for", "for"));
    }
