...
//! 対象の表記に可変の借用を与えるトレイト(`Box<dyn DerefMut<Target = 規則>>`。`impl_syntax/mutable_borrow_trait.rs`)が現れる実装と、見出しの境界の可変の借用を与えるトレイトの型引数が `Self` を指す実装(`impl<U: DerefMut<Target = Self>> 変更<U> for 規則`)も、同じく値で受ける `self` の関数を数える。
//! 見出しに名前が当たらない実装でも、本体の直下の関数か実装したトレイトの宣言の関数の署名の境界(型引数の並びの境界・`where` 句・引数の `impl Trait` と `dyn Trait`)の可変の借用を与えるトレイトの型引数に閉じた名前が現れれば、その関数を根拠にする(`mutable_impl_scan/signature_bound_question.rs`)。
...
impl<'a> 自己変更の検査<'a> {
    ...
    fn 実装の中の根拠(&self, 実装: &読んだ実装, 名前の集合: &[String]) -> Option<自己変更の根拠> {
        ...
        let 見出しの根拠 = 名前の集合.iter().find_map(|名前| {
            ...
            let 対象 = 自己変更を問う対象 {
                型名: 名前,
                可変参照を対象にするか: 構文.どの関数も自己変更を与える実装か() || 境界の場所.is_some(),
                当たった場所,
            };
