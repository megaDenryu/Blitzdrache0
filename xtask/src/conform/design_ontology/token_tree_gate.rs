...
//! 生の識別子(`r#名前`)が予約語の名前だけを名乗ることと、`extern crate` を宣言しないことと、`impl` と `type` の型引数に属性を書かないことと、実装の対象の型と型の別名の右辺のどの深さでもマクロを呼ばないことも、
//! 同じ字句の木の上で確かめる(`raw_identifier_assertion.rs`・`extern_crate_assertion.rs`・`generic_parameter_attribute_assertion.rs`・`type_notation_macro_assertion.rs`)。
//! この木の外が使うのは、字句の木の一覧(`token_tree_index.rs`)と、説明関数 `読み切れない宣言が無いこと`(`readable_form_assertion.rs`)と、工程 `予約語でない名前の生の識別子の違反一覧` と `外部クレートの宣言の違反一覧` と `型引数の属性の違反一覧` と `型の表記の中のマクロの呼び出しの違反一覧` の6つだけである。
