//! 1つのファイルの行ごとの字句位置(`module_path/enclosing_module.rs`)と、局所の位置(関数・実装・トレイトの本体や `const` のブロックの内側)に書いた `use` と項目が持ち込む名前。
...
//! 局所の名前は、どの波括弧の内側かを区別せずファイルの単位で持つ(`module_index/hiding_names.rs`)。隠しうる名前を広く見る側へ倒し、隠しうる名前を書いた局所の実装を、黙って外さず違反の側へ倒すためである。
...
impl ファイルの字句位置 {
    ...
    pub fn 局所の行を集めて組む(行ごとの字句位置: Vec<行の字句位置>, 行一覧: &[String]) -> Self {
        let mut 局所の名前 = 隠しうる名前の控え::default();
        for (行, 位置) in 行一覧.iter().zip(&行ごとの字句位置) {
            if 位置.局所の位置か() {
                局所の名前.行を足す(行);
            }
        }
