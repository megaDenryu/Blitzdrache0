...
//! 対象の型の表記か型の別名の右辺の先頭が関連型の射影(`<A as B>::C`)だと、その表記が指す型の名前が字面に現れず、検査器は実装を集められない。
//! 同じ理由で禁じるマクロの呼び出し(`名前!(..)`)は、先頭に限らずどの深さでも字句の木の側(`token_tree_gate/type_notation_macro_assertion.rs`)が違反にする。
...
impl クレート構文検査 {
    /// 実装の見出しの対象の型の位置と、型の別名の右辺の先頭が、関連型の射影でないこと。型の別名の型引数が既定値を持たないこと。
