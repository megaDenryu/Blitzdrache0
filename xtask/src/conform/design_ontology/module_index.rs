...
impl モジュールの索引 {
    ...
    fn ファイルを足す(&mut self, パス: &Path, 行一覧: &[String]) {
        ...
        self.モジュール一覧.extend([モジュール.clone(), モジュール.クレート()]);
        for (添字, (行, 位置)) in 行一覧.iter().zip(&行ごとの字句位置).enumerate().filter(|(_, (_, 位置))| !位置.局所の位置か()) {
            if 位置.モジュールの直下か() {
                self.モジュール一覧.insert(位置.モジュール.clone());
                self.型の別名の表.モジュールの直下の行を足す(行);
            }
            self.束縛を開く(&位置.モジュール).行を足す(行, 位置.区分, 条件付きの属性を持つか(行一覧, 添字));
        }
        ...
        for (書き出しの行, 項目) in 書き出しの行付きの取り込みの項目一覧(行一覧) {
            ...
            match 字句位置.行の字句位置(書き出しの行).cloned() {
                Some(位置) if !位置.局所の位置か() => self.束縛を開く(&位置.モジュール).取り込みを足す(項目, 位置.区分, 条件付きの属性を持つか(行一覧, 書き出しの行)),
                Some(_) | None => 字句位置.局所の取り込みを足す(項目),
            }
        }
    ...
    /// そのモジュールに書いた明示の `use` の項目が、走査範囲のクレートを指すか。走査範囲の外のクレート(`use thiserror::Error;`)を指す取り込みは、
    /// 検査器がその名前の名前空間(トレイトか derive のマクロか)を知らないため、同じ名前の glob の取り込みを隠すものとして数えない。
    pub fn 取り込みが走査範囲のクレートを指すか(&self, モジュール: &モジュールパス, 項目: &取り込みの項目) -> bool {
        self.モジュールが在るか(&self.書かれたパスのクレート(モジュール, &項目.パス))
    }
