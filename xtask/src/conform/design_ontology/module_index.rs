...
//! 実装の対象の型とトレイトの表記を定義へ結び付ける工程(`implementation_binding.rs`・`read_implementation.rs`・`implemented_trait.rs`)が使う。
...
impl<'a> モジュールの索引<'a> {
    ...
    /// そのクレートのすべてのモジュールの `use` の項目を、項目を書いたモジュールと組にした一覧。
    pub fn クレートの取り込みの組一覧(&self, クレート: &モジュールパス) -> Vec<(&モジュールパス, &取り込みの項目)> {
        self.モジュールごとの取り込みの項目
            .iter()
            .filter(|(モジュール, _)| モジュール.クレート() == *クレート)
            .flat_map(|(モジュール, 項目一覧)| 項目一覧.iter().map(move |項目| (モジュール, 項目)))
            .collect()
    }
