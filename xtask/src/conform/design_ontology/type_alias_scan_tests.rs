...
fn 別名と右辺のメタ変数は読み切れずクレートの根を指す決まったパスは読める() {
    assert!(matches!(原文の書き出しを読む("pub type 別の法則 = $元;"), 宣言を読んだ結末::読めない(_)));
    assert!(matches!(原文の書き出しを読む("type 包み = Vec<$型>;"), 宣言を読んだ結末::読めない(_)));
    assert!(matches!(原文の書き出しを読む("type $名 = crate::a::規則;"), 宣言を読んだ結末::読めない(_)));
...
fn 右辺の括弧の中のセミコロンで宣言を終えない() {
    ...
    for (原文, 右辺) in [("type 表 = [u8; 1 << 4];", "[u8; 1 << 4]"), ("type 表 = [u8; 16 >> 2];", "[u8; 16 >> 2]"), ("type 表 = fn() -> u8;", "fn() -> u8")] {
        assert_eq!(読んだ組(原文), Some(("表".to_string(), 右辺.to_string())), "{原文}");
    }
}

#[test]
fn 右辺を持たない関連型の宣言と別の予約語は別名の宣言でない() {
    assert!(!matches!(原文の書き出しを読む("typedef 何か = 別の何か;"), 宣言を読んだ結末::読めない(_)));
    assert!(読んだ組("typedef 何か = 別の何か;").is_none());
    assert!(読んだ組("type 状態: M状態 + PartialEq;").is_none());
    assert!(読んだ組("type 境界の等式: 甲<X = Y>;").is_none());
    for 原文 in ["type 状態: M状態;", "type 出力;", "type 境界の等式: 甲<X = Y>;"] {
        assert!(matches!(原文の書き出しを読む(原文), 宣言を読んだ結末::辺を作らない宣言を読んだ), "{原文}");
    }
    assert!(matches!(原文の書き出しを読む("typedef 何か = 別の何か;"), 宣言を読んだ結末::その宣言でない));
}

#[test]
fn 読み切れない綴りは読み切れないと答える() {
    assert!(matches!(原文の書き出しを読む("type 途中まで<T>"), 宣言を読んだ結末::読めない(_)));
    assert!(matches!(原文の書き出しを読む("type 閉じない<T = 規則;"), 宣言を読んだ結末::読めない(_)));
    assert!(matches!(原文の書き出しを読む("type 空の右辺 = ;"), 宣言を読んだ結末::読めない(_)));
}
