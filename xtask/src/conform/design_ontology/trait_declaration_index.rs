//! 走査範囲のトレイトの宣言を、宣言のモジュールパスと名前の組で引く索引。全ソースから1度だけ組み、組ごとに、宣言の本体の直下の関数の署名の一覧を持つ。
...
//! 見た宣言が複数あるときは、1つでも自己変更を与える関数があれば与えると答える(一意に決まらない宣言を違反の側へ倒す)。
...
impl トレイトの宣言の索引 {
    pub fn 全ソースから組む(ソース一覧: &[(PathBuf, Vec<String>)]) -> Self {
        ...
        for (パス, 行一覧) in ソース一覧 {
            ...
            for (開始, 行) in 行一覧.iter().enumerate() {
                ...
                中身.関数一覧.extend(本体の直下の関数の署名一覧(&見出し.本体の文字列(行一覧)));
            }
        }
        Self {
    ...
    /// 見るトレイトの宣言のうち、その名前の宣言を参照し、対象へ自己変更を与える関数を宣言しているかを答える。
