...
//! 索引へ入れるのはモジュールの直下の宣言だけである。関数・実装の本体などの局所の位置の `trait` はその波括弧の内側でしか見えず、モジュールのトレイトではないためである。
...
impl トレイトの宣言の索引 {
    pub fn 全ソースから組む(ソース一覧: &[(PathBuf, Vec<String>)]) -> Self {
        ...
        for (パス, 行一覧) in ソース一覧 {
            let 行ごとの字句位置 = モジュールパス::ファイルのパスから求める(パス).行ごとの字句位置一覧(行一覧);
            for (開始, 行) in 行一覧.iter().enumerate() {
                ...
                let Some(宣言のモジュール) = 行ごとの字句位置.get(開始).filter(|位置| 位置.モジュールの直下か()).map(|位置| &位置.モジュール) else {
                    continue;
                };
