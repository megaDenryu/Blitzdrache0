//! 同名の型が複数のクレートや同じクレートの別のモジュールにあるときの、定義と固有の `impl` の採り方の試験と、純粋データ規約を役割の実装から解決した定義に当てる試験。
...
fn 構文解析_同じファイルに定義が無く別のファイルに同名の定義が2つあれば違反にする() {
    ...
    let 期待 = "設計オントロジー: MDTO `位置` の同名の定義が複数のファイルにあり一意に決まらない(実装と同じファイルに定義を置くか、use で定義のモジュールパスを取り込む)".to_string();
...
",
    ...
    assert!(説明一覧[0].contains("MDTO `規則` の定義は内部可変性 `RefCell` を持てません"));
}
...
fn 構文解析_同じ型が複数の役割を持っても純粋データ規約の違反は1回だけ報告する() {
    ...
    let 説明一覧 = 全部の説明関数を連ねた違反の説明一覧(ソース一覧);
    assert_eq!(説明一覧.len(), 1);
    assert!(説明一覧[0].contains("MDTO `位置` の定義は内部可変性 `RefCell` を持てません"));
}
