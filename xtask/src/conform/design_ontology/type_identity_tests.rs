//! 同名の型が複数のクレートや同じクレートの別のモジュールにあるときの、定義の採り方の試験と、純粋データ規約を役割の実装からたどった定義に当てる試験。
//! 自己変更の禁止の検査は名前で実装を集めるため、同名の別の型の実装も当たる。それを台帳で除くことは `self_modification_tests/name_match_exclusion_tests.rs` が固定する。
...
use super::name_match_exclusion_ledger::{台帳の行, 名前が当たった別の型の実装の台帳};
use super::tests::{ソース, 全部の説明関数を連ねた違反の説明一覧, 台帳を与えた自己変更の違反の説明一覧};
...
#[test]
fn 構文解析_別クレートの同名の型の固有のimplも名前で検査され台帳の行で除ける() {
    let ソース一覧 = || {
        vec![
            ソース(
                "crates/a/src/x.rs",
...
",
    ...
    let 台帳 = 名前が当たった別の型の実装の台帳::行一覧から組む(vec![台帳の行 {
        マーカーの名前: "規則",
        パス: "crates/b/src/y.rs",
        見出し: "impl 規則",
        区分: "同じ名前の別の型",
        除外する理由: "この実装の対象はクレート b の別の型であり、マーカーを名乗るクレート a の型ではない",
    }]);
