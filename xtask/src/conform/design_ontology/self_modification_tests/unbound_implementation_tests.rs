...
//! 検査器はこれらを `M不変データ` の型へ結び付けられないため、自己変更を与える関数を持つものは台帳に理由が無い限り違反にする。本体が空の全称の実装が宣言の既定の関数から可変の関数を得る形(境界 `where Self: Sized` 付きを含む)も、この検査が捕まえる。
...
#[test]
fn 本体が空の全称の実装は宣言の境界付きの既定の関数でも違反になる() {
    let ソースの内容 = "pub trait 印 {}
pub trait 変更 {
    fn 変える(&mut self)
    where
        Self: Sized,
...
";
    let 説明一覧 = 台帳と突き合わせた説明一覧(vec![ソース("crates/a/src/x.rs", ソースの内容)], &対象の型を決められない実装の台帳::登録済みの台帳());
    assert_eq!(説明一覧.len(), 1, "{説明一覧:?}");
    assert!(説明一覧[0].contains("対象の型を決められない実装 `impl<T: 印> 変更 for T`"), "{説明一覧:?}");
}
