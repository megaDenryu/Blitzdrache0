...
//! 対象の型のパスの最後の名前だけで型に照らす形は、`&mut Vec<規則>` から私有のフィールドを書き換えうる `impl ローカル for Vec<規則> { fn 変える(&mut self) }` を通していた。いまは対象の表記に名前が識別子の境界で現れるかを問うため、包む実装も同じ1つの問いで当たる。
