...
//! 実装の本体の直下の項目の位置のマクロの呼び出し(式の位置のマクロは除く)と、マクロの中の実装の引数に現れるメタ変数の型への可変参照と、マクロの本体の中で不変データへトレイトを実装する形を固定する。`macro_rules!` の中の `impl` だけを見る形は、`impl 規則 { 生やす!(); }` のように関数だけを生やすマクロを通していた。
...
#[test]
fn マクロの本体の中の不変データへのトレイトの実装は名前と対象の型を決められない実装の両方で違反になる() {
    let 説明一覧 = 説明一覧(
        "pub struct 規則;
impl M不変データ for 規則 {}
pub trait 変更 {
...
",
    ...
    assert!(説明一覧.iter().any(|説明| 説明.contains("対象の型を決められない実装 `impl 変更 for 規則`")), "{説明一覧:?}");
}
...
#[test]
fn 本体の直下の式の位置のマクロは違反にならない() {
    let 説明一覧 = 説明一覧("pub struct 規則;\nimpl M不変データ for 規則 {}\nimpl 規則 {\n    const 真: bool = matches!(1, 1);\n    #[doc = include_str!(\"読む.md\")]\n    fn 読む(&self) {}\n}\n");
    assert!(説明一覧.is_empty(), "{説明一覧:?}");
}
