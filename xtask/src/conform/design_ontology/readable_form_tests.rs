//! 3つの読み口(`use`・`type`・`impl`)の全域性の試験。読み切れない宣言と読み口が読まなかった項目の予約語が違反になることと、読み口の誤読で落ちていた形が自己変更の検査へ届くことを固定する。
...
#[test]
fn 行の途中から書き始めたimplの見出しを読み口が読まなかった予約語として違反にする() {
    let ソース一覧 = vec![原文("crates/a/src/x.rs", "pub struct 規則; impl 規則 {\n    pub fn 変える(&mut self) {}\n}\n")];
    let 説明一覧 = 正規形の説明関数を連ねた違反の説明一覧(ソース一覧);
    assert_eq!(説明一覧.len(), 1, "{説明一覧:?}");
    assert!(説明一覧[0].contains("項目を始めうる `impl` が1個あるが、検査器の読み口が読んだ宣言は0個である"), "{}", 説明一覧[0]);
}
...
fn rustfmtが等号の後ろで折った長い型の別名は違反にならず名前の閉包へ入る() {
    ...
    let 本文 = format!("pub struct 規則;\nimpl M不変データ for 規則 {{}}\nimpl M規則 for 規則 {{}}\npub type {別名} =\n    crate::a::x::規則;\nimpl {別名} {{\n    pub fn 変える(&mut self) {{}}\n}}\n");
    assert!(本文.lines().any(|行| 行.chars().map(|文字| if 文字.is_ascii() { 1 } else { 2 }).sum::<usize>() > 250));
    assert!(正規形の説明関数を連ねた違反の説明一覧(vec![原文("crates/a/src/x.rs", &本文)]).is_empty());
    let 説明一覧 = 全部の説明関数を連ねた違反の説明一覧(vec![ソース("crates/a/src/x.rs", &本文)]);
    assert!(説明一覧.iter().any(|説明| 説明.contains("M不変データ `規則` は自分の型への可変参照を受け手か引数に持つ関数を持てません")), "{説明一覧:?}");
}

#[test]
fn 閉じない群のuseと読み切れないtypeと字句の木へ変えられないファイルを違反にする() {
    let ソース一覧 = vec![原文("crates/a/src/x.rs", "use crate::a::{b, c;\ntype 途中まで<T>\n")];
    let 説明一覧 = 正規形の説明関数を連ねた違反の説明一覧(ソース一覧);
    assert_eq!(説明一覧.len(), 3, "{説明一覧:?}");
    for 理由 in ["波括弧の群が同じ文の中で閉じていない", "`type` の宣言がファイルの最後まで `;` に届かない", "検査器がこのファイルを字句の木へ変えられない"] {
        assert!(説明一覧.iter().any(|説明| 説明.contains(理由)), "{理由}: {説明一覧:?}");
    }
}
