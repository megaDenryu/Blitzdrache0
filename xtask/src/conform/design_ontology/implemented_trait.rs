...
//! 宣言の関数が対象へ自己変更を与えるなら、その関数を根拠にする。宣言は、取り込み元から決めた宣言のクレートの候補(子のモジュール `implemented_trait/declaring_crate.rs`)で索引を引く。
...
use declaring_crate::トレイトの宣言のクレートの問い;
...
impl 実装したトレイト<'_> {
    ...
    pub fn 自己変更の根拠(&self, トレイトの索引: &トレイトの宣言の索引, モジュールの索引: &モジュールの索引<'_>, 対象: &自己変更を問う対象) -> Option<自己変更の根拠> {
        ...
        let 候補 = (トレイトの宣言のクレートの問い { 自分: &自分, 表記: self.表記 }).候補を答える(モジュールの索引);
        match トレイトの索引.宣言を参照する(&候補, 名前, 対象) {
    ...
    fn 走査範囲の外か(&self, モジュールの索引: &モジュールの索引<'_>) -> bool {
        ...
        if 起点 == パス && 明示の取り込み.is_none() && 取り込まずに書けるトレイトの名前の並び.split_whitespace().any(|書ける名前| 書ける名前 == 名前) {
            return true;
        }
