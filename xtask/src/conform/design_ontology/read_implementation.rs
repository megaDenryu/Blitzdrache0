...
//! その所在から答えられる問いのうち、実装の位置のモジュールと、書いた名前が `use` の別名を通しているかをここで答える。対象の型を定義へ結び付ける問いは `implementation_binding.rs`、
...
impl 読んだ実装<'_> {
    ...
    pub fn 位置のモジュール(&self) -> モジュールパス {
        let 囲むモジュールの一覧 = 行ごとの囲むモジュールの名前一覧(self.行一覧);
        let 名前一覧: Vec<&str> = 囲むモジュールの一覧.get(self.行番号.saturating_sub(1)).into_iter().flatten().map(String::as_str).collect();
        モジュールパス::ファイルのパスから求める(self.パス).下へ繋ぐ(&名前一覧)
    }
