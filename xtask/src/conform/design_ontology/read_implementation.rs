...
impl 読んだ実装<'_> {
    ...
    /// このファイルの取り込みと、取り込み元のモジュール(修飾があれば修飾が指しうるモジュール)の取り込みを1段たどる。再公開の `pub use … as` もこれで読む。
    pub fn 別名の元の名前<'b>(&self, 索引: &'b モジュールの索引<'_>, 修飾: Option<&str>, 名前: &str) -> Option<&'b str> {
        ...
        let 取り込み元 = match 修飾 {
            ...
            None => {
                ...
                索引.取り込みの項目一覧(&自分).iter().find(|項目| 項目.別名.is_none() && 項目.元の名前() == 名前)?.親のパス()
            }
        };
        自分.書かれたパスが指しうるモジュール一覧(取り込み元).iter().find_map(|モジュール| 索引.別名の元の名前(モジュール, 名前))
    }
}
