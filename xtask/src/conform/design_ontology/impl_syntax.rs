...
impl 実装の見出しの構文 {
    ...
    /// 値で受ける `self` の関数も含め、どの関数も対象の型の値を書き換えうる実装か。対象の表記のどの深さにでも可変参照か可変の借用を与えるトレイトが現れるとき(`Option<&mut 規則>`・`Box<dyn DerefMut<Target = 規則>>`)と、見出しの境界の可変の借用を与えるトレイトの型引数が `Self` を指すとき(`impl<U: DerefMut<Target = Self>> 変更<U> for 規則`)である。
    /// 境界の `Self` は対象の型そのものを指すため、`fn 変える(mut 相手: U)` は受け手と引数の字面に可変参照を持たずに対象の型の値を書き換える。
    pub fn どの関数も自己変更を与える実装か(&self) -> bool {
        self.対象.可変参照か() || 可変の借用を与えるトレイトが現れるか(self.対象.表記()) || self.境界.可変の借用で名前を指すか("Self")
    }
