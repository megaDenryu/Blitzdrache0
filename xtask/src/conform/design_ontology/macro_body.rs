...
//! 本体の中の関数で数えるのは、受け手が可変のものと `&mut Self` を引数に持つものである。
...
impl マクロの本体 {
    ...
    /// 本体の中の `impl` の本体の外にある関数のうち、受け手が可変か `&mut Self` を引数に持つものの、行の添字と署名の一覧。
    pub fn 実装の外の自己変更の関数一覧(&self, 行一覧: &[String]) -> Vec<(usize, 関数の署名)> {
        ...
        for 行 in self.範囲.clone().filter(|行| !実装の本体一覧.iter().any(|本体| 本体.contains(行))) {
            ...
            一覧.extend(関数一覧.filter(|関数| 対象.自己変更を与えるか(関数)).map(|関数| (行, 関数)));
        }
        一覧
    }
}
