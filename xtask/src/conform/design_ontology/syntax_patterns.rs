...
impl オントロジートレイト {
    ...
    /// 検査器が型の定義の解決を要するのはこの一覧と `M不変データ` と `MParameter` と `M結果`(標準の `Result` を除く)だけである。
...
/// `M結果` の実装の対象が標準の `Result` か。`Result` は標準ライブラリの型であり `crates` 配下に定義を持たないため、定義の解決を要求しない特例である。
