...
impl オントロジートレイト {
    ...
    /// 処理の役割の軸。`M工程` はデータの役割に含めない(2026-09-20 のオーナー裁定。PR #169)。`M不変データ` を上位トレイトに持たないため純粋データ規約と `&mut self` の禁止の対象にせず、正規形・別名・再公開・波括弧付きのモジュールの検査の対象にはする。
    ...
    /// 検査器が型の定義の解決を要するのはこの一覧と `M不変データ` と `MParameter` だけであり、ここに無いマーカー(`M結果` 等)の実装は定義をたどらない。
    ...
    /// `M結果` は上位トレイトを持たず、純粋データ規約・`&mut self` の禁止・定義の解決のどれも課さない(`純粋データの役割一覧` に入れない)。標準の `Result` への実装は `crates` 配下に定義が無く、定義をたどると「定義が見つからない」の違反になるためである。
