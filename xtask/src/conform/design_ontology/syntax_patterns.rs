...
impl オントロジートレイト {
    ...
    /// `M結果` は上位トレイトを持たず、純粋データ規約と `&mut self` の禁止を課さない(`純粋データの役割一覧` に入れない)。結果は判別型であるという構文の法則として、実装の対象の型が `enum` であることだけを課す(`すべての結果が列挙型であること`。正本 `blitz_design` が持つ標準の `Result` への包括の実装だけが定義をたどらない特例である)。
