...
//! 宣言と `use` が条件付きの属性(`#[cfg(..)]`・`#[cfg_attr(..)]`)を持つかも答える。名前の結び付けは cfg を評価せず、条件付きの宣言と取り込みを同時に見える候補として扱うためである。
...
/// 添字の行の宣言か `use` が、同じ行の頭か直前の属性だけの行に、条件付きの属性(名前に `cfg` を含む属性)を持つか。
/// 直前の属性だけの行とは、頭が `#[` で始まる行が続く範囲であり、複数の行にまたがる1つの属性の途中の行は数えない(rustfmt は短い属性を1行に書く)。
pub fn 条件付きの属性を持つか(行一覧: &[String], 添字: usize) -> bool {
    ...
    条件付きか(同じ行の属性) || 前の行一覧.iter().rev().take_while(|前の行| 前の行.trim_start().starts_with("#[")).any(|前の行| 条件付きか(前の行))
}
