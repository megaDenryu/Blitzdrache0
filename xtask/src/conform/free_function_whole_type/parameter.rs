...
pub fn 引数へ分ける(文字列: &str) -> Vec<String> {
...
fn 寿命注釈を飛ばす(文字列: &str) -> &str {
    let Some(注釈) = 文字列.strip_prefix('\'') else {
        return 文字列;
    };
    注釈.find(char::is_whitespace).map_or(文字列, |位置| 注釈[位置..].trim_start())
}

/// 経路の末尾の区切りだけを型名として読む。名前の後ろに山括弧以外が続く文字列は、丸ごと受け取る形でないため落とす。
fn 経路の末尾の型名(文字列: &str) -> Option<String> {
    let 先頭: String = 文字列.chars().take_while(|文字| 文字.is_alphanumeric() || *文字 == '_' || *文字 == ':').collect();
    let 残り = 文字列.get(先頭.len()..)?.trim();
