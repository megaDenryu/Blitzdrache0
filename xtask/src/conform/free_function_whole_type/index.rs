...
impl 型の定義の索引 {
    pub fn 観測から生成する(ファイル別観測: &[ファイルの観測]) -> Self {
        ...
        for ファイル in ファイル別観測 {
            for 観測 in &ファイル.観測一覧 {
                if let 観測::型定義 { 型名, .. } = 観測 {
                    木の表.entry(型名.clone()).or_default().push(モジュールの位置::定義ファイルから生成する(&ファイル.パス));
                }
            }
        }
        Self { 木の表 }
    ...
    pub fn 親の型を丸ごと受け取る自由関数を探す(&self, パス: &Path, 内容: &str) -> Vec<親の型を丸ごと受け取る自由関数> {
    ...
    fn 親の型か(&self, 型名: &str, パス: &Path) -> bool {
        self.木の表.get(型名).is_some_and(|木一覧| 木一覧.iter().any(|木| 木.この木の中のファイルか(パス)))
    }
}
