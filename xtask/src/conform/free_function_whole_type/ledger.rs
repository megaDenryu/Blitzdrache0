    ...
    let mut 違反一覧: Vec<違反> = 検出の表
        ...
        .map(|(位置, 行番号)| {
            違反::行単位(
                PathBuf::from(&位置.パス),
                *行番号,
                format!("自由関数「{}」が親の型「{}」を丸ごと受け取っている(工程が触るものだけを名前の付いた引数で受け取る)", 位置.関数名, 位置.型名),
            )
        })
        .collect();
...
fn 検出されない項目の違反(検出の表: &BTreeMap<自由関数の位置, usize>, 台帳の位置一覧: &BTreeSet<自由関数の位置>) -> Vec<違反> {
    台帳の位置一覧
        .iter()
        .filter(|位置| !検出の表.contains_key(*位置))
        .map(|位置| 違反::ファイル単位(PathBuf::from(&位置.パス), format!("台帳の自由関数「{}」が親の型「{}」を受け取らなくなった(台帳から削除する)", 位置.関数名, 位置.型名)))
        .collect()
}
