...
//! 畳んでよい境界は、GPU境界(`gpu_layout/rigid/`のバイト列化)と、`配置`が持つ描画の変換の生成と衝突数学層へ渡す形の中心の2つ
//! (`rigid_body/placement.rs`)である。それ以外のファイルにこの口の呼び出しが現れたら、CPUの物理の途中で2段を1本の単精度へ畳む経路が入ったことになる。
//! Rustの可視性は先祖のモジュールへしか絞れず、3つの境界が別のモジュールの木にあるため、可視性の代わりにこの検査が限定を課す。
...
// この口を呼んでよいファイル(リポジトリルートからの相対パス)。型の定義と、その型の試験と、3つの境界である。
