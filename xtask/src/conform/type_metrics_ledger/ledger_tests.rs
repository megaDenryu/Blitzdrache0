...
fn 台帳に載っているのに計測へ現れない型を違反にする() {
    let 台帳 = 台帳::全区画から組む();
    assert!(!台帳.計測に現れない項目一覧(&[]).is_empty());
    let アプリの所在 = 型の所在::区画の根と相対パスから生成する("crates/blitz_app/src", "app/mod.rs", "アプリ");
    assert!(台帳.上限を参照する(&アプリの所在).is_some());
}
...
fn 同じ名前でも定義ファイルが違えば別の行として台帳を引く() {
    let 台帳 = 台帳::全区画から組む();
    let 起動設定 = |定義ファイル: &str, フィールド数: usize, メソッド総数: usize| 計測(定義ファイル, "起動設定", 1, 宣言の分量::構造体のフィールド数(フィールド数), メソッド総数);
