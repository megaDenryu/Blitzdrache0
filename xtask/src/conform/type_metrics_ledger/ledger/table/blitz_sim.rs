...
const 行一覧: [台帳の行; 12] = [
    ...
    台帳の行::構造体("contact/normal_tangential_system/reduced_system.rs", "有効な行だけを抜き出した連立方程式", 6, 8, 20),
    台帳の行::構造体("contact/normal_tangential_system/solution_breakdown.rs", "単精度の解の内訳", 1, 11, 6),
    台帳の行::構造体("contact/normal_tangential_system/system.rs", "接触点集合の法線と接線の連立方程式", 9, 5, 37),
    台帳の行::構造体("contact/normal_tangential_system/tolerance_application.rs", "試験の許容差の適用規則", 4, 7, 21),
    台帳の行::構造体("contact/pipeline/pipeline_solver.rs", "接触の解法ソルバー", 5, 3, 10),
    台帳の行::構造体("contact/scene/numeric_contract_record/tolerance_dependence/substep_record.rs", "許容差依存の細分の記録", 3, 14, 9),
