...
const 行一覧: [台帳の行; 11] = [
    ...
    台帳の行::構造体("conform/design_ontology/syntax_checker.rs", "クレート構文検査", 6, 2, 26), // 自己変更の禁止の検査を外し、設計解釈マーカーを実装した型の実装の置き場の警告(2つのメソッド)を足した後の実測値。説明関数は観点ごとにファイルを分けるため、1つ足すと実装ファイル数とメソッド総数が1つずつ増える。
