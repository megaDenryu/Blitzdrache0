...
pub fn 全定数を検査する() -> Result<Vec<違反>, 規約検査の破れ> {
    ...
    for 組 in table::領域一覧.iter().copied().flatten() {
        ...
        if 正本 != 写し {
            違反一覧.push(違反::ファイル単位(
                PathBuf::from(組.写しパス),
                format!("{}の写しが正本と食い違う(正本{正本}・写し{写し}・正本は{})", 組.写しの前置き.trim(), 組.正本パス),
            ));
        }
    }
    Ok(違反一覧)
...
fn 値を読む(パス: &'static str, 前置き: &'static str) -> Result<f64, 規約検査の破れ> {
    ...
    let 行 = 内容.lines().find(|行| 行.trim_start().starts_with(前置き)).ok_or(規約検査の破れ::定数の宣言が無い { パス, 前置き })?;
    let 数値 = 行.trim_start().trim_start_matches(前置き).split(';').next().unwrap_or("").trim();
    数値を読む(数値).map_err(|誤り| 規約検査の破れ::定数の値を数として読めない {
        パス, 前置き, 文字列: 数値.to_string(), 誤り
    })
}

/// 数の表記と、`1.0 / 60.0`のような2つの数の商を読む。商を許すのは、基本刻みのように分母で書くのが正本の自然な表記である
/// 定数を、写しと突き合わせるために小数へ書き直させないためである。
fn 数値を読む(文字列: &str) -> Result<f64, std::num::ParseFloatError> {
    match 文字列.split_once('/') {
