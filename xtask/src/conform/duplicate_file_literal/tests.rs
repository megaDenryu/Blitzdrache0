//! ファイル名らしい文字列の選び方と、試験の項目の除かれ方と、取り込みの引数の見分け方の検査。
//! 走査と集計でなく規則だけを見る。
...
fn 文字列一覧(原文: &str) -> Vec<String> {
    ファイル内の出現を集める(原文).into_iter().map(|(文字列, _, _)| 文字列).collect()
}

fn 取り込みの引数か(原文: &str, 文字列: &str) -> bool {
    ファイル内の出現を集める(原文).into_iter().any(|(見つけた文字列, _, 引数か)| 見つけた文字列 == 文字列 && 引数か)
}
...
fn 一行で閉じる試験の宣言の後ろを打ち切らない() {
    ...
    assert_eq!(文字列一覧(原文), vec!["ui_vertex.spv".to_string()]);
}

#[test]
fn 試験の項目の中の波括弧の文字リテラルで範囲が伸びない() {
    let 原文 = "#[cfg(test)]\nfn t() {\n    let c = '{';\n}\nconst 後: &str = \"after.png\";";
    assert_eq!(文字列一覧(原文), vec!["after.png".to_string()], "試験の項目の範囲が末尾まで伸びている");
}
...
fn コメントと文字列の中の属性を項目の始まりと読まない() {
    ...
    assert_eq!(文字列一覧(原文), vec!["d.png".to_string(), "e.png".to_string()]);
}
