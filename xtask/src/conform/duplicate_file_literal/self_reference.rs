//! 検査の対象の文字列を自分で列挙する台帳の一覧と、その一覧が台帳でなくなったことの検査。
//!
//! 台帳はどのファイルとどのファイルを結ぶかを書く場所であり、そこに並ぶ文字列は検査の対象そのものである。
//! 正本を別に持てないうえ、文字列を誤ればその台帳を使う検査が「読み取りに失敗した」として止まるか、
//! 対象を1つも見つけられずに止まるため、この検査が重ねて見る必要がない。
//!
//! 置き場ごと外さずに1件ずつ挙げるのは、検査の実装が本番のファイル名を持ったときに見逃さないためである。
//! 一覧へ載せてよいのは、検査の一部として対象の文字列を列挙する台帳だけである。新しい台帳を作ったら足す。
//! 判定に使うのは今の衝突の有無ではなく役割である。衝突で判定すると、対象の文字列を列挙しているのに
...
const 対象の文字列を列挙する台帳一覧: [&str; 57] = [
    ...
    "xtask/src/conform/shader_constant/table/cloth.rs",
    "xtask/src/conform/shader_constant/table/editor_chunk_source.rs",
    ...
    "xtask/src/conform/shader_constant/table/time_step.rs",
    ...
    "xtask/src/conform/type_metrics_ledger/ledger/table/blitz_app.rs",
    "xtask/src/conform/type_metrics_ledger/ledger/table/blitz_asset_compiler.rs",
    "xtask/src/conform/type_metrics_ledger/ledger/table/blitz_collision.rs",
    "xtask/src/conform/type_metrics_ledger/ledger/table/blitz_engine.rs",
    "xtask/src/conform/type_metrics_ledger/ledger/table/blitz_math.rs",
    "xtask/src/conform/type_metrics_ledger/ledger/table/blitz_render.rs",
    "xtask/src/conform/type_metrics_ledger/ledger/table/blitz_sim.rs",
    "xtask/src/conform/type_metrics_ledger/ledger/table/editor_server.rs",
    "xtask/src/conform/type_metrics_ledger/ledger/table/other_crates.rs",
    "xtask/src/conform/type_metrics_ledger/ledger/table/xtask.rs",
...
/// 台帳であることの印は、対象の文字列(ファイル名らしい文字列リテラル)を1つ以上列挙していることである。
pub(super) fn 一覧の陳腐化を検査する() -> Result<Vec<違反>, 規約検査の破れ> {
    ...
    for 台帳 in 対象の文字列を列挙する台帳一覧 {
        ...
        if !対象の文字列を列挙しているか(&内容) {
            違反一覧.push(違反::ファイル単位(
                PathBuf::from(台帳),
                "台帳の一覧に載っているが、対象の文字列を1つも列挙していない(台帳でなくなったので一覧から削除する)".to_string(),
            ));
        }
    }
    Ok(違反一覧)
