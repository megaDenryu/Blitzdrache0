...
//! 英字始まりに限るのは、`0.05`のような数の文字列を拡張子と読み違えないためである。
//! リテラルの切り出しは`source_lexing`の字句の走査が持つ。ここは切り出された中身だけを見る。

/// 文字列のうちファイル名の部分。区切りより前のディレクトリを落とす。
///
/// 同じファイルを指す文字列が、`shaders/scene.slang`と`scene.slang`、`/ui_vertex.spv`と`ui_vertex.spv`のように
