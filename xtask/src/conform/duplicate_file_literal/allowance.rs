//! 2つ以上のファイルに書いてよい、ファイル名らしい文字列の台帳と、台帳を持つまでもない1つの規則。
//! ここへ載せてよいのは、正本を1箇所へ寄せられないものだけである。寄せられるものを載せるのは、
//! この検査を無効にすることと同じである。
//!
//! 台帳は文字列だけでなく、その文字列が現れてよい場所も持つ。文字列だけで許すと、台帳に載った名前を
...
/// 台帳が挙げた場所から文字列が消えていたら違反として報告し、台帳からの削除を強制する。
/// これがないと是正済みの項目が残り続け、次に同じ文字列を2箇所へ書いたときに見逃す穴になる。
...
fn 場所を失った違反(許容: &寄せられない文字列, 場所: &str) -> 違反 {
    違反::ファイル単位(
        ...
        format!("台帳が許した文字列「{}」が{場所}に無い(台帳の場所を直すか、文字列ごと台帳から削除する。載せた理由は「{}」)", 許容.文字列, 許容.寄せられない理由),
    )
}
