...
//! 受け取るのは無し、返すのは重複した文字列の違反一覧である。走査するファイルの範囲は`scan_scope`が持つ。
//!
//! 同じファイル名を2箇所へ書くと、片方だけを直した食い違いが実行するまで出ない。しかも文字列が合っている
...
/// 1ファイルの中のファイル名らしい文字列を、1つ残らず集める。`#[cfg(test)]`の付いた項目の中は数えない。
