...
//! 注意: 検査対象の文字列そのものをこのファイルに連続して書くと、conformが
...
pub fn 禁止語と絵文字の混入を検査する(パス: &Path, 内容: &str) -> Vec<違反> {
    ...
    for (行番号, 行) in 内容.lines().enumerate() {
        ...
        if let Some(語) = 行に経緯語を含むか(行) {
            経緯語を見つけた = true;
            if !未是正既知 {
                違反一覧.push(違反::行単位(パス.to_path_buf(), 行番号 + 1, format!("切り出しの経緯だけを述べる語({語})")));
            }
        }
        if 行に絵文字を含むか(行) {
...
mod tests {
    ...
    fn 台帳にないファイルの経緯語は違反になる() {
        let 違反一覧 = 禁止語と絵文字の混入を検査する(Path::new("crates/blitz_render/src/新規.rs"), concat!("//! ", "行数", "分割のための切り出し\n"));
