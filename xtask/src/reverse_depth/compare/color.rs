...
pub(super) fn 色を比べる(対照: &読み戻し画像, 候補: &読み戻し画像, 候補深度: &深度画像) -> Result<色差の集計, 逆Z検収エラー> {
    let 幅 = 候補.幅().画素数();
    let 高さ = 候補.高さ().画素数();
    ...
    if 輪郭外の幾何画素数 == 0 {
        return Err(逆Z検収エラー::判定が不合格("輪郭外で検査したジオメトリ画素が0件である".to_string()));
    }
    if 輪郭外の重大差 > 0 {
        return Err(逆Z検収エラー::判定が不合格(format!("輪郭から2px超離れた提示色の2階調以上の差が{輪郭外の重大差}画素ある")));
    }
    Ok(色差の集計 {
        最大差,
        差画素数,
