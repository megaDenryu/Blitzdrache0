...
pub(super) fn 標準深度の奥行き(深度: f32) -> f64 {
    遠クリップ * 近クリップ / (遠クリップ - f64::from(深度) * (遠クリップ - 近クリップ))
}

pub(super) fn 逆向き深度の奥行き(深度: f32) -> f64 {
    遠クリップ * 近クリップ / (近クリップ + f64::from(深度) * (遠クリップ - 近クリップ))
}

#[cfg(test)]
mod tests {
    use super::{標準深度の奥行き, 逆向き深度の奥行き};

    #[test]
    fn 端点の近面と遠面が逆になる() {
        assert!((標準深度の奥行き(0.0) - 0.1).abs() < 1.0e-12);
        assert!((標準深度の奥行き(1.0) - 2000.0).abs() < 1.0e-6);
        assert!((逆向き深度の奥行き(1.0) - 0.1).abs() < 1.0e-12);
        assert!((逆向き深度の奥行き(0.0) - 2000.0).abs() < 1.0e-6);
    }
