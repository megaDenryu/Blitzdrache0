...
impl 隅の位置 {
    ...
    pub(super) const 四隅: [Self; 4] = [Self { 横千分率: 80, 縦千分率: 80 }, Self { 横千分率: 920, 縦千分率: 80 }, Self { 横千分率: 80, 縦千分率: 920 }, Self { 横千分率: 920, 縦千分率: 920 }];
...
pub(super) fn 純色でない画素数を数える(画像: &読み戻し画像, 層: 層の色) -> u64 {
    画像.画素列().filter(|画素| !層.画素が一致するか([画素[0], 画素[1], 画素[2]])).count().try_into().unwrap_or(u64::MAX)
}
