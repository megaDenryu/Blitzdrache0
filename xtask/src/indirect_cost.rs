...
//!
//! 費用の合否は判定しない。
//! 予算との突き合わせは節目の定点で評価軸を更新するときに人が行う。
//! この入口が持つ機械判定は3つだけである。
//! 検証層の指摘が0件であること、積んだ生成パスの本数が本番の焼き上げの計画が答えた見込みどおりであること、区間の値がすべて有限であることである。
...
fn 計測する(引数一覧: &[String]) -> Result<String, 間接照明の費用計測エラー> {
    ...
    let シェーダー入口 = crate::shader_copy::一時コピーを作る(&検証の出力ルート::既定().名前が指す置き場(シェーダーコピー先)).map_err(間接照明の費用計測エラー::シェーダーの一時コピーを作れなかった)?;
