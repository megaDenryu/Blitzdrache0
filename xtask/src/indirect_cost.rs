//! 順3-Iの閉塞ゲートである全更新費用のGPU計測の入口。担当するのは、本番の地形世界を時計停止A・時計進行B・
//! 時計停止A'の順に起動し、間接照明の区間別GPU時間を集めて表と生値にすることである。
//!
//! 交互に起動するのは、同じ条件を続けて回すと機材の状態の移り変わりが条件の差に化けるためである。停止Aと停止A'は
//! 同じ条件であり、その食い違いがそのまま「この計測日の機材の揺れ」の大きさになる。
//!
//! 費用の合否は判定しない。予算との突き合わせはマイルストーンの定点で評価軸を更新するときに人が行う。
//! この入口が持つ機械判定は3つだけである。validationの指摘が0件であること、積んだ生成パスの本数が本番の焼き上げの
//! 計画が答えた見込みどおりであること、区間の値がすべて有限であることである。本数を見るのは費用の判定ではなく、
//! その実行が本当に測るべき状態を測ったかの判定である。
...
fn 計測する(引数一覧: &[String]) -> Result<String, String> {
    ...
    if !crate::release_build::実行する("indirect-cost") {
    ...
    table::表示する(&標本一覧);
    要約を組む(&標本一覧, &出力先, 指定.フレーム数)
}

/// 3実行がそろっていることを前提にせず、そろっていなければ何が足りないかを言って失敗にする。
fn 要約を組む(標本一覧: &[record::一標本], 出力先: &Path, フレーム数: u32) -> Result<String, String> {
    let [停止a, 進行b, 停止a2] = 標本一覧 else {
        return Err(format!("3実行そろわなかった(採れたのは{}件)", 標本一覧.len()));
    };
    Ok(format!(
        "1実行{フレーム数}フレームの停止A・進行B・停止A'で、{}、{}、{}、{}。区間別の表と生値は{}にある",
        summary::全更新合計の行(進行b)?,
