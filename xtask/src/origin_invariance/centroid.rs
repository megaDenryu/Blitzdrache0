...
pub(super) fn 移動を比較する(a: &読み戻し画像, a移動後: &読み戻し画像, b: &読み戻し画像, b移動後: &読み戻し画像) -> 判定結果 {
    let 重心一覧 = [a, a移動後, b, b移動後].map(重心を測る);
    let [Ok(a重心), Ok(a後重心), Ok(b重心), Ok(b後重心)] = 重心一覧 else {
        let 理由 = 重心一覧.into_iter().find_map(Result::err).map_or_else(|| "重心を取得できなかった".to_string(), |破れ| 破れ.to_string());
