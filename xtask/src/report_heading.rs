//! アプリの終了時報告の行の見出し。文字列を私有し、行との照合と、見出しに続く本文の取り出しだけを外へ出す。
//!
...
//!
//! 文字列を所有する形にするのは、パス別や距離区分別の見出しを実行時に組み立てる読み取りがあるためである。
...
impl 報告の見出し {
    ...
    /// 実行時に組み立てた文字列から作る。パス名や距離区分の番号を含む見出しがこの口を通る。
...
/// 誤りの文面へ文字列を埋めるための境界。生の文字列へ戻すのはここだけである。
