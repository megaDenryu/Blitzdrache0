...
pub(super) fn 測る(指定: &大規模世界の計測指定) -> Result<(), 物量計測エラー> {
    ...
    std::fs::create_dir_all(&出力先).map_err(|誤り| 物量計測エラー::出力先を作れなかった { パス: 出力先.clone(), 誤り })?;
    let シェーダー入口 = crate::shader_copy::一時コピーを作る(&検証の出力ルート::既定().名前が指す置き場(シェーダーコピー先)).map_err(物量計測エラー::シェーダーの一時コピーを作れなかった)?;
    let 検査候補数 = デバッグ実行で検査する(指定, &シェーダー入口)?;
    let 引数一覧 = crate::large_world_bench::launch::起動引数を作る(指定, &シェーダー入口);
    let 実行一覧 = (1..=反復回数).map(|回| 起動引数で走らせる(&出力先, &format!("large_world_{回}"), &引数一覧)).collect::<Result<Vec<_>, _>>()?;
