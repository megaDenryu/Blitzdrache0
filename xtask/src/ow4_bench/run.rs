...
pub(super) struct 一回の実行 {
    ...
    pub(super) 計数: 計数報告,
    pub(super) 世界メートル毎テクセル: Vec<f64>, // 距離区分ごとの、シャドウマップの1テクセルが世界で覆う長さ。添字が距離区分番号
    pub(super) 要約: ストリーミング要約報告,
    pub(super) プロセス: メモリ最大, // GPUカウンターを1標本も引けなかった実行では専用VRAMが不在になる
}
...
pub(super) fn 起動引数で走らせる(出力先: &Path, 名前: &str, 引数一覧: &[String]) -> Result<一回の実行, 物量計測エラー> {
    ...
    let プロセス = 実行しながら採取する(&条件).ok_or_else(|| 物量計測エラー::実行が標本を1つも採れなかった { 名前: 名前.to_string() })?;
    let 標準出力 = std::fs::read_to_string(&標準出力先).map_err(|誤り| 物量計測エラー::実行の標準出力を読めなかった { パス: 標準出力先.clone(), 誤り })?;
...
fn 読み取る(報告: &終了時報告, プロセス: メモリ最大) -> Result<一回の実行, 物量計測エラー> {
    let 計数 = crate::report_parse::終了時報告から計数報告を取り出す(報告)?;
    検証層のエラーと警告.零件であることを課す(計数.検証層の指摘件数)?;
    Ok(一回の実行 {
