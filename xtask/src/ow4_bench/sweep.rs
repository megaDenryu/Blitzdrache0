...
pub(super) fn 測る(物量点一覧: &[usize], 条件: &condition::計測条件) -> Result<Vec<point::物量点の結果>, 物量計測エラー> {
    ...
    物量点一覧.iter().map(|個体数| 一物量点を測る(&出力先, &シェーダー入口, *個体数, 条件)).collect()
}

fn 一物量点を測る(出力先: &Path, シェーダー入口: &Path, チャンクあたり個体数: usize, 条件: &condition::計測条件) -> Result<point::物量点の結果, 物量計測エラー> {
    let アセットルート = 出力先.join(format!("assets_{チャンクあたり個体数}"));
    if !crate::compile_assets::地形世界を個体数指定で生成する(&アセットルート, 同居植生の個体数::生成する(チャンクあたり個体数)) {
        return Err(物量計測エラー::物量点の実行時アセットを生成できなかった { チャンクあたり個体数 });
    }
