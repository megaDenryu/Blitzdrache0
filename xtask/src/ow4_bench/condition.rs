...
/// 一日内時刻を渡す引数の文字列。破れの文面が打ち直す語をそのまま名指す。
...
pub(super) fn 引数を読む(引数一覧: &[String]) -> Result<引数の読み, 物量計測エラー> {
    ...
    while let Some(語) = 残り.next() {
        ...
        // 注意: 一日内時刻の引数名は定数であり、この選択肢は文字列の一致で選ばれる。束縛でないことを保つため、
        // この名前を変数へ変えてはならない(変えるとこの選択肢が全部の語を受け、続く選択肢へ1語も届かない)。
        match 語.as_str() {
...
fn 一日内秒を読む(語: Option<&String>) -> Result<u32, 物量計測エラー> {
    let 語 = 語.ok_or(物量計測エラー::引数の次に値が無い { 引数名: 一日内時刻の引数名 })?;
    語.parse().map_err(|_| 物量計測エラー::引数の値を数として読めない {
        引数名: 一日内時刻の引数名, 語: 語.clone()
    })
}
