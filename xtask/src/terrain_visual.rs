...
//! 基準値を置くと変更のたびに基準の更新が要る。機械が見るのは検証層の指摘が0件であることと、
...
fn 検収する() -> Result<String, 目視見本の絵の撮影エラー> {
    crate::visual_sample_world::目視見本世界を用意する().map_err(目視見本の絵の撮影エラー::目視見本世界を用意できなかった)?;
    let 実行環境 = run::実行環境を作る(検証の出力ルート::既定().名前が指す置き場(出力ディレクトリ))?;

    let マスクの実行 = 実行環境.描いて読み戻す(crate::sample_world_region::領域マスクの実行名, &crate::sample_world_region::領域マスクの起動指定を組み立てる())?;
    ...
    for 時刻 in &代表時刻一覧 {
        ...
        帯の行一覧.push(band::破綻防止帯を判定する(時刻.名前, 実行.画像(), &マスク, &地面を照らす光を選ぶ(時刻))?);
        絵の置き場一覧.push(実行.書き出し先().目視用の絵へ変換する()?.display().to_string());
    }
    Ok(format!("5つの起動すべてで検証層の指摘0件、地面{}画素の破綻防止帯は{}。絵は{}", マスク.地面画素数, 帯の行一覧.join("、"), 絵の置き場一覧.join("と")))
}
