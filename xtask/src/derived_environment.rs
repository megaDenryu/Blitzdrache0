//! 順3-Ibの検収入口。与えた遠方環境の上に拡散照度・鏡面畳込み・反射率積分表を検証層を有効にした
...
fn 表を出す(報告: &rows::報告) {
    println!("派生表現の読み戻し:");
    for 行 in &報告.統計 {
        println!("  {}: テクセル数{} (非有限{} 負{} 予約が非零{})", 行.種別, 行.テクセル数, 行.非有限, 行.負, 行.予約が非零);
    }
    for 行 in &報告.分散 {
        println!("  鏡面段{}のばらつき: {:.3e}", 行.段, 行.分散);
    }
    for 行 in &報告.面境界 {
        println!("  面境界({} 段{} 一辺{}): 稜線{}本 最大相対差{:.3e}", 行.種別, 行.段, 行.一辺, 行.稜線数, 行.最大相対差);
    }
}

/// 派生表現を焼かせて報告を採る。検証層の指摘は判定の一部としてこの入口が読むため、
/// 実行環境が零件を確かめる口は通さない。
