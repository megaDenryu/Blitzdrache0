...
pub(super) fn 表を出す(名前一覧: &[&str], 実測一覧: &[領域別実測]) {
    println!("時刻別の領域実測(空の平均色、明部の第95百分位、明部の第99百分位、暗部画素数、地形の黒潰れ・飽和・全成分0の画素数):");
    for (名前, 実測) in 名前一覧.iter().zip(実測一覧) {
        let [赤, 緑, 青] = 実測.空平均色;
        println!(
            "  {名前}: 空({赤}, {緑}, {青}) 明部の第95百分位={} 明部の第99百分位={} 暗部={} 地形の黒潰れ={} 地形の飽和={} 全成分0={}",
            実測.明部の第95百分位, 実測.明部の第99百分位, 実測.暗部画素数, 実測.地形の黒潰れ画素数, 実測.地形の飽和画素数, 実測.黒つぶれ画素数
        );
    }
}
