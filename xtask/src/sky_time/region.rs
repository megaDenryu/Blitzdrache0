...
const 領域マスクの空の画素: 判定の名前 =
    判定の名前::定数から生成する("領域マスクで空だけの領域になった画素(1つも無ければマスクが片側に寄っている)");
const 領域マスクのジオメトリの画素: 判定の名前 =
    判定の名前::定数から生成する("領域マスクでジオメトリの領域になった画素(1つも無ければマスクが片側に寄っている)");
