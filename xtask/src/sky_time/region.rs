...
const 領域マスクの空の画素: 判定の名前 = 判定の名前::定数から生成する("領域マスクで空だけの領域になった画素(1つも無ければマスクが片側に寄っている)");
const 領域マスクのジオメトリの画素: 判定の名前 = 判定の名前::定数から生成する("領域マスクでジオメトリの領域になった画素(1つも無ければマスクが片側に寄っている)");
const 領域マスクと絵の画素数: 判定の名前 = 判定の名前::定数から生成する("領域マスクの画素数と突き合わせる絵の画素数");
...
impl 領域区分 {
    pub(super) fn 作る(マスク: &読み戻し画像) -> Result<Self, 判定の破れ> {
        ...
        if 空画素数 == 0 {
            return Err(領域マスクの空の画素.あるはずのものが無い破れ());
        }
        if 幾何画素数 == 0 {
            return Err(領域マスクのジオメトリの画素.あるはずのものが無い破れ());
        }
        Ok(Self { 区分一覧, 空画素数, 幾何画素数 })
    }
    ...
    pub(super) fn 寸法が合うか(&self, 絵: &読み戻し画像) -> Result<(), 判定の破れ> {
        領域マスクと絵の画素数.一致を課す(絵.画素数(), self.画素数())
    }
