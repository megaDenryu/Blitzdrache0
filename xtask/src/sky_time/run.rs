...
//!
//! 地形世界は空を持つ方針であるため、時刻の絵は本番の経路で描く。自動露出だけは`--no-auto-exposure`で外す。この入口が見るのは時刻から導く照明そのもの(夜の地表が環境光だけの帯・隣り合う時刻の明るさの変化・夜明けと夕方の対称性)であり、自動露出を通した最終色は「照明÷露出」になってその3つを測れなくなるためである。
