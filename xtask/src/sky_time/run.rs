...
pub(super) enum 条件<'引数> {
    時刻の絵 { 一日内秒: &'引数 str, カメラ方位度: &'引数 str }, // 本番の経路でその時刻の絵を描く。カメラ方位は既定の0度から動かす角度である
    領域マスク { カメラ方位度: &'引数 str },                     // 空も光も外して、どの画素が空でどの画素がジオメトリかだけを撮る
}
...
fn 条件別の選択肢を足す(指定: アプリの起動指定, 条件: &条件<'_>) -> アプリの起動指定 {
    match 条件 {
        条件::時刻の絵 { 一日内秒, カメラ方位度 } => 指定.値を持つ選択肢を足す("--time-of-day", 一日内秒).値を持つ選択肢を足す("--camera-yaw", カメラ方位度).選択肢を足す("--no-auto-exposure"),
        条件::領域マスク { カメラ方位度 } => 指定.選択肢をまとめて足す(&["--no-sky", "--no-post", "--unlit"]).値を持つ選択肢を足す("--camera-yaw", カメラ方位度),
    }
}
