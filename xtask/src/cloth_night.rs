...
fn 検収する() -> Result<String, 布の明るさの時刻追従の検収エラー> {
    ...
    let 入口 = crate::shader_copy::一時コピーを作る(&検証の出力ルート::既定().名前が指す置き場(シェーダーコピー先)).map_err(布の明るさの時刻追従の検収エラー::シェーダーの一時コピーを作れなかった)?;
...
fn 布領域を測る(実行環境: &描画検収の実行環境, 入口: &Path, 名前: &str, 一日内秒: &str) -> Result<(judgment::布領域, std::path::PathBuf), 布の明るさの時刻追従の検収エラー> {
...
fn 描く(実行環境: &描画検収の実行環境, 名前: &str, 入口: &Path, 一日内秒: &str, 追加: &[&str]) -> Result<検収の1回の実行, 布の明るさの時刻追従の検収エラー> {
    let mut 引数: Vec<&str> = vec!["--sky", "--time-of-day", 一日内秒];
    引数.extend_from_slice(追加);
    let 実行 = 実行環境.描いて読み戻す(検収の実行名::生成する(名前)?, &crate::vegetation_run::植生世界の起動指定を組み立てる(シーン, フレーム数, 入口, &引数))?;
    実行.報告().画面へ流す();
    Ok(実行)
}
