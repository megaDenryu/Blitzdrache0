...
//! 決定的に変わることを画素で確かめる。
//!
//! 判定は7つある。同一時刻の2回描画がバイト一致すること、隣り合う時刻の間で空色・明部・暗部がそれぞれ変わること、
...
fn 検収する() -> Result<String, String> {
    ...
    let 円盤の差分画素数 = image_judgment::太陽円盤を判定する(&絵)?;
    pixel_check::照合する(&出力先, 幅, &区分)?;

    report::表を出す(&名前一覧, &実測一覧);
    let 絵の置き場 = report::絵を書き出す(&出力先)?;
    Ok(format!(
        "空画素{}・幾何画素{}を4時刻で数え、隣り合う時刻の空色と明部と暗部がすべて変わり、夜明けと夕方の空平均色はバイト一致し、夜は環境光だけの明るさで黒つぶれ0、どの時刻も空は飽和せず、夕方の太陽円盤は対照との差分{円盤の差分画素数}画素、天頂寄りと地平線直上の代表画素がCPU正本と許容内で一致し、絵は{絵の置き場}",
        区分.空画素数, 区分.幾何画素数
    ))
}
