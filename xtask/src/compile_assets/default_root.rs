...
pub fn 既定を生成する() -> bool {
    実行時形式を生成する(ソースルート(), Path::new(既定出力ルート), 世界名::板の世界)
}

/// 地形世界は板の世界と同じ座標を占めるため、同じ出力ルートへは同居できない。専用の既定出力ルートへ焼く。
pub fn 地形世界を既定で生成する() -> bool {
    実行時形式を生成する(ソースルート(), Path::new(地形の既定出力ルート), 世界名::地形の世界)
}

/// 植生世界も原点チャンクを占めるため、専用の既定出力ルートへ焼く。
pub fn 植生世界を既定で生成する() -> bool {
    実行時形式を生成する(ソースルート(), Path::new(植生の既定出力ルート), 世界名::植生の世界)
}
...
pub fn 見本の集落世界を既定で生成する() -> bool {
    実行時形式を生成する(ソースルート(), Path::new(見本の集落の既定出力ルート), 世界名::見本の集落の世界)
}
...
pub fn 目視見本世界を既定で生成する() -> bool {
    実行時形式を生成する(ソースルート(), Path::new(目視見本の既定出力ルート), 世界名::目視見本の世界)
}

/// 夜の多光源の世界も原点チャンクを占めるため、専用の既定出力ルートへ焼く。
pub fn 夜の多光源世界を既定で生成する() -> bool {
    実行時形式を生成する(ソースルート(), Path::new(夜の多光源の既定出力ルート), 世界名::夜の多光源の世界)
}
...
pub fn 屋内の多光源世界を既定で生成する() -> bool {
    実行時形式を生成する(ソースルート(), Path::new(屋内の多光源の既定出力ルート), 世界名::屋内の多光源の世界)
}
...
pub fn 場所巡り世界を既定で生成する() -> bool {
    実行時形式を生成する(ソースルート(), Path::new(場所巡りの既定出力ルート), 世界名::場所巡りの世界)
}
