...
use crate::acceptance::{アプリの起こし方, アプリの起動指定, 世界を読ませて報告を採る実行環境, 描画フレーム数, 検収の実行名, 検収シーン名};
...
pub fn 流体試作を計測する() -> ExitCode {
    ...
    let 実行環境 = 世界を読ませて報告を採る実行環境::世界の置き場をアプリの既定に任せて作る(アプリの起こし方::毎回cargoに構築させて起動する);
...
fn 一条件を実行する(実行環境: &世界を読ませて報告を採る実行環境, 試作の起動指定: &str) -> bool {
    let 指定 = アプリの起動指定::シーンと枚数を決める(シーン名, フレーム数).選択肢を足す(試作の起動指定).選択肢を足す("--report-gpu-times");
