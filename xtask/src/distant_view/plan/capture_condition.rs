...
/// `後処理を使わない`は、光のにじみと明るさの圧縮を組まない構成で採る指定である。主判定はこの対で近傍不変を課す。
/// 参照: `_doc/設計/大規模世界の生成と遠景.md`第5段階の検査点4
///
/// `影可視度を可視化する`は、本番の色の代わりに影の欠落計器の診断色を出す指定である。影の検査点が色差の帰属に使う。
/// 参照: `_doc/設計/大規模世界の生成と遠景.md`第5段階の検査点5
///
/// `明示境界を使わない`は、大規模世界でも距離区分を再配分せず実用分割のまま採る指定である。影の検査点の対照側が使う。
/// 対照を再配分前のコミットの作業ツリーから採ると、比べている差に再配分以外のコミットの差が混ざるため、
/// 同じバイナリで旗だけを分けて2枚の違いを再配分だけにする。
///
/// `影のキャスターを使わない`は、地形も個体も布も影の距離区分のパスへ積まない指定である。散布の主判定が、
/// 幾何だけが変わる対を作るために使う。物を置けばその物が覆っていない地面の陰影も必ず変わるため、
/// 影と遮蔽を入れたままでは近傍不変を課せない。
...
impl 採取条件 {
    ...
    pub(super) fn 影のキャスターを切る(mut self) -> Self {
        self.影のキャスターを使わない = true;
        self
    }
