//! 遠景の固定構図を採る工程ごとの失敗。
//!
//! 採取を読む段の破れ(器の破れ)と、絵から採った数へ課した判定の破れを別の選択肢で持つのは、直す側が見る場所が
...
pub(super) enum 遠景構図の検収エラー {
    ...
    構図契約が失敗した(String),
    対照の焼き付けが失敗した(crate::game_fox_tour::error::場所巡りの通しの検収エラー), // 絵を採る前、散布の対照を焼く工程の破れ
    採取を読めなかった(採取の読み取りの破れ),                                          // 採った絵と由来のファイルを読む段の破れ。9つの前提を`採取の読み取りの破れ`が数え上げる
    判定が破れた(判定の破れ),                                                          // 絵から採った数へ課した判定の破れ。破れの形を`判定の破れ`が数え上げる
}
