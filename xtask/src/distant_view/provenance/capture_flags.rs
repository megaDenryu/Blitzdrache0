//! 採取1回の起動条件を、由来ファイルの1行として書き、判定の側が同じ文字列で読み戻す工程。
//! 受け取るのは採取条件または由来ファイルの置き場と名前、返すのは旗の行または読み戻した旗である。
//!
//! 書く側と読む側で文字列を1つのモジュールへ閉じるのは、判定が「後処理なしの対」のつもりで後処理ありの絵を
//! 突き合わせても、文字列がずれていれば誰も気づかないためである。旗の記録が無ければ、判定は自分がどの構成の絵を
//! 見ているかを絵からは知りようがない。
//!
//! `影可視度を可視化したか`・`明示境界を切ったか`・`影のキャスターを切ったか`が`None`の由来は、その欄を足す前に
//! 採った古い採取である。欄の有無で失敗させるのは、その欄を実際に要る段(影の検査点)だけにする。遠景の検査点は
//! 影可視度を読まないため、古い採取のままでも判定を続けられる。無いことを`false`へ丸めると、診断の絵を本番の絵
//! として突き合わせうる。
...
pub(in crate::distant_view) struct 採取の旗 {
    ...
    遠景の影を使うか: bool,
    後処理を使うか: bool,
    影可視度を可視化したか: Option<bool>,   // 影可視度の可視化の記録
    明示境界を切ったか: Option<bool>,       // 距離区分の再配分を切って走ったかの記録
    影のキャスターを切ったか: Option<bool>, // 影のキャスターを1つも積まずに走ったかの記録
}

impl 採取の旗 {
    pub(in crate::distant_view) fn 採取条件から作る(条件: &採取条件) -> Self {
        Self {
            局所可視性を使うか: !条件.局所可視性を使わない,
    ...
    /// 条件を確かめられないまま通ることを防ぐため、読めない文字列と同じく採取の読み取りの破れにする。
