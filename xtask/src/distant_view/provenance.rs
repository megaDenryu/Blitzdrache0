...
//!
//! 旗の行そのものの文字列と読み戻しは`capture_flags`が持つ。ここはファイルへ何をどの順で並べるかだけを決める。
...
pub(super) fn 由来を書く(置き場: &Path, 条件: &採取条件, 由来: &crate::release_build::構築の由来, 構図の実測: &str) -> Result<(), 遠景構図の検収エラー> {
    ...
    std::fs::write(&パス, 行一覧.join("\n")).map_err(|誤り| 遠景構図の検収エラー::由来を書けなかった { パス: PathBuf::from(&パス), 誤り })
}
