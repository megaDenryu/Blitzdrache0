...
//!
//! 同時に3つを要求する。差し替えが成功した行がちょうど1回出ること、検証層の指摘が0件であること、
...
fn 検収する() -> Result<String, 契約別のシェーダー束の差し替えの検収エラー> {
    ...
    let 監視先 = crate::shader_copy::一時コピーを作る(&検証の出力ルート::既定().名前が指す置き場(シェーダーコピー先)).map_err(契約別のシェーダー束の差し替えの検収エラー::シェーダーの一時コピーを作れなかった)?;
