...
fn 一時刻の対を撮る(出力先: &std::path::Path, 時刻: &代表時刻) -> Result<String, String> {
    ...
    crop::区画が絵に収まるか確かめる(効かせた.画像.幅, 効かせた.画像.高さ)?;
    let 食い違い = 食い違う画素を数える(&効かせた.画像.rgba8, &切った.画像.rgba8)?;
    if 食い違い == 0 {
...
fn 一条件を撮る(
    ...
    let 画像 = run::描画する(&ダンプ先, 時刻.一日内秒, &名前, 条件)?;
