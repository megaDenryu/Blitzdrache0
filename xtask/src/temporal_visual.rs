...
//! あり、その判断は絵を見る人が行うためである。機械が見るのは検証層の指摘が0件であることと、効かせた側と
...
const 出力ディレクトリ: 検証の出力の置き場名 = 検証の出力の置き場名::生成する("temporal_visual");
...
fn 撮る() -> Result<String, 時間再構成の目視材料の撮影エラー> {
    crate::visual_sample_world::目視見本世界を用意する().map_err(時間再構成の目視材料の撮影エラー::目視見本世界を用意できなかった)?;
...
fn 一時刻の対を撮る(実行環境: &描画検収の実行環境, 出力先: &std::path::Path, 時刻: &代表時刻) -> Result<String, 時間再構成の目視材料の撮影エラー> {
...
fn 一条件を撮る(実行環境: &描画検収の実行環境, 時刻: &代表時刻, 条件: 時間再構成の条件, 接尾辞: &str) -> Result<撮った絵, 時間再構成の目視材料の撮影エラー> {
