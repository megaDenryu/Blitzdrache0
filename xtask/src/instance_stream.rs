...
fn 検収する() -> Result<String, 植生ストリーミング統合の検収エラー> {
    ...
    let シェーダー入口 = crate::shader_copy::一時コピーを作る(&検証の出力ルート::既定().名前が指す置き場(シェーダーコピー先)).map_err(植生ストリーミング統合の検収エラー::シェーダーの一時コピーを作れなかった)?;

    let 静止 = run::走らせる(&実行環境, "preload", &シェーダー入口, 静止フレーム数, &[])?;
    let 往復 = run::走らせる(&実行環境, "roundtrip", &シェーダー入口, 往復フレーム数, &[])?;
    let カメラ往復 = run::走らせる(&実行環境, "camera_probe", &シェーダー入口, 往復フレーム数, &["--lod-probe-step", カメラ往復の刻み])?;
