...
fn 検収する() -> Result<String, 空の描画の検収エラー> {
    ...
    let 空パスms = gpu_time::空パスの平均ミリ秒を読む(本番.報告())?;
...
fn 空パスが枠を下回るかの判定名(空パスms: f64) -> 判定の名前 {
    判定の名前::組み立てた文字列から生成する(format!("空パスのGPU時間{空パスms:.4}msが独立枠{空パスの予算ミリ秒}msを下回るか"))
}
