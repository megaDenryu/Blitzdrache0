...
fn 走査順の入替で食い違った画素の判定名(再現性の床: usize) -> 判定の名前 {
    判定の名前::組み立てた綴りから生成する(format!("走査順の入替で食い違った布の影の画素(同一条件2回の差は{再現性の床}画素であり、超えるなら布の影が描画対象の走査順に依存している)"))
}

fn 描く(実行環境: &描画検収の実行環境, 実行名の綴り: &str, シェーダー入口: &Path, 条件: run::条件) -> Result<検収の1回の実行, 布の影の走査順非依存の検収エラー> {
