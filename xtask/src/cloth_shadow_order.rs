...
//! 落ちる(参照: `_doc/設計/空と時間帯と遠距離シャドウ.md`「描画の処理位置の資源の器と布専用シャドウ経路」)。
//!
...
const 布の影が覆う画素: 判定の名前 = 判定の名前::定数から生成する("布の影が覆う画素(下回るなら走査順の入替を比べる対象が無く検収が成立しない)");
...
fn 検収する() -> Result<String, 布の影の走査順非依存の検収エラー> {
    ...
    let 入口 = crate::shader_copy::一時コピーを作る(&検証の出力ルート::既定().名前が指す置き場(シェーダーコピー先)).map_err(布の影の走査順非依存の検収エラー::シェーダーの一時コピーを作れなかった)?;
...
fn 走査順の入替で食い違った画素の判定名(再現性の床: usize) -> 判定の名前 {
    判定の名前::組み立てた文字列から生成する(format!("走査順の入替で食い違った布の影の画素(同一条件2回の差は{再現性の床}画素であり、超えるなら布の影が描画対象の走査順に依存している)"))
}

fn 描く(実行環境: &描画検収の実行環境, 実行名の文字列: &str, シェーダー入口: &Path, 条件: run::条件) -> Result<検収の1回の実行, 布の影の走査順非依存の検収エラー> {
    Ok(実行環境.描いて読み戻す(検収の実行名::生成する(実行名の文字列)?, &run::起動指定を組み立てる(条件, シェーダー入口))?)
}
