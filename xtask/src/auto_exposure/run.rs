...
use crate::acceptance::{アプリの起こし方, アプリの起動指定, 描画フレーム数, 描画検収の実行環境, 書き出しの形式, 検収の実行名, 検収エラー};
...
pub(super) fn 実行環境を作る(出力ディレクトリ: PathBuf) -> Result<描画検収の実行環境, 検収エラー> {
    描画検収の実行環境::作る(アプリの起こし方::毎回cargoにリリース版を構築させて起動する, 目視見本世界のアセットルートを作る(), 出力ディレクトリ)
}

pub(super) fn 描画して報告を読む(実行環境: &描画検収の実行環境, 実行名の文字列: &str, 一日内秒: &str, 探り: &探り色の扱い) -> Result<自動露出の報告, 検収エラー> {
    println!("[xtask] auto-exposure描画: {実行名の文字列}");
    let 報告 = 実行環境.書き出させて報告を採る(検収の実行名::生成する(実行名の文字列)?, &起動指定を組み立てる(一日内秒, 探り), 書き出しの形式::明るさの圧縮前)?;
    読み解く(&報告)
}
...
fn 起動指定を組み立てる(一日内秒: &str, 探り: &探り色の扱い) -> アプリの起動指定 {
    ...
    match 探り {
        探り色の扱い::流さない => 指定,
        探り色の扱い::背景へ流す => 指定.値を持つ選択肢を足す("--auto-exposure-probe", 探り色).選択肢をまとめて足す(&["--no-sky", "--no-aerial-composite"]),
    }
}
