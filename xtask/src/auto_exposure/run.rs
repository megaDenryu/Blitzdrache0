...
pub(super) fn 描画して報告を読む(実行環境: &描画検収の実行環境, 実行名の綴り: &str, 一日内秒: &str, 探り: &探り色の扱い) -> Result<自動露出の報告, 検収エラー> {
    println!("[xtask] auto-exposure描画: {実行名の綴り}");
    let 報告 = 実行環境.書き出させて報告を採る(検収の実行名::生成する(実行名の綴り)?, &起動指定を組み立てる(一日内秒, 探り), 書き出しの形式::明るさの圧縮前)?;
    読み解く(&報告)
}
