...
fn 屋内を一条件で撮る(実行環境: &描画検収の実行環境, 名前: &str, 追加の選択肢: &[&str]) -> Result<(Vec<f64>, PathBuf), 検収エラー> {
    let 結果 = 実行環境.描いて読み戻す(検収の実行名::生成する(名前)?, &run::起動指定を組み立てる(world::屋内のシーン, world::絵の枚数, 追加の選択肢))?;
    let 平均輝度一覧 = 屋内の判定領域の一覧.iter().map(|領域| 矩形の平均輝度を採る(結果.画像(), &領域.矩形)).collect::<Result<Vec<f64>, 判定の破れ>>()?;
