...
pub(super) fn 名前付きの数を読む(標準出力: &str, 見出し: &str, 名前: &str) -> Result<u64, String> {
    let 本文 = 行の値を取る(標準出力, 見出し)?;
    for 語 in 本文.split_whitespace() {
        let Some((項目, 値)) = 語.split_once('=') else {
            continue;
        };
        if 項目 == 名前 {
            return 値.parse().map_err(|誤り| format!("「{見出し}」の{名前}の値を読めない: {誤り}"));
        }
    }
    Err(format!("「{見出し}」の行に{名前}が無い: {本文}"))
}
