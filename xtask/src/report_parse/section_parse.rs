...
pub fn 区画の行一覧<'a>(標準出力: &'a str, 見出し: &str) -> Result<Vec<&'a str>, String> {
...
pub fn 区画の行<'a>(区画: &[&'a str], 接頭辞: &str) -> Result<&'a str, String> {
    区画
        .iter()
...
pub fn 鍵の次の小数(行: &str, 鍵: &str) -> Result<f64, String> {
    鍵の次の語(行, 鍵)?
        .parse()
        .map_err(|誤り| format!("「{行}」の{鍵}を小数として読めない: {誤り}"))
}

/// 「点光源の影: 0.0754 (p50 0.0758 / p95 0.0778 / 標本60) ms」のように括弧へ触れている鍵の語を末尾一致で見つけ、
/// その次の語を小数として読む。括弧を語の一部として持つ表のために、完全一致で探す`鍵の次の小数`と分けている。
pub fn 括弧に触れた鍵の次の小数(行: &str, 鍵: &str) -> Result<f64, String> {
...
pub fn 鍵の次の整数(行: &str, 鍵: &str) -> Result<u64, String> {
    let 語 = 鍵の次の語(行, 鍵)?;
