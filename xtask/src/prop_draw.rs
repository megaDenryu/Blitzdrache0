...
const 検収世界: 検収世界の用意 = 検収世界の用意::生成する("小物の描画", crate::compile_assets::板の世界の実行時形式の置き場, 検証の出力のファイル名::生成する("prop_wooden_crate.blitzasset"));
...
fn 検収する() -> Result<String, 小物の描画の検収エラー> {
    検収世界
        .焼き上がりを確かめる(crate::gen_source_assets::検証用ソースアセットを生成して成否を返す() && crate::compile_assets::既定を生成する())
    ...
    Ok(format!("{判定}、ポストなしの絵は{}、本番経路の絵は{}", 平面png.display(), 本番png.display()))
}
