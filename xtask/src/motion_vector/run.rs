...
use crate::acceptance::{アプリの起こし方, アプリの起動指定, 世界を読ませて報告を採る実行環境, 描画フレーム数, 検収の実行名, 検収エラー};
...
pub(super) fn 観測を採る(実行環境: &世界を読ませて報告を採る実行環境, 条件: 条件) -> Result<観測, 検収エラー> {
    let 報告 = 実行環境.報告を採る(検収の実行名::生成する(条件.実行名の文字列())?, &起動指定を組み立てる(条件))?;
    super::observation::取り出す(&報告)
}
