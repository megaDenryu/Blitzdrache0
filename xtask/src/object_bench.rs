...
fn 実行環境を作る(起こし方: アプリの起こし方) -> 世界を読ませて報告を採る実行環境 {
    世界を読ませて報告を採る実行環境::作る(起こし方, 実行時アセットルート::パスから生成する(検証の出力ルート::既定().名前が指す置き場(crate::compile_assets::板の世界の実行時形式の置き場)))
}
...
fn 走らせる(実行環境: &世界を読ませて報告を採る実行環境, 実行名の文字列: &str, 指定: &アプリの起動指定) -> bool {
    let 実行名 = match 検収の実行名::生成する(実行名の文字列) {
        Ok(実行名) => 実行名,
        Err(誤り) => {
            eprintln!("[xtask] {誤り}");
            return false;
