...
pub(super) fn 全距離区分に影があることを検査する(地図: &距離区分の地図) -> Result<[usize; 4], String> {
    ...
    for (距離区分, &画素数) in 数.iter().enumerate() {
        if 画素数 == 0 {
            return Err(format!("距離区分{距離区分}に影の画素が1つも無い: 距離区分別の影画素数は{数:?}である"));
        }
    }
    Ok(数)
}
...
pub(super) fn 境界の両側に影があることを検査する(地図: &距離区分の地図) -> Result<Vec<(usize, usize)>, String> {
...
pub(super) fn 境界の輝度段差を検査する(地図: &距離区分の地図, 本番: &読み戻し画像) -> Result<Vec<(f64, f64)>, String> {
    ...
    for 境界番号 in 0..境界の数 {
        ...
        if 段差 > 勾配 || 段差 > 段差の絶対上限 {
            return Err(format!(
                "境界{境界番号}と{}の輝度段差{段差:.2}が許容を超えた(距離区分の内側の勾配{勾配:.2}、絶対上限{段差の絶対上限})",
                境界番号 + 1
            ));
        }
        実測.push((段差, 勾配));
