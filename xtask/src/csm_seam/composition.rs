...
//! 受け取るのは引数の文字列、返すのは構図と、その構図が要る世界・アセットルート・視点・判定の強さである。
//!
...
pub(super) enum 継ぎ目を見る構図 {
    検証用地形世界, // 5×5チャンクの検証用地形世界。既定であり、再配分の影響を受けない実用分割の側を守り続ける
    大規模世界,     // 8km×10kmの大規模世界。明示境界25・100・1000・10000メートルの再配分が効く唯一の構図
}
...
impl 継ぎ目を見る構図 {
    ...
    /// 開く世界の文字列と時刻。由来がそのまま書き、人が絵と突き合わせられるようにする。
