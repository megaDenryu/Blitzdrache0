...
//!
//! 起動するのは`ベンチ実行`(`--benchmark-frames`)であり、スモーク実行の自己操作を入れない。計測窓へ
//! 自己操作の描画を混ぜないためである。実行ファイルを直に叩くのは`shadow-probe`と同じで、計測窓へcargoの
//! ビルド判定を混ぜないためである。
//!
//! 標準出力をファイルへ落とすのは、誰も読まないパイプが埋まると子プロセスが書き込みで止まるためである。
//! 世界は本番の地形世界であり、地面は本番のストリーミング経路でしか現れないため`sky-lut`と同じ先読み半径と
//! 容量上限で渡す。カメラ俯角も`sky-lut`と揃えて地平線より上を画面へ入れる(空が画面に無いと遠方環境を
//! 消費する画素の割合が本番の構図と変わる)。
