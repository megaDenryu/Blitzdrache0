...
use crate::acceptance::{アプリの起こし方, アプリの起動指定, 世界を読ませて報告を採る実行環境, 実行時アセットルート, 検収シーン名, 終了時報告};
...
pub(super) fn 一回走らせる(材料: &実行の材料<'_>) -> Result<終了時報告, 間接照明の費用計測エラー> {
    let 標準出力先 = PathBuf::from(材料.出力先).join(format!("run_{}_{}.log", 材料.実行番号, 材料.条件.実行名));
    println!("[xtask] indirect-cost実行{}: {}", 材料.実行番号, 材料.条件.呼び名);
    let 報告 = 材料.実行環境.報告を採る(材料.条件.実行名.clone(), &起動指定を組み立てる(材料))?;
    let 書き出し = std::fs::write(&標準出力先, 報告.本文());
    書き出し.map_err(|誤り| 間接照明の費用計測エラー::実行の標準出力を書けなかった { パス: 標準出力先, 誤り })?;
    Ok(報告)
}
