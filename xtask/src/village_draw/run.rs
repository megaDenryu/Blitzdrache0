...
use crate::acceptance::{アプリの起こし方, アプリの起動指定, 実行時アセットルート, 描画フレーム数, 描画検収の実行環境, 検収の実行名, 検収エラー, 検収シーン名, 終了時報告};
...
const 報告を出させる選択肢: [&str; 5] = ["--sky", "--report-gpu-times", "--report-frame-times", "--report-draw-issue", "--report-memory"];
...
pub(super) fn 実行環境を作る(出力ディレクトリ: PathBuf) -> Result<描画検収の実行環境, 検収エラー> {
    描画検収の実行環境::作る(
        ...
        実行時アセットルート::パスから生成する(検証の出力ルート::既定().名前が指す置き場(crate::compile_assets::見本の集落の世界の実行時形式の置き場)),
        出力ディレクトリ,
    )
}
...
///
/// 注意: 報告に出た順で写すため、区画ごとの読み取りでなく本文を1度だけ走査する。区画ごとに読むと、
/// 見出し一覧の並びが画面に出る順を決めてしまい、アプリが出した順序を人が追えなくなる。
pub(super) fn 報告を書き出す(報告: &終了時報告) {
    let mut 報告の内側 = false;
