...
//!
//! 文字列を所有する形にするのは、パス別や区間別の判定が名前を実行時に組み立てるためである。
...
impl 判定の名前 {
    ...
    /// 実行時に組み立てた文字列から作る。パス名や区間名を含む判定がこの口を通る。
    ///
    /// 注意: 名前の無い判定は破れの文面が何も名指せないため、空の文字列は到達したらバグとして落とす。
    pub fn 組み立てた文字列から生成する(文字列: String) -> Self {
        assert!(!文字列.is_empty(), "判定の名前が空である");
        Self(Cow::Owned(文字列))
    }

    /// 名前と種類を1つの破れへ組む。比べで表せない破れ(文字列の食い違い等)を課す側がこの口を通る。
    ...
    /// 判定の材料に置いた文字列を数として読み、読めなければ破れにする。報告が文字列のまま運ぶ値がこの口を通る。
