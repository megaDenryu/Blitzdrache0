...
impl アプリの起動 {
    ...
    /// 最終フレームをRGBA8で書き出させる。
    pub(super) fn 画素の書き出し先を積む(&mut self, 書き出し先: &読み戻しの書き出し先) {
        self.コマンド.arg("--dump-frame").arg(書き出し先.起動引数として渡す綴り());
    }

    /// 最終フレームを明るさの圧縮を通す前の単精度で書き出させる。
    pub(super) fn 圧縮前の画素の書き出し先を積む(&mut self, 書き出し先: &読み戻しの書き出し先) {
        self.コマンド.arg("--dump-hdr-frame").arg(書き出し先.起動引数として渡す綴り());
    }
