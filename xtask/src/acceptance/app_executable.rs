//! アプリの起こし方: 検収がblitz_appを起こすやり方と、その文字列を私有する型。
//!
//! リリースの実行ファイルの文字列を裸の公開定数で配ると、起動する側と由来を採る側が別々にその文字列を持つ。
//! 実測ではその定数が6つのファイルへ流れており、置き場所を変える改定が6箇所の同時変更になっていた。
//! ここが文字列を1つ持ち、起動と由来の採取という2つの使い道だけを外へ出す。
//!
//! cargoへ渡す引数を選択肢ごとの表で持つのは、起こし方の違いが引数の並びの違いでしかないためである。
//! 表を1つにすると、起こし方を足す変更が1行の追加になり、起動の組み立てと文面の文字列が同時に決まる。
...
/// リリースの実行ファイルの、構築の置き場からの相対の文字列。生の文字列が在るのはこの1行だけである。
/// 構築の置き場を答えるのは`検証の出力ルート`である。起こす側がその置き場を別に綴ると、
/// 構築したものと別の版(または無いもの)を起こすことになる。
const リリース実行ファイルの相対の文字列: &str = "release/blitz_app.exe";
...
impl アプリの起こし方 {
    ...
    /// 起動できなかったときの文面へ出す文字列。どちらのやり方で起こそうとして失敗したのかを言う。
    pub fn 表示用の表記(self) -> &'static str {
        match self {
    ...
    /// cargoへ渡す引数の並び。実行ファイルを直に起こす選択肢はcargoを通さないため値を持たない。
    ///
    /// 末尾の`--`まで含めるのは、ここから先がアプリ自身の引数であることを選択肢ごとに決めさせないためである。
    ...
    pub(crate) fn リリース実行ファイルのパス() -> PathBuf {
        検証の出力ルート::いま使っている木().join(リリース実行ファイルの相対の文字列)
    }
}
