//! 骨格方式の世界が焼くシーンの文字列と、置き場から焼き先1つ分の値を組む書き方。
//!
//! **1ベイ1段の並びと骨格を連ねた見本は同じシーンを焼き、読む出力ルートだけが違う。** シーンの安定IDが同じである
//! ことが、初期カメラも検収計画も1つで足りることの根拠であり、その文字列が2つの入口へ写ると、片方だけ直した
//! 食い違いが起きる。文字列をここ1つに置く。
//!
...
/// 骨格方式の世界が焼くシーンの安定ID。初期カメラと検収計画をこの文字列が選ぶ。
pub(super) const 骨格方式のシーン名: 検収シーン名 = 検収シーン名::生成する("prop_part_frame_row");

/// 骨格方式の世界が焼く実行時形式のファイル名。シーンの安定IDと同じ文字列から決まる。
