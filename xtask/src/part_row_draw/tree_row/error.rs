//! 部品で組んだ木の並びの撮影が返す型付きエラー。破れうる前提を選択肢で数え上げ、どの前提が破れたかを型で言う。
//!
//! xtaskは外部のクレートへ依存しないため、thiserrorを使わず手書きのenumと`Display`で書く。
//! 様式は`acceptance/error.rs`と`part_house_row_draw/error.rs`に倣う。
//!
//! 検収の器の破れと判定の破れと世界の用意の破れを選択肢で内包するのは、この入口が焼き付けと起動と読み取りと判定を
...
impl std::error::Error for 木の並びの撮影エラー {}

impl From<検収エラー> for 木の並びの撮影エラー {
    fn from(破れ: 検収エラー) -> Self {
        Self::検収の器が破れた(破れ)
    }
}

impl From<判定の破れ> for 木の並びの撮影エラー {
    fn from(破れ: 判定の破れ) -> Self {
        Self::判定が破れた(破れ)
    }
}

impl From<生成器エラー> for 木の並びの撮影エラー {
