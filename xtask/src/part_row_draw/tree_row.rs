...
//! 1本の木が数画素にしかならず、幹の継ぎ目も選択肢の出方も葉房の付き方も絵から読み取れない。木の姿の合否は
//! 親エージェントの目視が確定する。
//!
...
fn 検収する() -> Result<String, 木の並びの撮影エラー> {
    ...
    let 標準出力 = crate::compile_assets::既定のソースから焼き標準出力を返す(&crate::compile_assets::部品で組んだ木の並びの出力ルート(), crate::asset_generator::世界名::部品で組んだ木の並びの世界)?;
    print!("{標準出力}");
    let 勘定 = 焼いた並びの勘定::標準出力から読む(標準出力)?;
    検収世界.焼き上がりを確かめる(true).map_err(木の並びの撮影エラー::検収世界を用意できなかった)?;
