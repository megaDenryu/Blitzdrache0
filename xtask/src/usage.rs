...
pub(crate) fn 使い方を表示する() {
    ...
    println!(
        "  instance-cull    可視判定のオンとオフで植生シーンを描き、画面内の絵がバイト一致すること・視錐台外の個体が描かれなくなること・帯ごとに可視数が絞られること・画面外の個体が落とす影が残ることを確かめる"
    );
    println!(
    ...
    println!(
        "  cloth-empty      群がカメラ視錐台にもどの帯のライト視錐台にも入らないシーンを布ありと布なしで描き、発行が0件のフレームでも布のシーン描画と影の記録が成立することを確かめる"
    );
    println!(
