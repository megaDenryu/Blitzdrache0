...
pub(crate) fn 使い方を表示する() {
    ...
    println!(
        "  shadow-probe <resolution|casters|margin|camera> [--rounds N] [--chunk-instances N] [--time-of-day 秒]  律速切り分けの計測バッチ。1つの軸の条件を子プロセスとして交互に起動し、距離区分別GPU時間の中央値とp95・投入インデックス数・可視数を条件ごとにまとめて出す(判定はしない。周回数は条件数の倍数へ切り上げる。生値と実行ログは軸ごとにtarget/shadow_probe/<軸名>/へ残す)"
    );
    println!(
