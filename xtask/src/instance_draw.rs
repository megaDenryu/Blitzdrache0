...
use crate::acceptance::{判定の名前, 描画フレーム数, 描画検収の実行環境, 検収の1回の実行, 検収の実行名, 検収シーン名};
...
const 画素判定シーンの名前: &str = "vegetation_4";
const 計数判定シーンの名前: &str = "vegetation_64";
const 画素判定シーン: 検収シーン名 = 検収シーン名::生成する(画素判定シーンの名前);
const 計数判定シーン: 検収シーン名 = 検収シーン名::生成する(計数判定シーンの名前);
...
/// 実測を文字列へ入れないのは、この判定が数を型のまま両側へ置けるためである。何を比べたかは名前が言う。
const 個体数を増やした後のGPU確保数: 判定の名前 = 判定の名前::定数から生成する("個体数を増やした後のGPU確保数(増やす前の実行と一致するはずである)");
...
fn 検収する() -> Result<String, 植生インスタンス描画の検収エラー> {
    ...
    let シェーダー入口 = crate::shader_copy::一時コピーを作る(&検証の出力ルート::既定().名前が指す置き場(シェーダーコピー先)).map_err(植生インスタンス描画の検収エラー::シェーダーの一時コピーを作れなかった)?;

    let 少数 = 一条件を描く(&実行環境, 画素判定シーン, 画素判定シーンの名前, &シェーダー入口)?;
    let 画素 = pixel_check::判定する(少数.画像())?;
    let 少数計数 = crate::report_parse::終了時報告から計数報告を取り出す(少数.報告())?;
    count_check::計数を検査する(&少数計数, 画素判定の個体数)?;

    let 多数 = 一条件を描く(&実行環境, 計数判定シーン, 計数判定シーンの名前, &シェーダー入口)?;
    let 多数計数 = crate::report_parse::終了時報告から計数報告を取り出す(多数.報告())?;
...
fn 一条件を描く(実行環境: &描画検収の実行環境, シーン名: 検収シーン名, 実行名の文字列: &str, シェーダー入口: &Path) -> Result<検収の1回の実行, 植生インスタンス描画の検収エラー> {
    let 実行 = 実行環境.描いて読み戻す(検収の実行名::生成する(実行名の文字列)?, &vegetation_run::植生世界の起動指定を組み立てる(シーン名, フレーム数, シェーダー入口, &起動引数))?;
    実行.報告().画面へ流す();
    Ok(実行)
}
