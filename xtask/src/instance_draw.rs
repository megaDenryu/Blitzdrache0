...
//! 個体が1体だけの対象が描画対象ユニフォームの先頭を個体変換として読む経路は、恒等でないTRSを与えた1体の世界で別に見る。
...
fn 検収する() -> Result<String, String> {
    ...
    let 単一 = single_instance::検収する(&出力先, &シェーダー入口)?;
    Ok(format!(
        "領域別非背景画素{:?}、シーン発行数1・帯別発行数{:?}、個体数{画素判定の個体数}と{計数判定の個体数}、GPU確保数はどちらも{}、単一個体は{単一}",
        画素.領域別非背景画素数,
        多数計数.帯別シャドウ.iter().map(|発行| 発行.発行数).collect::<Vec<u64>>(),
        少数計数.現在確保数
    ))
}
...
/// いずれかの帯へ影を落とす個体も全個体になる。帯ごとの個体数は帯の分割で変わるため、個々の帯へは期待値を置かない。
fn 計数を検査する(計数: &計数報告, 期待個体数: u64) -> Result<(), String> {
    ...
    // 群選択の候補数は可視判定へ登録された群の個体だけを数える。この世界は群1つだけであるため期待個体数と一致する。
    if 計数.群選択.帯和集合可視数 != 期待個体数 || 計数.群選択.候補数 != 期待個体数 {
        return Err(format!(
            "群の候補{}体のうちいずれかの帯へ影を落とす個体が{}体である: 期待はどちらも{期待個体数}体",
            計数.群選択.候補数, 計数.群選択.帯和集合可視数
        ));
    }
    let 帯別発行数: u64 = 計数.帯別シャドウ.iter().map(|発行| 発行.発行数).sum();
    if 帯別発行数 == 0 {
        return Err("どの帯も影の発行を積んでいない".to_string());
    }
