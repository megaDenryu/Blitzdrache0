//! 小物の描画の検収が返す型付きエラー。破れうる前提を選択肢で数え上げ、どの前提が破れたかを型で言う。
//!
...
#[derive(Debug)]
pub(super) enum 小物の描画の検収エラー {
    検収の器が破れた(検収エラー),
    判定が破れた(判定の破れ),
    検収世界を用意できなかった(検収世界の用意の破れ),
}

impl std::error::Error for 小物の描画の検収エラー {}

impl From<検収エラー> for 小物の描画の検収エラー {
    fn from(破れ: 検収エラー) -> Self {
        Self::検収の器が破れた(破れ)
    }
}

impl From<判定の破れ> for 小物の描画の検収エラー {
