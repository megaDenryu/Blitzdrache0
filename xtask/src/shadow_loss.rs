...
fn 欠落と余分を数える(指定: &args::指定) -> Result<String, 影の欠落計器のエラー> {
    ...
    report::比較結果を表にして表示する(&比較);
    diff_image::差分画像を書き出す(&差分先, 比較.幅, 比較.高さ, &比較)?;
    let 差分png = crate::raw_png::変換する(&差分先).map_err(|破れ| 検収エラー::目視用の絵へ変換できなかった { 書き出し先: 差分先.clone(), 破れ })?;
    Ok(format!("構図{}・候補{}、差分画像は{}", 指定.構図.文字列(), 候補の起動指定.join(" "), 差分png.display()))
}
