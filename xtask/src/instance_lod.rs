...
fn 検収する() -> Result<String, 植生個体別段選択の検収エラー> {
    ...
    let シェーダー入口 = crate::shader_copy::一時コピーを作る(&検証の出力ルート::既定().名前が指す置き場(シェーダーコピー先)).map_err(植生個体別段選択の検収エラー::シェーダーの一時コピーを作れなかった)?;
...
fn 描く(実行環境: &描画検収の実行環境, 名前: &str, シェーダー入口: &Path, 追加引数: &[&str]) -> Result<judgment::実行, 植生個体別段選択の検収エラー> {
    let mut 引数: Vec<&str> = 共通引数.to_vec();
    引数.extend_from_slice(追加引数);
    let 結果 = 実行環境.描いて読み戻す(検収の実行名::生成する(名前)?, &vegetation_run::植生世界の起動指定を組み立てる(検収シーン, フレーム数, シェーダー入口, &引数))?;
    結果.報告().画面へ流す();
    let 計数 = crate::report_parse::終了時報告から計数報告を取り出す(結果.報告())?;
    Ok(judgment::実行 {
        名前: 名前.to_string(),
        画像: 結果.画像を取り出す(),
        計数,
