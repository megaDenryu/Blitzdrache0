...
pub(crate) struct 大規模世界の計測指定 {
    ...
    pub(crate) フレーム数: u32,
    pub(crate) 先読み半径: u8,
    pub(crate) ramの上限バイト数: u64,
    pub(crate) vramの上限バイト数: u64,
...
impl Default for 大規模世界の計測指定 {
    fn default() -> Self {
        Self {
            ...
            フレーム数: 1_000,
            先読み半径: 8,
            ramの上限バイト数: 512 * 1024 * 1024,
            vramの上限バイト数: 512 * 1024 * 1024,
            ワーカー本数: 1,
            要求容量: 64,
            完了容量: 4,
            始点東: 0.0,
...
pub(crate) fn 引数を読む(引数一覧: &[String]) -> Result<大規模世界の計測指定, String> {
    ...
    while let Some(名前) = 残り.next() {
        ...
        match 名前.as_str() {
            ...
            "--ram-limit" => 指定.ramの上限バイト数 = 正の数を読む(名前, 値)?,
            "--vram-limit" => 指定.vramの上限バイト数 = 正の数を読む(名前, 値)?,
