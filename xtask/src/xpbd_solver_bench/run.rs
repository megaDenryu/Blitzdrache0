...
pub(super) fn 一回走らせる(
    実行環境: &世界を読まずに報告を採る実行環境, 出力先: &Path, 指定: &実行の指定, 条件: 実行条件, 位置: 周回の位置, 実行番号: usize
) -> Result<終了時報告, XPBDの並列方式の計測エラー> {
    let 標準出力先 = PathBuf::from(出力先).join(format!("run_{実行番号}_周回{}_{}.log", 位置.周回番号, 条件.名前()));
    println!("[xtask] xpbd-solver-bench実行{実行番号}: 周回{}の{}番目 {}", 位置.周回番号, 位置.順序位置, 条件.名前());
    let 引数 = 起動の引数を組み立てる(指定, 条件);
    let 引数の参照: Vec<&str> = 引数.iter().map(String::as_str).collect();
    let 報告 = 実行環境.報告を採る(条件.実行名を組む()?, &引数の参照)?;
    std::fs::write(&標準出力先, 報告.本文()).map_err(|誤り| XPBDの並列方式の計測エラー::実行の標準出力を書けなかった { パス: 標準出力先, 誤り })?;
    Ok(報告)
}
