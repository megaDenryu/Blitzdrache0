...
fn 走らせる(引数一覧: &[String]) -> Result<Vec<record::一標本>, 律速切り分けの計測エラー> {
    ...
    let 出力先 = ルート.join(指定.軸.文字列());
    std::fs::create_dir_all(&出力先).map_err(|誤り| 律速切り分けの計測エラー::出力先を作れなかった { パス: 出力先.clone(), 誤り })?;
    let シェーダー入口 = crate::shader_copy::一時コピーを作る(&検証の出力ルート::既定().名前が指す置き場(シェーダーコピー先)).map_err(律速切り分けの計測エラー::シェーダーの一時コピーを作れなかった)?;
