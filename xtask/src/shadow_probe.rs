...
//! 後から順序の影響を疑えなくなるためである。実行時アセットは物量が同じなら同じものであるため軸をまたいで共有する。
...
fn 走らせる(引数一覧: &[String]) -> Result<Vec<record::一標本>, String> {
    ...
    if !crate::release_build::実行する("shadow-probe") {
        return Err("リリースビルドに失敗した".to_string());
    }
    let ルート = PathBuf::from(出力ディレクトリ);
    let 出力先 = ルート.join(指定.軸.ディレクトリ名());
    std::fs::create_dir_all(&出力先).map_err(|誤り| format!("出力先を作れなかった: {誤り}"))?;
    let シェーダー入口 = crate::shader_copy::一時コピーを作る(Path::new(シェーダーコピー先))?;
    let アセットルート = ルート.join(format!("assets_{}", 指定.チャンクあたり個体数));
    if !crate::compile_assets::地形世界を個体数指定で生成する(&アセットルート, 指定.チャンクあたり個体数) {
        return Err("実行時アセット生成に失敗した".to_string());
    }
