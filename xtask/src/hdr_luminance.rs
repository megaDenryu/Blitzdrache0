...
//! 担当するのは、目視見本世界を一日の代表4時刻で起動し、明るさの圧縮を通す前のHDRカラーを読み戻して、
//! その画面全体のlog2輝度の分布を報告することである。
//!
//! 圧縮前を読むのは、明るさの圧縮とsRGB符号化を通った後の8ビット値から圧縮前の値を復元できないためである。
//! 画面全体で採るのは、自動露出が画面全体のヒストグラムを見るためであり、空も太陽円盤も除かない。
//!
//! 画素の基準値は持たない。範囲の制定はここで採った値を人が読んで決めることであり、
//! 期待値をこの入口に置くと、まだ決めていない範囲を検収側が先に決めたことになる。
