//! 大気のベイク済み画像生成パス数の報告行を読む工程。受け取るのは標準出力と条件の名前、返すのはフレームごとの本数の列である。
//! 行の綴りは`crates/blitz_app/src/reports/atmosphere_passes.rs`の出力と一致させている。

const 見出し: &str = "大気のベイク済み画像生成パス数";

pub(super) struct 生成パス数の列 {
    pub(super) 数えたフレーム数: u64,
    pub(super) 総数: u64,
    /// 報告側が数えた列の長さ。実際に読めた要素数と別に持つのは、区切りの欠けで列が切れた場合に
    /// その食い違いを整合検査が捉えられるようにするためである。
    pub(super) 報告された列の長さ: u64,
