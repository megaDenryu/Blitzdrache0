...
use crate::acceptance::{アプリの起こし方, アプリの起動指定, 実行時アセットルート, 描画フレーム数, 描画検収の実行環境, 検収の1回の実行, 検収の実行名, 検収エラー, 検収シーン名};
...
pub(super) fn 描く(実行環境: &描画検収の実行環境, 実行名: 検収の実行名, 条件: 条件, 一日内秒: Option<&str>) -> Result<検収の1回の実行, 検収エラー> {
