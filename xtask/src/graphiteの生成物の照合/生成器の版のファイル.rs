...
//! 注意: この確かめ方には3つの限界がある。cargo の内部ファイル `.crates.toml` を読むため、cargo がその書式を変えると読めなくなる。
//! `CARGO_INSTALL_ROOT` で別の置き場へ入れた生成器は記録が見つからず、食い違いとして落ちる。実際に走る実行ファイルそのものは確かめていない。
//! 前の2つは正しい生成器を落とす向きに働く。3つ目は、cargo を通さずに実行ファイルを置き換えた場合に限り、記録と違う生成器を通しうる。
