...
//! 検証層が有効なデバッグビルドで走らせるのは、描画束の解除がこの経路でしか実行されず、解除時の検証エラーを見逃さないためである。

use super::{order_compare, order_sequence};
use crate::acceptance::{アプリの起こし方, アプリの起動指定, 世界を読ませて報告を採る実行環境, 実行時アセットルート, 描画フレーム数, 検収の実行名, 検収シーン名};
...
fn 一度実行する(回: u32) -> Option<Vec<String>> {
    println!("[xtask] 条件B: {回}回目をデバッグビルド(検証層を有効にした状態)で{}フレーム実行", フレーム数.枚数());
    let 実行環境 = 世界を読ませて報告を採る実行環境::作る(
        アプリの起こし方::毎回cargoに構築させて起動する,
        実行時アセットルート::パスから生成する(検証の出力ルート::既定().名前が指す置き場(crate::compile_assets::板の世界の実行時形式の置き場)),
    );
    let 実行名 = 検収の実行名::生成する(&format!("condition_b_{回}")).map_err(|誤り| eprintln!("[xtask] {誤り}")).ok()?;
    let 報告 = 実行環境.報告を採る(実行名, &起動指定を組み立てる()).map_err(|誤り| eprintln!("[xtask] 条件Bの{回}回目が失敗した: {誤り}")).ok()?;
    let 標準出力 = 報告.本文();
    for 行 in 標準出力.lines().filter(|行| 検証層の指摘件数の見出し.行の始まりか(行)) {
        println!("  {行}");
    }
