//! 順3-Iaの検収入口。遠方環境の立方体画像を検証層を有効にしたウィンドウなし実行のGPUで焼いて読み戻し、
//! 鍵の判定・検証層の指摘・円盤非二重計上の負の対照・全テクセルの健全性・代表テクセルのCPU正本との一致・
//! 面境界の連続性を判定する。
//!
...
fn 表を出す(報告: &parse::報告) {
    println!("遠方環境の読み戻し:");
    println!("  テクセル数: {} (非有限{} 負{} 予約が非零{})", 報告.テクセル数, 報告.非有限, 報告.負, 報告.予約が非零);
...
/// 遠方環境を焼かせて報告を採る。検証層の指摘は判定の一部としてこの入口が読むため、
/// 実行環境が零件を確かめる口は通さない。
