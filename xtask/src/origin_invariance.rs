...
fn 四条件を描く(実行環境: &描画検収の実行環境, 条件一覧: &[(&str, 検査条件); 4]) -> Option<[読み戻し画像; 4]> {
    ...
    for (実行名の文字列, 条件) in 条件一覧 {
        絵一覧.push(一条件を描く(実行環境, 実行名の文字列, 条件)?);
    }
    絵一覧.try_into().ok()
}

fn 一条件を描く(実行環境: &描画検収の実行環境, 実行名の文字列: &str, 条件: &検査条件) -> Option<読み戻し画像> {
    let 実行名 = 検収の実行名::生成する(実行名の文字列).map_err(|誤り| eprintln!("[xtask] {誤り}")).ok()?;
    match 実行環境.描いて読み戻す(実行名, &run::起動指定を組み立てる(条件)) {
        Ok(実行) => Some(実行.画像を取り出す()),
        Err(誤り) => {
            eprintln!("[xtask] {実行名の文字列}の描画か読み戻しに失敗した: {誤り}");
            None
        }
    }
}
