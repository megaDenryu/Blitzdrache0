...
pub fn 実行する() -> ExitCode {
    ...
    let 近傍 = match run::読み戻しを取る(出力先, "near", &静止条件([0.0, 0.0, 0.0])) {
        Some(画像) => 画像,
        None => return ExitCode::FAILURE,
    };
    let 遠方一回目 = match run::読み戻しを取る(出力先, "far_first", &静止条件(大移動メートル)) {
        Some(画像) => 画像,
        None => return ExitCode::FAILURE,
    };
    let 遠方二回目 = match run::読み戻しを取る(出力先, "far_second", &静止条件(大移動メートル)) {
        Some(画像) => 画像,
        None => return ExitCode::FAILURE,
    };
    let 遠方カメラずらし = match run::読み戻しを取る(出力先, "far_nudged", &カメラずらし条件()) {
        Some(画像) => 画像,
        None => return ExitCode::FAILURE,
    };

    let 判定一覧 = [
        compare::一致を求める("同じ指定の2回実行が一致する", &遠方一回目, &遠方二回目),
        compare::一致を求める("オフセット0と大移動が一致する", &近傍, &遠方一回目),
        compare::不一致を求める("大移動のままカメラだけ動かすと変わる", &遠方一回目, &遠方カメラずらし),
    ];
