...
fn 未コミット変更が無いことを確かめる(名前: &'static str, 作業コピー: &Path) -> Result<(), 大気の期待値の焼き出しエラー> {
...
fn gitの出力を読む(作業コピー: &Path, 引数一覧: &[&str]) -> Result<String, 大気の期待値の焼き出しエラー> {
    let 出力 = Command::new("git").arg("-C").arg(作業コピー).args(引数一覧).output().map_err(|誤り| 大気の期待値の焼き出しエラー::作業コピーでgitを起こせなかった {
        作業コピー: 作業コピー.to_path_buf(), 誤り
    })?;
    ...
    let 文字列 = String::from_utf8(出力.stdout).map_err(|誤り| 大気の期待値の焼き出しエラー::作業コピーの照合の標準出力がUTF8でない { 誤り })?;
