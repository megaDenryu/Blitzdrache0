//! 計測用のリリースビルドが返す型付きエラー。破れうる前提を選択肢で数え上げ、どの前提が破れたかを型で言う。
//!
//! xtaskは外部のクレートへ依存しないため、thiserrorを使わず手書きのenumと`Display`で書く。
//! 様式は`acceptance/error.rs`に倣う。
//!
//! 「起こせなかった」と「失敗して終わった」を別の選択肢で持つのは、直す側が見る場所が違うためである。
...
impl std::fmt::Display for 計測用の構築の破れ {
    fn fmt(&self, 書き手: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::構築が失敗して終わった { コマンド名, 終了状態 } => write!(書き手, "{コマンド名}のリリースビルドが終了コード{終了状態}で失敗した(直そうとした版で測れないためここで止める)"),
        }
    }
}
