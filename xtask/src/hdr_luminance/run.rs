//! 1時刻ぶんのblitz_app起動。受け取るのはダンプ先と代表時刻、返すものは無く、
//! 成功したときには`<ダンプ先>.hdr32`と`<ダンプ先>.size`が書かれている。
//!
//! シーン名を`terrain_visual`にすることが、庭を正面から見る初期カメラと、書き換えもピクセル判定も持たない検収計画と、
//! 天空の遠方環境の間接照明方針を同時に選ばせる。空もポスト処理も本番の経路のままにするのは、
//! 自動露出が見るのが本番の絵と同じHDRカラーだからである。監視対象シェーダーは`terrain-visual`と同じく指定しない。
//!
