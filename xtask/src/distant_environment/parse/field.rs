...
pub(crate) fn 語句値(語一覧: &[&str], 鍵: &str) -> Result<String, String> {
    Ok(値を探す(語一覧, 鍵)?.to_string())
}

pub(crate) fn 整数値(語一覧: &[&str], 鍵: &str) -> Result<usize, String> {
    値を探す(語一覧, 鍵)?
        .parse::<usize>()
        .map_err(|誤り| format!("{鍵}を整数として読めない: {誤り}"))
}

pub(crate) fn 実数値(語一覧: &[&str], 鍵: &str) -> Result<f64, String> {
    値を探す(語一覧, 鍵)?
        .parse::<f64>()
        .map_err(|誤り| format!("{鍵}を実数として読めない: {誤り}"))
}

pub(crate) fn 添字値(語一覧: &[&str], 鍵: &str) -> Result<u32, String> {
    let 値 = 整数値(語一覧, 鍵)?;
