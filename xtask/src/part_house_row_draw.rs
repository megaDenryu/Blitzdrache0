//! 部品で建てた家並みの検収入口。同じ規則で10軒と100軒を焼き、どちらも本番の描画経路へ通して、
//! シーンパスの発行数が軒数に依らないことと、個体数が焼く工程の勘定と一致することを確かめ、目視用のPNGを書き出す。
//!
//! **これが段4の実証である。** 発行数が軒数に比例し始めたら、部品の組み合わせでバリエーションを作るという
//! 設計は前提から誤っている。
//!
...
use error::部品の家並みの検収エラー;
use measurement::規模ごとの実測;
use scale::家並みの規模;
use tally_line::焼いた家並みの勘定;
...
fn 検収する() -> Result<String, 部品の家並みの検収エラー> {
    if !crate::gen_source_assets::生成する() {
        return Err(部品の家並みの検収エラー::ソースアセットを生成できなかった);
    }
    let [少ないほう, 多いほう] = 家並みの規模::全規模();
...
/// 先に言うためである。
fn 規模1つを測る(規模: 家並みの規模) -> Result<規模ごとの実測, 部品の家並みの検収エラー> {
    規模
        .前回の焼き上がりを消す()
        .map_err(部品の家並みの検収エラー::前回の焼き上がりを消せなかった)?;
    let 標準出力 = crate::compile_assets::既定のソースから焼き標準出力を返す(規模.出力ルート(), 規模.世界())?;
    print!("{標準出力}");
    let 勘定 = 焼いた家並みの勘定::標準出力から読む(標準出力)?;
    規模
        .焼き上がりの確かめ方()
        .焼き上がりを確かめる(true)
        .map_err(部品の家並みの検収エラー::検収世界を用意できなかった)?;
