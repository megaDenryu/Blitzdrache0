...
const シーンの名前: &str = "instance_all_culled";
...
fn 検収する() -> Result<String, 描画入力が空のフレームの布の検収エラー> {
    ...
    let シェーダー入口 = crate::shader_copy::一時コピーを作る(&検証の出力ルート::既定().名前が指す置き場(シェーダーコピー先)).map_err(描画入力が空のフレームの布の検収エラー::シェーダーの一時コピーを作れなかった)?;

    let 布あり = 描く(&実行環境, "cloth", &シェーダー入口, &布ありの引数)?;
    let 布なし = 描く(&実行環境, "no_cloth", &シェーダー入口, &布なしの引数)?;
    let 布あり計数 = crate::report_parse::終了時報告から計数報告を取り出す(布あり.報告())?;
    let 布なし計数 = crate::report_parse::終了時報告から計数報告を取り出す(布なし.報告())?;
    ...
    Ok(format!(
        "{シーンの名前}はシーンと全距離区分のどれも候補{}体・可視0体・発行0件で、布ありの実行だけが{布の画素数}画素を描いた(布ありはどのパスもパイプライン切替1回・描画1回、布なしはどちらも0回。validationは両実行とも0件)",
        布あり計数.シーン.候補数
    ))
}

fn 描く(実行環境: &描画検収の実行環境, 名前: &str, シェーダー入口: &Path, 追加引数: &[&str]) -> Result<検収の1回の実行, 描画入力が空のフレームの布の検収エラー> {
    let 実行 = 実行環境.描いて読み戻す(検収の実行名::生成する(名前)?, &vegetation_run::植生世界の起動指定を組み立てる(シーン, フレーム数, シェーダー入口, 追加引数))?;
    実行.報告().画面へ流す();
    Ok(実行)
}
