...
fn 検収する() -> Result<String, String> {
    ...
    let 停止 = run::描画する(&出力先, "lut_stopped", run::条件::時計停止)?;
    let 進行 = run::描画する(&出力先, "lut_advancing", run::条件::時計進行)?;
    ...
    Ok(format!(
        "時計停止は{}フレームで生成パス{}本(列の先頭{:?})、時計進行は{}フレームで生成パス{}本(うちスカイビュー1本のフレームが{}回)、計器は{}、絵は{}",
        停止の列.数えたフレーム数,
        停止の列.総数,
        停止の列.先頭の並び(4),
        進行の列.数えたフレーム数,
