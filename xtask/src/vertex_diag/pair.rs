...
impl 診断の対 {
    ...
    pub(super) fn 描き比べる(&self, 条件名: &str, 追加の選択肢: &[&str]) -> Result<(run::実行結果, run::実行結果), 頂点量の診断の検収エラー> {
...
fn 一世界を焼く(出力先: &Path, 名前: &str, 世界: 世界名, チャンクあたり個体数: usize) -> Result<PathBuf, 頂点量の診断の検収エラー> {
