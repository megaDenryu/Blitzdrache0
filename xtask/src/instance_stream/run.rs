...
use crate::acceptance::{アプリの起こし方, アプリの起動指定, 実行時アセットルート, 描画フレーム数, 描画検収の実行環境, 検収の実行名, 検収エラー, 検収シーン名, 読み戻し画像};
...
const 共通の選択肢: [&str; 8] = ["--streaming", "--instance-stream-route", "--report-streaming-summary", "--report-memory", "--report-draw-issue", "--unlit", "--no-sky", "--no-post"];
...
pub(super) fn 走らせる(
    実行環境: &描画検収の実行環境, 名前: &str, シェーダー入口: &Path, フレーム数: 描画フレーム数, 追加の選択肢: &[&str]
) -> Result<実行, 植生ストリーミング統合の検収エラー> {
    let 実行結果 = 実行環境.描いて読み戻す(検収の実行名::生成する(名前)?, &起動指定を組み立てる(フレーム数, シェーダー入口, 追加の選択肢))?;
    実行結果.報告().画面へ流す();
...
fn 起動指定を組み立てる(フレーム数: 描画フレーム数, シェーダー入口: &Path, 追加の選択肢: &[&str]) -> アプリの起動指定 {
