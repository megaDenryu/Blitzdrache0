...
pub(super) fn 走らせる(
    ...
    実行結果.報告().画面へ流す();
    // 注意: ストリーミングの要約報告の読み取りだけ標準出力の綴りを直に受ける。この工程は検収の器の口へまだ移っていない。
    let 要約 = crate::streaming_report::取り出す(実行結果.報告().本文())
        .map_err(|理由| 植生ストリーミング統合の検収エラー::ストリーミングの要約報告を読めなかった { 理由 })?;
    Ok(実行 {
        名前: 名前.to_string(),
