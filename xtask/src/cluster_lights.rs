...
//! 読み戻しの決定性・検証層の指摘が0件であること・光の件数を振ったときのGPU時間と割り当ての統計を採る。
//!
//! 判定は4つである。同一の起動の中で撮った2枚がバイト単位で一致することと、そのとき出た割り当ての2行も一致すること、
//! どの実行も検証層の指摘が0件であること、屋内の灯を消したときより灯を点けたときのほうが屋内が明るいことである。
//! **GPU時間には判定を置かない。**
...
fn 検収する() -> Result<String, クラスタ多光源の検収エラー> {
    ...
    let 夜の絵 = determinism::同一起動の2枚の一致を確かめる(&夜の実行環境)?;
    let 結果一覧 = scale::光の件数を振って測る(&夜の実行環境)?;
    let 屋内 = interior::屋内の明るさを測る(&屋内の実行環境)?;
