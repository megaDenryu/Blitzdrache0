...
pub(super) enum 計器の様式 {
    影の欠落を数える, // 診断出力を読み、受光距離帯ごとの欠落と余分を数える
    最終色の絵を撮る, // 本番の見た目で2枚を撮る。比較も判定もしない
}
...
pub(super) fn 引数を読む(引数一覧: &[String]) -> Result<指定, 影の欠落計器の引数の破れ> {
    ...
    while let Some(語) = 残り.next() {
        if 語 == "--layout" {
            ...
            構図 = Some(scene_choice::文字列から読む(値)?);
            continue;
        }
        if 語 == "--final-color" {
            様式 = 計器の様式::最終色の絵を撮る;
        ...
        let 軸 = 計測軸::文字列から読む(語).ok_or_else(|| 影の欠落計器の引数の破れ::知らない引数を渡された { 語: 語.clone() })?;
        let 値 = 残り.next().ok_or_else(|| 値が無い破れ(語))?;
...
fn 組み立てる(構図: scene_choice::構図, 候補: Option<候補の計測指定>, 様式: 計器の様式) -> Result<指定, 影の欠落計器の引数の破れ> {
    let 候補 = 候補.ok_or_else(|| 影の欠落計器の引数の破れ::候補の指定が1つも無い {
        選べる軸: 計測軸::文字列を並べる(&計測軸::全軸),
    })?;
