...
pub(super) fn 二枚を撮る(指定: &指定) -> Result<String, 影の欠落計器のエラー> {
    ...
    Ok(format!("最終色の2枚を撮った(比較も判定もしない)。基準は{}、候補は{}", 基準.display(), 候補.display()))
}

/// 撮った絵の名前。軸と距離を名前へ入れるのは、αの組とβの組を続けて撮ったときに上書きし合わないためである。
/// 4枚を並べて選ぶには4枚が同時にディスクへ残っている必要がある。
fn 実行名を組む(候補: &候補の計測指定, 役: &str) -> String {
    let 軸 = 候補.文字列().trim_start_matches("--").replace('-', "_");
    format!("final_{軸}_{}_{役}", 候補.距離の文字列())
}

fn 撮る(実行環境: &描画検収の実行環境, 実行名の文字列: &str, 構図: 構図, 候補の起動指定: &[String]) -> Result<PathBuf, 検収エラー> {
    let 指定 = super::launch::共通の起動指定を組み立てる(構図)
        .選択肢を足す("--no-taa")
        .選択肢をまとめて足す(&候補の起動指定.iter().map(String::as_str).collect::<Vec<&str>>());
    let 実行 = 実行環境.描いて読み戻す(検収の実行名::生成する(実行名の文字列)?, &指定)?;
    実行.書き出し先().目視用の絵へ変換する()
}
