...
pub(super) const コマンドの説明一覧: [&str; 40] = [
    ...
    "  indirect-probe   金属と誘電体の板を粗さの水準ごとに並べた専用世界を本番のシーン描画経路で描き、遠方環境の派生表現へ注入した解析入力から組んだ期待値と読み戻した画素の一致を、拡散だけ・鏡面の段別・鏡面の面別(2つの時刻)の4条件で判定し、注入なしの昼の描画で飽和と黒潰れの割合を測り、5条件ともvalidationの指摘0件を確かめる",
