...
pub(super) fn 計測の入口を表示する() {
    ...
    println!(
        "  shadow-loss [--layout terrain|range] <--max-shadow-distance メートル | --shadow-caster-range メートル>  同じ構図を基準(現行設定)と候補で1回ずつ描き、シーンの画素段の診断出力(多段影の評価直後の影可視度と固定25メートル刻みの受光距離帯)から、受光距離帯ごとの影の欠落と余分と影可視度の弱まり・強まりを数えて差分画像を書き出す(合否は判定しない。ただしrangeは遠方キャスターと近距離対照だけを置いた負の対照の世界であり、遠方キャスターの影が消え近距離対照の影が残ることを判定する)"
    );
}
