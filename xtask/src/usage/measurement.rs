...
pub(super) fn 計測の入口を表示する() {
    ...
    println!(
        "  reverse-depth <--capture-reference|--compare-candidate|--print-plan>  terrain_originを固定条件で提示RGBA8と最終深度へ撮る。反転前は対照を保存し、反転後はmask完全一致・線形奥行き誤差・色の1階調/0.01%上限・有限性をA/B判定する"
    );
    println!(
