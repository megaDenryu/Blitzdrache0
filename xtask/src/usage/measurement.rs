...
pub(super) fn 計測の入口を表示する() {
    ...
    println!(
        "  depth-prepass-cost [--frames N] [--time-of-day 秒]  深度プリパスの費用のGPU計測と同値性の検収。本番の地形世界をA(現行)・B(深度プリパス追加で色は同値以下)・C(深度プリパス追加で色は等値・深度書込なし)の順に2周起動し、深度プリパス・色パス・両者のフレーム内合計の区間別GPU時間をp50とp95で出す。合計は計器がフレーム内で2区間を足した合成区間の分位から採る(分位の加法性は成立しないため内訳の分位の和では語らない)。Aは深度プリパスを積まないため合計の区間が立たず、その条件の1フレームの合計は色パスの区間そのものである。続けて三条件の最終深度と提示前HDRを1枚ずつ撮り、AとB・AとCをそれぞれ別々にバイトで突き合わせる。費用の合否も同値性の合否も判定しない(予算の突き合わせはマイルストーンの定点で行い、Cの不変性が立たないことは事実として残して後段を止めない)。機械判定はvalidationの指摘0件と、条件と実際に立った区間が食い違わないことと、測れた値がすべて有限であることの3つだけである。区間別の生値と実行ごとの標準出力と撮った画像はtarget/depth_prepass_cost/へ残す"
    );
    println!(
