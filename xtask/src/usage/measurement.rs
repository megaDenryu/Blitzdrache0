...
pub(super) fn 計測の入口を表示する() {
    ...
    println!(
        "  indirect-cost [--frames N] [--time-of-day 秒]  間接照明の全更新費用のGPU計測。本番の地形世界を時計停止A・時計進行B・時計停止A'の順に起動し、遠方環境生成・鏡面畳込み最詳細段の複製・拡散照度生成・鏡面畳込み生成(全縮小段の合計)・シーン描画・反射率積分表生成の区間別GPU時間をp50とp95で出す。全更新合計(進行Bの生成4区間の分位の和)と更新なし定常(停止Aのシーン描画)と反射率積分表の初回の単発値を別行で出し、停止Aと停止A'の食い違いを機材の状態の揺れとして明示する。費用の合否は判定せず(予算との突き合わせはマイルストーンの定点で行う)、機械判定はvalidationの指摘0件と区間の値がすべて有限であることだけである。区間別の生値と実行ごとの標準出力はtarget/indirect_cost/へ残す"
    );
    println!(
