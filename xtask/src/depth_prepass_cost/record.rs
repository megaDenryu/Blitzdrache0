...
pub(super) enum 区間の観測 {
    積まない, // その条件はその区間を積まない。値が無いことがその条件の性質である
    測れた(区間の分布),
}
...
pub(super) struct 一標本 {
    pub(super) 実行番号: usize, // 起動した順の通し番号。0から始まり、回転の並びがそのまま並ぶ
    pub(super) 周回番号: usize,
    pub(super) 順序位置: usize,
    pub(super) 条件名: &'static str,
    pub(super) 区間別: Vec<区間の観測>,                   // 添字が`intervals::全区間一覧`の添字である
    pub(super) 窓の標本数: usize,                         // 報告する側が分位を採った窓の長さ
    pub(super) フレーム別の値一覧: Vec<フレーム別の生値>, // 読み取り順に並んだフレーム別の値。全区間ぶんが混ざった1本の列である
}
