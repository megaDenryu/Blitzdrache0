...
impl ダンプの別 {
    /// 器が知る書き出しの形式。選択肢の文字列も拡張子もそちらが持つ。
...
pub(super) fn 撮る(実行環境: &描画検収の実行環境, 条件: &実行条件, 別: ダンプの別, 実行名: 検収の実行名, 一日内秒: Option<&String>, フレーム数: 描画フレーム数) -> Result<(), 検収エラー> {
    実行環境.書き出させて報告を採る(実行名, &world::撮影の起動指定を組み立てる(フレーム数, 条件, 一日内秒), 別.書き出しの形式())?;
    Ok(())
}
