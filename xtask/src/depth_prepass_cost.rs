...
//! 機械判定は4つである。validationの指摘が0件であること、条件と実際に立った区間が食い違わないこと、
//! 測れた値がすべて有限であること、報告した分位がフレーム別の生標本から独立に計算し直しても同じ値になることである。
...
fn 計測する(引数一覧: &[String]) -> Result<String, String> {
    ...
    record::窓の集約を書く(&出力先.join(窓の集約ファイル名), &標本一覧)?;
...
fn 要約を組む(観測一覧: &[equality::観測], 出力先: &Path, フレーム数: u32) -> String {
    ...
    format!(
        "1実行{フレーム数}フレームのA・B・Cを回転順で{}周した。報告した分位はフレーム別の生標本から再現された。同値性の突き合わせ{}件のうち{}件がバイト一致した。区間別の表とフレーム別の生値と撮った画像は{}にある",
        schedule::周回数,
        観測一覧.len(),
        一致した数,
        出力先.display()
