...
//!
//! 費用の合否も同値性の合否も判定しない。予算との突き合わせはマイルストーンの定点で人が行い、Cの不変性が立たないことは事実として残すだけで後段を止めない(参照: `_doc/設計/放射輝度問い合わせ階層.md`「IIaの実装設計」)。機械判定は4つである。検証層の指摘が0件であること、条件と実際に立った区間が食い違わないこと、測れた値がすべて有限であること、報告した分位がフレーム別の生標本から独立に計算し直しても同じ値になることである。
//!
