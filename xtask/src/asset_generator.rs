...
//! 実測では5つの入口がそれぞれ`Command::new("cargo")`から書き起こしており、example名の文字列が5箇所、
//! 「起動に失敗」「終了コードで失敗」の文面が5組に散っていた。文字列が散ると、生成器の置き場を変える改定が
//! 5箇所の同時変更になり、直し漏れた入口だけが静かに動かなくなる。
//!
//! 焼ける世界の数え上げは`world_name`が持つ。これが生成器の語彙であるのは、文字列の正本が生成器側にあり、
//! 焼ける対象として渡せるものの集合を決めるのが生成器だからである。
//! 何を生成させるかは`specification`が選択肢で数え上げ、生成器へ渡す語の並びは`arguments`が、
