...
const 正判定シーンの名前: &str = "vegetation_4";
const 正判定シーン: 検収シーン名 = 検収シーン名::生成する(正判定シーンの名前);
/// 視錐台外の番兵と、画面外から画面内へ影を落とす個体を持つ世界。
const 負対照シーンの名前: &str = "vegetation_cull";
const 負対照シーン: 検収シーン名 = 検収シーン名::生成する(負対照シーンの名前);
...
fn 検収する() -> Result<String, 植生個体の可視判定の検収エラー> {
    ...
    let シェーダー入口 = crate::shader_copy::一時コピーを作る(&検証の出力ルート::既定().名前が指す置き場(シェーダーコピー先)).map_err(植生個体の可視判定の検収エラー::シェーダーの一時コピーを作れなかった)?;

    let 正判定 = run::対で描く(&実行環境, 正判定シーン, 正判定シーンの名前, &シェーダー入口)?;
    judgment::正の判定を検査する(&正判定)?;
    let 負対照 = run::対で描く(&実行環境, 負対照シーン, 負対照シーンの名前, &シェーダー入口)?;
    let 影 = judgment::負の対照と影を検査する(&負対照)?;
    Ok(format!(
        "{正判定シーンの名前}は可視判定の有無でバイト一致(シーン可視{}体・距離区分の和集合{}体)、{負対照シーンの名前}は候補{}体のうちシーン可視{}体・距離区分の和集合{}体・距離区分別の可視{:?}でバイト一致、影の暗部{}画素・明部{}画素",
        正判定.オン計数.シーン.可視数,
