...
pub(super) fn 遠景容量を確かめる(報告: &str, 広がり: 世界の広がり) -> Result<String, String> {
    ...
    let 期待頂点数 = (東セル数 + 1).checked_mul(南セル数 + 1).ok_or_else(|| "期待頂点数が桁あふれした".to_string())?;
    let 期待インデックス数 = 東セル数.checked_mul(南セル数).and_then(|値| 値.checked_mul(6)).ok_or_else(|| "期待インデックス数が桁あふれした".to_string())?;
    let 頂点数 = 数を読む(報告, "頂点数=")?;
