...
pub(super) fn ディレクトリの全ファイルを畳む(ルート: &Path) -> Result<BTreeMap<PathBuf, u64>, 場所巡りの通しの検収エラー> {
...
fn 畳む対象のファイルの判定名(ルート: &Path) -> 判定の名前 {
    判定の名前::組み立てた文字列から生成する(format!("{}の下の畳む対象のファイル", ルート.display()))
}

fn ディレクトリを辿る(ルート: &Path, ディレクトリ: &Path, 対応表: &mut BTreeMap<PathBuf, u64>) -> Result<(), 場所巡りの通しの検収エラー> {
    let 一覧 = std::fs::read_dir(ディレクトリ).map_err(|誤り| 場所巡りの通しの検収エラー::ディレクトリを開けなかった {
        パス: ディレクトリ.to_path_buf(), 誤り
    })?;
    for 項目 in 一覧 {
        let パス = 項目
        ...
        let 相対パス = パス.strip_prefix(ルート).map_err(|_| 場所巡りの通しの検収エラー::相対パスを求められなかった { パス: パス.clone() })?;
...
fn ファイルの中身を畳む(パス: &Path) -> Result<u64, 場所巡りの通しの検収エラー> {
    let mut ファイル = std::fs::File::open(パス).map_err(|誤り| 場所巡りの通しの検収エラー::ファイルを開けなかった { パス: パス.to_path_buf(), 誤り })?;
    let mut 区切り = vec![0_u8; 読み取りの一区切りのバイト数];
    let mut 畳んだ値 = 畳み込みの開始値;
    loop {
        let 読み取ったバイト数 = ファイル.read(&mut 区切り).map_err(|誤り| 場所巡りの通しの検収エラー::ファイルを読めなかった { パス: パス.to_path_buf(), 誤り })?;
        if 読み取ったバイト数 == 0 {
            return Ok(畳んだ値);
        }
