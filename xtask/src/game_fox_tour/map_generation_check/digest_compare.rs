...
pub(super) fn 畳んだ値を突き合わせる(一度目: &BTreeMap<PathBuf, u64>, 二度目: &BTreeMap<PathBuf, u64>, 何の突き合わせか: &str) -> Result<usize, 場所巡りの通しの検収エラー> {
...
fn 判定名を組む(文字列: &str) -> 判定の名前 {
    判定の名前::組み立てた文字列から生成する(文字列.to_string())
}
