...
//! 生成器を1つ足すと両方へ選択肢が増えるが、変わる理由は別である。片方は作らせるものの種類、もう片方は起こし方である。
//!
...
/// 生成器を収めているクレート。全部の選択肢が同じクレートのexampleである。
...
impl 生成の指定<'_> {
    ...
    fn 構築の知らせを伏せるか(&self) -> bool {
        matches!(self, Self::入力するglTFの契約を検査する { .. } | Self::部品カタログを組み上げる { .. } | Self::組み立てを正解表と突き合わせる { .. })
    }
}
...
impl 生成の指定<'_> {
    ...
    ///
    /// 置き場をcargoの出力の文字列から読まず、自分の実行ファイルの隣の`examples`から導くのは、
    ...
    pub fn 生成器を組み立てて実行ファイルのパスを求める(&self) -> Result<PathBuf, 生成器エラー> {
        ...
        if !状態.success() {
            return Err(生成器エラー::生成器が失敗して終わった { 生成器, 終了状態: 状態.to_string() });
        }
        Ok(self.実行ファイルの置き場を求める()?.join(format!("{生成器}{}", std::env::consts::EXE_SUFFIX)))
    }

    fn 実行ファイルの置き場を求める(&self) -> Result<PathBuf, 生成器エラー> {
        let 自分の実行ファイル = std::env::current_exe().map_err(|誤り| 生成器エラー::生成器の実行ファイルの置き場を決められない { 生成器: self.生成器の名前(), 誤り })?;
