...
use super::spelling::{南北チャンク数の文字列, 東西チャンク数の文字列, 種の文字列};
...
pub(super) fn ソース生成の引数を足す(並び: &mut 語の並び, 種: Option<マップ生成の乱数の種>, ソースルート: Option<&ソースルート>, 広がり: Option<世界の広がり>) -> Result<(), 生成器エラー> {
    並び.値を足す(Some(種の選択肢の文字列), "場所巡りの世界の種", 種.map(種の文字列))?;
    if let Some(ルート) = ソースルート {
        並び.語を足す(ソースルートの選択肢の文字列);
        並び.パスを足す(ルート.プロセスの引数へ渡すディレクトリ());
    }
    並び.値を足す(Some(東西チャンク数の選択肢の文字列), "世界の東西チャンク数", 広がり.map(東西チャンク数の文字列))?;
    並び.値を足す(Some(南北チャンク数の選択肢の文字列), "世界の南北チャンク数", 広がり.map(南北チャンク数の文字列))
}
