...
impl<'a> 起動設定<'a> {
    pub(super) fn 生成する(フレーム数: 描画フレーム数, シェーダーパス: &'a Path, アセットルート: Option<&'a Path>, シーン名: 検収シーン名) -> Self {
        Self {
            フレーム数,
            シェーダーパス,
            アセットルート,
    ...
    pub(super) fn 起動指定へ写す(&self) -> アプリの起動指定 {
        ...
        有無と文字列.into_iter().filter(|(有るか, _)| *有るか).fold(基本の指定(self), |指定, (_, 文字列)| 指定.選択肢を足す(文字列))
    }
}

fn 基本の指定(設定: &起動設定<'_>) -> アプリの起動指定 {
    アプリの起動指定::シーンと枚数を決める(設定.シーン名, 設定.フレーム数).パスを値に持つ選択肢を足す("--shader-source", 設定.シェーダーパス)
}
