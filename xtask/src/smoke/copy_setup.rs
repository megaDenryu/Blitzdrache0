...
/// 写す前に生成物のソースアセットを揃えるのは、写す一覧に共有バッファ(`.bin`)が4件並んでおり、
/// それがGitの追跡に入っていない生成物だからである。ここが元のディレクトリを読む最初の工程であり、
/// スモークが焼く工程へ進むより前にある。
pub(super) fn アセットを一時コピーする() -> Result<PathBuf, スモークのアセットの一時コピーの破れ> {
    if !crate::gen_source_assets::生成物のソースアセットを揃えて成否を返す() {
        return Err(スモークのアセットの一時コピーの破れ::生成物のソースアセットが揃わなかった);
    }
...
fn 写す(元パス: PathBuf, 先: &Path) -> Result<(), スモークのアセットの一時コピーの破れ> {
    std::fs::copy(&元パス, 先).map(|_| ()).map_err(|誤り| スモークのアセットの一時コピーの破れ::アセットの1枚を写せなかった { 元パス, 誤り })
}
