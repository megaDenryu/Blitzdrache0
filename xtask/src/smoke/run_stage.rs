...
use crate::acceptance::{アプリの起こし方, 世界を読ませて報告を採る実行環境, 実行時アセットルート, 検収の実行名};
...
pub(super) fn 必須ステージを実行する(実行名: 検収の実行名, 設定: 起動設定<'_>) -> bool {
    ...
    if !ステージを起動して合否を判定する(実行名.clone(), &設定) {
...
fn ステージを起動して合否を判定する(実行名: 検収の実行名, 設定: &起動設定<'_>) -> bool {
...
fn 実行環境を作る(設定: &起動設定<'_>) -> 世界を読ませて報告を採る実行環境 {
    ...
    match 設定.アセットルート {
        ...
        Some(ルート) => 世界を読ませて報告を採る実行環境::作る(起こし方, 実行時アセットルート::パスから生成する(ルート.to_path_buf())),
    }
}
