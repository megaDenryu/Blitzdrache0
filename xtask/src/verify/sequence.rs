...
//!
//! 段が落ちたときの行と、全部通ったときの行の文言は読む側との契約である。読む側はログをその行で探すため、
//! 文字列を変えると探せなくなる。参照: `.claude/agents/ビルド員.md`
...
/// Graphiteの生成物の照合を規約検査の次に置くのは、規約検査が生成物を見出しで見分けて検査から外すだけであり、本文の手編集はこの段でしか捕まらないためである。
/// 生成物の取り揃えをそこへ続けるのは、cloneした直後の作業ツリーにglTFが指す共有バッファが1件も無く、
/// 揃える前にtestの段まで進むとアセットを読む試験がまとめて落ちるためである。
const 自分の実行ファイルで走らせる段の一覧: [&str; 3] = ["conform", "graphite-check", "supply-source-assets"];
