...
//!
//! **この木の名前の文字列を持つのは、リポジトリの中でこのファイルだけである。** `cargo xtask conform`の
//! 検証の出力の置き場の検査が、ほかのファイルにこの木を指す文字列が現れることを違反として止める。
//!
//! 木の下の場所を導くメソッドをこの型が持つのは、役割を持つパスが自分の配置を知るためである。
//! 呼び出し側が`join`を並べて場所を綴ると、同じ場所の書き方が入口の数だけ増える。

use std::path::PathBuf;

use super::file_name::検証の出力のファイル名;
use super::name::検証の出力の置き場名;

/// 検証の出力とビルドの成果物が載る木の名前。Cargoが`CARGO_TARGET_DIR`の既定として使う文字列と同じである。
...
/// 作業ツリーごとの出力先を見分ける印。木の名前に続けてこれを書いた兄弟が、その作業ツリー専用の出力先である。
...
impl 検証の出力ルート {
    ...
    pub(crate) fn 名前が指す置き場(self, 名前: 検証の出力の置き場名) -> PathBuf {
        PathBuf::from(self.0).join(名前.文字列を見せる())
    }

    /// 置き場1つの直下にあるファイル。焼き上がりの実在を確かめる側と、書き出す側が同じ口を通る。
    pub(crate) fn 置き場の中のファイル(self, 名前: 検証の出力の置き場名, ファイル名: 検証の出力のファイル名) -> PathBuf {
        self.名前が指す置き場(名前).join(ファイル名.文字列を見せる())
    }

    /// 作業ツリーごとの出力先を見分ける接頭辞。木の名前に印を続けた名前を持つ兄弟が、その作業ツリー専用の出力先である。
    /// 印の文字列をこの型が持つのは、規則が木の名前から決まるためである。掃除の入口はこの答えを使うだけにする。
    pub(crate) fn 作業ツリーごとの出力先の接頭辞(self) -> String {
        format!("{}{作業ツリーごとの出力先の印}", self.0)
    }
