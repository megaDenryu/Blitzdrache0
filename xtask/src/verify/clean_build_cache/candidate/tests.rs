//! 候補の振り分けの検査。いま使っている出力の木と同じ場所の候補を、候補の種類に関わらず残す選択肢にすること。
//!
...
fn 残す選択肢か(役割: 掃除の対象の役割, 置き場: &Path, いま使っている木: &Path) -> bool {
    let 対象 = 掃除の対象::生成する(役割, 置き場.to_path_buf());
    matches!(掃除の候補::対象から生成する(対象, いま使っている木), 掃除の候補::いま使っているため残す(_))
}
...
fn codexレビューの出力先をいま使っているならその候補を残す() {
    ...
    let 残すか = 残す選択肢か(掃除の対象の役割::Codexレビューのビルドの出力先, &codexレビューの出力先, &codexレビューの出力先);
    std::fs::remove_dir_all(&木).unwrap();
    assert!(残すか, "いまのビルドが使っている出力の木そのものを消す選択肢へ積んでいる");
}
...
fn 木の一部分である差分ビルドの中間データはその木を使っていても消す() {
    ...
    let 残すか = 残す選択肢か(掃除の対象の役割::差分ビルドの中間データ, &中間データの置き場, &木);
    std::fs::remove_dir_all(&木).unwrap();
