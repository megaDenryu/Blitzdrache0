...
//! 拡張子を付けた綴りである。切り離した状態では枝の名前が無いため、先端の短いハッシュから始まる綴りになる。
//!
...
/// 切り離した状態のときにgitが枝の名前として返す綴り。枝の名前ではないため、前置きに使わない。
...
impl 検証のログのファイル名 {
    pub fn 枝と先端と時刻から組み立てる() -> Result<Self, 検証列の破れ> {
        let 枝の前置き = 枝の名前を前置きへ写す(&枝または先端をgitから読む(枝の名前の問い合わせ)?);
        let 先端の短いハッシュ = 枝または先端をgitから読む(先端の短いハッシュの問い合わせ)?;
        let 時刻 = 協定世界時の時刻::いまを読む()?.綴り();
        Ok(Self {
            綴り: format!("{枝の前置き}{先端の短いハッシュ}_{時刻}.log"),
        })
    }
...
/// gitが返した枝の名前を、ファイル名の前置きへ写す。切り離した状態では前置きを空にする。
/// ビルド専用のチェックアウトは依頼された先端を切り離して置くため、`HEAD`という綴りが名前に残ると
/// どの枝の検証だったのか読めない名前が並ぶ。
fn 枝の名前を前置きへ写す(gitの答え: &str) -> String {
...
mod tests {
    ...
    #[test]
    fn 切り離した状態では枝の前置きを付けない() {
        assert_eq!(枝の名前を前置きへ写す("HEAD"), "");
    }

    #[test]
    fn 枝の名前の斜線を横棒へ置き換えて前置きにする() {
        assert_eq!(枝の名前を前置きへ写す("feat/verify-writes-log"), "feat-verify-writes-log_");
    }
}
