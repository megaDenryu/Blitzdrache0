...
//! その判断は親エージェントの目視が行うためである。機械が見るのは検証層の指摘が0件であることだけである。
...
const 検収世界: 検収世界の用意 = 検収世界の用意::生成する("見本の集落", crate::compile_assets::見本の集落の世界の実行時形式の置き場, 検証の出力のファイル名::生成する("prop_village.blitzasset"));
...
fn 検収する() -> Result<String, 見本の集落の撮影エラー> {
    検収世界
        .焼き上がりを確かめる(crate::gen_source_assets::検証用ソースアセットを生成して成否を返す() && crate::compile_assets::見本の集落世界を既定で生成する())
