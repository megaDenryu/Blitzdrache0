...
//! 機械が見るのは検証層の指摘0件と、2枚の絵が実際に食い違うことだけである。後者を見るのは、
...
const 二枚の絵で食い違った画素: 判定の名前 = 判定の名前::定数から生成する("DamagedHelmetの2枚の絵で食い違った画素(1画素も食い違わないなら方針の取り違えの疑いがある)");
...
pub(super) fn ヘルメットの目視材料を作る(出力先: &Path) -> Result<Vec<String>, ブロック圧縮の検収エラー> {
    ...
    let 非圧縮 = draw::条件1つを描いて読み戻しをpngへ書き出す(&非圧縮の実行環境, ヘルメットのシーン名, "helmet_rgba8", 非圧縮の条件の呼び名)?;
    let 圧縮 = draw::条件1つを描いて読み戻しをpngへ書き出す(&圧縮の実行環境, ヘルメットのシーン名, "helmet_bc1", 圧縮の条件の呼び名)?;
    ...
    Ok(vec![
        ...
        summary::格納バイト数の行を作る("DamagedHelmet", baked_scene::ヘルメットの格納バイト数を読む(&ルート.非圧縮)?, baked_scene::ヘルメットの格納バイト数を読む(&ルート.ブロック圧縮)?)?,
        format!("DamagedHelmetの絵は{}と{}(拡大は{}と{})", 非圧縮.png.display(), 圧縮.png.display(), 非圧縮の拡大.display(), 圧縮の拡大.display()),
    ])
}
...
fn 画面全体の統計を採る(非圧縮: &読み戻し画像, 圧縮: &読み戻し画像) -> Result<difference::画素の成分差の統計, ブロック圧縮の検収エラー> {
