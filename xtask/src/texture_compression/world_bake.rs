...
pub(super) fn 対照の板の世界を方針違いで焼く() -> Result<(焼いた出力ルート, PathBuf), ブロック圧縮の検収エラー> {
    ...
    方針を指定して世界を焼く(世界名::ブロック圧縮の対照世界, 対照のブロック圧縮ルート, テクスチャ格納方針::ベースカラーのブロック圧縮)?;
    方針を指定して世界を焼く(世界名::ブロック圧縮の対照世界, 対照のブロック圧縮の再現ルート, テクスチャ格納方針::ベースカラーのブロック圧縮)?;
    Ok((
        焼いた出力ルート {
...
pub(super) fn ヘルメットの世界を方針違いで焼く() -> Result<焼いた出力ルート, ブロック圧縮の検収エラー> {
    ...
    方針を指定して世界を焼く(世界名::板の世界, ヘルメットのブロック圧縮ルート, テクスチャ格納方針::ベースカラーのブロック圧縮)?;
    Ok(焼いた出力ルート {
...
fn 方針を指定して世界を焼く(世界: 世界名, 出力ルート: 検証の出力の置き場名, 方針: テクスチャ格納方針) -> Result<(), ブロック圧縮の検収エラー> {
