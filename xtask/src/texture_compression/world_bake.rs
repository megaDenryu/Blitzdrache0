...
use crate::compile_assets::{ブロック圧縮の対照世界, 方針を名指しして生成する, 板の世界};
...
pub(super) fn 対照の板の世界を方針違いで焼く() -> Result<(焼いた出力ルート, PathBuf), String> {
    焼く(ブロック圧縮の対照世界, 対照の非圧縮ルート, 全てRGBA8)?;
    焼く(ブロック圧縮の対照世界, 対照のブロック圧縮ルート, ベースカラーのブロック圧縮)?;
    焼く(ブロック圧縮の対照世界, 対照のブロック圧縮の再現ルート, ベースカラーのブロック圧縮)?;
    Ok((
        焼いた出力ルート {
...
pub(super) fn ヘルメットの世界を方針違いで焼く() -> Result<焼いた出力ルート, String> {
    焼く(板の世界, ヘルメットの非圧縮ルート, 全てRGBA8)?;
    焼く(板の世界, ヘルメットのブロック圧縮ルート, ベースカラーのブロック圧縮)?;
    Ok(焼いた出力ルート {
...
fn 焼く(世界名: &str, 出力ルート: &str, 方針の名前: &str) -> Result<(), String> {
    if 方針を名指しして生成する(Path::new(出力ルート), 世界名, 方針の名前) {
        return Ok(());
    }
