...
use crate::acceptance::{アプリの起こし方, 世界を読まずに報告を採る実行環境, 検収の実行名, 検収エラー, 終了時報告};
...
/// 大気のベイク済み画像を焼かせて報告を採る。検証層の指摘は判定の一部としてこの入口が読むため、
/// 実行環境が零件を確かめる口は通さない。
fn 報告を採る() -> Result<終了時報告, 検収エラー> {
    ...
    実行環境.報告を採る(大気の焼き上げの実行名, &["--report-atmosphere-lut"])
}
