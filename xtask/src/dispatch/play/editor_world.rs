...
/// エディターが焼いた実行時形式の置き場。編集サーバーが同じ場所へ書き出す。
const エディター世界の実行時形式の置き場: 検証の出力の置き場名 = 検証の出力の置き場名::生成する("editor_world_assets");
...
pub(super) fn エディター世界を起動する() -> ExitCode {
    let 環境 = 世界を読ませて報告を採る実行環境::作る(
        ...
        実行時アセットルート::パスから生成する(検証の出力ルート::既定().名前が指す置き場(エディター世界の実行時形式の置き場)),
    );
