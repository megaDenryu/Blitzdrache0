//! 遠景の撮影と判定が受け取る、ちょうど1つの実行の別の文字列の一覧。1コマンドで独立したファイルへ置くのは、
//! この1件だけで22の文字列を持ち、分類の一覧と同居させると一覧の見通しを潰すためである。
//! 文字列と実行の別の対応の正本は`xtask/src/distant_view/plan/argument_name.rs`が持つ。
...
const 実行の別の選択肢一覧: &[選択肢] = &[
    ...
    選択肢::生成する("--capture-shadow-reference", "影の検査点のための対照の絵を、ポスト処理と明示境界を切って採る"),
    ...
    選択肢::生成する("--capture-scatter-reference-bare", "ポスト処理と画面空間の環境遮蔽と影のキャスターを切って散布の対照の絵を採る"),
    選択肢::生成する("--capture-scatter-candidate-bare", "ポスト処理と画面空間の環境遮蔽と影のキャスターを切って散布の候補の絵を採る"),
...
pub(super) const 遠景の撮影と判定の引数定義: &[引数定義] = &[引数定義::文字列から1つ選ぶ値を定義する(
    None,
    "実行の別",
