...
pub(super) const 一覧: &[コマンド項目] = &[
    コマンド項目::引数なしで生成する("固定シーンの性能計測", "  bench            リリース版の固定シーンを600フレーム実行し、GPU時間とCPU側フレーム間隔分布を表示する"),
    コマンド項目::引数なしで生成する("実表示間隔つき性能計測", "  bench-display-timing  benchに実表示間隔の計測を足して実行する(計測が描画ループを止めるため既存の時系列とは比較できない)"),
    コマンド項目::引数なしで生成する("流体試作の性能計測", "  m10-bench        M10流体GPU試作を固定条件で実行し、検証件数とGPU時間を表示する"),
    コマンド項目::引数なしで生成する("連続実行のメモリ推移計測", "  m11-soak         3600フレーム連続実行し、RAM・VRAM推移を約5秒間隔で表示する"),
    コマンド項目::引数なしで生成する("対象数別の性能計測", "  object-bench     二対象の画素判定後、1・10・100対象のGPU/CPU時間とGPUメモリを計測する"),
    コマンド項目::引数なしで生成する(
        "原点移動の不変性確認",
        "  origin-invariance  世界全体へkm級の大域平行移動を加えても読み戻し画像がバイト一致し、カメラだけを微小に動かすと変わることを確かめる",
    ),
    コマンド項目::引数なしで生成する("地形段差の継ぎ目確認", "  lod-crack        地形LODの段差0・1・最大を四方向と細粗入替でGPU描画し、内側の継ぎ目に背景色が露出しないことを確かめる"),
    コマンド項目::引数定義を添えて生成する(
        "大規模世界の固定経路計測",
        "  large-world-bench [条件...]  8km×10km世界をkm級固定経路で3回走らせ、CPU区間・GPU時間・会計・検証層の指摘件数・p50/p99を表示する(--print-planはGPUを起動しない)",
    ...
    コマンド項目::引数なしで生成する("原点移動と地形段の一括計測", "  ow3-dod          原点移動・LOD継ぎ目・半径2ストリーミングを本番経路でまとめて測り、複数LOD画像をPNGへ書き出す"),
];
