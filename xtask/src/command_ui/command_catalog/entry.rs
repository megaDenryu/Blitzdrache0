...
impl コマンド項目 {
    ...
    pub(super) const fn 引数定義を添えて生成する(日本語名: &'static str, 説明文: &'static str, 引数定義一覧: &'static [引数定義]) -> Self {
        Self { 日本語名, 説明文, 引数定義一覧 }
    }
    ...
    /// この部分だけで突き合わせる(日本語の説明の中の文字列は引数の宣言ではないため見ない)。
