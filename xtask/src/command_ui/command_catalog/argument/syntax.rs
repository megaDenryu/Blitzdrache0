//! 使い方の行が見せている引数の構文。所有するのは、人が読む文字列を1つの値として保つことと、
//! その文字列の中で名指しされている引数(ハイフン2つで始まる語)を数え上げる規則である。
//!
//! 数え上げの規則をこの型の中へ置くのは、文字列の形と読み方が別々の場所で変わるのを防ぐためである。
//! 実行時の経路は文字列を使い方の行へ出すだけであり、文字列を読み直すのは引数定義との突き合わせだけである。
...
impl 引数の構文 {
    ...
    /// 使い方の表示に出す文字列そのもの。生の文字列へ戻すのはこの1箇所だけである。
    pub(crate) fn 文字列(self) -> &'static str {
        self.文字列
    }

    /// 構文の中で名指しされている引数の文字列を、現れた順に全部並べる。位置で意味が決まる引数は
    /// 名指しされないため、ここには現れない。
    #[cfg(test)]
