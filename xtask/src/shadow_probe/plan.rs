...
//! 軸そのものと文字列は`axis`が、軸ごとにどの条件を並べるかは`axis_cases`が、条件1つが何を既定から変えるかは`case`が、
    ...
    while let Some(語) = 残り.next() {
        match 語.as_str() {
            ...
            "--time-of-day" => {
                ...
                一日内時刻の秒 = u32::try_from(秒).map_err(|_| 律速切り分けの計測エラー::一日内時刻の秒が32ビットに収まらない { 秒 })?;
            }
            _ => 軸 = Some(axis::文字列から読む(語)?),
        }
    }
    let 軸 = 軸.ok_or_else(|| 律速切り分けの計測エラー::振る軸を渡されなかった { 選べる軸: axis::文字列を並べる() })?;
    if 指定された周回数 == Some(0) {
...
fn 数を読む(引数名: &str, 語: Option<&String>) -> Result<usize, 律速切り分けの計測エラー> {
    let 語 = 語.ok_or_else(|| 律速切り分けの計測エラー::引数の次に値が無い { 引数名: 引数名.to_string() })?;
    語.parse().map_err(|_| 律速切り分けの計測エラー::引数の値を数として読めない {
        引数名: 引数名.to_string(), 語: 語.clone()
    })
}
