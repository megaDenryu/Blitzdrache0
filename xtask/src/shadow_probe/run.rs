...
pub(super) fn 一回走らせる(材料: &実行の材料<'_>) -> Result<一標本, 律速切り分けの計測エラー> {
    let 標準出力先 = PathBuf::from(材料.出力先).join(format!("{}.log", 実行名の文字列を組む(材料.実行番号)));
    let 引数一覧 = 引数を作る(材料);
    println!("[xtask] shadow-probe実行{}: {}", 材料.実行番号, 材料.条件.名前);
    let 起こし方 = アプリの起こし方::構築済みのリリース版を直に起動する;
    let 出力 = 起こし方.コマンドを作る().args(&引数一覧).output().map_err(|誤り| 律速切り分けの計測エラー::アプリを起こせなかった {
        起こし方: 起こし方.表示用の表記(), 誤り
    })?;
    let 標準出力 = String::from_utf8_lossy(&出力.stdout).into_owned();
    std::fs::write(&標準出力先, &標準出力).map_err(|誤り| 律速切り分けの計測エラー::実行の標準出力を書けなかった { パス: 標準出力先.clone(), 誤り })?;
    ...
    let 報告 = 終了時報告::取り込む(&検収の実行名::生成する(&実行名の文字列を組む(材料.実行番号))?, 標準出力, String::from_utf8_lossy(&出力.stderr).into_owned());
    Ok(parse::標本を取り出す(&報告, 材料.実行番号, 材料.条件.名前)?)
}

/// 報告の読み取りが破れたときの文面が名指す実行名。標準出力を落としたログのファイル名と同じ文字列にして、
/// 破れた実行の証拠を読み手がそのまま開けるようにする。
fn 実行名の文字列を組む(実行番号: usize) -> String {
    format!("run_{実行番号:03}")
}
