...
fn 複数ファイルのimplを合算して降順に並べる() {
    let 観測 = vec![
        ...
        ファイル("src/b.rs", vec![実装("大", 1), 定義("小", 宣言の分量::列挙の選択肢数(1))]),
    ];
    ...
    assert_eq!(一覧[1].宣言.ただ1つの宣言().unwrap().指標名(), "選択肢数");
}

#[test]
fn 同じ名前の型が2つあれば別々の型として数える() {
    let 観測 = vec![
        ファイル("crates/blitz_app/src/cli/types.rs", vec![定義("起動設定", 宣言の分量::構造体のフィールド数(40))]),
        ファイル("xtask/src/smoke/launch_setting.rs", vec![定義("起動設定", 宣言の分量::構造体のフィールド数(10)), 実装("起動設定", 9)]),
    ];
    let 一覧 = 集計する(&観測).型ごとの計測一覧;
    assert_eq!(一覧.len(), 2);
...
fn 同じファイルの同名の定義は小さい側で大きい側を隠さない() {
    let 観測 = vec![ファイル("src/a.rs", vec![定義("設定", 宣言の分量::構造体のフィールド数(40)), 定義("設定", 宣言の分量::構造体のフィールド数(3))])];
    let 一覧 = 集計する(&観測).型ごとの計測一覧;
    assert_eq!(一覧.len(), 1);
    assert_eq!(一覧[0].宣言の件数(), 40);
    assert_eq!(一覧[0].宣言.定義の件数(), 2);
    assert!(一覧[0].宣言.ただ1つの宣言().is_none());
}
