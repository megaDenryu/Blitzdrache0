//! 型本体の1行が、構造体のフィールド宣言か、列挙の選択肢の開始か、メソッド定義かを判定する。
...
/// 選択肢の名前で始まる行を選択肢の開始とみなす。呼び出し元が列挙本体の直下の行だけを渡すため、選択肢が持つ
/// 名前付きフィールドはここへ来ない。属性・注釈・複数行にまたがる属性の中身・閉じ括弧は、
/// どれも識別子で始まらないことで外れる。
pub fn 列挙の選択肢の開始か(行: &str) -> bool {
...
mod tests {
    ...
    #[test]
    fn 選択肢の名前で始まる行だけを選択肢の開始とみなす() {
        assert!(列挙の選択肢の開始か("    値なし,"));
        assert!(列挙の選択肢の開始か("    範囲外(u32),"));
        assert!(列挙の選択肢の開始か("    長さ不一致 { 期待: usize, 実際: usize },"));
        assert!(!列挙の選択肢の開始か("    #[error(\"説明\")]"));
        assert!(!列挙の選択肢の開始か("    /// 説明"));
        assert!(!列挙の選択肢の開始か("    )]"));
        assert!(!列挙の選択肢の開始か("    },"));
        assert!(!列挙の選択肢の開始か(""));
    }
