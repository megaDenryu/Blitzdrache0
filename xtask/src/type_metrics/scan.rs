...
enum 走査状態 {
    外側,
    本体 { 種別: 本体種別, 宣言の文字列: String, 件数: usize },
}
...
fn 一行を処理する(状態: 走査状態, 行: &str, 行前の深さ: usize, 行後の深さ: usize, 観測一覧: &mut Vec<観測>) -> 走査状態 {
    let 走査状態::本体 { 種別, 宣言の文字列, 件数 } = 状態 else {
        return 宣言の開始を判定する(行, 行前の深さ, 行後の深さ, 観測一覧);
    };
    let 次の件数 = 件数 + usize::from(行前の深さ == 1 && 種別.数える対象か(行));
    if 行後の深さ == 0 {
        観測一覧.push(種別.観測にする(宣言の文字列, 次の件数));
        return 走査状態::外側;
    }
    走査状態::本体 {
        種別, 宣言の文字列, 件数: 次の件数
...
fn 宣言の開始を判定する(行: &str, 行前の深さ: usize, 行後の深さ: usize, 観測一覧: &mut Vec<観測>) -> 走査状態 {
    ...
    let 宣言 = definition_line::宣言の種別と型名を読み取る(行).or_else(|| impl_line::実装対象の経路の文字列(行).map(|文字列| (本体種別::実装, 文字列)));
    let Some((種別, 宣言の文字列)) = 宣言 else {
        return 走査状態::外側;
    };
    if 行後の深さ == 0 {
        観測一覧.push(種別.観測にする(宣言の文字列, 0));
        return 走査状態::外側;
    }
    走査状態::本体 { 種別, 宣言の文字列, 件数: 0 }
}
