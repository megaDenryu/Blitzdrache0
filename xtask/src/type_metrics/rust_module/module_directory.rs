//! モジュールが子のモジュールを置くディレクトリ。段を1つ進む・上位へ戻る・本体のファイルの文字列へ変える、の
//! 3つの操作をこの型が持つ。
//!
//! 裸のパスで運ばないのは、経路の解決がこの3つの操作だけで書けることを型で示すためである。文字列は斜線区切りへ
//! 揃えて持ち、走査が実行環境ごとの区切りで得たパスと突き合わせられるようにする。

use std::path::Path;

/// モジュールの本体になれるファイルの語幹。拡張子を除いた語幹で持つのは、ファイル名らしい文字列の重複の検査に
/// この規則の内側の文字列を数えさせないためである。`lib`と`main`はクレートの根の本体の語幹である。
...
impl モジュールのディレクトリ {
    ...
    /// このディレクトリのモジュールの本体になりうるファイルの文字列。`x`のモジュールは`x.rs`か`x/mod.rs`に書かれ、
