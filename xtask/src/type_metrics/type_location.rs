...
//!
//! 文字列は、定義ファイルの相対パスに二重コロンと型名を続けた形である。`crates/blitz_app/src/cli/types.rs`の
//! 起動設定なら crates/blitz_app/src/cli/types.rs::起動設定 になる。ファイルと名前を二重コロンで分けるのは、
//! 読み手がどちらがどちらか判断できる形にするためであり、この文字列を決めるのはこの型の`Display`だけである。
...
impl 型の所在 {
    ...
    /// 台帳の1行から組み立てる。台帳は文字列を区画の根と根からの相対パスへ分けて持つため、繋ぐ規則をここへ閉じる。
    /// 繋ぎを呼び出し側に書かせると、走査の側と台帳の側で区切りの文字列が割れる余地が残る。
    ...
    /// 経路の解決が出したファイルの文字列と突き合わせるための文字列。区切りは斜線へ揃えてある。
    pub fn 定義ファイルの文字列(&self) -> &str {
        &self.定義ファイル
    }
}

/// 走査で得たパスを、台帳と経路の解決が使う斜線区切りの文字列へ揃える。実行環境で区切り文字が変わるためである。
