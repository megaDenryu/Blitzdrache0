...
/// 同じファイルに`#[cfg]`で切り替わる同名の定義が2つ並ぶ形である。2つの定義の所在は同じ組であり、
/// implがどちらの構成のものであっても引き当ての答えは1つに定まるため、ここでは曖昧にしない。
/// 決まらないのは宣言の分量がどちらの構成の数かであり、そちらは`location_declarations`が保持し、
/// `conform/type_metrics_ledger`が隠れると困る場面で違反として報告する。
#[test]
