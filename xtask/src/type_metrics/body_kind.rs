...
impl 本体種別 {
    pub fn 数える対象か(&self, 行: &str) -> bool {
        match self {
            ...
            Self::列挙 => member_line::列挙の選択肢の開始か(行),
            Self::実装 => member_line::メソッド定義か(行),
        }
    }

    /// 宣言の文字列は、構造体と列挙なら型名そのものであり、implブロックなら実装対象として書かれた経路である。
    pub fn 観測にする(&self, 宣言の文字列: String, 件数: usize) -> 観測 {
        match self {
            Self::構造体 => 観測::型定義 {
                型名: 宣言の文字列,
            ...
            Self::実装 => 観測::実装ブロック {
                自己型の経路: 自己型の経路::文字列から生成する(&宣言の文字列),
                メソッド数: 件数,
            },
        }
    }
